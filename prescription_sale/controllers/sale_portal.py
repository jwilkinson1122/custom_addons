...
class CustomerPortal(CustomerPortal):
    @http.route(
        ["/my/orders/<int:order_id>/requestprescriptions"],
        type="http",
        auth="public",
        methods=["POST"],
        website=True,
    ...
    def request_prescription(self, order_id, access_token=None, **post):
        ...
        # Create wizard an generate prescriptions
        ...
        for rec in prescription:
            rec.origin += _(" (Portal)")
        # Add the user as follower of the created Prescriptions so they can later view them.
        ...
        if len(prescription) == 0:
            ...
            route = (
                ...
                else "/my/prescriptions?sale_id=%d" % order_id
            )
        return request.redirect(route)

    @http.route(
        ["/my/requestprescriptions/<int:order_id>"], type="http", auth="public", website=True
    )
