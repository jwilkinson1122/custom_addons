from odoo.addons.product_device_configurator.tests.common import (
    TestProductDeviceConfiguratorCommon,
)


class TestDeviceConfigureSale(TestProductDeviceConfiguratorCommon):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
    ...
    def _assert_device_order_line(self, data, lines, description=None):
    ...
    def test_01_multi_device_configure_on_sale_die(self):
        # GIVEN
        ctx = self.sale_1.action_open_device_configurator()['context']
        # WHEN
        cfg_1 = self.DeviceConfigure.with_context(**ctx).create(
            {
        ...
        self._assert_device_order_line(res_1['die'], order_lines)
        self._assert_device_order_line(
            res_1['counter_die'],
            order_lines,
        ...
        cfg_2 = self.DeviceConfigure.with_context(**ctx).create(
            {
                'die_id': self.device_die_default.id,
                'design_id': self.device_design_f.id,
                'material_id': self.device_material_brass_7.id,
        ...
        self._assert_device_order_line(res_2['die'], order_lines)
        self._assert_device_order_line(
            res_2['counter_die'],
            order_lines,
    ...
    def test_02_multi_device_configure_on_sale_insert_die_ref(self):
        # GIVEN
        ...
        self.assertEqual(
            ...
            [('device_type', '=', 'die'), ('is_insert_die', '=', False)],
        )
    ...
    def test_03_device_configure_sale_contact(self):
        # GIVEN
        ctx = self.sale_1.action_open_device_configurator()['context']
        self.sale_1.partner_id = self.partner_deco_fletcher.id
        # WHEN
        cfg = self.DeviceConfigure.with_context(**ctx).create(
            {
                'die_id': self.device_die_default.id,
                'design_id': self.device_design_f.id,
                'material_id': self.device_material_brass_7.id,
