...
class DifferedCheckHistory(models.Model):
    _name = "report.podiatry.order"
    _description = "Podiatry Order Analysis"
    _auto = False
    ...
    def _select(self):
        select_str = """
             SELECT
                    ...
                    t.podiatry_person as podiatry_person,
    ...
    def init(self):
        tools.sql.drop_view_if_exists(self._cr, 'report_podiatry_order')
        self._cr.execute("""
            CREATE view report_podiatry_order as
              %s
              FROM podiatry_order t
                %s
