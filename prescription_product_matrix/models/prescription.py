...
class PrescriptionOrder(models.Model):
    ...
    def get_report_matrixes(self):
        ...
        if self.report_grids:
            ...
            # configurable products are only configured through the matrix in prescription, so no need to check product_add_mode.
...
class PrescriptionOrderLine(models.Model):
    ...
    product_template_id = fields.Many2one('product.template', string='Product Template', related="product_id.product_tmpl_id", domain=[('prescription_ok', '=', True)])
    ...
    def _get_product_prescription_description(self, product):
        name = super(PrescriptionOrderLine, self)._get_product_prescription_description(product)
