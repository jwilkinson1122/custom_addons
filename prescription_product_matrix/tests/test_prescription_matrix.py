...
class TestPrescriptionMatrixUi(TestMatrixCommon):

    def test_prescription_matrix_ui(self):
        self.start_tour("/web", 'prescription_matrix_tour', login="admin")
        ...
        self.env['prescription.order.line'].search([('product_id', 'in', self.matrix_template.product_variant_ids.ids)]).order_id.button_confirm()

        self.matrix_template.flush_recordset()
        self.assertEqual(round(self.matrix_template.prescriptiond_product_qty, 2), 56.8)
        for variant in self.matrix_template.product_variant_ids:
            # 5 and 9.2 because of no variant attributes
            self.assertIn(round(variant.prescriptiond_product_qty, 2), [5, 9.2])
        ...
        self.assertEqual(
            len(self.env['prescription.order.line'].search([('product_id', 'in', self.matrix_template.product_variant_ids.ids)])),
