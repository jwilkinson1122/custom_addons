...
class BranchStockMove(models.Model):
    ...
    branch_id = fields.Many2one(
        "res.branch", readonly=True, store=True, related="picking_id.branch_id"
    )
...
class BranchStockMoveLine(models.Model):
    ...
    branch_id = fields.Many2one(
        "res.branch", readonly=True, store=True, related="move_id.branch_id"
    )
...
class BranchStockValuationLayer(models.Model):
    ...
    branch_id = fields.Many2one(
        "res.branch", readonly=True, store=True, related="stock_move_id.branch_id"
    )
