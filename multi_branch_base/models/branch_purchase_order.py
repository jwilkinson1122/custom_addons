...
class PurchaseOrder(models.Model):
    ...
    def _compute_branch(self):
        for order in self:
            ...
            branch = branch_ids.filtered(lambda branch: branch.company_id == so_company)
            if branch:
    ...
    def _get_picking_type(self, company_id):
        ...
        if len(self.env.user.branch_ids) == 1:
            picking_type = self.env["stock.picking.type"].search(
                [("code", "=", "incoming"), ("warehouse_id.branch_id", "=", company_id)]
            )
            if not picking_type:
                picking_type = self.env["stock.picking.type"].search(
                    [("code", "=", "incoming"), ("warehouse_id.branch_id", "=", False)]
                )
            if not picking_type:
                error_msg = _(
    ...
    picking_type_id = fields.Many2one(
        ...
        "'|', ('warehouse_id.branch_id', '=', branch_id), "
        "('warehouse_id.branch_id', '=', False)]",
        help="This will determine the operation type of the incoming shipment.",
    )
