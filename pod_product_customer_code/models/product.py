...
class ProductTemplate(models.Model):
    ...
    product_customer_ids = fields.One2many(
        'product.customer.info', 'product_tmpl_id', string="Customer Code")

    code_id = fields.Char(
        related='product_customer_ids.product_code', string='Code', readonly=False)

    product_code_name = fields.Char(
        related='product_customer_ids.product_name', string='Customer Product Name', readonly=False)
    ...
    def create(self, vals):
        ...
        if res:
            ...
            if (
                customer_code := self.env['product.customer.info']
                .sudo()
...
class ProductProduct(models.Model):
    ...
    product_customer_ids = fields.One2many(
        'product.customer.info', 'product_id', string="Customer Code")
    ...
    def _name_search(self, name, domain=None, operator='ilike', limit=None, order=None):
        ...
        if name:
            ...
            if partner_id:
                ...
                if customer_ids := self.env['product.customer.info']._search([('name', 'in', partners), '|', ('product_code', operator, name), ('product_name', operator, name)], order=order):
                    return self._search(
                        [('product_tmpl_id.product_customer_ids', 'in', customer_ids)], limit=limit, order=order)
            # SH
