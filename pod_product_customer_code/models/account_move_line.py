...
class AccountMoveLine(models.Model):
    ...
    order_line_customer_code = fields.Char(string='Customer Product Code')
    order_line_customer_product_name = fields.Char(string='Customer Product Name')
    ...
    def create(self, vals_list):
        ...
        for rec in res_super:
            if rec.product_id:
                ...
                customer_code = self.env['product.customer.info'].sudo(
                ).search([('name', 'in', partners),
                          ('product_id', '=', rec.product_id.id)],
                         limit=1)
                rec.order_line_customer_code = customer_code.product_code if customer_code and customer_code.product_code else False
                rec.order_line_customer_product_name = customer_code.product_name if customer_code and customer_code.product_name else False
