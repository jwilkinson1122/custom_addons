...
class Client(models.Model):
    ...
    # Pet
    pet = fields.One2many('pod_clinic.pet', 'owner', string='Pet')
    ...
    def compute_pet_count(self):
        for record in self:
            record.pet_count = self.env['pod_clinic.pet'].search_count(
                [('owner', '=', self.id)])
