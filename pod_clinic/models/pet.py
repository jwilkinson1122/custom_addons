...
class Pet(models.Model):
    ...
    # Pet -> Type
    pet_type = fields.Many2one('pod_clinic.pet.type',
                               string='Species')
    pet_type_name = fields.Char(related='pet_type.name',
                                string='Species')

    # Pet -> Breed
    pet_breed = fields.Many2one('pod_clinic.pet.breed', domain="[('pet_type','=',pet_type)]",
                                string='Breed')
    ...
    owner = fields.Many2one(
        'pod_clinic.client', string='Owner', store=True, readonly=False)
    ...
    def open_pet_appointment(self):
        return {
            ...
            'domain': [('pet', '=', self.id)],
            'view_type': 'form',
            'res_model': 'pod_clinic.appointment',
            'view_id': False,
            'view_mode': 'tree,form',
    ...
    def compute_appointment_count(self):
        for record in self:
            record.appointment_count = self.env['pod_clinic.appointment'].search_count(
                [('pet', '=', self.id)])


class PetType(models.Model):
    _name = 'pod_clinic.pet.type'
    name = fields.Char(string='Name', required=True)


class PetBreed(models.Model):
    _name = 'pod_clinic.pet.breed'
    name = fields.Char(string='Name', required=True)
    pet_type = fields.Many2one('pod_clinic.pet.type',
