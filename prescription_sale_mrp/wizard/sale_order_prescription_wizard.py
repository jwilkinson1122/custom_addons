...
class SaleOrderPrescriptionWizard(models.TransientModel):
    ...
    def create_prescription(self, from_portal=None):
        """We kept the component lines in the shade and now we need to take
        the ones linked to the phatom lines (kits) to create the proper Prescription
                ...
                if not product_kit_component_lines:
                    raise ValidationError(
                        _(
                            ...
                            "put in the Prescription. Either all or some of the components "
                            "where already put in another Prescription"
                        )
        ...
        return super().create_prescription(from_portal=from_portal)


class SaleOrderLinePrescriptionWizard(models.TransientModel):
    _inherit = "sale.order.line.prescription.wizard"
    ...
    kit_qty_done = fields.Float(
        readonly=True,
        help="Used to inform kit qty used in the prescription. Will be useful to refund",
    )
    ...
    def _compute_move_id(self):
        ...
        not_kit = self.filtered(
            ...
            and not x.sale_line_id._prescription_is_kit_product()
        )
        res = super(SaleOrderLinePrescriptionWizard, not_kit)._compute_move_id()
    ...
    def _prepare_prescription_values(self):
        ...
        res = super()._prepare_prescription_values()
            ...
            res.update(
                {
                    ...
                    "prescription_kit_register": unique_register,
                }
            )
        return res


class SaleOrderLinePrescriptionWizardComponent(models.TransientModel):
    _name = "sale.order.line.prescription.wizard.component"
    _inherit = "sale.order.line.prescription.wizard"
