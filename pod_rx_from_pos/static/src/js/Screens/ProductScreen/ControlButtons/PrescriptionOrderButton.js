...
export class PrescriptionOrderButton extends Component {
	static template = "pod_create_rx_from_pos.PrescriptionOrderButton";
	setup() {
		this.pos = usePos();
	...
	async onClick() {
		...
		if (selectedOrder.get_partner()){
			if (this.pos.get_order().get_orderlines().length > 0){
				...
				const result = await this.orm.call('prescription.order','create_prescriptionorder_from_pos', [oderdetails]);
				if(result){
					this.pos.showScreen('ProductScreen');
					await this.popup.add(ConfirmPopup, {
						title: this.env._t('Successfully!'),
						body: this.env._t(
							'Prescription Order ' + result.name +' Created Successfully!!!!'
						),
					});
				}
