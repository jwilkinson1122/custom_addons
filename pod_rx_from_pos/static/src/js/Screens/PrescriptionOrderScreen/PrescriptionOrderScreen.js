...
import { PrescriptionOrderList } from "@pos_prescription/js/OrderManagementScreen/PrescriptionOrderList";
import { PrescriptionOrderManagementControlPanel } from "@pos_prescription/js/OrderManagementScreen/PrescriptionOrderManagementControlPanel";
import { Component, onMounted, useRef } from "@odoo/owl";
import { usePos } from "@point_of_sale/app/store/pos_hook";
import { PrescriptionOrderPopup } from "@pod_create_rx_from_pos/js/Popups/PrescriptionOrderPopup";
...
export class PrescriptionOrderScreen extends ControlButtonsMixin(Component) {
	...
	static template = "pod_create_rx_from_pos.PrescriptionOrderScreenWidget";
	...
	async _onClickPrescriptionOrder(clickedOrder) {
		const { confirmed, payload: selectedOption } = await this.popup.add(PrescriptionOrderPopup, {
			title: this.env._t('Sale Order')  + '  ' + clickedOrder.name,
			list: [
				{
					id: "1",
					label: this.env._t("Confirm Prescription Order"),
					item: 'confirm',
					icon: 'fa fa-check-circle',
				},
				{
					id: "2",
					label: this.env._t("Cancel Prescription Order"),
					item: 'cancel',
					icon: 'fa fa-close'
				},
			],
				...
				if(selectedOption === 'confirm'){
					if(clickedOrder.state !== 'prescription'){
                        ...
                        await this.popup.add(ConfirmPopup, {
                            ...
                            body: this.env._t(
                                'This Prescription Order is Already in confirmed state!!!!'
                            ),
                        });
                    }
				}
				if(selectedOption === 'cancel'){
					if(clickedOrder.state !== 'cancel'){
                        ...
                        await this.popup.add(ConfirmPopup, {
                            ...
                            body: this.env._t(
                                'This Prescription Order is Already in Cancel State!!!!'
                            ),
                        });
                    }
				}
