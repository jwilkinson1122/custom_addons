...
class TestWorkflowPlandefinition(TransactionCase):
        ...
        self.type = self.env["workflow.type"].create(
            {
                ...
                "model_id": self.browse_ref(
                    "podiatry_administration.model_podiatry_patient"
                ).id,
                "model_ids": [
                    (
                        4,
    ...
    def test_add_plan_definition_on_patients(self):
        ...
        wzd = self.env["podiatry.add.plan.definition"]
        workflow_type = self.browse_ref("podiatry_workflow.podiatry_workflow")
        patient = self.browse_ref("podiatry_administration.patient_01")
