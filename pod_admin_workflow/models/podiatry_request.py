...
class PodiatryRequest(models.AbstractModel):
    # : Request (https://www.hl7.org/fhir/request.html)
    _name = "podiatry.request"
    ...
    order_date = fields.Datetime(
        string="Submitted date", help="Start of the order."
    )  # Field: authoredOn
    observations = fields.Text(string="Observations")  # Field: note
    ...
    is_prescription_request = fields.Boolean(
        "Podiatry request", compute="_compute_is_prescription_request"
    )

    def _compute_is_prescription_request(self):
        for rec in self:
            rec.is_prescription_request = True

    def _get_podiatry_request_context(self, context):
        for name, field in self._fields.items():
            if (
                field.comodel_name
                and "is_prescription_request"
                in self.env[field.comodel_name]._fields
                and field.type == "many2one"
            ):
                context.update({"default_%s" % field.name: self[name].id})
                if "is_prescription_request" in self._fields:
    ...
    def action_view_request(self):
        ...
        context = self._get_podiatry_request_context(context)
