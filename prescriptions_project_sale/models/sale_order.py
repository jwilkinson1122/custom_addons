    ...
    def _action_confirm(self):
        ...
        for order in self:
            ...
            for project, sols in sols_per_project.items():
                if not project.use_prescriptions or project.prescriptions_folder_id:
                    continue
                template_folders = sols.product_template_id.template_folder_id

                if len(template_folders) > 1:
                    project.prescriptions_folder_id = template_folders._copy_and_merge({
                        'name': project.name,
                        'company_id': project.company_id.id,
                    })
                    # It is necessary to set the parent after the copy to avoid
                    # infinite recursion issues.
                    project.prescriptions_folder_id.parent_folder_id = self.env.ref('prescriptions_project.prescriptions_project_folder').id,
                elif len(template_folders) == 1:
                    project.prescriptions_folder_id = template_folders.copy({
                        'name': project.name,
                        'company_id': project.company_id.id,
