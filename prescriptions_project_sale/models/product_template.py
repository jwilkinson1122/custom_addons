...
class ProductTemplate(models.Model):
    ...
    prescriptions_allowed_company_id = fields.Many2one('res.company', compute='_compute_prescriptions_allowed_company_id')
    project_template_use_prescriptions = fields.Boolean(string="Project Template - Prescriptions", related='project_template_id.use_prescriptions')
    template_folder_id = fields.Many2one('prescriptions.folder', "Workspace Template", company_dependent=True, copy=True,
        domain="['|', ('company_id', '=', False), ('company_id', '=', prescriptions_allowed_company_id)]",
    ...
    def _compute_prescriptions_allowed_company_id(self):
        for template in self:
            template.prescriptions_allowed_company_id = template.company_id if template.company_id else self.env.company
    ...
    @api.depends('project_template_id.use_prescriptions')
    def _compute_template_folder_id(self):
        for template in self:
            if template.project_template_id and not template.project_template_id.use_prescriptions:
