...
class PracticeServices(models.Model):
    ...
    _name = "practice.services"
    _description = "Practice Services and its charges"
    ...
    def create(self, vals):
        if "service_categ_id" in vals:
            service_categ = self.env["practice.service.type"].browse(
                vals.get("service_categ_id")
            )
            vals.update({"categ_id": service_categ.product_categ_id.id})
        return super(PracticeServices, self).create(vals)
...
class PracticeServiceType(models.Model):
    ...
    def create(self, vals):
        ...
        return super(PracticeServiceType, self).create(vals)
    ...
    def write(self, vals):
        if "service_id" in vals:
            ...
            vals.update({"parent_id": service_categ.product_categ_id.id})
        return super(PracticeServiceType, self).write(vals)
