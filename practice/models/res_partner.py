...
class ResPartner(models.Model):
    ...
    practice_count = fields.Integer(
        '# Practices', compute='_compute_practice_count', groups='practice.group_practice_confirmation_desk',
        help='Number of practices the partner has participated.')

    def _compute_practice_count(self):
        self.practice_count = 0
        for partner in self:
            partner.practice_count = self.env['practice.practice'].search_count([('confirmation_ids.partner_id', 'child_of', partner.ids)])

    def action_practice_view(self):
        action = self.env["ir.actions.actions"]._for_xml_id("practice.action_practice_view")
        action['context'] = {}
        action['domain'] = [('confirmation_ids.partner_id', 'child_of', self.ids)]
        return action
