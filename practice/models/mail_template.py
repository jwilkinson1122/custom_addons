...
class MailTemplate(models.Model):
    ...
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        ...
        key `filter_template_on_practice` on the template reference field. If this
        key is set, we add our domain in the `args` in the `_name_search`
        method to filtrate the mail templates.
        """
        if self.env.context.get('filter_template_on_practice'):
            args = expression.AND([[('model', '=', 'practice.confirmation')], args])
