...
class Job(models.Model):
    ...
    employee_ids = fields.One2many('podiatry.employee', 'job_id', string='Employees', groups='base.group_user')
    description = fields.Html(string='Job Description')
    requirements = fields.Text('Requirements')
    practice_id = fields.Many2one('podiatry.practice', string='Practice', domain="['|', ('company_id', '=', False), ('company_id', '=', company_id)]")
    ...
    _sql_constraints = [
        ('name_company_uniq', 'unique(name, company_id, practice_id)', 'The name of the job position must be unique per practice in company!'),
    ...
    def _compute_employees(self):
        employee_data = self.env['podiatry.employee'].read_group([('job_id', 'in', self.ids)], ['job_id'], ['job_id'])
