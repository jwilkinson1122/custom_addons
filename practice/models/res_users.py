...
class User(models.Model):
    ...
    # note: a user can only be linked to one employee per company (see sql constraint in ´podiatry.employee´)
    employee_ids = fields.One2many('podiatry.employee', 'user_id', string='Related employee', domain=_employee_ids_domain)
    employee_id = fields.Many2one('podiatry.employee', string="Company employee",
    ...
    practice_id = fields.Many2one(related='employee_id.practice_id', readonly=False, related_sudo=False)
    address_id = fields.Many2one(related='employee_id.address_id', readonly=False, related_sudo=False)
    practice_location_id = fields.Many2one(related='employee_id.practice_location_id', readonly=False, related_sudo=False)
    ...
    podiatry_presence_state = fields.Selection(related='employee_id.podiatry_presence_state')
    ...
    def _compute_can_edit(self):
        can_edit = self.env['ir.config_parameter'].sudo().get_param('podiatry.podiatry_employee_self_edit') or self.env.user.has_group('podiatry.group_podiatry_user')
        for user in self:
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        ...
        profile_view = self.env.ref("podiatry.res_users_view_form_profile")
    ...
    def write(self, vals):
        """
        Syncpodiatryonize user and its related employee
        ...
        podiatry_fields = {
            ...
            if field.related_field and field.related_field.model_name == 'podiatry.employee' and field_name in vals
        }
        can_edit_self = self.env['ir.config_parameter'].sudo().get_param('podiatry.podiatry_employee_self_edit') or self.env.user.has_group('podiatry.group_podiatry_user')
        if podiatry_fields and not can_edit_self:
        ...
        if employee_values:
            ...
            if 'image_1920' in vals:
                without_image = self.env['podiatry.employee'].sudo().search([('user_id', 'in', self.ids), ('image_1920', '=', False)])
                with_image = self.env['podiatry.employee'].sudo().search([('user_id', 'in', self.ids), ('image_1920', '!=', False)])
                without_image.write(employee_values)
                if not can_edit_self:
                    employee_values.pop('image_1920')
                with_image.write(employee_values)
            else:
                self.env['podiatry.employee'].sudo().search([('user_id', 'in', self.ids)]).write(employee_values)
        return result

    @api.model
    def action_get(self):
        if self.env.user.employee_id:
            return self.env['ir.actions.act_window']._for_xml_id('podiatry.res_users_action_my')
    ...
    def _compute_company_employee(self):
        employee_per_user = {
            ...
            for employee in self.env['podiatry.employee'].search([('user_id', 'in', self.ids), ('company_id', '=', self.env.company.id)])
        }
        for user in self:
    ...
    def action_create_employee(self):
        self.ensure_one()
        self.env['podiatry.employee'].create(dict(
            name=self.name,
            company_id=self.env.company.id,
            **self.env['podiatry.employee']._sync_user(self)
        ))
