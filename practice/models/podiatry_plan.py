...
class PodiatryPlanActivityType(models.Model):
    _name = 'podiatry.plan.activity.type'
    ...
    activity_type_id = fields.Many2one(
        ...
        domain=lambda self: ['|', ('res_model', '=', False), ('res_model', '=', 'podiatry.employee')],
        ondelete='restrict'
    )
...
class PodiatryPlan(models.Model):
    _name = 'podiatry.plan'
    _description = 'plan'

    name = fields.Char('Name', required=True)
    plan_activity_type_ids = fields.Many2many('podiatry.plan.activity.type', string='Activities')
