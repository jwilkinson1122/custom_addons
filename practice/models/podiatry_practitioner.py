...
class Practitioner(models.Model):
    ...
    practitioner_practice_id = fields.One2many(
    ...
    practice_count = fields.Integer(
        string='Practice Count', compute='_compute_practice_count')
    ...
    # practice_line = fields.One2many(
    #     'practice.practice.line', 'practice_id', 'Practice Line')

    def _compute_practice_count(self):
        for rec in self:
            practice_count = self.env['practice.practice'].search_count(
                [('practitioner_id', '=', rec.id)])
            rec.practice_count = practice_count

    practice_date = fields.Datetime(
        'Practice Date', default=fields.Datetime.now)
    ...
    def action_open_practices(self):
        return {
