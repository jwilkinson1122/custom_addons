...
class BaseModel(models.AbstractModel):
    ...
    def _alias_get_error_message(self, message, message_dict, alias):
        if alias.alias_contact == 'employees':
            ...
            employee = self.env['podiatry.employee'].search([('work_email', 'ilike', email_address)], limit=1)
            if not employee:
                employee = self.env['podiatry.employee'].search([('user_id.email', 'ilike', email_address)], limit=1)
            if not employee:
