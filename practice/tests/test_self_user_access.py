...
class TestSelfAccessProfile(TestPodiatryCommon):
    ...
    def test_access_my_profile(self):
        ...
        self.env['podiatry.employee'].create({
            'name': 'James',
            'user_id': james.id,
        })
        view = self.env.ref('podiatry.res_users_view_form_profile')
    ...
    def test_readonly_fields(self):
        ...
        self.env['ir.config_parameter'].sudo().set_param('podiatry.podiatry_employee_self_edit', False)
        james = new_test_user(self.env, login='hel', groups='base.group_user', name='Simple employee', email='ric@example.com')
        james = james.with_user(james)
        self.env['podiatry.employee'].create({
            'name': 'James',
...
class TestSelfAccessRights(TestPodiatryCommon):
    ...
    def setUp(self):
        ...
        self.richard_emp = self.env['podiatry.employee'].create({
            'name': 'Richard',
        ...
        self.hubert_emp = self.env['podiatry.employee'].create({
            'name': 'Hubert',
            'user_id': self.hubert.id,
            'address_home_id': self.env['res.partner'].create({'name': 'Hubert', 'type': 'private'}).id,
        })

        self.protected_fields_emp = OrderedDict([(k, v) for k, v in self.env['podiatry.employee']._fields.items() if v.groups == 'podiatry.group_podiatry_user'])
        # Compute fields and id field are always readable by everyone
        self.read_protected_fields_emp = OrderedDict([(k, v) for k, v in self.env['podiatry.employee']._fields.items() if not v.compute and k != 'id'])
        self.self_protected_fields_user = OrderedDict([
            (k, v)
            for k, v in self.env['res.users']._fields.items()
            if v.groups == 'podiatry.group_podiatry_user' and k in self.env['res.users'].SELF_READABLE_FIELDS
        ])

    # Read podiatry.employee #
    ...
    def testWriteSelfUserEmployee(self):
        self.env['ir.config_parameter'].set_param('podiatry.podiatry_employee_self_edit', True)
        for f, v in self.self_protected_fields_user.items():
            val = None
    ...
    def testWriteSelfUserPreferencesEmployee(self):
        # self should always be able to update non podiatry.employee fields if
        # they are in SELF_READABLE_FIELDS
        self.env['ir.config_parameter'].set_param('podiatry.podiatry_employee_self_edit', False)
        # should not raise
        vals = [
            {'tz': "Australia/ACT"},
