...
class TestMultiCompany(TestPodiatryCommon):

    def setUp(self):
        super().setUp()
        self.company_1 = self.env['res.company'].create({'name': 'Opoo'})
        self.company_2 = self.env['res.company'].create({'name': 'Otoo'})
        self.employees = self.env['podiatry.employee'].create([
            {'name': 'Bidule', 'company_id': self.company_1.id},
            {'name': 'Machin', 'company_id': self.company_2.id},
        ])
        self.res_users_podiatry_officer.company_ids = [
            (4, self.company_1.id),
            (4, self.company_2.id),
        ]
        self.res_users_podiatry_officer.company_id = self.company_1.id
    ...
    def test_multi_company_report(self):
        content, content_type = self.env.ref('podiatry.podiatry_employee_print_badge').with_user(self.res_users_podiatry_officer).with_context(
    ...
    def test_single_company_report(self):
        with self.assertRaises(QWebException):  # CacheMiss followed by AccessError
            content, content_type = self.env.ref('podiatry.podiatry_employee_print_badge').with_user(self.res_users_podiatry_officer).with_company(
                self.company_1
            )._render_qweb_pdf(res_ids=self.employees.ids)
