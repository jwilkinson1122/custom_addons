...
class TestPodiatryEmployee(TestPodiatryCommon):
    ...
    def setUp(self):
        ...
        self.employee_without_image = self.env['podiatry.employee'].create({
    ...
    def test_employee_resource(self):
        _tz = 'Pacific/Apia'
        self.res_users_podiatry_officer.company_id.resource_calendar_id.tz = _tz
        Employee = self.env['podiatry.employee'].with_user(self.res_users_podiatry_officer)
    ...
    def test_employee_from_user(self):
        ...
        self.res_users_podiatry_officer.company_id.resource_calendar_id.tz = _tz
        self.res_users_podiatry_officer.tz = _tz2
        Employee = self.env['podiatry.employee'].with_user(self.res_users_podiatry_officer)
        employee_form = Form(Employee)
        employee_form.name = 'Raoul Grosbedon'
        employee_form.work_email = 'raoul@example.com'
        employee_form.user_id = self.res_users_podiatry_officer
        employee = employee_form.save()
        self.assertEqual(employee.name, 'Raoul Grosbedon')
        self.assertEqual(employee.work_email, self.res_users_podiatry_officer.email)
        self.assertEqual(employee.tz, self.res_users_podiatry_officer.tz)

    def test_employee_from_user_tz_no_reset(self):
        _tz = 'Pacific/Apia'
        self.res_users_podiatry_officer.tz = False
        Employee = self.env['podiatry.employee'].with_user(self.res_users_podiatry_officer)
        employee_form = Form(Employee)
        employee_form.name = 'Raoul Grosbedon'
        employee_form.work_email = 'raoul@example.com'
