...
import { Many2OneAvatarEmployee } from '@podiatry/js/m2x_avatar_employee';
import { dom, mock } from 'web.test_utils';

QUnit.module('podiatry', {}, function () {
    QUnit.module('M2XAvatarEmployee', {
        beforeEach() {
            ...
            Object.assign(this.data, {
                'foo': {
                    fields: {
                        employee_id: { string: "Employee", type: 'many2one', relation: 'podiatry.employee.public' },
                        employee_ids: { string: "Employees", type: "many2many", relation: 'podiatry.employee.public' },
                    },
                    records: [
            ...
            this.data['podiatry.employee.public'].records.push(
    ...
    QUnit.test('many2one_avatar_employee widget in list view', async function (assert) {
        ...
        assert.verifySteps(
            ['read podiatry.employee.public 11'],
        ...
        assert.verifySteps(
            ['read podiatry.employee.public 7'],
    ...
    QUnit.test('many2one_avatar_employee widget in kanban view', async function (assert) {
        ...
        assert.strictEqual(kanban.$('.o_m2o_avatar:nth(0) > img').data('src'), '/web/image/podiatry.employee.public/11/avatar_128');
        assert.strictEqual(kanban.$('.o_m2o_avatar:nth(1) > img').data('src'), '/web/image/podiatry.employee.public/7/avatar_128');
        assert.strictEqual(kanban.$('.o_m2o_avatar:nth(2) > img').data('src'), '/web/image/podiatry.employee.public/11/avatar_128');
        assert.strictEqual(kanban.$('.o_m2o_avatar:nth(3) > img').data('src'), '/web/image/podiatry.employee.public/23/avatar_128');
    ...
    QUnit.test('many2one_avatar_employee: click on an employee not associated with a user', async function (assert) {
        ...
        this.data['podiatry.employee.public'].records[0].user_id = false;
        this.data['podiatry.employee.public'].records[0].user_partner_id = false;
    ...
    QUnit.test('many2many_avatar_employee widget in form view', async function (assert) {
        ...
        assert.strictEqual(form.$('.o_field_many2manytags.avatar.o_field_widget .badge:first img').data('src'),
            '/web/image/podiatry.employee.public/11/avatar_128',
        ...
        assert.verifySteps([
            "read foo 1",
            'read podiatry.employee.public 11,23',
            "read podiatry.employee.public 11",
            "read podiatry.employee.public 23",
        ]);
    ...
    QUnit.test('many2many_avatar_employee widget in list view', async function (assert) {
        ...
        assert.verifySteps(
            ['read podiatry.employee.public 11,23', "read podiatry.employee.public 11"],
            "first employee should have been read to find its partner"
        );
        assert.containsOnce(
            document.body,
    ...
    QUnit.test('many2many_avatar_employee widget in kanban view', async function (assert) {
        ...
        assert.strictEqual(kanban.$('.o_kanban_record:first .o_field_many2manytags img.o_m2m_avatar:eq(1)').data('src'),
            "/web/image/podiatry.employee.public/23/avatar_128",
            "should have correct avatar image");
