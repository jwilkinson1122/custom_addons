...
function factory(dependencies) {
    ...
    class Employee extends dependencies['mail.model'] {
        ...
        /**
         * Performs the `read` RPC on the `podiatry.employee.public`.
         *
         * @static
         * @param {Object} param0
        ...
        static async performRpcRead({ context, fields, ids }) {
            const employeesData = await this.env.services.rpc({
                model: 'podiatry.employee.public',
                method: 'read',
                args: [ids, fields],
                kwargs: {
                    context,
            ...
            this.messaging.models['podiatry.employee'].insert(employeesData.map(employeeData =>
                this.messaging.models['podiatry.employee'].convertData(employeeData)
            ));
        }

        /**
         * Performs the `search_read` RPC on `podiatry.employee.public`.
         *
         * @static
         * @param {Object} param0
         * @param {Object} param0.context
         ...
         */
        async checkIsUser() {
            return this.messaging.models['podiatry.employee'].performRpcRead({
                ids: [this.id],
         ...
         */
        async openProfile(model = 'podiatry.employee.public') {
    ...
    Employee.modelName = 'podiatry.employee';

    return Employee;
}

registerNewModel('podiatry.employee', factory);
