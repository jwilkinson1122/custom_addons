...
class SaleAdvancePaymentInv(models.TransientModel):
    ...
    def create_invoices(self):
        ...
        if self._context.get("active_model") == "practice.prescription":

            PracticePrescription = self.env["practice.prescription"]
            prescription = PracticePrescription.browse(
                self._context.get("active_ids", []))
            prescription.practice_line_ids.mapped(
                "product_id").write({"ispractice": True})
            ctx.update(
                {
