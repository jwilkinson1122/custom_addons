...
class wizard_sales_details(models.TransientModel):
    ...
    user_ids = fields.Many2many(
        'res.users', 'acespritech_pos_details_report_user_rel', 'user_id', 'wizard_id', 'Salespeople')
    ...
    def get_total_sales(self, user_lst=None):
        if self:
            ...
            if not user_lst:
                user_ids = [
                    user.id for user in self.user_ids] or self.get_all_users()
            else:
                user_ids = user_lst
            ...
            pos_ids = pos_obj.search([('date_order', '>=', self.start_date + ' 00:00:00'),
                                      ...
                                      ('user_id', 'in', user_ids), ('state',
                                                                    'in', ['done', 'paid', 'invoiced']),
