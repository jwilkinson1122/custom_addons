...
class ResConfigSettings(models.TransientModel):
    ...
    def get_values(self):
        ...
        res.update(
            google_api_key=self.env['ir.config_parameter'].sudo(
            ).get_param('google_api_key'),
            theme_selector=self.env['ir.config_parameter'].sudo(
            ).get_param('theme_selector'),
            gen_ean13=self.env['ir.config_parameter'].sudo(
            ).get_param('gen_ean13')
        )
        return res
    ...
    def set_values(self):
        ...
        self.env['ir.config_parameter'].sudo().set_param(
            'theme_selector', self.theme_selector or False)
        self.env['ir.config_parameter'].sudo().set_param(
            'gen_ean13', self.gen_ean13 or '')
        return res

    google_api_key = fields.Char('Google API key')
    theme_selector = fields.Selection(
        [('blue-green', 'Blue Green'), ('purple-pink', 'Purple Pink')])
...
class res_company(models.Model):
    ...
    shop_ids = fields.Many2many(
        "pos.shop", 'pos_shop_company_rel', 'shop_id', 'company_id', string='Allow Shops')
