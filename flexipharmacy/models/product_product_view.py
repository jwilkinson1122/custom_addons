...
class ProductProduct(models.Model):
    ...
    def get_near_expiry(self):
        ...
        if self.tracking != 'none':
            ...
            stock_lot = stock_production_lot_obj.search(
                [('product_id', 'in', self.ids)])
            for each_stock_lot in stock_lot.filtered(lambda l: l.alert_date):
                alert_date = datetime.strptime(
                    each_stock_lot.alert_date, '%Y-%m-%d %H:%M:%S').date()
                if each_stock_lot.expiration_date:
                    expiration_date = datetime.strptime(
                        each_stock_lot.expiration_date, '%Y-%m-%d %H:%M:%S').date()
    ...
    def get_expiry(self):
        ...
        if self.tracking != 'none':
            today_date = date.today()
            stock_lot = self.env['stock.production.lot'].search(
                [('product_id', 'in', self.ids)])
            for each_stock_lot in stock_lot.filtered(lambda l: l.expiration_date):
                expiration_date = datetime.strptime(
                    each_stock_lot.expiration_date, '%Y-%m-%d %H:%M:%S').date()
    ...
    def nearly_expired(self):
        ...
        action['domain'] = [
            ('id', 'in', [each_lot.id for each_lot in stock_production_lot_obj])]
        return action
        ...
        if product_expiry_detail:
            for each_product in product_expiry_detail:
                ...
                for each_quant in quant_detail:
                    expiration_date = datetime.strptime(
                        each_quant.lot_id.expiration_date, '%Y-%m-%d %H:%M:%S').date()
                    if not expiration_date < today_date:
    ...
    def search_product_expiry(self):
        ...
        for exp_day in exp_in_day:
            ...
            if today_date_end_exp == today_exp_date:
                self._cr.execute("select sq.lot_id "
                                 ...
                                 "where spl.expiration_date >= '%s'" % today_date_exp + " and"
                                 " spl.expiration_date <= '%s'" % today_exp_date + "and"
                                 " sq.company_id = '%s'" % company_id + "group by sq.lot_id")
            else:
                self._cr.execute("select sq.lot_id "
                                 "from stock_quant sq left join stock_production_lot spl on spl.id = sq.lot_id "
                                 "where spl.expiration_date >= '%s'" % today_date + " and"
                                 " spl.expiration_date <= '%s'" % today_exp_date + "and"
                                 " sq.company_id = '%s'" % company_id + "group by sq.lot_id")
        ...
        expire_product = self.env['stock.production.lot'].search(
            [('state_check', '=', 'expired')])
        ...
        quant_detail = self.env['stock.quant'].search([('state_check', '=', 'near_expired'),
                                                       ...
                                                       ('lot_id.expiration_date',
                                                        '>=', today_date),
                                                       ('lot_id.expiration_date', '<=', today_end_date)])
        for each_quant in quant_detail:
    ...
    def graph_date(self, start, end):
        ...
        filter_date_record = self.env['stock.quant'].search([('state_check', '!=', False),
                                                             ...
                                                             ('lot_id.expiration_date',
                                                              '>=', new_start_date),
                                                             ('lot_id.expiration_date', '<=', new_end_date)])
        for each_filter in filter_date_record.filtered(lambda l: l.quantity):
            graph_data_list.append(
                {'product_name': each_filter.product_id.name, 'qty': each_filter.quantity})
        data_res = []
        def key(x): return x['product_name']
...
class StockProductionLot(models.Model):
    ...
    def _check_dates(self):
        if self.alert_date and self.expiration_date:
            if not self.alert_date <= self.expiration_date:
                raise ValidationError(
                    _('Dates must be: Alert Date < Expiry Date'))
    ...
    def name_search(self, name, args=None, operator='=', limit=100):
        if self._context.get('default_product_id'):
            ...
            args = args or []
            recs = self.search(
                [('product_id', operator, self._context.get('default_product_id'))])
            if recs:
