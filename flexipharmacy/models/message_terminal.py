...
class message_terminal(models.Model):
    ...
    message_session_id = fields.Many2one(
        'pos.session', string="Message To Session", readonly=True)
    receiver_user = fields.Many2one(
        'res.users', string="Message To", readonly=True)
    sender_user = fields.Many2one(
        'res.users', string="Message By User", readonly=True)
    ...
    def create(self, vals):
        ...
        for user in res_users:
            notifications.append(
                ((self._cr.dbname, 'lock.data', user.id), ('terminal_message', res.read())))
