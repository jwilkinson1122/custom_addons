...
class PosConfig(models.Model):
    ...
    def search(self, args, offset=0, limit=None, order=None, count=False):
        ...
        if user_rec and erp_manager_id not in user_rec.groups_id.ids:
            if user_rec.shop_ids:
                args += ['|', ('multi_shop_id', 'in', user_rec.shop_ids.ids),
                         ('multi_shop_id', '=', False)]
            res = super(PosConfig, self).search(
    ...
    rounding_options = fields.Selection(
        [("digits", 'Digits'), ('points', 'Points'), ], string='Rounding Options', default='digits')
    rounding_journal_id = fields.Many2one(
        'account.journal', "Rounding Payment Method")
    ...
    enable_journal_id = fields.Many2one(
        'account.journal', string="Enable Journal")
    ...
    gift_voucher_journal_id = fields.Many2one(
        "account.journal", string="Gift Voucher Journal")
    ...
    payment_product_id = fields.Many2one(
        'product.product', "Payment Charge Product")
#     Wallet Functionality
    enable_wallet = fields.Boolean('Wallet')
    wallet_product = fields.Many2one(
        'product.product', string="Wallet Product")
# Order Reservation
    ...
    cancellation_charges_type = fields.Selection(
        [('fixed', 'Fixed'), ('percentage', 'Percentage')], 'Cancellation Charges Type')
    ...
    allow_reservation_with_no_amount = fields.Boolean(
        "Allow Reservation With 0 Amount")
# Discard Product
    discard_product = fields.Boolean(string="Discard Product")
    discard_location = fields.Many2one(
        'stock.location', string="Discard Location")
...
# User Operation Restrict
    ...
    pos_managers_ids = fields.Many2many(
        'res.users', 'posconfig_partner_rel', 'location_id', 'partner_id', string='Authorised Managers')
...
# Order Summary Report
    ...
    order_summary_no_of_copies = fields.Integer(
        string="No. of Copy Receipt", default=1)
...
# Customer Display
    ...
    customer_display_details_ids = fields.One2many(
        'customer.display', 'config_id', "Customer Display Details")
# Expire Dashboard
...
# POS Multi-Shop
    ...
    header_info = fields.Selection([('company', 'Company'), (
        'store', 'Store')], string="Receipt Header", required=True, default="company")
# Credit Management
    prod_for_payment = fields.Many2one('product.product', string='Paid Amount Product',
                                       help="This is a dummy product used when a customer pays partially. This is a workaround to the fact that Odoo needs to have at least one product on the order to validate the transaction.")
    ...
    pos_journal_id = fields.Many2one(
        'account.journal', string='Select Journal')
# Debit Management
...
class pos_order(models.Model):
    ...
    def search(self, args, offset=0, limit=None, order=None, count=False):
        ...
        if user_rec and erp_manager_id not in user_rec.groups_id.ids:
            if user_rec.shop_ids:
                args += ['|', ('shop_id', 'in', user_rec.shop_ids.ids),
                         ('shop_id', '=', False)]
            res = super(pos_order, self).search(
                args=args, offset=offset, limit=limit, order=order, count=count)
        else:
            res = super(pos_order, self).search(
                args=args, offset=offset, limit=limit, order=order, count=count)
    ...
    def load_ir_config_parameter(self):
        sys_param = self.env['ir.config_parameter'].sudo().search(
            [('key', '=', 'google_api_key')], order='id desc', limit=1)
        if sys_param:
    ...
    def get_dashboard_data(self):
        ...
        if pos_ids:
            ...
            res_pos_order['client_based_sale'] = {'new_client_sale': new_partner_sale,
                                                  'existing_client_sale': existing_partner_sale, 'without_client_sale': without_partner_sale}
...
#             orders = []
            ...
            for each in result_data_hour:
                if each['date_order_hour'] != 23:
                    each['date_order_hour'] = [
                        each['date_order_hour'], each['date_order_hour'] + 1]
                else:
            ...
            for hrs in hour_lst:
                ...
                if hrs != 23:
                    ...
                    hr += [hrs, 0]
                result_data_hour.append(
                    {'date_order_hour': hr, 'price_total': 0.0})
            sorted_hour_data = sorted(
                result_data_hour, key=lambda l: l['date_order_hour'][0])
...
#             Last 6 month data
    ...
    def pos_order_month_based(self, month_count):
        ...
        eddate = (c_time + relativedelta(day=1, months=+month_count,
                  days=-1)).strftime("%Y-%m-%d 23:59:59")
        # this month data
        if sign == '-':
            mon_stdate = (datetime.strptime(stdate, '%Y-%m-%d %H:%M:%S') +
                          timedelta(hours=hour_tz, minutes=min_tz)).strftime("%Y-%m-%d %H:%M:%S")
            mon_eddate = (datetime.strptime(eddate, '%Y-%m-%d %H:%M:%S') +
                          timedelta(hours=hour_tz, minutes=min_tz)).strftime("%Y-%m-%d %H:%M:%S")
        if sign == '+':
            mon_stdate = (datetime.strptime(stdate, '%Y-%m-%d %H:%M:%S') -
                          timedelta(hours=hour_tz, minutes=min_tz)).strftime("%Y-%m-%d %H:%M:%S")
    ...
    def session_details_on_canvas(self):
        ...
        if active_pos_ids:
            ...
            session_group = {}
            sort_group = sorted(active_session_data,
                                key=itemgetter('pos_session_id'))
        ...
        if posted_pos_ids:
            ...
            session_group = {}
            sort_group = sorted(posted_session_data,
                                key=itemgetter('pos_session_id'))
            for key, value in itertools.groupby(sort_group, key=itemgetter('pos_session_id')):
                if key not in session_group:
                    session_group.update({key: [x for x in value]})
    ...
    def update_delivery_date(self, reserve_delivery_date):
        res = self.write({'reserve_delivery_date': datetime.strptime(
            reserve_delivery_date, '%Y-%m-%d')})
        if res:
            return self.read()[0]
        return False
            ...
            if vals.get('state') == 'paid' and each.reserved:
                ...
                if picking_type_id:
                    ...
                    stock_transfer_id = self.env['stock.immediate.transfer'].search(
                        [('pick_ids', '=', picking_id.id)], limit=1).process()
                ...
                if line.product_id.type != 'service' and not line.cancel_item and line.line_status == 'nothing':
                    ...
                    if picking_type_out:
                        ...
                        if not picking_id_cust:
                            picking_id_cust = self.env['stock.picking'].create(
                                picking_vals_rev)
        ...
        for tmp_order in orders_to_save:
            ...
            if order.get('redeem') and pos_order:
                for redeem_details in order.get('redeem'):
                    ...
                    use_giftcard = self.env['aspl.gift.card.use'].create(
                        redeem_vals)
                    if use_giftcard:
            ...
            if order.get('recharge'):
                for recharge_details in order.get('recharge'):
                    ...
                    recharge_giftcard = self.env['aspl.gift.card.recharge'].create(
                        recharge_vals)
                ...
                for line in order.get('lines'):
                    if line[2].get('cancel_process'):
                        if line[2].get('product_id') in to_be_cancelled_items:
                            to_be_cancelled_items[line[2].get('product_id')] = to_be_cancelled_items[line[2].get(
                                'product_id')] + line[2].get('qty')
                        else:
                            to_be_cancelled_items.update(
                                {line[2].get('product_id'): line[2].get('qty')})
                    if line[2].get('return_process'):
                        if line[2].get('product_id') in to_be_returned_items:
                            to_be_returned_items[line[2].get('product_id')] = to_be_returned_items[line[2].get(
                                'product_id')] + line[2].get('qty')
                        else:
                            to_be_returned_items.update(
                                {line[2].get('product_id'): line[2].get('qty')})
                for line in order.get('lines'):
                    for item_id in to_be_cancelled_items:
                        cancel_lines = []
                        if line[2].get('cancel_process'):
                            cancel_lines = self.browse(
                                [line[2].get('cancel_process')[0]]).lines
                        for origin_line in cancel_lines:
                            ...
                            if origin_line.qty > 0 and item_id == origin_line.product_id.id:
                                if (to_be_cancelled_items[item_id] * -1) >= origin_line.qty:
                                    ret_from_line_qty = 0
                                    to_be_cancelled_items[item_id] = to_be_cancelled_items[item_id] + \
                                        origin_line.qty
                                else:
                                    ret_from_line_qty = to_be_cancelled_items[item_id] + \
                                        origin_line.qty
                            ...
                            for origin_line in self.browse([line[2].get('return_process')[0]]).lines:
                                ...
                                if origin_line.return_qty > 0 and item_id == origin_line.product_id.id:
                                    if (to_be_returned_items[item_id] * -1) >= origin_line.return_qty:
                                        ret_from_line_qty = 0
                                        to_be_returned_items[item_id] = to_be_returned_items[item_id] + \
                                            origin_line.return_qty
                                    else:
                                        ret_from_line_qty = to_be_returned_items[item_id] + \
                                            origin_line.return_qty
    ...
    def _process_order(self, order):
        ...
        if order.get('draft_order'):
            if not draft_order_id:
                ...
                pos_line_ids = pos_line_obj.search(
                    [('order_id', '=', order_id)])
                if pos_line_ids:
        ...
        if not order.get('draft_order') and draft_order_id:
            ...
            if picking_id_cust and picking_id_cust.move_lines:
                ...
                stock_transfer_id = stock_imm_tra_obj.search(
                    [('pick_ids', '=', picking_id_cust.id)], limit=1).process()
                if stock_transfer_id:
                    stock_transfer_id.process()
            ...
            if picking_id_rev and picking_id_rev.move_lines:
                ...
                stock_transfer_id = stock_imm_tra_obj.search(
                    [('pick_ids', '=', picking_id_rev.id)], limit=1)
                if stock_transfer_id:
                    stock_transfer_id.process()
                order_obj.with_context({'out_order': True}).write(
                    {'picking_id': picking_id_rev.id, 'unreserved': True})
            elif picking_id_rev:
            ...
            if res:
                if order.get('wallet_type'):
                    if order.get('change_amount_for_wallet'):
                        ...
                        cash_bocx_in_obj = self.env['x_cash.box.in'].create(
                            {'x_name': 'Credit', 'x_amount': order.get('change_amount_for_wallet')})
        ...
        if res.session_id.config_id.enable_pos_loyalty and res.partner_id:
            loyalty_setting_id = self.env['loyalty.config.settings'].sudo().search(
                    ...
                    if loyalty and res.partner_id.send_loyalty_mail:
                        try:
                            template_id = self.env['ir.model.data'].get_object_reference(
                                'flexipharmacy', 'email_template_pos_loyalty')
                            template_obj = self.env['mail.template'].browse(
                                template_id[1])
                            template_obj.send_mail(
                                loyalty.id, force_send=True, raise_exception=True)
                        except Exception as e:
                            _logger.error(
                                'Unable to send email for order %s', e)
    ...
    def set_pack_operation_lot(self, picking=None):
        ...
        for order in self:
            for move in (picking or self.picking_id).move_lines:
                ...
                pack_lot_names = [
                    pos_pack.lot_name for pos_pack in pos_pack_lots]
                ...
                if pack_lot_names and lots_necessary:
                    for lot_name in list(set(pack_lot_names)):
                        ...
                        if stock_production_lot:
                            ...
                            qty_done += qty
                            pack_lots.append(
                                {'lot_id': stock_production_lot.id, 'qty': qty})
                        else:
                            has_wrong_lots = True
...
#                 _logger.error('Unable to send email for order %s',e)
    ...
    def _calculate_loyalty_points_by_order(self, loyalty_config):
        if loyalty_config.point_calculation:
            ...
            amount_total = (
                earned_points * loyalty_config.to_amount) / loyalty_config.points
            return {
                            ...
                            if not month_year in salesmen_detail[each_order.user_id.name]:
                                ...
                                for payment_line in each_order.statement_ids:
                                    if payment_line.statement_id.journal_id.name in salesmen_detail[each_order.user_id.name][month_year]:
                                        payment = salesmen_detail[each_order.user_id.name][
                                            month_year][payment_line.statement_id.journal_id.name]
                                        payment += payment_line.amount
                                    else:
                                        payment = payment_line.amount
            ...
            if ('category_summary' in vals.get('summary') or len(vals.get('summary')) == 0):
                if (order_detail):
                    ...
                    if(False in category_summary_dict):
                        category_summary_dict['Others'] = category_summary_dict.pop(
                            False)
        ...
        if(vals):
            if(vals['state'] == ''):
                if('order_summary_report' in vals['summary'] or len(vals['summary']) == 0):
                    orders = self.search([('date_order', '>=', vals.get(
                        'start_date')), ('date_order', '<=', vals.get('end_date'))])
                ...
                if('category_summary_report' in vals['summary'] or len(vals['summary']) == 0):
                    ...
                    for each_order in orders:
                        ...
                        if(False in category_list[each_order.state]):
                            category_list[each_order.state]['others'] = category_list[each_order.state].pop(
                                False)
                ...
                order_list = []
                if('order_summary_report' in vals['summary'] or len(vals['summary']) == 0):
                    orders = self.search([('date_order', '>=', vals.get('start_date')), (
                        'date_order', '<=', vals.get('end_date')), ('state', '=', vals.get('state'))])
                    for each_order in orders:
                        order_list.append({
                            'order_ref': each_order.name,
                            'order_date': each_order.date_order,
                    ...
                    order_list_sorted = sorted(
                        order_list, key=itemgetter('order_ref'))
...
class pos_session(models.Model):
    ...
    current_cashier_id = fields.Many2one(
        'res.users', string="Cashier", readonly=True)
    ...
    shop_id = fields.Many2one(
        'pos.shop', string='Shop', related='config_id.multi_shop_id')
    ...
    def search(self, args, offset=0, limit=None, order=None, count=False):
        ...
        if user_rec and erp_manager_id not in user_rec.groups_id.ids:
            if user_rec.shop_ids:
                args += ['|', ('shop_id', 'in', user_rec.shop_ids.ids),
                         ('shop_id', '=', False)]
            res = super(pos_session, self).search(
                args=args, offset=offset, limit=limit, order=order, count=count)
        else:
            res = super(pos_session, self).search(
                args=args, offset=offset, limit=limit, order=order, count=count)
    ...
    def get_proxy_ip(self):
        proxy_id = self.env['res.users'].browse(
            [self._uid]).company_id.report_ip_address
    ...
    def get_payments(self):
        if self:
            ...
            if pos_ids:
                ...
                st_line_ids = statement_line_obj.search(
                    [('pos_statement_id', 'in', pos_ids)])
                if st_line_ids:
                    a_l = []
    ...
    def get_total_sales(self):
        ...
        if self:
            for order in self.order_ids:
                total_price += sum([(line.qty * line.price_unit)
                                   for line in order.lines])
        return total_price
    ...
    def get_vat_tax(self):
        ...
        if self:
            tax_list = []
            tax_list = [tax.id for order in self.order_ids for line in order.lines.filtered(
                lambda line: line.tax_ids_after_fiscal_position) for tax in line.tax_ids_after_fiscal_position]
...
#             sql query for get payments total of "In Progress" session
            ...
            for pay_id in session_detail:
                ...
                for i in bank_detail:
                    if i[2] != None:
                        lst.append(
                            {'session_name': i[0], 'journals': i[1], 'total': i[2]})
            ...
            for cate_id in session_detail:
                ...
                for j in cate_detail:
                    cate_lst.append(
                        {'cate_name': j[0], 'cate_total': j[1], 'session_name': j[2]})
            categ_null = []
            for cate_id_null in session_detail:
                ...
                for k in categ_null_detail:
                    categ_null.append(
                        {'cate_name': 'Undefined Category', 'cate_total': k[0], 'session_name': k[1]})
            all_cat = []
    ...
    def cash_in_out_operation(self, vals):
        ...
        if vals:
            if vals.get('operation') == "put_money":
                cash_obj = self.env['x_cash.box.in']
        ...
        if session_id:
            for session in session_id:
                bank_statements = [
                    session.cash_register_id for session in session_id if session.cash_register_id]
            if not bank_statements:
                return {'error': _('There is no cash register for this PoS Session')}
            cntx = {'active_id': session_id.id, 'uid': vals.get('cashier')}
            res = cash_obj.with_context(cntx).create(
                {'name': vals.get('name'), 'amount': vals.get('amount')})
...
#         try:
#             cash_out_obj = self.env['x_cash.box.in']
#             total_amount = 0.0
#             active_model = 'pos.session'
#             active_ids = [session_id]
#             if active_model == 'pos.session':
...
class CashControl(models.Model):
    ...
    pos_session_id = fields.Many2one(
        comodel_name='pos.session', string="Session Id")
...
class cash_in_out_history(models.Model):
    ...
    operation = fields.Selection(
        [('Dr', 'Dr'), ('Cr', 'Cr')], string="Operation")


# Put money in from backend
class PosBoxIn(CashBox):
    _inherit = 'x_cash.box.in'
    ...
    def create(self, vals):
        ...
        if res and self._context:
            ...
            history_val = {'user_id': user_id, 'session_id': session_record_id, 'x_amount': vals.get('x_amount'),
                           'operation': 'Cr'}
