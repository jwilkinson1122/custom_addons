...
class pos_cache(models.Model):
    ...
    config_id = fields.Many2one(
        'pos.config', ondelete='cascade', required=True)
    compute_user_id = fields.Many2one(
        'res.users', 'Cache compute user', required=True)
...
class pos_config(models.Model):
    ...
    def _get_oldest_cache_time(self):
        ...
        oldest_cache = pos_cache.search(
            [('config_id', '=', self.id)], order='write_date', limit=1)
        if oldest_cache:
    ...
    def _get_cache_for_user(self):
        ...
        cache_for_user = pos_cache.search(
            [('id', 'in', self.cache_ids.ids), ('compute_user_id', '=', self.env.uid)])
