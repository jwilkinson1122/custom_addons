...
class account_journal(models.Model):
    ...
    fees_type = fields.Selection(selection=[(
        'fixed', 'Fixed'), ('percentage', 'Percentage')], string="Fees type", default="fixed")
    ...
    def payment(self, get_journal_id, amount, pos_session_id, partner_id, pay_due):
        ...
        if pay_due:
            res = pos_order_obj.search(
                [('partner_id', '=', partner_id), ('state', '=', 'draft')], order='date_order')
            for each in res:
                if amount > 0:
        ...
        if amount > 0:
            ...
            result = account_payment_obj.with_context(
                {'default_from_pos': 'credit'}).create(vals)
            result.post()
        res = pos_order_obj.search(
    ...
    class InvoiceInfo(models.Model):
        ...
        def get_outstanding_info(self, vals):
            if (vals):
                ...
                comp_id = self.env['res.partner']._find_accounting_partner(
                    partner_id).id
                ...
                for each in vals:
                    if each['partner_id']:
                        partner_id = self.env['res.partner'].browse(
                            each['partner_id'])
                    credit_aml = self.env['account.move.line'].browse(
                        each['journal_id'])
