...
class lock_data(models.Model):
    ...
    locked_user_id = fields.Many2one(
        'res.users', string="Locked User", readonly=True)
    locked_by_user_id = fields.Many2one(
        'res.users', string="Locked By User", readonly=True)
        ...
        if rec_session.id:
            if rec_session.lock_status == True:
                if rec_session.locked_by_user_id.id == locked_by:
                    ...
                    session.write(
                        {'locked': status, 'locked_by_user_id': False})
                    return session.read()
                else:
                    return False
            else:
                ...
                session.write(
                    {'locked': status, 'locked_by_user_id': locked_by})
                return session.read()
        else:
            self.create({
                ...
                if lock:
                    if(session.get('locked') != True):
                        ...
                        if rec_session.id:
                            ...
                            session_rec.write(
                                {'locked': True, 'locked_by_user_id': locked_by})
                            session.update({'locked': True, 'locked_by_user_id': [
                                           user_locked_by.get('id'), user_locked_by.get('name')]})
                        else:
                            self.create({
                                'session_id': session.get('id'),
                                'locked_user_id': session.get('current_cashier_id')[0],
                        ...
                        if rec_session.id:
                            if rec_session.lock_status == True:
                                if rec_session.locked_by_user_id.id == locked_by:
                                    ...
                                    session_rec.write(
                                        {'locked': False, 'locked_by_user_id': False})
                                    session.update(
                                        {'locked': False, 'locked_by_user_id': False})
#                                 else:
...
#                                     session.update({'locked': True,'locked_by_user_id': [rec_session.locked_by_user_id.id,user_locked_by.get('name')]})
    ...
    def write(self, vals):
        ...
        for user in res_users:
            notifications.append(
                ((self._cr.dbname, 'lock.data', user.id), ('terminal_lock', self.read())))
    ...
    def create(self, vals):
        ...
        for user in res_users:
            notifications.append(
                ((self._cr.dbname, 'lock.data', user.id), ('terminal_lock', res.read())))
            self.env['bus.bus'].sendmany(notifications)
        return res
