...
class ProductProduct(models.Model):
    ...
    def calculate_product(self, config_id):
        ...
        setting = self.env['res.config.settings'].search(
            [], order='id desc', limit=1, offset=0)
    ...
    def search_product_expiry(self):
        ...
        for exp_day in exp_in_day:
            ...
            self._cr.execute("select sq.product_id, sq.lot_id, sq.company_id, sq.id "
                             ...
                             "where spl.expiration_date >= '%s'" % datetime.today().strftime(
                                 "%Y-%m-%d %H:%M:%S") + " and"
                             " spl.expiration_date <= '%s'" % exp_date + "and"
        ...
        for k, v in groupby(sorted(category_list, key=key), key=key):
            ...
            for each in v:
                ...
                categ_id = each['catid']
            category_res.append(
                {'categ_id': categ_id, 'categ_name': k, 'qty': qty})
    ...
    def graph_date_on_canvas(self, start, end):
        company_id = self.get_current_company()
        graph_data_list = []
        domain = [('state_check', '!=', False),
                  ('company_id.id', '=', company_id)]
        if start:
            domain += [('lot_id.expiration_date', '>=', start)]
        if end:
            domain += [('lot_id.expiration_date', '<=', end)]
