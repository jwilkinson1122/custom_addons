            ...
            if picking_id:
                if vals.get('data').get('state') == 'done':
                    ...
                    stock_transfer_id = self.env['stock.immediate.transfer'].search(
                        [('pick_ids', '=', picking_id.id)], limit=1)
...
class stock_location(models.Model):
    ...
    def get_warehouse_expiry_detail(self, company_id):
        ...
        warehouse_deatil = self.env['stock.warehouse'].search(
            [('company_id', '=', company_id)])
        for each in warehouse_deatil:
            warehouse_location = self.search(
                [('id', '=', each.lot_stock_id.id)])
            stock_detail = self.env['stock.quant'].search(
...
class StockQuantity(models.Model):
    ...
    state_check = fields.Selection(
        related='lot_id.state_check', string="state", store=True)
