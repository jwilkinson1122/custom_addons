    ...
    var PackLotLinePopupWidget = PopupWidget.extend({
	                	...
	                	if(serials[i].name == lot_name){
	                		if((serials[i]['remaining_qty'] - 1) < 0){
	                			...
	                			if(serials[i].expiration_date){
	                				if(moment(new moment().add(this.pos.config.product_exp_days, 'd').locale("en").format('YYYY-MM-DD HH:mm:mm')).format('DD/MM/YYYY') < moment(serials[i].expiration_date).format('DD/MM/YYYY')){
		                				serials[i]['remaining_qty'] = serials[i]['remaining_qty'] - 1;
			                			flag = false;
		                			}
	                			}else{
	    ...
	    render_list: function(orders){
            ...
            for(var i = 0, len = Math.min(orders.length,1000); i < len; i++){
                ...
                serial.check_serial_life =moment(serial.expiration_date).locale("en").format('YYYY/MM/DD');
