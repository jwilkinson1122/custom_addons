         ...
         _add_to_orderline (event) {
            if (event.target.className == 'add-button'){
                    ...
                    if (addon_obj.taxes_id.length > 0){
                        if (this.env.pos.taxes_by_id[addon_obj.taxes_id[0]].include_base_amount == true){
                        //checks the tax included or excluded product
                            var addon_obj_tax = 0;
            ...
            if (event.target.className == 'remove-button'){
                ...
                if (addons.length >0) {
                    for (var i=0; i<addons.length; i++) {
                            if (addons[i]['addon_id'] == addon_obj.id && addons[i]['addon_count'] != 0) {
                                has_already = true;
                                addons[i]['addon_count'] -= 1
                                addons[i]['total_without'] -= addon_obj.lst_price
                                addons[i]['total_without_including'] -= round_pr((addon_obj.lst_price)/(1+(tax_amt/100)),rounding)
