...
class ActivityDefinition(models.Model):
    ...
    def _get_request_group_values(self, vals, parent=False, plan=False, action=False):
        ...
        if vals.get("coverage_id", False):
            ...
            if not cai:
                raise ValidationError(
                    _("An element should exist on an agreement if it is billable")
                )
            ...
            new_vals = cai._check_authorization(cai.authorization_method_id, **res)
            res.update(new_vals)
        return res
    ...
    def execute_activity(self, vals, parent=False, plan=False, action=False):
        ...
        if parent and action.is_billable:
            ...
            result = super().execute_activity(vals, request_group, plan, action)
            request_group.write({"child_model": result._name, "child_id": result.id})
            return result
