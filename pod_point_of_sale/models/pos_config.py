...
class PosConfig(models.Model):
    ...
    def _get_group_pos_manager(self):
        return self.env.ref('pod_point_of_sale.group_pos_manager')

    def _get_group_pos_user(self):
        return self.env.ref('pod_point_of_sale.group_pos_user')

    def _get_default_tip_product(self):
        tip_product_id = self.env.ref("pod_point_of_sale.product_product_tip", raise_if_not_found=False)
    ...
    def _reset_default_on_vals(self, vals):
        if 'tip_product_id' in vals and not vals['tip_product_id'] and 'iface_tipproduct' in vals and vals['iface_tipproduct']:
            default_product = self.env.ref('pod_point_of_sale.product_product_tip', False)
            if default_product:
    ...
    def _update_preparation_printers_menuitem_visibility(self):
        prepa_printers_menuitem = self.sudo().env.ref('pod_point_of_sale.menu_pos_preparation_printer', raise_if_not_found=False)
    ...
    def _force_http(self):
        enforce_https = self.env['ir.config_parameter'].sudo().get_param('pod_point_of_sale.enforce_https')
    ...
    def _get_special_products(self):
        return self.env.ref('pod_point_of_sale.product_product_tip', raise_if_not_found=False) or self.env['product.product']
