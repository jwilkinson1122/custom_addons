...
class PosSession(models.Model):
    ...
    def _compute_cash_control(self):
        # Only one cash register is supported by pod_point_of_sale.
        for session in self:
    ...
    def create(self, vals_list):
        ...
        if self.user_has_groups('pod_point_of_sale.group_pos_user'):
    ...
    def _validate_session(self, balancing_account=False, amount_to_balance=0, bank_payment_method_diffs=None):
        ...
        self.ensure_one()
        data = {}
        sudo = self.user_has_groups('pod_point_of_sale.group_pos_user')
    ...
    def get_closing_control_data(self):
        if not self.env.user.has_group('pod_point_of_sale.group_pos_user'):
        ...
        return {
            ...
            'is_manager': self.user_has_groups("pod_point_of_sale.group_pos_manager"),
    ...
    def action_view_order(self):
        return {
            ...
            'view_mode': 'tree,form',
            'views': [
                (self.env.ref('pod_point_of_sale.view_pos_order_tree_no_session_id').id, 'tree'),
                (self.env.ref('pod_point_of_sale.view_pos_pos_form').id, 'form'),
                ],
            'type': 'ir.actions.act_window',
        ...
        for session in sessions:
            if self.env['mail.activity'].search_count([('res_id', '=', session.id), ('res_model', '=', 'pos.session')]) == 0:
                session.activity_schedule(
                    'pod_point_of_sale.mail_activity_old_session',
    ...
    def _get_pos_fallback_nomenclature_id(self):
        ...
        fallback_nomenclature_id = self.env['ir.config_parameter'].sudo().get_param('pod_point_of_sale.fallback_nomenclature_id')
    ...
    def _load_onboarding_data(self):
        convert.convert_file(self.env, 'pod_point_of_sale', 'data/point_of_sale_onboarding.xml', None, mode='init', kind='data')
        shop_config = self.env.ref('pod_point_of_sale.pos_config_main', raise_if_not_found=False)
        if shop_config:
            convert.convert_file(self.env, 'pod_point_of_sale', 'data/point_of_sale_onboarding_main_config.xml', None, mode='init', kind='data')
    ...
    def _after_load_onboarding_data(self):
        config = self.env.ref('pod_point_of_sale.pos_config_main', raise_if_not_found=False)
        if config:
            config.with_context(bypass_categories_forbidden_change=True).write({
                'limit_categories': True,
                'iface_available_categ_ids': [Command.link(self.env.ref('pod_point_of_sale.pos_category_miscellaneous').id), Command.link(self.env.ref('pod_point_of_sale.pos_category_desks').id), Command.link(self.env.ref('pod_point_of_sale.pos_category_chairs').id)]
            })
