...
from odoo.addons.pod_point_of_sale.tests.common_setup_methods import setup_pos_combo_items
...
class TestPointOfSaleHttpCommon(AccountTestInvoicingHttpCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        env = cls.env
        cls.env.user.groups_id += env.ref('pod_point_of_sale.group_pos_manager')
        ...
        cls.pos_user = cls.env['res.users'].create({
            ...
            'groups_id': [
                ...
                (4, cls.env.ref('pod_point_of_sale.group_pos_user').id),
        ...
        cls.pos_admin = cls.env['res.users'].create({
            ...
            'password': 'pos_admin',
            'groups_id': [
                (4, cls.env.ref('pod_point_of_sale.group_pos_manager').id),
            ],
        })
        ...
        discount = env.ref('pod_point_of_sale.product_product_consumable')
        cls.tip = env.ref('pod_point_of_sale.product_product_tip')
        (all_pos_product - discount - cls.tip)._write({'active': False})

        # In DESKS categ: Desk Pad
        pos_categ_desks = env.ref('pod_point_of_sale.pos_category_desks')

        # In DESKS categ: Whiteboard Pen
        pos_categ_misc = env.ref('pod_point_of_sale.pos_category_miscellaneous')

        # In CHAIR categ: Letter Tray
        pos_categ_chairs = env.ref('pod_point_of_sale.pos_category_chairs')
        ...
        env['product.pricelist'].create({
            ...
            'item_ids': [(0, 0, {
                ...
                'applied_on': '0_product_variant',
                'min_quantity': 2,
                'product_id': env.ref('pod_point_of_sale.product_product_consumable').id,
            })],
        })
...
class TestUi(TestPointOfSaleHttpCommon):
    def test_01_pos_basic_order(self):
        ...
        self.env['ir.module.module'].search([('name', '=', 'pod_point_of_sale')], limit=1).state = 'installed'
    ...
    def test_07_pos_barcodes_scan(self):
        barcode_rule = self.env.ref("pod_point_of_sale.barcode_rule_client")
        ...
        self.env.user.write({'groups_id': [
            Command.set(
                [
                    ...
                    self.env.ref('pod_point_of_sale.group_pos_user').id,
                ]
            )
        ]})
        self.main_pos_config.open_ui()
