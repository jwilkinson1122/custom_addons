odoo.define('product_reconfiguration', function (require) {
    ...
    var ProductReConfiguratorFormController = ProductConfiguratorFormController.include({
        _onFieldChanged: function (event) {
            this._super.apply(this, arguments); // Simplified the call to the super method

            var self = this;
            var product_id = event.data.changes.product_template_id.id;

            if (!product_id) { // Check to prevent traceback when emptying the field
                return;
            }
            ...
            var params = {
                ...
                product_product_id: record.data.product_id || false,
                order_line_id: record.data.order_line_id || false,
                order_id: record.data.order_id || false
            };
        ...
        _handleAdd: function ($modal) {
            var self = this;
            var productId = this._getProductId($modal);
            ...
            productReady.done(function (productId) {
                self._handleProductReady($modal, productId);
            });
        },

        _getProductId: function ($modal) {
            var productSelector = [ 'input[type="hidden"][name="product_id"]', 'input[type="radio"][name="product_id"]:checked'
            ];
            return parseInt($modal.find(productSelector.join(', ')).first().val(), 10);
        },

        // _handleProductReady: function ($modal, productId) {
        // }

        _handleProductReady: function ($modal, productId) {
            var self = this; $modal.find('input[type="hidden"][name="product_id"], input[type="radio"][name="product_id"]:checked').val(productId);
    ...
    ProductConfiguratorFormRenderer.include({
        ...
        handleCustomValues: function ($target) {
            var $variantContainer = $target.closest('li');
