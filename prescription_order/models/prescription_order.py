...
class PrescriptionOrder(models.Model):
    ...
    #ofdatetime
    prescription_start = fields.Datetime(string='Prescription Start')
    prescription_end = fields.Datetime(string='Prescription End')
    ...
    def _compute_wo_count(self):
        wo_data = self.env['work.order'].sudo().read_group([('prescription_order_reference', 'in', self.ids)], ['prescription_order_reference'],
                                                                   ['prescription_order_reference'])
        
        result = {
            data['prescription_order_reference'][0]: data['prescription_order_reference_count'] for data in wo_data
        }
    ...
    def action_work_order_create(self):
        ...
        for order in self:
            wo_obj.create([{'prescription_order_reference': order.id,
                            'team': order.team.id,
