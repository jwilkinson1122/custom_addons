...
class PosSession(models.Model):
    ...
    def _pos_data_process(self, loaded_data):
        ...
        if self.config_id and self.config_id.pod_enable_order_list:
            ...
            elif self.config_id.pod_load_order_by == "session_wise":
                # Load session wise orders
                orders = self.load_session_wise_pos_orders()
                loaded_data['pos_order_by_id'] = {order['id']: [order, list(
                    filter(lambda x: x.get('order_id') == order['id'], lines))] for order in orders}
            elif self.config_id.pod_load_order_by == "day_wise":
    ...
    def load_day_wise_pos_orders(self):
        ...
        if self.config_id.pod_day_wise_option == "current_day":
        ...
        elif self.config_id.pod_day_wise_option == "last_no_day":

            start_date = fields.Date.today()
            previus_day = start_date - \
                timedelta(days=self.config_id.pod_last_no_days - 1)
    ...
    def load_session_wise_pos_orders(self):
        ...
        if self.config_id.pod_session_wise_option == "current_session":
            # Give only currunt session id
            session_ids = [self.id]
        elif self.config_id.pod_session_wise_option == "last_no_session":
            ...
            session_query = f""" SELECT pos_session.id FROM pos_session
                                ...
                                LIMIT {self.config_id.pod_last_no_session} """
        ...
        if session_ids:
            self.env.cr.execute(f""" SELECT {table_fields} FROM pos_order pos_order
                                ...
                                AND pos_order.session_id IN %(pod_session)s """, {
                'company_id': self.env.company.id,
                'pod_session': tuple(session_ids),
            })
