...
class ShOptionGroup(models.Model):
    ...
    toppinds_ids = fields.Many2many('product.product', string="Options", domain="[('available_in_pos', '=', True)]")

class MassUpdateOptions(models.TransientModel):
    _name = 'pod.mass.update.topings'
    ...
    def _onchange_pod_option_group_ids(self):
            ...
            if option_groups:
                for option_groupid in option_groups: 
                    for tid in option_groupid.toppinds_ids:
