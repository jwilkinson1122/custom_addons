...
patch(PosStore.prototype, {
    ...
    async _processData(loadedData) {
        ...
        this.db.key_payment_screen_by_grp = {};
        this.db.temp_key_by_id = {};
        this.keyboard_keys_temp = loadedData['pod.keyboard.key.temp'];
        this.loadKeyboardKeysTemp(loadedData['pod.keyboard.key.temp']);
        this.keyboard_keys = loadedData['pod.pos.keyboard.shortcut'];
    ...
    loadKeyboardKeys(keyboard_keys){
        ...
        if(keyboard_keys && keyboard_keys.length > 0){
            ...
            for(let each_key_data of keyboard_keys){
                var key_combine = "";
                for(let each_key of each_key_data['pod_key_ids']){
                    if (key_combine != "") {
                ...
                if (each_key_data.payment_method_id && each_key_data.payment_method_id) {
                    ...
                    if (each_key_data["pod_payment_shortcut_screen_type"]) {
                        if (self.db.key_payment_screen_by_grp[each_key_data["pod_payment_shortcut_screen_type"]]) {
                            self.db.key_payment_screen_by_grp[each_key_data["pod_payment_shortcut_screen_type"]].push(each_key_data["payment_method_id"]);
                        } else {
                            self.db.key_payment_screen_by_grp[each_key_data["pod_payment_shortcut_screen_type"]] = [each_key_data["payment_method_id"]];
                        }
                    }
                } else {
                    self.db.key_screen_by_id[each_key_data["pod_shortcut_screen"]] = key_combine;
                    if (each_key_data.pod_shortcut_screen_type) {
                        if (self.db.key_screen_by_grp[each_key_data.pod_shortcut_screen_type]) {
                            self.db.key_screen_by_grp[each_key_data.pod_shortcut_screen_type].push(each_key_data["pod_shortcut_screen"]);
                        } else {
                            self.db.key_screen_by_grp[each_key_data.pod_shortcut_screen_type] = [each_key_data["pod_shortcut_screen"]];
                        }
