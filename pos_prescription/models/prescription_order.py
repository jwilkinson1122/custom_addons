...
class PrescriptionOrder(models.Model):
    ...
    amount_unpaid = fields.Monetary(string='Unpaid Amount', compute='_compute_amount_unpaid', store=True, help="The amount due from the prescription ordder.")
    ...
    def _compute_amount_unpaid(self):
        for prescription_order in self:
            total_invoice_paid = sum(prescription_order.order_line.filtered(lambda l: not l.display_type).mapped('invoice_lines').filtered(lambda l: l.parent_state != 'cancel').mapped('price_total'))
            total_pos_paid = sum(prescription_order.order_line.filtered(lambda l: not l.display_type).mapped('pos_order_line_ids.price_subtotal_incl'))
            prescription_order.amount_unpaid = prescription_order.amount_total - (total_invoice_paid + total_pos_paid)
...
class PrescriptionOrderLine(models.Model):
    ...
    def read_converted(self):
        ...
        results = []
        for prescription_line in self:
            if prescription_line.product_type:
    ...
    def unlink(self):
        ...
        return super(PrescriptionOrderLine, self - pos_downpayment_lines).unlink()
