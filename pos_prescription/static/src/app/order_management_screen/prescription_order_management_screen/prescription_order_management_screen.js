...
import { PrescriptionOrderList } from "@pos_prescription/app/order_management_screen/prescription_order_list/prescription_order_list";
import { PrescriptionOrderManagementControlPanel } from "@pos_prescription/app/order_management_screen/prescription_order_management_control_panel/prescription_order_management_control_panel";
...
export class PrescriptionOrderManagementScreen extends ControlButtonsMixin(Component) {
    ...
    static template = "pos_prescription.PrescriptionOrderManagementScreen";
    ...
    setup() {
        ...
        this.saleOrderFetcher = useService("prescription_order_fetcher");
    ...
    async onClickPrescriptionOrder(clickedOrder) {
        ...
        if (confirmed) {
            ...
            const currentSaleOrigin = this._getPrescriptionOrderOrigin(currentPOSOrder);
            const currentSaleOriginId = currentSaleOrigin && currentSaleOrigin.id;

            if (currentSaleOriginId) {
                const linkedRX = await this._getPrescriptionOrder(currentSaleOriginId);
                if (
                    getId(linkedRX.partner_id) !== getId(prescription_order.partner_id) ||
                    getId(linkedRX.partner_invoice_id) !== getId(prescription_order.partner_invoice_id) ||
                    getId(linkedRX.partner_shipping_id) !== getId(prescription_order.partner_shipping_id)
                ) {
            ...
            if (selectedOption == "settle") {
                // settle the order
                const lines = prescription_order.order_line;
                ...
                for (var i = 0; i < lines.length; i++) {
                    ...
                    new_line.setQuantityFromRXL(line);
                ...
                if (this.pos.config.down_payment_product_id) {
                    const lines = prescription_order.order_line.filter((line) => {
                    ...
                    if (down_payment_tax) {
                        down_payment = down_payment_tax.price_include
                            ? prescription_order.amount_total
                            : prescription_order.amount_untaxed;
                    } else {
                        down_payment = prescription_order.amount_total;
                    }

                    let popupTitle = "";
                    let popupInputSuffix = "";
                    const popupTotalDue = prescription_order.amount_total;
                    ...
                    const { confirmed, payload } = await this.popup.add(NumberPopup, {
                        ...
                        subtitle: sprintf(
                            popupSubtitle,
                            this.env.utils.formatCurrency(prescription_order.amount_total)
                        ),
                    ...
                    if (down_payment > prescription_order.amount_unpaid) {
                        const errorBody = _t(
                            ...
                            this.env.utils.formatCurrency(prescription_order.amount_unpaid),
                            prescription_order.amount_unpaid > 0
                                ? this.env.utils.formatCurrency(prescription_order.amount_unpaid)
                        ...
                        down_payment = prescription_order.amount_unpaid > 0 ? prescription_order.amount_unpaid : 0;
                    }
