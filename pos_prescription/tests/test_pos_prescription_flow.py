...
class TestPoSSale(TestPointOfSaleHttpCommon):
    def test_settle_order_with_kit(self):
        ...
        self.assertEqual(prescription_order.order_line.qty_delivered, 1)

        self.pos_user.write({
            'groups_id': [
                (4, self.env.ref('stock.group_stock_user').id),
                (4, self.env.ref('prescription_team.group_sale_salesman_all_leads').id),
            ]
        })
        ...
        self.assertEqual(prescription_order.order_line.qty_delivered, 3)
        self.assertEqual(prescription_order.picking_ids[0].move_ids.product_qty, 2100) # 7 left to deliver => 300 * 7 = 2100
        self.assertEqual(prescription_order.picking_ids[0].move_ids.quantity, 0)
        self.assertEqual(prescription_order.picking_ids[1].move_ids.product_qty, 300)
        self.assertEqual(prescription_order.picking_ids[1].move_ids.quantity, 300) # 1 delivered => 300 * 2 = 600

    def test_settle_order_with_incompatible_partner(self):
        """ If the partner of the prescription ordder is not compatible with the current pos order,
            then a new pos order should be to settle the newly selected prescription ordder.
        """
    ...
    def test_settle_order_with_different_product(self):
        """This test create an order and settle it in the PoS. But only one of the product is delivered.
            And we need to make sure the quantity are correctly updated on the prescription ordder.
        """
        #create 2 products
        ...
        #create a prescription ordder with 2 lines
        prescription_order = self.env['prescription.order'].create({
            'partner_id': self.env.ref('base.res_partner_2').id,
            'order_line': [(0, 0, {
        ...
        self.assertEqual(prescription_order.order_line[0].qty_delivered, 0)
        self.assertEqual(prescription_order.order_line[1].qty_delivered, 0)

        self.main_pos_config.open_ui()
        self.start_tour("/pos/ui?config_id=%d" % self.main_pos_config.id, 'PosSettleOrder2', login="accountman")

        self.assertEqual(prescription_order.order_line[0].qty_delivered, 1)
        self.assertEqual(prescription_order.order_line[1].qty_delivered, 0)
        orderline_product_a = prescription_order.order_line.filtered(lambda l: l.product_id.id == product_a.id)
        orderline_product_b = prescription_order.order_line.filtered(lambda l: l.product_id.id == product_b.id)
    ...
    def test_downpayment_refund(self):
        ...
        self.assertEqual(len(prescription_order.order_line), 3)
        self.assertEqual(prescription_order.order_line[1].qty_invoiced, 1)
        self.assertEqual(prescription_order.order_line[2].qty_invoiced, -1)
    def test_settle_order_unreserve_order_lines(self):
        ...
        self.assertEqual(prescription_order.order_line.move_ids.move_line_ids[0].quantity, 2)
        self.assertEqual(prescription_order.order_line.move_ids.move_line_ids[0].location_id.id, self.shelf_1.id)
        self.assertEqual(prescription_order.order_line.move_ids.move_line_ids[1].quantity, 2)
        self.assertEqual(prescription_order.order_line.move_ids.move_line_ids[1].location_id.id, self.shelf_2.id)
        ...
        self.assertEqual(prescription_order.order_line.move_ids.move_lines_count, 0)
    ...
    def test_settle_order_with_multistep_delivery(self):
        ...
        self.assertEqual(prescription_order.order_line[0].qty_delivered, 1)
        self.assertEqual(prescription_order.picking_ids.mapped('state'), ['cancel', 'cancel', 'cancel'])
    ...
    def test_pos_not_groupable_product(self):
        ...
        #create a prescription ordder with product_a
        self.env['prescription.order'].create({
            'partner_id': self.env.ref('base.res_partner_2').id,
            'order_line': [(0, 0, {
                'product_id': product_a.id,
