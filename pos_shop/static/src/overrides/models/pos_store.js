...
import { FloorScreen } from "@pos_shop/app/floor_screen/floor_screen";
import { TipScreen } from "@pos_shop/app/tip_screen/tip_screen";
...
patch(PosStore.prototype, {
    ...
    async setup() {
        ...
        if (this.config.module_pos_shop) {
    ...
    closeScreen() {
        if (this.config.module_pos_shop && !this.get_order()) {
    ...
    showBackButton() {
        return (
            ...
            (this.mainScreen.component === ProductScreen && this.config.module_pos_shop)
        );
    },
    //@override
    async _processData(loadedData) {
        ...
        if (this.config.module_pos_shop) {
            this.floors = loadedData["shop.floor"];
            this.loadShopFloor();
        }
    },
    //@override
    ...
    isOpenOrderShareable() {
        return super.isOpenOrderShareable(...arguments) || this.config.module_pos_shop;
    },
    toggleEditMode() {
        this.isEditMode = !this.isEditMode;
    },
    async updateModelsData(models_data) {
        const floors = models_data["shop.floor"];
        if (floors) {
            this.floors = floors;
            this.loadShopFloor();
