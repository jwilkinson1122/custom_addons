...
// New orders are now associated with the current section, if any.
patch(Order.prototype, {
    setup(_defaultObj, options) {
        super.setup(...arguments);
        if (this.pos.config.module_pos_shop) {
            if (this.defaultSectionNeeded(options)) {
                this.sectionId = this.pos.section.id;
            }
    ...
    export_as_JSON() {
        ...
        if (this.pos.config.module_pos_shop) {
            json.section_id = this.sectionId;
    ...
    getSection() {
        if (this.pos.config.module_pos_shop) {
            return this.pos.sections_by_id[this.sectionId];
        }
        return null;
    },
    defaultSectionNeeded(options) {
        return !this.sectionId && !options.json && this.pos.section;
    },
    export_for_printing() {
        return {
