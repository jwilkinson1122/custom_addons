...
class TestFrontend(odoo.tests.HttpCase):
    def setUp(self):
        ...
        section_05 = self.env['shop.section'].create({
            'name': '5',
        ...
        section_04 = self.env['shop.section'].create({
            'name': '4',
            'floor_id': main_floor.id,
            'seats': 4,
            'shape': 'square',
        ...
        section_02 = self.env['shop.section'].create({
            'name': '2',
            'floor_id': main_floor.id,
            'seats': 4,
            'position_h': 250,
        ...
        second_floor = self.env['shop.floor'].create({
            'name': 'Second Floor',
            'pos_config_ids': [(4, pos_config.id)],
        })

        section_01 = self.env['shop.section'].create({
            'name': '1',
        ...
        section_03 = self.env['shop.section'].create({
            'name': '3',
            'floor_id': second_floor.id,
            'seats': 4,
            'position_h': 100,
        ...
        pricelist = self.env['product.pricelist'].create({'name': 'Manufacturing Shop Pricelist'})
        pos_config.write({'pricelist_id': pricelist.id})

        self.pos_config = pos_config

    def test_01_pos_shop(self):

        self.pos_config.with_user(self.env.ref('base.user_demo')).open_ui()

        self.start_tour("/pos/ui?config_id=%d" % self.pos_config.id, 'pos_shop_sync', login="demo")
        ...
        self.start_tour("/pos/ui?config_id=%d" % self.pos_config.id, 'pos_shop_sync_second_login', login="demo")
