...
class TestPrescriptionRefund(TestPrescriptionCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        payment = cls.env['prescription.advance.payment.inv'].with_context({
        ...
        cls.invoice = cls.prescription.invoice_ids[0]
    ...
    def test_refund_create(self):
        ...
        for line in self.prescription_line:
            if line.product_id.invoice_policy == 'delivery':
                ...
                if line == self.sol_prod_order:
                    ...
                    self.assertEqual(line.qty_to_invoice, 0.0, "The ordered prescription line are totally invoiced (qty to invoice is zero)")
                    self.assertEqual(line.qty_invoiced, 3.0, "The ordered (serv) prescription line are totally invoiced (qty invoiced = the invoice lines)")
        ...
        invoice_refund = self.prescription.invoice_ids.sorted(key=lambda inv: inv.id, reverse=False)[-1]  # the first invoice, its refund, and the new invoice
        ...
        self.assertEqual(self.prescription.invoice_count, 2, "The SO should have 2 related invoices: the original, the new refund")
        self.assertEqual(len(self.prescription.invoice_ids.filtered(lambda inv: inv.move_type == 'out_refund')), 1, "The SO should be linked to only one refund")
        self.assertEqual(len(self.prescription.invoice_ids.filtered(lambda inv: inv.move_type == 'out_invoice')), 1, "The SO should be linked to only one customer invoices")
    ...
    def test_refund_modify(self):
        ...
        self.assertEqual(invoice_refund.state, 'draft', 'Last Customer invoices should be in draft')
        self.assertEqual(self.prescription.invoice_count, 3, "The SO should have 3 related invoices: the original, the refund, and the new one")
        self.assertEqual(len(self.prescription.invoice_ids.filtered(lambda inv: inv.move_type == 'out_refund')), 1, "The SO should be linked to only one refund")
        self.assertEqual(len(self.prescription.invoice_ids.filtered(lambda inv: inv.move_type == 'out_invoice')), 2, "The SO should be linked to two customer invoices")
        ...
        for line in self.prescription_line:
            if line.product_id.invoice_policy == 'delivery':
                ...
                if line == self.sol_prod_order:
                    ...
                    self.assertEqual(line.qty_invoiced, 2.0, "The ordered (serv) prescription line does not change on invoice 2 confirmation")
        ...
        for line in self.prescription_line:
            if line.product_id.invoice_policy == 'delivery':
                ...
                if line == self.sol_prod_order:
                    ...
                    self.assertEqual(line.qty_invoiced, 2.0, "The ordered prescription line are totally invoiced (qty invoiced = ordered qty)")
    ...
    def test_refund_invoice_with_downpayment(self):
        prescription_refund = self.env['prescription'].create({
            'partner_id': self.partner_a.id,
            'partner_invoice_id': self.partner_a.id,
            'partner_shipping_id': self.partner_a.id,
            'pricelist_id': self.company_data['default_pricelist'].id,
        })
        sol_product = self.env['prescription.line'].create({
            'product_id': self.company_data['product_order_no'].id,
            'product_uom_qty': 5,
            'order_id': prescription_refund.id,
            'tax_id': False,
        })
        ...
        prescription_refund.action_confirm()
        ...
        so_context = {
            'active_model': 'prescription',
            'active_ids': [prescription_refund.id],
            'active_id': prescription_refund.id,
        ...
        sol_downpayment = prescription_refund.order_line[2]
        dp_invoice = prescription_refund.invoice_ids[0]
        ...
        so_invoice = max(prescription_refund.invoice_ids)
        self.assertEqual(len(so_invoice.invoice_line_ids.filtered(lambda l: not (l.display_type == 'line_section' and l.name == "Down Payments"))),
                         len(prescription_refund.order_line.filtered(lambda l: not (l.display_type == 'line_section' and l.name == "Down Payments"))), 'All lines should be invoiced')
        ...
        self.assertEqual(so_invoice.amount_total, prescription_refund.amount_total - sol_downpayment.price_unit, 'Downpayment should be applied')
        ...
        credit_note_wizard.refund_moves()
        invoice_refund = prescription_refund.invoice_ids.sorted(key=lambda inv: inv.id, reverse=False)[-1]
        invoice_refund.action_post()
