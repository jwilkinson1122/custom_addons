...
class TestPrescriptionToInvoice(TestPrescriptionCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.prescription = cls.env['prescription'].with_context(tracking_disable=True).create({
        ...
        ) = cls.prescription_line

        # Context
        cls.context = {
            'active_model': 'prescription',
            'active_ids': [cls.prescription.id],
            'active_id': cls.prescription.id,
    ...
    def _check_order_search(self, orders, domain, expected_result):
        ...
        result = self.env['prescription'].search(domain)
    ...
    def test_search_invoice_ids(self):
        ...
        self._check_order_search(self.prescription, [('invoice_ids', '=', False)], self.prescription)
        self._check_order_search(self.prescription, [('invoice_ids', '!=', False)], self.env['prescription'])

        # Create invoice
        moves = self.prescription._create_invoices()

        # Tests after creating the invoice
        self._check_order_search(self.prescription, [('invoice_ids', 'in', moves.ids)], self.prescription)
        self._check_order_search(self.prescription, [('invoice_ids', '=', False)], self.env['prescription'])
        self._check_order_search(self.prescription, [('invoice_ids', '!=', False)], self.prescription)
    ...
    def test_downpayment(self):
        ...
        self.assertEqual(len(self.prescription.invoice_ids), 2, 'Invoice should be created for the SO')
        downpayment_line = self.prescription_line.filtered(lambda l: l.is_downpayment and not l.display_type)
        ...
        self.assertEqual(len(self.prescription.invoice_ids), 3, 'Invoice should be created for the SO')

        invoice = max(self.prescription.invoice_ids)
        self.assertEqual(len(invoice.invoice_line_ids.filtered(lambda l: not (l.display_type == 'line_section' and l.name == "Down Payments"))),
                         len(self.prescription_line.filtered(lambda l: not (l.display_type == 'line_section' and l.name == "Down Payments"))), 'All lines should be invoiced')
        ...
        self.assertEqual(invoice.amount_total, self.prescription.amount_total - sum(downpayment_line.mapped('price_unit')), 'Downpayment should be applied')
    ...
    def test_downpayment_validation(self):
        ...
        self.env.user.groups_id += self.env.ref('prescription.group_auto_done_setting')
        ...
        # Validate invoice
        self.prescription.invoice_ids.action_post()
    ...
    def test_downpayment_line_remains_on_SO(self):
        ...
        prescription = self.env['prescription'].with_context(tracking_disable=True).create({
        ...
        prescription_line.write({'qty_delivered': 5.0})
        context = {
            'active_model': 'prescription',
            'active_ids': [prescription.id],
        ...
        for invoice in prescription.invoice_ids:
            invoice.action_post()
        downpayment_line = prescription_line.filtered(lambda l: l.is_downpayment and not l.display_type)
    ...
    def test_downpayment_percentage_tax_icl(self):
        ...
        self.assertEqual(len(self.prescription.invoice_ids), 1, 'Invoice should be created for the SO')
        downpayment_line = self.prescription_line.filtered(lambda l: l.is_downpayment and not l.display_type)
        self.assertEqual(len(downpayment_line), 1, 'SO line downpayment should be created on SO')
        self.assertEqual(downpayment_line.price_unit, self.prescription.amount_total/2, 'downpayment should have the correct amount')

        invoice = self.prescription.invoice_ids[0]
        downpayment_aml = invoice.line_ids.filtered(lambda l: not (l.display_type == 'line_section' and l.name == "Down Payments"))[0]
        self.assertEqual(downpayment_aml.price_total, self.prescription.amount_total/2, 'downpayment should have the correct amount')
        self.assertEqual(downpayment_aml.price_unit, self.prescription.amount_total/2, 'downpayment should have the correct amount')
        invoice.action_post()
        self.assertEqual(downpayment_line.price_unit, self.prescription.amount_total/2, 'downpayment should have the correct amount')
    ...
    def test_invoice_with_discount(self):
        ...
        for line in self.prescription_line.filtered(lambda l: l.discount):
        ...
        # 'untaxed_amount_to_invoice' is invalid when 'prescription_stock' is installed.
        ...
        for line, inv_line in zip(self.prescription_line, invoice.invoice_line_ids):
    ...
    def test_multiple_prescriptions_on_same_invoice(self):
        ...
        prescription_data = self.prescription.copy_data()[0]
        prescription_data['order_line'] = [
            (0, 0, line.copy_data({
                'invoice_lines': [(6, 0, line.invoice_lines.ids)],
            })[0])
            for line in self.prescription_line
        ]
        self.prescription.create(prescription_data)
    ...
    def test_invoice_with_sections(self):
        ...
        PrescriptionLine = self.env['prescription.line'].with_context(tracking_disable=True)
        PrescriptionLine.create({
            'name': 'Section',
            'display_type': 'line_section',
            'order_id': prescription.id,
        })
        sol_prod_deliver = PrescriptionLine.create({
            'product_id': self.company_data['product_order_no'].id,
            'product_uom_qty': 5,
            'order_id': prescription.id,
            'tax_id': False,
        ...
        invoice = prescription.invoice_ids[0]
    ...
    def test_qty_invoiced(self):
        ...
        quantity = 5.13
        move_form = Form(prescription.invoice_ids)
        with move_form.invoice_line_ids.edit(0) as line_form:
    ...
    def test_multi_company_invoice(self):
        ...
        """
        so_company_id = self.prescription.company_id.id
        yet_another_company_id = self.company_data_2['company'].id
        so_for_downpayment = self.prescription.copy()
        ...
        self.prescription.with_context(self.context).action_confirm()
        payment = self.env['prescription.advance.payment.inv'].with_context(no_journal_ctxt).create({
            'advance_payment_method': 'percentage',
            'amount': 50,
        })
        payment.create_invoices()
        self.assertEqual(self.prescription.invoice_ids[0].company_id.id, so_company_id, "The company of the invoice should be the same as the one from the SO")
    ...
    def test_invoice_analytic_distribution_model(self):
        ...
        so_form = Form(self.env['prescription'])
    ...
    def test_invoice_analytic_rule_with_account_prefix(self):
        ...
        so = self.env['prescription'].create({'partner_id': self.partner_a.id})
        self.env['prescription.line'].create({
            'order_id': so.id,
            'name': 'test',
            'product_id': self.product_a.id
        })
    ...
    def test_invoice_after_product_return_price_not_default(self):
        so = self.env['prescription'].create({
    ...
    def test_group_invoice(self):
        ...
        so1 = self.prescription.with_context(mail_notrack=True).copy()
    ...
    def test_so_note_to_invoice(self):
        ...
        self.prescription_line = [Command.create({
            'name': 'This is a note',
    ...
    def test_cost_invoicing(self):
        ...
        self.assertTrue(sol, 'Prescription: cost invoicing does not add lines when confirming vendor invoice')
        self.assertEqual(
    ...
    def test_prescription_standard_flow_with_invoicing(self):
        ...
        """
        self.prescription_line.product_uom_qty = 2.0
        # TODO?: validate invoice and register payments
        self.prescription_line.read(['name', 'price_unit', 'product_uom_qty', 'price_total'])

        self.assertEqual(self.prescription.amount_total, 1240.0, 'Prescription: total amount is wrong')
        self.prescription_line._compute_product_updatable()
        self.assertTrue(self.prescription_line[0].product_updatable)
        # send quotation
        email_act = self.prescription.action_quotation_send()
        email_ctx = email_act.get('context', {})
        self.prescription.with_context(**email_ctx).message_post_with_source(
            self.env['mail.template'].browse(email_ctx.get('default_template_id')),
            subtype_xmlid='mail.mt_comment',
        )
        self.assertTrue(self.prescription.state == 'sent', 'Prescription: state after sending is wrong')
        self.prescription_line._compute_product_updatable()
        self.assertTrue(self.prescription_line[0].product_updatable)

        # confirm quotation
        self.prescription.action_confirm()
        self.assertTrue(self.prescription.state == 'prescription')
        self.assertTrue(self.prescription.invoice_status == 'to invoice')
        ...
        self.assertTrue(self.prescription.invoice_status == 'no', 'Prescription: SO status after invoicing should be "nothing to invoice"')
        self.assertTrue(len(self.prescription.invoice_ids) == 1, 'Prescription: invoice is missing')
        self.prescription_line._compute_product_updatable()
        self.assertFalse(self.prescription_line[0].product_updatable)
        ...
        self.assertTrue(self.prescription.invoice_status == 'to invoice', 'Prescription: SO status after delivery should be "to invoice"')
        invoice2 = self.prescription._create_invoices()
        ...
        self.assertTrue(self.prescription.invoice_status == 'invoiced', 'Prescription: SO status after invoicing everything should be "invoiced"')
        self.assertTrue(len(self.prescription.invoice_ids) == 2, 'Prescription: invoice is missing')

        # go over the sold quantity
        self.sol_serv_order.write({'qty_delivered': 10})
        self.assertTrue(self.prescription.invoice_status == 'upselling', 'Prescription: SO status after increasing delivered qty higher than ordered qty should be "upselling"')
        ...
        invoice3 = self.prescription._create_invoices()
        ...
        self.assertTrue(self.prescription.invoice_status == 'invoiced', 'Prescription: SO status after invoicing everything (including the upsel) should be "invoiced"')
    ...
    def test_so_create_multicompany(self):
        ...
        so_1 = self.env['prescription'].with_user(self.company_data['default_user_personnel']).create({
        ...
        self.assertEqual(so_1.order_line.tax_id, self.company_data['default_tax_prescription'],
    ...
    def test_partial_invoicing_interaction_with_invoicing_switch_threshold(self):
        ...
        line = prescription_line[0]

        prescription.action_confirm()

        line.qty_delivered = 10

        invoice = prescription._create_invoices()
        invoice.action_post()
    ...
    def test_prescriptionperson_in_invoice_followers(self):
        ...
        prescriptionperson = self.env['res.users'].create({
            ...
            'email': 'test@test.com',
            'groups_id': [(6, 0, [self.env.ref('prescription_team.group_prescription_personnel').id])]
        })
        ...
        prescription.action_confirm()
        invoice = prescription._create_invoices(final=True)

        # check if the prescriptionperson is in the followers list of invoice created from SO
        self.assertIn(prescriptionperson.partner_id, invoice.message_partner_ids, 'Prescriptionperson not in the followers list of '
    ...
    def test_amount_to_invoice_multiple_so(self):
        ...
        """
        prescription_1 = self.env['prescription'].create({
            'partner_id': self.partner_a.id,
            'order_line': [
                Command.create({
                    'product_id': self.company_data['product_delivery_no'].id,
        ...
        prescription_2 = self.env['prescription'].create({
            'partner_id': self.partner_a.id,
            'order_line': [
                Command.create({
                    'product_id': self.company_data['product_delivery_no'].id,
        ...
        prescription_1.action_confirm()
        prescription_2.action_confirm()
        prescription_1.order_line.qty_delivered = 10
        prescription_2.order_line.qty_delivered = 20

        self.env['prescription.advance.payment.inv'].create({
            'advance_payment_method': 'delivered',
            'prescription_ids': [Command.set((prescription_1 + prescription_2).ids)],
        }).create_invoices()

        prescription_1.invoice_ids.action_post()

        self.assertEqual(prescription_1.amount_to_invoice, 0.0)
        self.assertEqual(prescription_2.amount_to_invoice, 0.0)
    ...
    def test_amount_to_invoice_one_line_multiple_so(self):
        ...
        self.env['prescription.advance.payment.inv'].create({
            'advance_payment_method': 'delivered',
            'prescription_ids': [Command.set((prescription_2).ids)],
        }).create_invoices()

        prescription_1.invoice_ids = prescription_2.invoice_ids
        prescription_1.invoice_ids.line_ids.prescription_line_ids += prescription_1.order_line

        prescription_1.invoice_ids.action_post()

        self.assertEqual(prescription_1.amount_to_invoice, -700.0)
        self.assertEqual(prescription_2.amount_to_invoice, 0.0)
