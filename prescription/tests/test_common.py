...
class TestPrescriptionCommon(PrescriptionCommon):
    ...
    def test_common(self):
        ...
        self.assertEqual(self.prescription.partner_id, self.partner)
        self.assertEqual(self.prescription.partner_invoice_id, self.partner)
        self.assertEqual(self.prescription.partner_shipping_id, self.partner)
        self.assertEqual(self.prescription.pricelist_id, self.pricelist)
        self.assertEqual(self.prescription.currency_id.name, self.currency.name)
        self.assertEqual(self.prescription.team_id, self.prescription_team)
        self.assertEqual(self.prescription.state, 'draft')

        consumable_line, service_line = self.prescription_line
        ...
        self.assertEqual(self.prescription.amount_total, 725.0)
