...
class TestPrescription(PrescriptionCommon):
    ...
    def test_computes_auto_fill(self):
        ...
        order = self.env['prescription'].create({
    ...
    def test_prescription_standard_flow(self):
        self.assertEqual(self.prescription.amount_total, 725.0, 'Prescription: total amount is wrong')
    ...
    def test_prescription_send_to_self(self):
        # when sender(logged in user) is also present in recipients of the mail composer,
        # user should receive mail.
        prescription = self.env['prescription'].with_user(self.prescription_user).create({
            'partner_id': self.prescription_user.partner_id.id,
        })
        email_ctx = prescription.action_quotation_send().get('context', {})
        ...
        prescription.with_context(**email_ctx).with_user(self.prescription_user).message_post_with_source(
            mail_template,
            subtype_xmlid='mail.mt_comment',
        )
        self.assertTrue(prescription.state == 'sent', 'Prescription : state should be changed to sent')
        mail_message = prescription.message_ids[0]
        self.assertEqual(mail_message.author_id, prescription.partner_id, 'Prescription: author should be same as customer')
    ...
    def test_invoice_state_when_ordered_quantity_is_negative(self):
        ...
        prescription._create_invoices(final=True)
        self.assertTrue(prescription.invoice_status == 'invoiced', 'Prescription: The invoicing status of the SO should be "invoiced"')
    ...
    def test_prescription_sequence(self):
        ...
        prescription = self.prescription.copy({'date_order': '2019-01-01'})
        self.assertTrue(prescription.name.startswith('SO/2019/'))
        prescription = self.prescription.copy({'date_order': '2020-01-01'})
        self.assertTrue(prescription.name.startswith('SO/2020/'))
        # In EU/BXL tz, this is actually already 01/01/2020
        prescription = self.prescription.with_context(tz='Europe/Brussels').copy({'date_order': '2019-12-31 23:30:00'})
        self.assertTrue(prescription.name.startswith('SO/2020/'))
    ...
    def test_unlink_cancel(self):
        ...
        so_copy = self.prescription.copy()
        ...
        self.prescription.action_confirm()
        self.assertTrue(self.prescription.state == 'prescription', 'Prescription: SO should be in state "prescription"')
        with self.assertRaises(UserError):
            self.prescription.unlink()

        self.prescription.action_lock()
        self.assertTrue(self.prescription.state == 'prescription')
        self.assertTrue(self.prescription.locked)
        with self.assertRaises(UserError):
            self.prescription.unlink()
    ...
    def test_compute_packaging_00(self):
        ...
        so2 = self.env['prescription'].create({
            'partner_id': self.partner.id,
        })
        so2_form = Form(so2)
    ...
    def test_compute_packaging_01(self):
        ...
        so2 = self.env['prescription'].with_company(company2).create({
            'partner_id': self.partner.id,
        })
        so2_form = Form(so2)
        with so2_form.order_line.new() as line:
    ...
    def _create_prescription(self):
        ...
        return self.env['prescription'].with_context(
            default_prescription_template_id=False
            # Do not modify test behavior even if prescription_management is installed
        ).create({
            'partner_id': self.partner.id,
        })
    ...
    def test_invoicing_terms(self):
        ...
        self.assertEqual(prescription.note, "<p>Coin coin</p>")

        # Html invoice terms (/terms page)
        self.env.company.terms_type = 'html'
        prescription = self._create_prescription()
        self.assertTrue(prescription.note.startswith("<p>Terms &amp; Conditions: "))
    ...
    def test_validity_days(self):
        ...
        with freeze_time("2020-05-02"):
            ...
            self.assertEqual(prescription.validity_date, fields.Date.today() + timedelta(days=5))
    ...
    def test_so_names(self):
        ...
        """
        Prescription = self.env['prescription'].with_context(prescription_show_partner_name=True)

        res = Prescription.name_search(name=self.prescription.partner_id.name)
        self.assertEqual(res[0][0], self.prescription.id)

        self.assertNotIn(self.prescription.partner_id.name, self.prescription.display_name)
        self.assertIn(
            self.prescription.partner_id.name,
            self.prescription.with_context(prescription_show_partner_name=True).display_name)

    def test_state_changes(self):
        """Test some untested state changes methods & logic."""
        self.prescription.action_quotation_sent()

        self.assertEqual(self.prescription.state, 'sent')
        self.assertIn(self.prescription.partner_id, self.prescription.message_follower_ids.partner_id)

        self.env.user.groups_id += self.env.ref('prescription.group_auto_done_setting')
        self.prescription.action_confirm()
        self.assertEqual(self.prescription.state, 'prescription')
        self.assertTrue(self.prescription.locked)
        with self.assertRaises(UserError):
            self.prescription.action_confirm()

        self.prescription.action_unlock()
        self.assertEqual(self.prescription.state, 'prescription')

    def test_sol_name_search(self):
        # Shouldn't raise
        self.env['prescription']._search([('order_line', 'ilike', 'product')])

        name_search_data = self.env['prescription.line'].name_search(name=self.prescription.name)
        sol_ids_found = dict(name_search_data).keys()
        self.assertEqual(list(sol_ids_found), self.prescription_line.ids)
    ...
    def test_zero_quantity(self):
        ...
        """
        order_line = self.prescription_line[0]
    ...
    def test_discount_rounding(self):
        ...
        self.assertEqual(prescription_line.price_subtotal, 49.44, "Subtotal should be equal to 192 * (1 - 0.7425)")
        self.assertEqual(prescription_line.discount, 74.25)
    ...
    def test_tax_amount_rounding(self):
        ...
        self.assertEqual(prescription.amount_total, 15.42, "")
        ...
        self.assertEqual(prescription.amount_total, 15.41, "")
...
class TestPrescriptionTeam(PrescriptionCommon):
    ...
    def test_assign_prescription_team_from_partner_user(self):
        ...
        self.assertEqual(prescription.team_id.id, self.prescription_team.id, 'Should assign to team of prescription person')
    ...
    def test_assign_prescription_team_from_partner_team(self):
        ...
        self.assertEqual(prescription.team_id.id, self.prescription_team_2.id, 'Should assign to team of partner')
    ...
    def test_assign_prescription_team_when_changing_user(self):
        ...
        prescription.user_id = self.user_in_team
        self.assertEqual(prescription.team_id.id, self.prescription_team.id, 'Should assign to team of prescription person')
    ...
    def test_keep_prescription_team_when_changing_user_with_no_team(self):
        ...
        prescription.user_id = self.user_not_in_team
        self.assertEqual(prescription.team_id.id, self.prescription_team_2.id, 'Should not reset the team to default')

    def test_prescription_analytic_distribution_change(self):
        ...
        sol = self.env['prescription.line'].create({
        ...
        so_no_analytic_account = self.env['prescription'].create({
            'partner_id': self.env.ref('base.res_partner_1').id,
        })
        sol_no_analytic_account = self.env['prescription.line'].create({
            'name': super_product.name,
            'product_id': super_product.id,
        ...
        prescription.action_confirm()
        sol_on_confirmed_order = self.env['prescription.line'].create({
            'name': super_product.name,
            'product_id': super_product.id,
            'order_id': prescription.id,
        })
    ...
    def test_cannot_assign_tax_of_mismatch_company(self):
        ...
        sol = self.env['prescription.line'].sudo().create({
            'name': product.name,
    ...
    def test_downpayment_amount_constraints(self):
        ...
        self.prescription.require_payment = True
        with self.assertRaises(ValidationError):
            self.prescription.prepayment_percent = -1
        with self.assertRaises(ValidationError):
            self.prescription.prepayment_percent = 1.01
