...
class TestPrescriptionDiscount(PrescriptionCommon):

    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.wizard = cls.env['prescription.discount'].create({
    ...
    def test_amount(self):
        ...
        discount_line = self.prescription_line[-1]
    ...
    def test_so_discount(self):
        solines = self.prescription_line
        amount_before_discount = self.prescription.amount_total
        ...
        discount_line = self.prescription_line - solines
        ...
        self.wizard.action_apply_discount()
        discount_lines = self.prescription_line - solines
    ...
    def test_sol_discount(self):
        so_amount = self.prescription.amount_untaxed
        self.wizard.write({
            'discount_percentage': 0.5,  # 50%
        ...
        self.assertTrue(
            all(line.discount == 50 for line in self.prescription_line)
        )
        self.assertAlmostEqual(self.prescription.amount_untaxed, so_amount*0.5)
