...
class TestPrescriptionPrices(PrescriptionCommon):
    ...
    def test_pricelist_dates(self):
        ...
        with freeze_time(today):
            ...
            order_line = self.env['prescription.line'].create({
    ...
    def test_no_pricelist_rules(self):
        ...
        with freeze_time('2022-08-19'):
            ...
            order_in_other_currency = self.env['prescription'].create({
    ...
    def test_multi_currency_discount(self):
        ...
        prescription = product_1_ctxt.with_context(mail_notrack=True, mail_create_nolog=True).env['prescription'].create({
    ...
    def test_update_prices(self):
        ...
        so_amount = prescription.amount_total
        start_so_amount = so_amount
        prescription._recompute_prices()
        self.assertEqual(
            prescription.amount_total, so_amount,
            "Updating the prices of an unmodified SO shouldn't modify the amounts")

        pricelist = prescription.pricelist_id
        pricelist.item_ids = [
        ...
        self.assertTrue(all(line.discount == 5 for line in prescription_line))
        self.assertEqual(prescription.amount_undiscounted, so_amount)
        self.assertEqual(prescription.amount_total, 0.95*so_amount)

        pricelist.discount_policy = "with_discount"
        prescription._recompute_prices()

        self.assertTrue(all(line.discount == 0 for line in prescription_line))
        self.assertEqual(prescription.amount_undiscounted, so_amount)
        self.assertEqual(prescription.amount_total, 0.95*so_amount)

        # Test taking off the pricelist
        prescription.pricelist_id = False
        prescription._recompute_prices()

        self.assertTrue(all(line.discount == 0 for line in prescription_line))
        self.assertEqual(prescription.amount_undiscounted, so_amount)
        self.assertEqual(
            prescription.amount_total, start_so_amount,
    ...
    def test_fiscalposition_application(self):
        ...
        prescription = order_form.save()
        self.assertRecordValues(prescription_line, [{'price_unit': 106, 'price_subtotal': 100}])
        ...
        with order_form.order_line.new() as line:
            ...
            line.product_uom_qty = 1.0
        prescription = order_form.save()
        self.assertRecordValues(prescription_line, [{'price_unit': 100, 'price_subtotal': 94.34}])
        ...
        with order_form.order_line.new() as line:
            ...
            line.product_uom_qty = 1.0
        prescription = order_form.save()
        self.assertRecordValues(prescription_line, [{'price_unit': 100, 'price_subtotal': 100}])
        ...
        with order_form.order_line.new() as line:
            ...
            line.product_uom_qty = 1.0
        prescription = order_form.save()
        self.assertRecordValues(prescription_line, [{'price_unit': 100, 'price_subtotal': 84.91}])
    ...
    def test_prescription_with_taxes(self):
        ...
        self.prescription_line[0].write({'tax_id': [Command.link(tax_include.id)]})
        self.prescription_line[1].write({'tax_id': [Command.link(tax_exclude.id)]})
        ...
        self.assertAlmostEqual(
            self.prescription.amount_total,
            self.prescription.amount_untaxed + self.prescription.amount_tax,
            places=2)
