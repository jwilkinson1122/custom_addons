...
class PrescriptionTypeMail(models.Model):
    """ Template of prescription.mail to attach to prescription.type. Those will be copied
    upon all prescriptions created in that type to ease prescription creation. """
    _name = 'prescription.type.mail'
    _description = 'Mail Scheduling on Prescription Category'
    ...
    prescription_type_id = fields.Many2one(
        'prescription.type', string='Prescription Type',
    ...
    def _prepare_prescription_mail_values(self):
        self.ensure_one()
        return {
...
class PrescriptionMailScheduler(models.Model):
    """ Prescription automated mailing. This model replaces all existing fields and
    configuration allowing to send emails on prescriptions since Odoo 9. A cron exists
    that periodically checks for mailing to run. """
    _name = 'prescription.mail'
    _rec_name = 'prescription_id'
    _description = 'Prescription Automated Mailing'

    @api.model
    def _selection_template_model(self):
        return [('mail.template', 'Mail')]

    prescription_id = fields.Many2one('prescription.prescription', string='Prescription', required=True, ondelete='cascade')
    ...
    @api.depends('prescription_id.date_begin', 'prescription_id.date_end', 'interval_type', 'interval_unit', 'interval_nbr')
    def _compute_scheduled_date(self):
        for scheduler in self:
            if scheduler.interval_type == 'after_sub':
                date, sign = scheduler.prescription_id.create_date, 1
            elif scheduler.interval_type == 'before_prescription':
                date, sign = scheduler.prescription_id.date_begin, -1
            else:
                date, sign = scheduler.prescription_id.date_end, 1
    ...
    def execute(self):
        for scheduler in self:
            ...
            if scheduler.interval_type == 'after_sub':
                ...
                # before or after prescription -> one shot email
                ...
                if not scheduler.template_ref:
                    continue
                # do not send emails if the mailing was scheduled before the prescription but the prescription is over
                if scheduler.scheduled_date <= now and (scheduler.interval_type != 'before_prescription' or scheduler.prescription_id.date_end > now):
                    scheduler.prescription_id.mail_attendees(scheduler.template_ref.id)
                    scheduler.update({
                        'mail_done': True,
                        'mail_count_done': scheduler.prescription_id.seats_reserved + scheduler.prescription_id.seats_used,
                    })
        return True
    ...
    def _warn_template_error(self, scheduler, exception):
        ...
        if random.random() < 0.1666 or scheduler.interval_unit in ('now', 'hours'):
            ...
            try:
                prescription, template = scheduler.prescription_id, scheduler.template_ref
                emails = list(set([prescription.organizer_id.email, prescription.user_id.email, template.write_uid.email]))
                subject = _("WARNING: Prescription Scheduler Error for prescription: %s", prescription.name)
                body = _("""Prescription Scheduler for:
  - Prescription: %(prescription_name)s (%(prescription_id)s)
  - Scheduled: %(date)s
...
You receive this email because you are:
  - the organizer of the prescription,
  - or the responsible of the prescription,
...
class PrescriptionMailRegistration(models.Model):
    ...
    def execute(self):
        ...
        for reg_mail in todo:
            organizer = reg_mail.scheduler_id.prescription_id.organizer_id
