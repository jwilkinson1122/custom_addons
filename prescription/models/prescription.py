...
class Prescription(models.Model):
    ...
    user_id = fields.Many2one(
        ...
        domain=lambda self: "[('groups_id', '=', {}), ('share', '=', False), ('company_ids', '=', company_id)]".format(
            self.env.ref("prescription_team.group_prescription_personnel").id
        ))
    ...
    transaction_ids = fields.Many2many(
        ...
        relation='prescription_transaction_rel', column1='prescription_id', column2='transaction_id',
        string="Transactions",
    ...
    tag_ids = fields.Many2many(
        ...
        relation='prescription_tag_rel', column1='order_id', column2='tag_id',
        string="Tags")
    ...
    def init(self):
        create_index(self._cr, 'prescription_date_order_id_idx', 'prescription', ["date_order desc", "id desc"])
        ...
        for order in self:
            if order.partner_id and not (order._origin.id and order.user_id):
                ...
                order.user_id = (
                    ...
                    or (self.user_has_groups('prescription_team.group_prescription_personnel') and self.env.user)
                )
        ...
        if operator == 'in' and value:
            self.env.cr.execute("""
                SELECT array_agg(so.id)
                    FROM prescription so
                    JOIN prescription_line sol ON sol.order_id = so.id
                    JOIN prescription_line_invoice_rel soli_rel ON soli_rel.order_line_id = sol.id
    ...
    def _compute_invoice_status(self):
        ...
        line_invoice_status_all = [
            ...
            for order, invoice_status in self.env['prescription.line']._read_group([
    ...
    def _compute_expected_date(self):
        """ For service and consumable, we only take the min dates. This method is extended in prescription_stock to
        ...
        for vals in vals_list:
            ...
            if vals.get('name', _("New")) == _("New"):
                ...
                vals['name'] = self.env['ir.sequence'].next_by_code(
                    'prescription', sequence_date=seq_date) or _("New")
    ...
    def action_open_discount_wizard(self):
        ...
        return {
            ...
            'res_model': 'prescription.discount',
            'view_mode': 'form',
            'target': 'new',
        }
    ...
    def _find_mail_template(self):
        ...
        if self.env.context.get('proforma') or self.state != 'prescription':
            return self.env.ref('prescription.email_template_edi_prescription', raise_if_not_found=False)
        else:
    ...
    def _get_confirmation_template(self):
        ...
        default_confirmation_template_id = self.env['ir.config_parameter'].sudo().get_param(
            'prescription.default_confirmation_template'
        )
        ...
        if default_confirmation_template:
            ...
            return self.env.ref('prescription.mail_template_prescription_confirmation', raise_if_not_found=False)
    ...
    def action_confirm(self):
        ...
        for order in self:
            order.validate_taxes_on_prescription()
        ...
        if self.env.user.has_group('prescription.group_auto_done_setting'):
            self.action_lock()
    ...
    def _send_payment_succeeded_for_order_mail(self):
        ...
        mail_template = self.env.ref(
            'prescription.mail_template_prescription_payment_executed', raise_if_not_found=False
        )
        for order in self:
            order._send_order_notification_mail(mail_template)
    ...
    def action_cancel(self):
        ...
        if cancel_warning:
            ...
            template_id = self.env['ir.model.data']._xmlid_to_res_id(
                'prescription.mail_template_prescription_cancellation', raise_if_not_found=False
            )
            lang = self.env.context.get('lang')
            ...
            return {
                ...
                'view_mode': 'form',
                'res_model': 'prescription.cancel',
                'view_id': self.env.ref('prescription.prescription_cancel_view_form').id,
                'type': 'ir.actions.act_window',
                'context': ctx,
                'target': 'new'
            }
    ...
    def _show_cancel_wizard(self):
        """ Decide whether the prescription.cancel wizard should be shown to cancel specified orders.
    ...
    def action_preview_prescription(self):
        self.ensure_one()
        return {
    ...
    def _default_order_line_values(self):
        ...
        new_default_data = self.env['prescription.line']._get_product_catalog_lines_data()
    ...
    def _get_invoiceable_lines(self, final=False):
        ...
        return self.env['prescription.line'].browse(invoiceable_line_ids + down_payment_line_ids)
        ...
        if len(invoice_vals_list) < len(self):
            ...
            for invoice in invoice_vals_list:
                ...
                for line in invoice['invoice_line_ids']:
                    line[2]['sequence'] = PrescriptionLine._get_invoice_line_sequence(new=sequence, old=line[2]['sequence'])
                    sequence += 1
    ...
    def message_post(self, **kwargs):
        ...
        return super(Prescription, self.with_context(**so_ctx)).message_post(**kwargs)
    ...
    def _track_subtype(self, init_values):
        ...
        if 'state' in init_values and self.state == 'prescription':
            return self.env.ref('prescription.mt_order_confirmed')
        elif 'state' in init_values and self.state == 'sent':
            return self.env.ref('prescription.mt_order_sent')
    ...
    def _get_portal_return_action(self):
        ...
        self.ensure_one()
        return self.env.ref('prescription.action_quotations_with_onboarding')

    def _get_name_portal_content_view(self):
        """ This method can be inherited by localizations who want to localize the online quotation view. """
        self.ensure_one()
        return 'prescription.prescription_portal_content'
    ...
    def _get_name_tax_totals_view(self):
        ...
        return 'prescription.document_tax_totals'
    ...
    def _create_upsell_activity(self):
        ...
        self.activity_unlink(['prescription.mail_act_prescription_upsell'])
        for order in self:
            order_ref = order._get_html_link()
            customer_ref = order.partner_id._get_html_link()
            order.activity_schedule(
                'prescription.mail_act_prescription_upsell',
    ...
    def _get_product_catalog_record_lines(self, product_ids):
        grouped_lines = defaultdict(lambda: self.env['prescription.line'])
        for line in self.order_line:
    ...
    def _filter_product_documents(self, documents):
        return documents.filtered(
            lambda document:
                ...
                or (self.state == 'prescription' and document.attached_on == 'prescription')
        )
    ...
    def validate_taxes_on_prescription(self):
