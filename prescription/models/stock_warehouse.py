...
class StockWarehouse(models.Model):
    ...
    prescription_type_id = fields.Many2one('stock.picking.type', 'Prescription Operation Type', check_company=True)

    def _get_sequence_values(self, name=False, code=False):
        values = super(StockWarehouse, self)._get_sequence_values(name=name, code=code)
        values.update({
            'prescription_type_id': {
                'name': self.name + ' ' + _('Sequence prescription'),
                'prefix': self.code + '/RX/',
                'padding': 5,
    ...
    def _get_picking_type_update_values(self):
        ...
        data.update({
            'prescription_type_id': {
                'active': self.active,
                'barcode': self.code.replace(" ", "").upper() + "-RX",
            },
        })
        return data
