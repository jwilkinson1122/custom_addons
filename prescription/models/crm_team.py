...
class CrmTeam(models.Model):
    ...
    prescription_count = fields.Integer(compute='_compute_prescription_count', string='# Prescription Orders')

    def _compute_quotations_to_invoice(self):
        query = self.env['prescription']._where_calc([
            ('team_id', 'in', self.ids),
            ('state', 'in', ['draft', 'sent']),
        ])
        self.env['prescription']._apply_ir_rules(query, 'read')
    ...
    def _compute_prescription_to_invoice(self):
        prescription_data = self.env['prescription']._read_group([
            ('team_id', 'in', self.ids),
            ('invoice_status','=','to invoice'),
        ], ['team_id'], ['__count'])
        data_map = {team.id: count for team, count in prescription_data}
        for team in self:
    ...
    def _compute_prescription_count(self):
        ...
        data_map = {team.id: count for team, count in prescription_data}
        for team in self:
            team.prescription_count = data_map.get(team.id, 0)
    ...
    def action_primary_channel_button(self):
        if self._in_prescription_scope():
            return self.env["ir.actions.actions"]._for_xml_id("prescription.action_order_report_so_prescription_team")
    ...
    def _unlink_except_used_for_prescription(self):
        ...
        SO_COUNT_TRIGGER = 5
        for team in self:
            if team.prescription_count >= SO_COUNT_TRIGGER:
                raise UserError(
                    _('Team %(team_name)s has %(prescription_count)s active prescription orders. Consider canceling them or archiving the team instead.',
                      team_name=team.name,
                      prescription_count=team.prescription_count
                      ))
