...
class AccountMoveLine(models.Model):
    ...
    def _prescription_create_reinvoice_prescription_line(self):
        ...
        prescription_map = self._prescription_determine_order()
        ...
        #   - value is either a prescription.line record (existing case), or an integer representing the index of the prescription line to create in
        ...
        for move_line in self:
            prescription = prescription_map.get(move_line.id)
            ...
            if prescription.state in ('draft', 'sent'):
                raise UserError(_(
                    ...
                    account=prescription.analytic_account_id.name,
                ))
            elif prescription.state == 'cancel':
                raise UserError(_(
            ...
            price = move_line._prescription_get_invoice_price(prescription)
            ...
            if move_line.product_id.expense_policy == 'prescription_price' and move_line.product_id.invoice_policy == 'delivery':  # for those case only, we can try to reuse one
                map_entry_key = (prescription.id, move_line.product_id.id, price)  # cache entry to limit the call to search
                prescription_line = existing_prescription_line_cache.get(map_entry_key)
                if prescription_line:  # already search, so reuse it. prescription_line can be prescription.line record or index of a "to create values" in `prescription_line_values_to_create`
                    ...
                    if prescription_line:  # found existing one, so keep the browse record
                        ...
                        prescription_line_values_to_create.append(move_line._prescription_prepare_prescription_line_values(prescription, price))
        ...
        new_prescription_lines = self.env['prescription.line'].create(prescription_line_values_to_create)

        # build result map by replacing index with newly created record of prescription.line
        result = {}
        for move_line_id, unknown_prescription_line in map_move_prescription_line.items():
            ...
            elif isinstance(unknown_prescription_line, models.BaseModel):  # already record of prescription.line
                result[move_line_id] = unknown_prescription_line
        return result

    def _prescription_determine_order(self):
        """ Get the mapping of move.line with the prescription record on which its analytic entries should be reinvoiced
            :return a dict where key is the move line id, and value is prescription record (or None).
        """
        mapping = {}
        for move_line in self:
            if move_line.analytic_distribution:
                distribution_json = move_line.analytic_distribution
                prescription = self.env['prescription'].search([('analytic_account_id', 'in', list(int(account_id) for account_id in distribution_json.keys())),
                                                            ('state', '=', 'prescription')], order='create_date ASC', limit=1)
                if prescription:
                    mapping[move_line.id] = prescription
                else:
                    prescription = self.env['prescription'].search([('analytic_account_id', 'in', list(int(account_id) for account_id in distribution_json.keys()))], order='create_date ASC', limit=1)
                    mapping[move_line.id] = prescription
    ...
    def _prescription_prepare_prescription_line_values(self, order, price):
        ...
        self.ensure_one()
        last_so_line = self.env['prescription.line'].search([('order_id', '=', order.id)], order='sequence desc', limit=1)
