...
class AccountMove(models.Model):
    ...
    prescription_count = fields.Integer(compute="_compute_origin_so_count", string='Prescription Order Count')
    ...
    def _compute_origin_so_count(self):
        for move in self:
            move.prescription_count = len(move.line_ids.prescription_line_ids.order_id)
    ...
    def action_view_source_prescriptions(self):
        self.ensure_one()
        source_orders = self.line_ids.prescription_line_ids.order_id
        result = self.env['ir.actions.act_window']._for_xml_id('prescription.action_prescription')
        if len(source_orders) > 1:
            result['domain'] = [('id', 'in', source_orders.ids)]
        elif len(source_orders) == 1:
            result['views'] = [(self.env.ref('prescription.view_order_form', False).id, 'form')]
    ...
    def _compute_partner_credit(self):
        ...
        for move in self.filtered(lambda m: m.is_invoice(include_receipts=True)):
            prescriptions = move.line_ids.prescription_line_ids.order_id
            ...
            amount_to_invoice_currency = sum(
                prescription.currency_id._convert(
                    prescription.amount_to_invoice,
                    move.company_currency_id,
                    move.company_id,
                    move.date
