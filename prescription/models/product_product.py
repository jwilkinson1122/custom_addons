...
class ProductProduct(models.Model):
    ...
    product_catalog_product_is_in_prescription = fields.Boolean( compute='_compute_product_is_in_prescription', search='_search_product_is_in_prescription',
    )
    ...
    def _compute_prescription_count(self):
        ...
        if not self.user_has_groups('prescription_team.group_prescription_personnel'):
            return r
    ...
    def _compute_product_is_in_prescription(self):
        order_id = self.env.context.get('order_id')
        if not order_id:
            self.product_catalog_product_is_in_prescription = False
            return

        read_group_data = self.env['prescription.line']._read_group( domain=[('order_id', '=', order_id)], groupby=['product_id'], aggregates=['__count'],
        )
        data = {product.id: count for product, count in read_group_data}
        for product in self:
            product.product_catalog_product_is_in_prescription = bool(data.get(product.id, 0))

    def _search_product_is_in_prescription(self, operator, value):
        if operator not in ['=', '!='] or not isinstance(value, bool):
            raise UserError(_("Operation not supported"))
        product_ids = self.env['prescription.line'].search([
            ('order_id', 'in', [self.env.context.get('order_id', '')]),
        ]).product_id.ids
        return [('id', 'in', product_ids)]

    def action_view_prescription(self):
        action = self.env["ir.actions.actions"]._for_xml_id("prescription.report_all_channels_prescription_action")
    ...
    def _filter_to_unlink(self):
        ...
        lines = self.env['prescription.line']._read_group(domain, ['product_id'])
...
class ProductAttributeCustomValue(models.Model):
    ...
    _sql_constraints = [
        ('sol_custom_value_unique', 'unique(custom_product_template_attribute_value_id, prescription_line_id)', "Only one Custom Value is allowed per Attribute Value per Prescription Order Line.")
    ]
