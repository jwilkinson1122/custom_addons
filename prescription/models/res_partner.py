...
class ResPartner(models.Model):
    ...
    prescription_count = fields.Integer(compute='_compute_prescription_count', string='Prescription Order Count')
    prescription_ids = fields.One2many('prescription', 'partner_id', 'Prescription Order')
    ...
    @api.model
    def _get_prescription_domain_count(self):
        return []
    ...
    def _compute_prescription_count(self):
        ...
        prescription_groups = self.env['prescription']._read_group(
            domain=expression.AND([self._get_prescription_domain_count(), [('partner_id', 'in', all_partners.ids)]]),
            groupby=['partner_id'], aggregates=['__count']
        )
        self_ids = set(self._ids)

        self.prescription_count = 0
        for partner, count in prescription_groups:
            while partner:
                if partner.id in self_ids:
                    partner.prescription_count += count
                partner = partner.parent_id

    def _has_order(self, partner_domain):
        self.ensure_one()
        prescription = self.env['prescription'].sudo().search(
            expression.AND([
                partner_domain,
                [
    ...
    def action_view_prescription(self):
        action = self.env['ir.actions.act_window']._for_xml_id('prescription.act_res_partner_2_prescription')
    ...
    def _compute_credit_to_invoice(self):
        ...
        group = self.env['prescription']._read_group(domain, ['partner_id'], ['amount_to_invoice:sum'])
    ...
    def unlink(self):
        ...
        self.env['prescription'].sudo().search([
