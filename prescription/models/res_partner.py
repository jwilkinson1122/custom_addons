...
class ResPartner(models.Model):
    ...
    prescription_count = fields.Integer(
        '# Prescriptions', compute='_compute_prescription_count', groups='prescription.group_prescription_confirmation_desk',
        help='Number of prescriptions the partner has participated.')

    def _compute_prescription_count(self):
        self.prescription_count = 0
        for partner in self:
            partner.prescription_count = self.env['prescription.prescription'].search_count([('confirmation_ids.partner_id', 'child_of', partner.ids)])
