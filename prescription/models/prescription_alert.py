...
from .prescription_partner import float_to_time
...
class PrescriptionAlert(models.Model):
    """ Alerts to display during a prescription order. An alert can be specific to a
    given day, weekly or daily. The alert is displayed from start to end hour. """
    _name = 'prescription.alert'
    _description = 'Prescription Alert'
    ...
    location_ids = fields.Many2many('prescription.location', string='Location')
    ...
    def _sync_cron(self):
        ...
        for alert in self:
            ...
            cron.name = f"Prescription: alert chat notification ({alert.name})"
    ...
    def create(self, vals_list):
        crons = self.env['ir.cron'].sudo().create([
            {
                ...
                'numbercall': -1,
                'doall': False,
                'name': "Prescription: alert chat notification",
        ...
        self.env['ir.model.data'].sudo().create([{
            'name': f'prescription_alert_cron_sa_{cron.ir_actions_server_id.id}',
            'module': 'prescription',
    ...
    def _notify_chat(self):
        ...
        if self.location_ids.ids:
            order_domain = expression.AND([order_domain, [('user_id.last_prescription_location_id', 'in', self.location_ids.ids)]])
        ...
        partners = self.env['prescription.order'].search(order_domain).user_id.partner_id
        if partners:
