...
from odoo.addons.prescription.models.prescription import SALE_ORDER_STATE
...
class PrescriptionReport(models.Model):
    ...
    order_reference = fields.Reference(string='Related Order', selection=[('prescription', 'Prescription Order')], group_operator="count_distinct")
    ...
    def _with_prescription(self):
        return ""

    def _select_prescription(self):
        select_ = f"""
            MIN(l.id) AS id,
            ...
            concat('prescription', ',', s.id) AS order_reference"""
    ...
    def _from_prescription(self):
        return """
            prescription_line l
            LEFT JOIN prescription s ON s.id=l.order_id
    ...
    def _query(self):
        with_ = self._with_prescription()
        return f"""
            {"WITH" + with_ + "(" if with_ else ""}
            SELECT {self._select_prescription()}
            FROM {self._from_prescription()}
            WHERE {self._where_prescription()}
            GROUP BY {self._group_by_prescription()}
            {")" if with_ else ""}
        """
