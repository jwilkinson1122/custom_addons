...
class CashmoveReport(models.Model):
    _name = "prescription.cashmove.report"
        ...
        self._cr.execute("""
            CREATE or REPLACE view %s as (
                ...
                SELECT
                    ...
                    format('Order: %%s x %%s %%s', lol.quantity::text, lp.name, lol.display_options) as description
                FROM prescription_order lol
                JOIN prescription_product lp ON lp.id = lol.product_id
                WHERE
