...
class PrescriptionAdvancePaymentInv(models.TransientModel):
    ...
    prescription_ids = fields.Many2many(
        'prescription', default=lambda self: self.env.context.get('active_ids'))
    ...
    amount_to_invoice = fields.Monetary(
        ...
        compute="_compute_invoice_amounts",
        help="The amount to invoice = Prescription Order Total - Confirmed Down Payments.")
    ...
    def _compute_count(self):
        for wizard in self:
            wizard.count = len(wizard.prescription_ids)

    @api.depends('prescription_ids')
    def _compute_has_down_payments(self):
        for wizard in self:
            wizard.has_down_payments = bool(
                wizard.prescription_ids.order_line.filtered('is_downpayment')
            )
    ...
    def _compute_currency_id(self):
        ...
        for wizard in self:
            if wizard.count == 1:
                wizard.currency_id = wizard.prescription_ids.currency_id
    ...
    def _compute_company_id(self):
        ...
        for wizard in self:
            if wizard.count == 1:
                wizard.company_id = wizard.prescription_ids.company_id
    ...
    def _compute_display_invoice_amount_warning(self):
        for wizard in self:
            ...
            if wizard.advance_payment_method == 'percentage':
                invoice_amount = wizard.amount / 100 * sum(wizard.prescription_ids.mapped('amount_total'))
    ...
    def _compute_display_draft_invoice_warning(self):
        for wizard in self:
            wizard.display_draft_invoice_warning = wizard.prescription_ids.invoice_ids.filtered(lambda invoice: invoice.state == 'draft')

    @api.depends('prescription_ids')
    def _compute_invoice_amounts(self):
        for wizard in self:
            wizard.amount_invoiced = sum(wizard.prescription_ids._origin.mapped('amount_invoiced'))
            wizard.amount_to_invoice = sum(wizard.prescription_ids._origin.mapped('amount_to_invoice'))
    ...
    def create_invoices(self):
        ...
        invoices = self._create_invoices(self.prescription_ids)
        return self.prescription_ids.action_view_invoice(invoices=invoices)
    ...
    def view_draft_invoices(self):
        return {
            ...
            'domain': [('line_ids.prescription_line_ids.order_id', 'in', self.prescription_ids.ids), ('state', '=', 'draft')],
        }

    #=== BUSINESS METHODS ===#

    def _create_invoices(self, prescriptions):
        self.ensure_one()
        if self.advance_payment_method == 'delivered':
            return prescriptions._create_invoices(final=self.deduct_down_payments, grouped=not self.consolidated_billing)
        else:
            self.prescription_ids.ensure_one()
            self = self.with_company(self.company_id)
            order = self.prescription_ids
            ...
            Prescriptionline = self.env['prescription.line'].with_context(prescription_no_log_for_new_lines=True)
            ...
            down_payment_lines = Prescriptionline.create(
