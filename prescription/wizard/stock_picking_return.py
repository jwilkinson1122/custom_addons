...
class ReturnPicking(models.TransientModel):
    ...
    create_prescription = fields.Boolean(string="Create Prescription")
    picking_type_code = fields.Selection(related="picking_id.picking_type_id.code")
    prescription_location_ids = fields.Many2many(
        comodel_name="stock.location", compute="_compute_prescription_location_id"
    )
    # Expand domain for Prescription
    location_id = fields.Many2one(
        domain="create_prescription and [('id', 'child_of', prescription_location_ids)]"
        "or "
    ...
    def _compute_prescription_location_id(self):
        for record in self:
            record.prescription_location_ids = (
                self.env["stock.warehouse"]
                .search([("company_id", "=", record.picking_id.company_id.id)])
                .prescription_loc_id
            )

    @api.onchange("create_prescription")
    def _onchange_create_prescription(self):
        if self.create_prescription:
            warehouse = self.picking_id.picking_type_id.warehouse_id
            self.location_id = warehouse.prescription_loc_id.id
            ...
            # If self.create_prescription is not True, the value of the location will be the
    ...
    def create_returns(self):
        ...
        'confirmed' Prescription after return a delivery picking in case
        'Create Prescription' checkbox is checked in this wizard.
        New Prescription will be linked to the delivery picking as the origin
        delivery and also Prescription will be linked to the returned picking
        as the 'Receipt'.
        """
        if self.create_prescription:
            ...
            self_with_context = self.with_context(set_prescription_picking_type=True)
            ...
            if not self.picking_id.partner_id:
                raise ValidationError(
                    _(
                        ...
                        "'Stock Picking' from which Prescription will be created"
                    )
                )
            returned_picking = self.env["stock.picking"].browse(res["res_id"])
            vals_list = [
                move._prepare_return_prescription_vals(self.picking_id)
                for move in returned_picking.move_ids
            ]
            self.env["prescription"].create(vals_list)
            return res
        else:
