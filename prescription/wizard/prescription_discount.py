...
class PrescriptionDiscount(models.TransientModel):
    _name = 'prescription.discount'
    _description = "Discount Wizard"

    prescription_id = fields.Many2one(
        'prescription', default=lambda self: self.env.context.get('active_id'), required=True)
    company_id = fields.Many2one(related='prescription_id.company_id')
    ...
    def _prepare_discount_line_values(self, product, amount, taxes, description=None):
        ...
        vals = {
            'order_id': self.prescription_id.id,
    ...
    def _create_discount_lines(self):
        ...
        if self.discount_type == 'amount':
            ...
            for line in self.prescription_id.order_line:
        ...
        return self.env['prescription.line'].create(vals_list)

    def action_apply_discount(self):
        self.ensure_one()
        self = self.with_company(self.company_id)
        if self.discount_type == 'sol_discount':
            self.prescription_id.order_line.write({'discount': self.discount_percentage*100})
        else:
