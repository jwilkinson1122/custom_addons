odoo.define('prescription.prescriptionKanbanTests', function (require) {
"use strict";
...
QUnit.module('PrescriptionKanbanView', {
    beforeEach() {
    ...
    QUnit.test('basic rendering', async function (assert) {
        ...
        assert.containsOnce(kanban, '.o_prescription_content > span > .o_prescription_banner',
    ...
    QUnit.test('no flickering at reload', async function (assert) {
        ...
        const kanban = await createPrescriptionView({
            ...
            mockRPC: function (route, args) {
                if (route === '/prescription/user_location_get') {
                    return Promise.resolve(self.data['prescription.location'].records[0].id);
                }
                ...
                if (args.method === 'xmlid_to_res_id') {
                    // delay the rendering of the prescription widget
        ...
        assert.strictEqual(kanban.$('.o_prescription_widget').length, 1,
    ...
    QUnit.module('PrescriptionWidget', function () {
        ...
        QUnit.test('empty cart', async function (assert) {
            ...
            const $kanbanWidget = kanban.$('.o_prescription_widget');

            assert.containsN($kanbanWidget, '> .o_prescription_widget_info', 3,
                "should have 3 columns");
            assert.isVisible($kanbanWidget.find('> .o_prescription_widget_info:first'),
                "should have the first column visible");
            assert.strictEqual($kanbanWidget.find('> .o_prescription_widget_info:not(:first)').html().trim(), "",
        ...
        QUnit.test('non-empty cart', async function (assert) {
            ...
            assert.containsOnce($kanbanWidget, '.o_prescription_widget_info:eq(1)',
                "should have a second column");

            const $widgetSecondColumn = $kanbanWidget.find('.o_prescription_widget_info:eq(1)');

            assert.containsOnce($widgetSecondColumn, '.o_prescription_widget_unlink',
            ...
            const $firstLine = $widgetSecondColumn.find('.o_prescription_widget_lines > li:first');
            ...
            assert.containsOnce($firstLine, '.o_prescription_product_quantity > :eq(1)',
                "should have the line's quantity");
            assert.strictEqual($firstLine.find('.o_prescription_product_quantity > :eq(1)').text().trim(), "1",
                "should have 1 as the line's quantity");
            assert.containsOnce($firstLine, '.o_prescription_open_wizard',
                "should have the line's product name to open the wizard");
            assert.strictEqual($firstLine.find('.o_prescription_open_wizard').text().trim(), "Tuna sandwich",
            ...
            assert.containsOnce($kanbanWidget, '.o_prescription_widget_info:eq(2)',
                "should have a third column");

            const $widgetThirdColumn = kanban.$('.o_prescription_widget .o_prescription_widget_info:eq(2)');
            ...
            assert.containsOnce($widgetThirdColumn, '.o_prescription_widget_order_button',
                "should have a button to validate the order");
            assert.strictEqual($widgetThirdColumn.find('.o_prescription_widget_order_button').text().trim(), "Order now",
        ...
        QUnit.test('ordered cart', async function (assert) {
            ...
            assert.containsOnce($widgetSecondColumn, '.o_prescription_widget_unlink',
                "should have a button to clear the order");
            assert.containsOnce($widgetSecondColumn, '.badge.badge-warning.o_prescription_ordered',
                "should have an ordered state badge");
            assert.strictEqual($widgetSecondColumn.find('.o_prescription_ordered').text().trim(), "Ordered",
            ...
            assert.strictEqual($kanbanWidget.find('> .o_prescription_widget_info:eq(2)').html().trim(), "",
        ...
        QUnit.test('confirmed cart', async function (assert) {
            ...
            assert.containsNone($widgetSecondColumn, '.o_prescription_widget_unlink',
                "shouldn't have a button to clear the order");
            assert.containsOnce($widgetSecondColumn, '.badge.badge-success.o_prescription_confirmed',
                "should have a confirmed state badge");
            assert.strictEqual($widgetSecondColumn.find('.o_prescription_confirmed').text().trim(), "Received",
            ...
            assert.containsOnce($firstLine, '.o_prescription_product_quantity',
                "should have the line's quantity");
            assert.strictEqual($firstLine.find('.o_prescription_product_quantity').text().trim(), "1",
                "should have 1 as the line's quantity");
            assert.containsOnce($firstLine, '.o_prescription_open_wizard',
                "should have the line's product name to open the wizard");
            assert.strictEqual($firstLine.find('.o_prescription_open_wizard').text().trim(), "Tuna sandwich",
        ...
        QUnit.test('regular user', async function (assert) {
            ...
            assert.containsOnce($kanbanWidget, '.o_prescription_widget_info:first',
                "should have a first column");

            const $widgetFirstColumn = $kanbanWidget.find('.o_prescription_widget_info:first');

            assert.containsOnce($widgetFirstColumn, 'img.rounded-circle',
                "should have a rounded avatar image");

            assert.containsOnce($widgetFirstColumn, '.o_prescription_user_field',
                "should have a user field");
            assert.containsNone($widgetFirstColumn, '.o_prescription_user_field > .o_field_widget',
                "shouldn't have a field widget in the user field");
            assert.strictEqual($widgetFirstColumn.find('.o_prescription_user_field').text().trim(), "Marc Demo",
                "should have 'Marc Demo' in the user field");

            assert.containsOnce($widgetFirstColumn, '.o_prescription_location_field',
                "should have a location field");
            assert.containsOnce($widgetFirstColumn, '.o_prescription_location_field > .o_field_many2one[name="locations"]',
            ...
            assert.containsOnce($widgetFirstColumn, '.o_prescription_location_field + div',
                "should have an account balance");
            assert.strictEqual($widgetFirstColumn.find('.o_prescription_location_field + div .o_field_monetary').text().trim(), "36.50€",
        ...
        QUnit.test('manager user', async function (assert) {
            ...
            assert.containsOnce($widgetFirstColumn, '.o_prescription_user_field',
                "should have a user field");
            assert.containsOnce($widgetFirstColumn, '.o_prescription_user_field > .o_field_many2one[name="users"]',
                "shouldn't have a field widget in the user field");
            ...
            assert.containsOnce($widgetFirstColumn, '.o_prescription_location_field + div',
                "should have an account balance");
                assert.strictEqual($widgetFirstColumn.find('.o_prescription_location_field + div .o_field_monetary').text().trim(), "47.60€",
        ...
        QUnit.test('clear order', async function (assert) {
                ...
                mockRPC: function (route) {
                    if (route.startsWith('/prescription')) {
                        ...
                        return mockPrescriptionRPC({
                            ...
                            userLocation: self.data['prescription.location'].records[0].id,
                        }).apply(this, arguments);
                    }
                    return this._super.apply(this, arguments);
                },
            ...
            await testUtils.dom.click($widgetSecondColumn.find('button.o_prescription_widget_unlink'));
        ...
        QUnit.test('validate order: success', async function (assert) {
            ...
            await testUtils.dom.click($widgetThirdColumn.find('button.o_prescription_widget_order_button'));
            ...
            const kanban = await createPrescriptionView({
                ...
                mockRPC: function (route) {
                    if (route.startsWith('/prescription')) {
                        ...
                        if (route === '/prescription/payment_message') {
