odoo.define('prescription.PrescriptionWidget', function (require) {
"use strict";
...
var PrescriptionMany2One = FieldMany2One.extend({
    start: function () {
...
var PrescriptionWidget = Widget.extend({
    template: 'PrescriptionWidget',
    custom_events: {
        field_changed: '_onFieldChanged',
    },
    events: {
        'click .o_add_product': '_onAddProduct',
        'click .o_prescription_widget_order_button': '_onOrderNow',
        'click .o_remove_product': '_onRemoveProduct',
        'click .o_prescription_widget_unlink': '_onUnlinkOrder',
        'click .o_prescription_open_wizard': '_onPrescriptionOpenWizard',
    },
    ...
    init: function (parent, params) {
        ...
        this.prescriptionUserField = null;

        this.locations = params.locations || [];
        this.userLocation = params.user_location[1] || '';

        const company_ids = [false].concat(session.user_context.allowed_company_ids || []);
        this.prescriptionLocationField = this._createMany2One('locations', 'prescription.location', this.userLocation, () => [
    ...
    willStart: function () {
        ...
        if (this.is_manager) {
            this.prescriptionUserField = this._createMany2One('users', 'res.users', this.username,
    ...
    renderElement: function () {
        this._super.apply(this, arguments);
        if (this.prescriptionUserField) {
            this.prescriptionUserField.appendTo(this.$('.o_prescription_user_field'));
        } else {
            this.$('.o_prescription_user_field').text(this.username);
        }

        if (this.userLocation) {
            this.prescriptionLocationField.appendTo(this.$('.o_prescription_location_field'));
        }
    },
    ...
    _createMany2One: function (name, model, value, domain, context) {
        ...
        return new PrescriptionMany2One(this, name, record, options);
    },
    ...
    _onPrescriptionOpenWizard: function (ev) {
        ev.preventDefault();
        ev.stopPropagation();
