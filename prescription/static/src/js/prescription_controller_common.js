odoo.define('prescription.PrescriptionControllerCommon', function (require) {
"use strict";

/**
 * This file defines the common events and functions used by Controllers for the Prescription view.
 */
...
var PrescriptionPaymentDialog = require('prescription.PrescriptionPaymentDialog');

var _t = core._t;

var PrescriptionControllerCommon = {
    custom_events: {
     ...
     */
    start: function () {
        // create a div inside o_content that will be used to wrap the prescription
        // banner and renderer (this is required to get the desired
        // layout with the searchPanel to the left)
        var self = this;
        this.$('.o_content').append($('<div>').addClass('o_prescription_content'));
        return this._super.apply(this, arguments).then(function () {
            self.$('.o_prescription_content').append(self.$('.o_prescription_view'));
        });
    },
    ...
    _fetchPaymentInfo: function () {
        return this._rpc({
            route: '/prescription/payment_message',
            params: {
    ...
    /**
     * Renders and appends the prescription banner widget.
     *
     * @private
     */
    _renderPrescriptionWidget: function () {
        var oldWidget = this.widget;
        this.widget = new PrescriptionWidget(this, Object.assign(this.widgetData, {edit: this.editMode}));
        return this.widget.appendTo(document.createDocumentFragment()).then(() => {
            this.$('.o_prescription_content').prepend(this.widget.$el);
            if (oldWidget) {
                oldWidget.destroy();
            }
        });
    ...
    _showPaymentDialog: function (title) {
        ...
        this._fetchPaymentInfo().then(function (data) {
            var paymentDialog = new PrescriptionPaymentDialog(self, _.extend(data, {title: title}));
            paymentDialog.open();
        });
    },
    /**
     * Override to fetch and display the prescription data. Because of the presence of
     * the searchPanel, also wrap the prescription widget and the renderer into
     ...
     */
    _update: function () {
        var def = this._fetchWidgetData().then(this._renderPrescriptionWidget.bind(this));
...
return PrescriptionControllerCommon;
