...
class Prescription(models.Model):
    ...
    def _populate_factories(self):
        ...
        def get_company_info(iterator, field_name, model_name):
            random = populate.Random("prescription_company")
...
class PrescriptionLine(models.Model):
    ...
    _populate_dependencies = ["prescription", "product.product"]

    def _populate(self, size):
        so_line = super()._populate(size)
        self.confirm_prescription(0.60, so_line)
        return so_line

    def confirm_prescription(self, sample_ratio, so_line):
        # Confirm sample_ratio * 100 % of so
        random = populate.Random('confirm_prescription')
        order_ids = self.filter_confirmable_prescriptions(so_line.order_id).ids
        orders_to_confirm = self.env['prescription'].browse(random.sample(order_ids, int(len(order_ids) * sample_ratio)))
        _logger.info("Confirm %d prescription orders", len(orders_to_confirm))
        orders_to_confirm.action_confirm()
        return orders_to_confirm

    @classmethod
    def filter_confirmable_prescriptions(cls, prescription):
        return prescription

    def _populate_factories(self):
        order_ids = self.env.registry.populated_models["prescription"]
