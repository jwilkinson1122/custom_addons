...
class PrescriptionProductCategory(models.Model):
    _inherit = 'prescription.product.category'
    _populate_sizes = {'small': 5, 'medium': 150, 'large': 400}
    _populate_dependencies = ['res.company']

    def _populate_factories(self):
        # TODO option_ids_{1,2,3}, toppping_label_{1,2,3}, option_quantity{1,2,3}
        company_ids = self.env.registry.populated_models['res.company']

        return [
            ('name', populate.constant('prescription_product_category_{counter}')),
...
class PrescriptionProduct(models.Model):
    _inherit = 'prescription.product'
    _populate_sizes = {'small': 10, 'medium': 150, 'large': 10000}
    _populate_dependencies = ['prescription.product.category', 'prescription.partner']

    def _populate_factories(self):
        category_ids = self.env.registry.populated_models['prescription.product.category']
        category_records = self.env['prescription.product.category'].browse(category_ids)
        category_by_company = {k: list(v) for k, v in groupby(category_records, key=lambda rec: rec['company_id'].id)}

        partner_ids = self.env.registry.populated_models['prescription.partner']
        company_by_partner = {rec.id: rec.company_id.id for rec in self.env['prescription.partner'].browse(partner_ids)}

        def get_category(random=None, values=None, **kwargs):
            company_id = company_by_partner[values['partner_id']]
            return random.choice(category_by_company[company_id]).id

        return [
            ('active', populate.iterate([True, False], [0.9, 0.1])),
            ('name', populate.constant('prescription_product_{counter}')),
...
class PrescriptionLocation(models.Model):
    _inherit = 'prescription.location'
    ...
    def _populate_factories(self):
        ...
        return [
            ('name', populate.constant('prescription_location_{counter}')),
            ('address', populate.constant('prescription_address_location_{counter}')),
            ('company_id', populate.randomize(company_ids))
        ]


class PrescriptionPartner(models.Model):
    _inherit = 'prescription.partner'

    _populate_sizes = {'small': 3, 'medium': 50, 'large': 1500}

    _populate_dependencies = ['prescription.location', 'res.partner', 'res.users']

    def _populate_factories(self):

        location_ids = self.env.registry.populated_models['prescription.location']
...
class PrescriptionOrder(models.Model):
    ...
    _populate_dependencies = ['prescription.product', 'res.users', 'res.company']

    def _populate_factories(self):
        # TODO option_ids_{1,2,3}, option_label_{1,3}, option_quantity_{1,3}
        user_ids = self.env.registry.populated_models['res.users']
        product_ids = self.env.registry.populated_models['prescription.product']
        company_ids = self.env.registry.populated_models['res.company']
        ...
        return [
            ...
            ('note', populate.constant('prescription_note_{counter}')),
            ('company_id', populate.randomize(company_ids)),
...
class PrescriptionAlert(models.Model):
    _inherit = 'prescription.alert'
    _populate_sizes = {'small': 10, 'medium': 40, 'large': 150}

    _populate_dependencies = ['prescription.location']
