...
class S3Credentials(models.Model):
    _name = constants.S3_CREDENTIALS_MODEL
    ...
    def connect(self):
        ...
        try:
            ...
            if db_rows and len(db_rows) > 0:
                ...
                db_rows[constants.DEFAULT_INDEX].access_key = val_struct["access_key"]
                db_rows[constants.DEFAULT_INDEX].secret_key = val_struct["secret_key"]
                db_rows[constants.DEFAULT_INDEX].bucket = val_struct["bucket"]
                db_cursor.update(db_rows[constants.DEFAULT_INDEX])
                rep_message += constants.CREDENTIAL_UPDATE_MSG
            else:
                _logging.info("Create CRD record")
                super().create(val_struct)
                rep_message += constants.CREDENTIAL_SAVE_MSG

        except Exception as ex:
            _logging.exception("Exception CRD: " + str(ex))
            rep_message += constants.CREDENTIAL_EXCEPT_MSG
        return {
    ...
    def get_s3_credentials(self):
        ...
        try:
            ...
            db_rows = db_cursor.search([])
            if db_rows and len(db_rows) > 0:
                cred_response["access_key"] = db_rows[constants.DEFAULT_INDEX].access_key
                cred_response["secret_key"] = db_rows[constants.DEFAULT_INDEX].secret_key
                cred_response["bucket"] = db_rows[constants.DEFAULT_INDEX].bucket
            else:
                cred_response["err_message"] = constants.CREDENTIAL_NOT_FND_MSG
                cred_response["error"] = "Credentials are not found"
        except Exception as ex:
            cred_response["error"] = str(ex)
            cred_response["err_message"] = constants.CREDENTIAL_EXCEPT_FETCH_MSG
        return cred_response

    @api.model
    def _get_default_access_key(self):
        latest_record = self.env[constants.S3_CREDENTIALS_MODEL].search([])
        return latest_record[constants.DEFAULT_INDEX].access_key if len(latest_record) > 0 else ''

    @api.model
    def _get_default_secret_key(self):
        latest_record = self.env[constants.S3_CREDENTIALS_MODEL].search([])
        return latest_record[constants.DEFAULT_INDEX].secret_key if len(latest_record) > 0 else ''

    @api.model
    def _get_default_bucket(self):
        latest_record = self.env[constants.S3_CREDENTIALS_MODEL].search([])
        return latest_record[constants.DEFAULT_INDEX].bucket if len(latest_record) > 0 else ''


class S3Connect(models.Model):
    _inherit = constants.CONTACT_MODEL
    attachment_ids = fields.Many2many(constants.IR_ATTACHMENT_MODEL,
                                      constants.CLASS_IR_ATTACHMENT_REL_MODEL,
                                      'class_id',
                                      'attachment_id',
                                      'Attachments')
    ...
    def s3_upload_btn(self):
        ...
        r_partner = self.env[constants.CONTACT_MODEL].search([('id', '=', self.id)])
        file_records = {
        ...
        if len(file_records["files"]) > 0:
            credentials = self.env[constants.S3_CREDENTIALS_MODEL].get_s3_credentials()
            ...
            if 'error' not in credentials and 'err_message' not in credentials:
                ...
                if not _sp_response["err_status"]:
                    pop_message += "Files uploaded successfully: " + constants.AWS_S3_OPT_KEY
                else:
                    pop_message += str(_sp_response["response"])
            else:
                pop_message += credentials["err_message"]
        else:
            pop_message += constants.AWS_S3_FILE_NOT_FND
            
        if constants.AWS_S3_OPT_KEY in pop_message:
            return {
                'type': 'ir.actions.client',
                'tag': 'reload',
            }
    ...
    def s3_download_btn(self):
        ...
        if 'error' not in credentials and 'err_message' not in credentials:
            ...
            if not _sp_response["err_status"]:
                pop_message += "Files downloaded successfully: " + constants.AWS_S3_OPT_KEY
            else:
                pop_message += str(_sp_response["response"])
        else:
            pop_message += credentials["err_message"]
