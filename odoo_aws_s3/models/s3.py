...
class S3:
    def __init__(self, aws_credential):
        ...
        self.__aws_client = boto3.client(constants.AWS_S3_DEFAULT,
    ...
    def export_documents(self, res_data):
        ...
        try:
            for file in res_data["files"]:
                ...
                if constants.AWS_S3_RESPONSE_KEY in server_rp:
            ...
            self.__js_resp["response"] = constants.AWS_S3_EXP_EXCEPT
        return self.__js_resp

    def check_file_exists(self, db_cursor, file_name, file_type, user_rec):
        chk_eir_file = False
        try:
            db_cursor.execute("select id from " + constants.DB_IR_ATTACHMENT_MODEL +
    ...
    def import_documents(self, db_cursor, user_rcd, self_env=None):
        ...
        try:
            dir_serv_resp = self.__aws_client.list_objects_v2(
                ...
                MaxKeys=100)
            if constants.AWS_S3_RESPONSE_KEY in dir_serv_resp and constants.AWS_S3_LIST_KEY in dir_serv_resp:
                if len(dir_serv_resp[constants.AWS_S3_LIST_KEY]) > 0:
                    self.__js_resp["err_status"] = False
                    for serv_file in dir_serv_resp[constants.AWS_S3_LIST_KEY]:
                        file_serv_resp = self.__aws_client.get_object(
                            Bucket=self.__default_bucket, Key=serv_file['Key'])
                        if constants.AWS_S3_RESPONSE_KEY in file_serv_resp:
                            ...
                            try:
                                ...
                                if not chk_status:
                                    ...
                                    save_rec = self_env[constants.IR_ATTACHMENT_MODEL].create({
                                                                            ...
                                                                            'type': 'binary',
                                                                            'res_model': constants.CONTACT_MODEL,
                                                                            'res_id': 0,
                                                                            })

                                    db_cursor.execute("insert into " + constants.CLASS_IR_ATTACHMENT_REL_MODEL +
                    ...
                    self.__js_resp["response"] = constants.AWS_S3_IMP_SERV_DIR_NOT_FND
            else:
                self.__js_resp["response"] = constants.AWS_S3_IMP_SERV_ERR

        except Exception as ex:
            self.__logging.exception("Import Directory Exception: " + str(ex))
            self.__js_resp["response"] = constants.AWS_S3_IMP_SERV_EXCEPT
        return self.__js_resp
