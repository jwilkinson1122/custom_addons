...
class PrescriptionOrder(models.Model):
    _inherit = "pod.prescription.order"

    def action_config_start(self):
        """Return action to start configuration wizard"""
        configurator_obj = self.env["product.configurator.prescription"]
        return configurator_obj.with_context( default_prescription_order_id=self.id, wizard_model="product.configurator.prescription", allow_preset_selection=True,
        ).get_wizard_action()
...
class PrescriptionOrderLine(models.Model):
    ...
    custom_value_ids = fields.One2many(
        ...
        inverse_name="cfg_session_id", related="config_session_id.custom_value_ids", 
    ...
    def reconfigure_product(self):
        ...
        self = self.with_context(
            default_prescription_order_id=self.prescription_order_id.id, 
            default_prescription_order_line_id=self.id,
        )
