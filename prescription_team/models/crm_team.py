...
class CrmTeam(models.Model):
    ...
    member_ids = fields.Many2many(
        'res.users', string='Prescriptionpersons',
    ...
    def _compute_is_membership_multi(self):
        multi_enabled = self.env['ir.config_parameter'].sudo().get_param('prescription_team.membership_multi', False)
    ...
    def _unlink_except_default(self):
        default_teams = [
            self.env.ref('prescription_team.prescription_team_website_prescription'),
            self.env.ref('prescription_team.pos_prescription_team'),
            self.env.ref('prescription_team.ebay_prescription_team')
        ]
        for team in self:
    ...
    def _get_dashboard_graph_data(self):
        ...
        weeks_in_start_year = int(date(start_date.year, 12, 28).isocalendar()[1]) # This date is always in the last week of IRX years
