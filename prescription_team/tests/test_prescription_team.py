...
from odoo.addons.prescription_team.tests.common import PrescriptionTeamCommon, TestPrescriptionCommon, TestPrescriptionMC


class TestDefaultTeam(TestPrescriptionCommon):
    ...
    def setUpClass(cls):
        ...
        cls.env['ir.config_parameter'].set_param('prescription_team.membership_multi', True)

        # Prescriptionmen organization
        # ------------------------------------------------------------
        # Role: M (team member) R (team manager)
        # SALESMAN---------------prescription_team_1---C2Team1---LowSequ---Team3
        # admin------------------M-------------- --------- ---------
        # user_prescription_manager-----R-------------- --------- ---------R
        # user_prescription_leads-------M-------------- ---------M---------
        ...
        cls.team_sequence = cls.env['crm.team'].create({
            ...
            'member_ids': [(4, cls.user_prescription_leads.id)],
            'sequence': 0,
            'user_id': False,
        })
    ...
    def test_default_team_fallback(self):
        ...
        domain, order """
        self.prescription_team_1.member_ids = [(5,)]
        self.team_sequence.member_ids = [(5,)]
        (self.prescription_team_1 + self.team_sequence).flush_model()
        self.assertFalse(self.env['crm.team.member'].search([('user_id', '=', self.user_prescription_leads.id)]))

        # default is better sequence matching company criterion
        with self.with_user('user_prescription_leads'):
        ...
        self.user_prescription_leads.write({
    ...
    def test_default_team_member(self):
        ...
        with self.with_user('user_prescription_leads'):
            team = self.env['crm.team']._get_default_team_id()
            self.assertEqual(team, self.prescription_team_1)

        # responsible with lower sequence better than member with higher sequence
        self.team_responsible.user_id = self.user_prescription_leads.id
        with self.with_user('user_prescription_leads'):
            team = self.env['crm.team']._get_default_team_id()
            self.assertEqual(team, self.team_responsible)

        # in case of same sequence: take latest team
        self.team_responsible.sequence = self.prescription_team_1.sequence
        with self.with_user('user_prescription_leads'):
            team = self.env['crm.team']._get_default_team_id()
            self.assertEqual(team, self.team_responsible)
    ...
    def test_default_team_wcontext(self):
        ...
        with self.with_user('user_prescription_leads'):
            ...
            team = self.env['crm.team'].with_context(
                default_team_id=self.prescription_team_1.id
            )._get_default_team_id()
            self.assertEqual(
                team, self.prescription_team_1,
                'PrescriptionTeam: default takes over ordering when member / responsible'
            )
        ...
        with self.with_user('user_prescription_leads'):
            ...
            self.assertEqual(
                team, self.prescription_team_1,
                'PrescriptionTeam: default taken into account when no member / responsible'
            )

class TestMultiCompany(TestPrescriptionMC):
    """Tests to check multi company management with prescription team and their
    members. """
    ...
    def test_team_members(self):
        ...
        with self.assertRaises(exceptions.UserError):
            team_c2.write({'member_ids': [(4, self.user_prescription_prescriptionman.id)]})

        # reset members, change company
        team_c2.write({'member_ids': [(5, 0)], 'company_id': self.company_main.id})
        self.assertEqual(team_c2.member_ids, self.env['res.users'])
        team_c2.write({'member_ids': [(4, self.user_prescription_prescriptionman.id)]})
        self.assertEqual(team_c2.member_ids, self.user_prescription_prescriptionman)
    ...
    def test_team_memberships(self):
        ...
        # cannot add someone from another company
        with self.assertRaises(exceptions.UserError):
            team_c2.write({'crm_team_member_ids': [(0, 0, {'user_id': self.user_prescription_prescriptionman.id})]})
...
class TestAccessRights(PrescriptionTeamCommon):

    @users('prescriptionmanager')
    def test_access_prescription_manager(self):
        """ Test prescription manager's access rights """
        # Manager can create a Prescription Team
        india_channel = self.env['crm.team'].with_context(tracking_disable=True).create({
            'name': 'India',
        })
        self.assertIn(
            india_channel.id, self.env['crm.team'].search([]).ids,
            'Prescription manager should be able to create a Prescription Team')

        # Manager can edit a Prescription Team
        india_channel.write({'name': 'new_india'})
        self.assertEqual(
            india_channel.name, 'new_india',
            'Prescription manager should be able to edit a Prescription Team')

        # Manager can delete a Prescription Team
        india_channel.unlink()
        self.assertNotIn(
            india_channel.id, self.env['crm.team'].search([]).ids,
            'Prescription manager should be able to delete a Prescription Team')
