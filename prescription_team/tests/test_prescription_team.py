...
class TestMultiCompany(TestPrescriptionMC):
    ...
    def test_team_members(self):
        ...
        with self.assertRaises(exceptions.UserError):
            team_c2.write({'member_ids': [(4, self.user_prescription_personnel.id)]})

        # reset members, change company
        team_c2.write({'member_ids': [(5, 0)], 'company_id': self.company_main.id})
        self.assertEqual(team_c2.member_ids, self.env['res.users'])
        team_c2.write({'member_ids': [(4, self.user_prescription_personnel.id)]})
        self.assertEqual(team_c2.member_ids, self.user_prescription_personnel)
    ...
    def test_team_memberships(self):
        ...
        # cannot add someone from another company
        with self.assertRaises(exceptions.UserError):
            team_c2.write({'crm_team_member_ids': [(0, 0, {'user_id': self.user_prescription_personnel.id})]})
