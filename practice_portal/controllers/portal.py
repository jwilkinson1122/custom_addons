...
class CustomerPortal(portal.CustomerPortal):
    def _prepare_home_portal_values(self, counters):
        ...
        if "prescription_bookout_count" in counters:
            count = request.env["practice.bookout"].search_count([])
    ...
    def my_prescription_bookouts(self, page=1, **kw):
        Bookout = request.env["practice.bookout"]
        domain = []
        # Prepare pager data
        ...
        return request.render("practice_portal.my_prescription_bookouts", values)

    @route(["/my/prescription-bookout/<model('practice.bookout'):doc>"], auth="user", website=True)
    def portal_my_project(self, doc=None, **kw):
        return request.render("practice_portal.prescription_bookout", {"doc": doc})
