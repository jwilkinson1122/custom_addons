...
class ProductMatrixWizard(models.TransientModel):
    _name = 'product.matrix.wizard'
    _description = 'Product Matrix'
    _rec_name = "name"

    name = fields.Char("Name", default='Name')
    matrix_lines = fields.One2many("product.matrix.wizard.line", 'wizard_id', "Lines")
    product_tmpl_id = fields.Many2one("product.template",
                                      domain="['|',('is_foot_size','=',True),('is_material_size','=',True)]")
...
class ProductMatrixWizardLine(models.TransientModel):
    _name = 'product.matrix.wizard.line'
    _description = 'Product Matrix'
    _rec_name = "name"

    name = fields.Char("Name", default='Name')
    active_bool = fields.Boolean('Select')
    wizard_id = fields.Many2one("product.matrix.wizard")
