...
class ProductTemplate(models.Model):
    ...
    product_dimension = fields.Char(
        'Dimension', help="Size Package Ex: 30x30x30")
    product_package_po = fields.Float(
        'Quantity on Carton', help="Qty in Package ext:10", digits='Product Unit of Measure')
    package_uom = fields.Many2one(
        'product.packaging', help="Package ex:Dozen, Pack,", readonly=False)
    #package_uom = fields.Char('Package Name' ,default='Carton', help="Package ex:Dozen, Pack,", readonly=True)
    packaging_supp_yes = fields.Boolean('yes')
    packaging_supp_no = fields.Boolean('No')
    separator_x = fields.Char(
        'X', default='x', related='product_variant_ids.separator_x')
    separator_x2 = fields.Char(
        'X', default='x', related='product_variant_ids.separator_x2')
    ...
    size = fields.Float('Volume Carton', digits=(10, 4),
                        )  # compute='_compute_weight_unit', inverse='_set_weight_unit', store=True)
...
#         inverse='_set_weight_unit', store=True)
    ...
    def on_change_weight_package(self):
        if self.packaging_weight > 0 and self.product_package_po > 0:
            ...
            self.weight = round_down(
                self.packaging_weight / self.product_package_po, 3)
            self.weight_unit = round_down(
                self.packaging_weight / self.product_package_po, 3)
            # DIMENSI DALAM METER CUBIC  / QUANTITY DALAM PACKING
            self.volume = ((self.height_dimension * self.length_dimension *
                           self.width_dimension) / 1000000) / (self.product_package_po or 1)
            self.volume_unit = ((self.height_dimension * self.length_dimension *
                                self.width_dimension) / 1000000) / (self.product_package_po or 1)
    ...
    def on_change_size_dimension(self):
        # DIMENSI DALAM M3
        self.size = (self.height_dimension *
                     self.length_dimension * self.width_dimension) / 1000000
...
class ProductProduct(models.Model):
    ...
    product_dimension = fields.Char(
        'Dimension', help="Size Package Ex: 30x30x30")
    product_package_po = fields.Float(
        'Quantity on Carton', help="Qty in Package ext:10")
    package_uom = fields.Many2one(
        'product.packaging', help="Package ex:Dozen, Pack,", readonly=False)
    #package_uom = fields.Char('Package Name',default='Carton', help="Package ex:Dozen, Pack,", readonly=True)
    ...
    packaging_supp_quest = fields.Selection([('yes', 'Yes'), ('no', 'No')], string='Standard Packaging',
                                            help='Do You Have Standard Packaging For This Product?', required=True, default='no')
    packaging_supp_quest_dimension = fields.Selection(
        [('yes', 'Yes'), ('no', 'No')], string='Standard Size', help='Do You Have Size This Product?')
