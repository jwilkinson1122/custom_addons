...
class Prescription(models.Model):
    ...
    def _compute_is_shared(self):
        ...
        doc_share_count_per_doc_id = {prescription.id: count for prescription, count in doc_share_read_group}

        for prescription in self:
            prescription.is_shared = doc_share_count_per_doc_id.get(prescription.id) or prescription.folder_id.is_shared
    ...
    def _search_is_shared(self, operator, value):
        ...
        share_links = self.env['prescriptions.share'].search_read(
            ['|', ('date_deadline', '=', False), ('date_deadline', '>', fields.Date.today())],
            ['prescription_ids', 'folder_id', 'include_sub_folders', 'type'],
        )

        shared_folder_ids = set()
        shared_folder_with_descendants_ids = set()
        shared_prescription_ids = set()
        ...
        for link in share_links:
            if link['type'] == 'domain':
                ...
                shared_prescription_ids |= set(link['prescription_ids'])
        ...
        domain = [
            '|',
                ...
                ('id', 'in', list(shared_prescription_ids)),
        ]
    ...
    def _search_project_id(self, operator, value):
        if operator in ('=', '!=') and isinstance(value, bool): # needs to be the first condition as True and False are instances of int
            ...
            # We only apply security rules to projects as security rules on prescriptions will be applied prior
            # to this leaf. Not applying security rules on tasks might give more result than expected but it would not allow
            # access to an unauthorized prescription.
            return [
                ("id", "inselect", (f"""
                    WITH helper as (
                        {project_select}
                    ...
                    FROM prescriptions_prescription prescription
                    LEFT JOIN project_project project ON project.id=prescription.res_id AND prescription.res_model = 'project.project'
                    LEFT JOIN project_task task ON task.id=prescription.res_id AND prescription.res_model = 'project.task'
    ...
    def _search_task_id(self, operator, value):
        if operator in ('=', '!=') and isinstance(value, bool):
            ...
            prescription_task_alias = query_task.make_alias('project_task', 'prescription')
            query_task.add_join("JOIN", prescription_task_alias, 'prescriptions_prescription', SQL(
                "%s = %s AND %s = %s",
                SQL.identifier('project_task', 'id'),
                SQL.identifier(prescription_task_alias, 'res_id'),
                SQL.identifier(prescription_task_alias, 'res_model'),
                'project.task',
            ))
            return [
                ("id", "inselect", query_task.select(f"{prescription_task_alias}.id")),
            ]
        else:
    ...
    def search_panel_select_range(self, field_name, **kwargs):
        ...
        prescription_read_group = self.env['prescriptions.prescription']._read_group(kwargs.get('search_domain', []), [], ['folder_id:array_agg'])
        folder_ids = prescription_read_group[0][0]
        records = self.env['prescriptions.folder'].with_context(hierarchical_naming=False).search_read([
            '|',
                ('id', 'child_of', project.prescriptions_folder_id.id),
                ('id', 'in', folder_ids),
        ], fields)
