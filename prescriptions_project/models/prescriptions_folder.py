...
class PrescriptionFolder(models.Model):
    _inherit = 'prescriptions.folder'

    def default_get(self, fields):
        res = super().default_get(fields)
        if self.env.context.get('prescriptions_project') and not res.get('parent_folder_id'):
            res['parent_folder_id'] = self.env.ref('prescriptions_project.prescriptions_project_folder').id
        return res

    project_ids = fields.One2many('project.project', 'prescriptions_folder_id')

    @api.model
    def _name_search(self, name, domain=None, operator='ilike', limit=None, order=None):
        domain = domain or []
        if 'project_prescriptions_template_folder' in self.env.context:
            template_folder_id = self.env.context.get('project_prescriptions_template_folder')
    ...
    def unlink_except_project_folder(self):
        project_folder = self.env.ref('prescriptions_project.prescriptions_project_folder')
    ...
    def _check_no_company_on_projects_folder(self):
        if not self.company_id:
            return
        projects_folder = self.env.ref('prescriptions_project.prescriptions_project_folder')
    ...
    def _get_project_from_closest_ancestor(self):
        ...
        eligible_projects = self.env['project.project'].sudo()._read_group(
            [('prescriptions_folder_id', 'parent_of', self.id)],
            ['prescriptions_folder_id'],
            having=[('__count', '=', 1)],
        )
        ...
        return self.env['project.project'].sudo().search([('prescriptions_folder_id', '=', eligible_projects[0][0].id)], limit=1)
