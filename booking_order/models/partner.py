...
class Partner(models.Model):
    ...
    @api.depends('booking_order_ids')
    def _compute_booking_order_count(self):
        for record in self.filtered('id'):
            record.booking_order_count = len(self.env['sale.order'].search([('partner_id', 'child_of', self.id)]))

    def booking_order_history(self):
        act = self.env.ref('booking_order.open_order_form_tree_all').read()[0]
