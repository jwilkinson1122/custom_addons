odoo.define('podiatry_pos_modifier.ModifierProductPopup', function (require) {
	'use strict';
	...
	class ModifierProductPopup extends AbstractAwaitablePopup {
		...
		click_on_modifier_product(event) {
			...
			if (product) {
				...
				vals_list.option = self.props.option;
			...
			if (modifier_list.length > 0) {
				if (_.findWhere(modifier_list, { id: vals_list.id, })) {
					var increase = _.findWhere(modifier_list, { id: vals_list.id, option: vals_list.option })
					if (increase && increase.option == vals_list.option) {
						increase.qty += 1;
					}
					else {
