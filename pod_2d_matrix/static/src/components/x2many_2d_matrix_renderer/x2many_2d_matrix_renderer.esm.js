...
export class X2Many2DMatrixRenderer extends Component {
  ...
  _getColumns(records = this.list.records) {
    const columns = [
      { value: "left_only", text: "Left Only" },
      { value: "right_only", text: "Right Only" },
  ...
  _getRows(records = this.list.records) {
    const rows = [];
    const categories = this.env["product.category"].search([]);
    categories.forEach((category) => {
      rows.push({ value: category.id, text: category.name });
    });
    return rows;
  }
  ...
  _getMatrix(records = this.list.records) {
    ...
    records.forEach((record) => {
      ...
      matrix[y][x].value += 1; // Update value (or handle selection)
  ...
  _aggregateRow(row) {
    ...
    const total = this.matrix[y]
      .map((r) => r.value)
      .reduce((aggr, x) => aggr + x);
...
X2Many2DMatrixRenderer.template = "pod_2d_matrix.X2Many2DMatrixRenderer";
