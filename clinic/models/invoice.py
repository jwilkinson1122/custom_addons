...
class AccountInvoice(models.Model):
    ...
    date_invoice = fields.Date(string='Invoice Date', readonly=True, states={'draft': [(
        'readonly', False)]}, index=True, copy=False, default=lambda self: fields.datetime.now())
    date_due = fields.Date(string='Due Date', readonly=True, states={'draft': [(
        'readonly', False)]}, index=True, copy=False, default=lambda self: fields.datetime.now())
    ...
    def create(self, vals):
        vals['number_invoice'] = self.env['ir.sequence'].next_by_code(
            'clinic.invoice.sequence')
    ...
    def action_invoice_cancel(self):
        if self.filtered(lambda inv: inv.state not in ['draft', 'open']):
            raise UserError(
                _("Invoice must be in draft or open state in order to be cancelled."))
    ...
    def name_get(self):
        result = []
        for me in self:
            result.append((me.id, "%s - %s" %
                          (me.number_invoice, me.partner_id.name)))
        return result
...
class AccountInvoiceLine(models.Model):
    ...
    invoice_id = fields.Many2one(
        'clinic.invoice', string='Invoice Reference', ondelete='cascade', index=True)
    product_id = fields.Many2one(
        'product.product', string='Product', ondelete='restrict', index=True)
