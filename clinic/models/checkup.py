...
class ClinicCheckup(models.Model):
    ...
    @api.depends('patient_id', 'dob')
    def _get_age(self):
        for patient_id in self:
            if patient_id.dob:
                years = relativedelta(date.today(), patient_id.dob).years
                patient_id.age = str(int(years))
            else:
                patient_id.age = 0

        # age = False
        # if self.patient_id:
        #     age = datetime.now().year - datetime.strptime('%Y-%m-%d',
        #                                                   self.patient_id.birth_date).year
        # self.age = age
    ...
    dob = fields.Date()
    age = fields.Integer(compute='_get_age', store=True, readonly=True)
