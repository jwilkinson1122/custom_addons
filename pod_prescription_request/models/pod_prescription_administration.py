...
class PodPrescriptionAdministration(models.Model):
    _name = "pod.prescription.administration"
    _inherit = "pod.event"
    _description = "pod prescription administration"

    def _default_patient_location(self):
        # return self.env.ref('stock.stock_location_customers')
        return self.env.ref("pod_prescription_request.location_patient")

    internal_identifier = fields.Char(string="Prescription")
    ...
    move_ids = fields.One2many(
        "stock.move", inverse_name="prescription_administration_id"
    )
    ...
    def in_progress2completed(self):
        ...
        for event in self:
            ...
            if float_compare(qty, event.qty, precision_digits=precision) >= 0:
                continue
    ...
    def _prepare_procurement_values(self, group):
        ...
        return {
            "group_id": group,
            "prescription_administration_id": self.id,
            "warehouse_id": wh,
    ...
    def action_view_stock_moves(self):
        ...
        action["domain"] = [("prescription_administration_id", "=", self.id)]
        if len(self.move_ids) == 1:
            action["views"] = [(False, "form")]
            action["res_id"] = self.move_ids.id
        return action

    @api.constrains("prescription_request_id", "patient_id")
    def _check_patient_prescription(self):
        if not self.env.context.get("no_check_patient", False):
            if (
                self.prescription_request_id
                and self.patient_id != self.prescription_request_id.patient_id
            ):
