...
class podiatry_patient_evaluation(models.Model):
    ...
    practitioner_partner_id = fields.Many2one(
        'res.partner', domain=[('is_practitioner', '=', True)], string="Practitioner")
    ...
    symptom_disturb_sleep = fields.Boolean('Disturpractice Sleep')
    ...
    diagnosis_id = fields.Many2one(
        'podiatry.pathology', 'Presumptive Diagnosis')
    ldl = fields.Integer(
        string='Last LDL',
        help='Last LDL Cholesterol reading. Can be approximative'
    )
    visit_type = fields.Selection([('new', 'New Health Condition'), ('follow', 'FollowUp'), ('chronic', 'Chronic Condition ChechUp'), (
        'child', 'Well Child Visit'), ('women', 'Well Woman Visit'), ('man', 'Well Man Visit')], string="Visit")
    urgency = fields.Selection(
        [('a', 'Normal'), ('b', 'Urgent'), ('c', 'Medical Emergency')], string='Urgency')
    ...
    user_id = fields.Many2one(
        'res.users', 'Practitioner user ID', readonly=True)
    derived_from_practitioner_id = fields.Many2one(
        'podiatry.practitioner', 'Derived from Practitioner')
    ...
    procedure_ids = fields.One2many(
        'podiatry.directions', 'patient_evaluation_id', 'Pathologies')
