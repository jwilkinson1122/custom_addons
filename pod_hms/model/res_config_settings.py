...
PARAMS = [
    ("pod_hms_attributes_option", safe_eval, "False"),
    ("pod_hms_export_option", safe_eval, "False"),
    ("pod_hms_eshop_categories_option", safe_eval, "False"),
]
ALREADYADDEDFIELDS = [
...
class res_config_settings(models.TransientModel):
    ...
    @api.onchange("module_pod_hms_website_sale")
    def _onchange_module_pod_hms_website_sale(self):
        """
        Onchange method for module_pod_hms_website_sale
        """
        for conf in self:
            if not conf.module_pod_hms_website_sale:
                conf.pod_hms_eshop_categories_option = False

    pod_hms_attributes_option = fields.Boolean(string="Filter by attribute values")
    pod_hms_export_option = fields.Boolean(string="Export products")
    ...
    module_pod_hms_website_sale = fields.Boolean(string="E-commerce mass actions")
    module_pod_hms_accounting = fields.Boolean(string="Accounting mass actions")
    module_pod_hms_stock = fields.Boolean(string="Warehouse mass actions")
    module_pod_hms_purchase = fields.Boolean(string="Purchase mass actions")
    pod_hms_eshop_categories_option = fields.Boolean(string="Filter by eCommerce categories")
    ...
    def get_values(self):
        ...
        ir_actions_server_ids =  safe_eval(Config.get_param("pod_hms.ir_actions_server_ids", "[]"))
        ...
        kanban_fields_ids =  safe_eval(Config.get_param("pod_hms.kanban_fields_ids", "[]"))
    ...
    def set_values(self):
        ...
        Config.set_param("pod_hms.ir_actions_server_ids", self.ir_actions_server_ids.ids)
        kanban_fields_ids =  safe_eval(Config.get_param("pod_hms.kanban_fields_ids", "[]"))
        if kanban_fields_ids != self.kanban_fields_ids.ids:
            Config.set_param("pod_hms.kanban_fields_ids", self.kanban_fields_ids.ids)
    ...
    def _update_kanban_view(self, cfields):
        ...
        if not view_id:
            ...
            values = {
                ...
                "type": "kanban",
                "inherit_id": self.sudo().env.ref("pod_hms.product_template_kanban_view").id,
            }
