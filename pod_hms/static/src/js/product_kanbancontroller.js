...
const ProductKanbanController = KanbanController.extend({
    events: _.extend({}, KanbanController.prototype.events, {
        ...
        "click .pod_hms_product_selected_row": "_removeProductSelected",
        "click .mass_action_button": "_proceedMassAction",
        "click .mass_action_export": "_massActionExport",
    }),
    jsLibs: [
        '/pod_hms/static/lib/jstree/jstree.js',
    ],
    cssLibs: [
        '/pod_hms/static/lib/jstree/themes/default/style.css',
    ],
    ...
    */ 
    start: function () {
        this.$('.o_content').addClass('pod_hms_products prm_d_flex');
    ...
    _renderEshopCategories: function () {
        ...
        self._rpc({
            ...
            if (availableCategories.length > 0) {
                ...
                var toHideCategories = self.$('.pod_hms_content').find(".not_shown_eshop_categories");
                toHideCategories.addClass("pod_hms_hidden");
            }
            defer.resolve();
        });
        return defer
    ...
    _renderAttributes: function () {
        ...
        self._rpc({
            ...
            if (availableAttributes.length > 0) {
                ...
                var toHideAttributes = self.$('.pod_hms_content').find(".not_shown_attributes");
                toHideAttributes.addClass("pod_hms_hidden");
            };
    ...
    */ 
    _renderNavigationPanel: function () {
        var self = this;
        var scrollTop = self.$('.pod_hms_navigation_panel').scrollTop();
        self.$('.pod_hms_navigation_panel').remove();
        ...
        self.$('.pod_hms_navigation_panel').scrollTop(scrollTop || 0);
    ...
    */ 
    _renderRightNavigationPanel: function () {
        var self = this;
        var scrollTop = self.$('.pod_hms_right_navigation_panel').scrollTop();
        self.$('.pod_hms_right_navigation_panel').remove();
        ...
        if (selectedRecords.length) {
            self._rpc({
                ...
                self.$('.pod_hms_right_navigation_panel').scrollTop(scrollTop || 0);
            });
        }
    },
    /*
    ...
    */
    _highlightParent(jsTreeAnchor, checkedNodes, jsSelector) {
        $(jsSelector + "* .pod_hms_selected_parent").removeClass("pod_hms_selected_parent");
        var allParentNodes = [];
        ...
        _.each(allParentNodes, function(nodeID) {
            $(jsSelector + " * .jstree-node#" + nodeID).addClass("pod_hms_selected_parent");
        });
    },
    /*
