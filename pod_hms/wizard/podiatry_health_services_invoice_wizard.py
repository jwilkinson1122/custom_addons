...
class podiatry_health_services_invoice(models.TransientModel):
    _name = 'podiatry.health.service.invoice'
    _description = 'podiatry health service invoice'

    def create_podiatry_service_invoice(self):
        active_id = self._context.get('active_id')
        rx_req_obj = self.env['podiatry.health_service']
        ...
        sale_journals = self.env['account.journal'].search(
            [('type', '=', 'sale')])
        invoice_vals = {
            'name': self.env['ir.sequence'].next_by_code('podiatry_health_service_inv_seq'),
            'origin': rx_req.name or '',
            'type': 'out_invoice',
            'reference': False,
            'account_id': rx_req.patient_id.patient_id.property_account_receivable_id.id or False,
