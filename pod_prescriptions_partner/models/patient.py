...
class Patient(models.Model):
    ...
    _description = "Patient at a prescriptions medicine practice."
    ...
    date_of_birth = fields.Date(
        groups="pod_prescriptions_partner.group_prescriptions_partner_treatment_professional",
        tracking=True)
    age = fields.Integer(compute='_compute_age',
                         groups="pod_prescriptions_partner.group_prescriptions_partner_treatment_professional",
                         tracking=True)
    contact_ids = fields.One2many(comodel_name='prescriptions.patient.contact',
                                  inverse_name='patient_id',
                                  string='Patient Contacts',
                                  groups="pod_prescriptions_partner.group_prescriptions_partner_user")
    prescriptions_partner_ids = fields.Many2many(comodel_name='prescriptions.partner',
                                relation='prescriptions_partner_patient_rel',
                                column1='patient_id',
                                column2='prescriptions_partner_id',
                                string='Partners', )
    ...
    injury_ids = fields.One2many(comodel_name='prescriptions.patient.injury',
                                 inverse_name='patient_id',
                                 string='Injuries', )
    ...
    return_date = fields.Date(tracking=True,
                              help="When the patient was cleared by medical personnel to "
    ...
    partner_info_notes = fields.Html(string="Notes")

    def default_get(self, fields_list):
        res = super().default_get(fields_list)
        if 'prescriptions_partner_ids' in fields_list and 'params' in self.env.context \
                and self.env.context.get('params')['model'] == 'prescriptions.partner':
            partner = self.env['prescriptions.partner'].browse(self.env.context.get('params')['id'])
            prescriptions_partner_ids = [Command.set([partner.id])]
            if prescriptions_partner_ids:
                res.update({'prescriptions_partner_ids': prescriptions_partner_ids})
        return res
...
class PatientContact(models.Model):
    _name = 'prescriptions.patient.contact'
    ...
    patient_id = fields.Many2one(comodel_name='prescriptions.patient', string='Patient')
...
class PatientInjury(models.Model):
    _name = 'prescriptions.patient.injury'
    _description = "A patient's injury."
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _rec_name = 'diagnosis'

    patient_id = fields.Many2one(comodel_name='prescriptions.patient',
                                 string="Patient",
                                 readonly=True,
                                 required=True)
    ...
    def action_view_injury_form(self):
        ...
        return {
            'type': 'ir.actions.act_window',
            'view_mode': 'form',
            'res_model': 'prescriptions.patient.injury',
            'res_id': self.id,
            'context': self._context,
        }
