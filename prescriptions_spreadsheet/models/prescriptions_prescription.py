...
class Prescription(models.Model):
    ...
    _inherit = ["prescriptions.prescription", "spreadsheet.mixin"]
    ...
    def create(self, vals_list):
        ...
        prescriptions._update_spreadsheet_contributors()
    ...
    def write(self, vals):
        ...
        if all(prescription.handler == 'spreadsheet' for prescription in self):
    ...
    def _check_spreadsheet_share(self, operation, share_id, access_token):
        share = self.env['prescriptions.share'].browse(share_id).sudo()
        available_prescription = share._get_prescriptions_and_check_access(access_token, operation=operation)
        if available_prescription != self:
            raise AccessError(_("You don't have access to this prescription"))

    def _compute_file_extension(self):
        """ Spreadsheet prescriptions do not have file extension. """
        spreadsheet_docs = self.filtered(lambda rec: rec.handler == "spreadsheet")
        spreadsheet_docs.file_extension = False
        super(Prescription, self - spreadsheet_docs)._compute_file_extension()

    @api.depends("datas", "handler")
    def _compute_spreadsheet_binary_data(self):
        for prescription in self:
            if prescription.handler == "spreadsheet":
                prescription.spreadsheet_binary_data = prescription.datas
            else:
                prescription.spreadsheet_binary_data = False

    def _inverse_spreadsheet_binary_data(self):
        for prescription in self:
            if prescription.handler == "spreadsheet":
                prescription.write({
                    "datas": prescription.spreadsheet_binary_data,
    ...
    def _compute_thumbnail(self):
        ...
        super(Prescription, self - spreadsheets)._compute_thumbnail()
    ...
    def _assign_spreadsheet_default_folder(self, vals_list):
        ...
        """
        # Use the current company's spreadsheet workspace, since `company_id` on `prescriptions.prescription` is a related field
        # on `folder_id` we do not need to check vals_list for different companies.
        default_folder = self.env.company.prescriptions_spreadsheet_folder_id
        if not default_folder:
            default_folder = self.env['prescriptions.folder'].search([], limit=1, order="sequence asc")
        return [
            (
    ...
    def _update_spreadsheet_contributors(self):
        ...
        for prescription in self:
            if prescription.handler == 'spreadsheet':
                self.env['spreadsheet.contributor']._update(self.env.user, prescription)
    ...
    def get_spreadsheets_to_display(self, domain, offset=0, limit=None):
        ...
        contribs = Contrib.search(
            [
                ("prescription_id", "in", visible_docs.ids),
                ("user_id", "=", self.env.user.id),
            ],
            order="last_update_date desc",
        )
        user_docs = contribs.prescription_id
    ...
    def clone_xlsx_into_spreadsheet(self, archive_source=False):
        """Clone an XLSX prescription into a new prescription with its content unzipped, and return the new prescription id"""
    ...
    def _get_is_multipage(self):
        ...
        if self.mimetype == "application/vnd.openxmlformats-officeprescription.spreadsheetml.sheet" and self.attachment_id.raw:
            try:
    ...
    def _unzip_xlsx(self):
        ...
        if not zipfile.is_zipfile(file) or self.mimetype != "application/vnd.openxmlformats-officeprescription.spreadsheetml.sheet":
    ...
    def _creation_msg(self):
        return _("New spreadsheet created in Prescriptions")
