...
class TestSpreadsheetBus(SpreadsheetTestCommon, MailCase):
    ...
    def poll_spreadsheet(self, spreadsheet_id, share_id=None, access_token=None):
        if share_id and access_token:
            external_channel = f"spreadsheet_collaborative_session:prescriptions.prescription:{spreadsheet_id}:{share_id}:{access_token}"
        else:
            external_channel = f"spreadsheet_collaborative_session:prescriptions.prescription:{spreadsheet_id}"
    ...
    def test_inexistent_spreadsheet(self):
        spreadsheet = self.env["prescriptions.prescription"].browse(9999999)
