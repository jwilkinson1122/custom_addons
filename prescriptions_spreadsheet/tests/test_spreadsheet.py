...
class SpreadsheetPrescriptions(SpreadsheetTestCommon):
    @classmethod
    def setUpClass(cls):
        super(SpreadsheetPrescriptions, cls).setUpClass()
        cls.folder = cls.env["prescriptions.folder"].create({"name": "Test folder"})

    def test_action_open_new_spreadsheet(self):
        action = self.env["prescriptions.prescription"].action_open_new_spreadsheet()
        ...
        self.assertEqual(prescription.name, "Untitled spreadsheet")
        self.assertEqual(prescription.datas, prescription._empty_spreadsheet_data_base64())
    ...
    def test_action_open_new_spreadsheet_with_locale(self):
        ...
        action = self.env["prescriptions.prescription"].with_user(user).action_open_new_spreadsheet()
        spreadsheet_id = action["params"]["spreadsheet_id"]
        prescription = self.env["prescriptions.prescription"].browse(spreadsheet_id)
        self.assertTrue(prescription.exists())

        data = prescription.join_spreadsheet_session()["data"]
        expected_locale = {
            "code": "en_FR",
            "name": "Custom Locale",
            "thousandsSeparator": " ",
    ...
    def archive_existing_spreadsheet(self):
        ...
        self.env["prescriptions.prescription"].search([("handler", "=", "spreadsheet")]).active = False
    ...
    def test_spreadsheet_default_folder(self):
        ...
        self.assertEqual(
            ...
            self.env.company.prescriptions_spreadsheet_folder_id,
            "It should have been assigned the default Spreadsheet Folder"
        )
        self.env.company.prescriptions_spreadsheet_folder_id = self.env['prescriptions.folder'].create({
    ...
    def test_normal_doc_default_folder(self):
        """Default spreadsheet folder is not assigned to normal prescriptions"""
    ...
    def test_spreadsheet_no_default_folder(self):
        ...
        self.assertEqual(prescription.folder_id, self.folder, "It should be in the specified folder")
    ...
    def test_spreadsheet_to_display_with_domain(self):
        ...
        spreadsheets = self.env["prescriptions.prescription"].get_spreadsheets_to_display([("name", "ilike", "My")])
        spreadsheet_ids = [s["id"] for s in spreadsheets]
        self.assertEqual(spreadsheet_ids, [spreadsheet1.id])
        spreadsheets = self.env["prescriptions.prescription"].get_spreadsheets_to_display([("name", "ilike", "Untitled")])
        spreadsheet_ids = [s["id"] for s in spreadsheets]
        self.assertEqual(spreadsheet_ids, [spreadsheet2.id])
        spreadsheets = self.env["prescriptions.prescription"].get_spreadsheets_to_display([("name", "ilike", "Spreadsheet")])
        spreadsheet_ids = [s["id"] for s in spreadsheets]
    ...
    def test_spreadsheet_to_display_with_offset_limit(self):
        ...
        spreadsheets = self.env["prescriptions.prescription"].get_spreadsheets_to_display([], offset=0, limit=1)
        spreadsheet_ids = [s["id"] for s in spreadsheets]
        self.assertEqual(spreadsheet_ids, [spreadsheet6.id])

        # Two last opened spreadsheets.
        spreadsheets = self.env["prescriptions.prescription"].get_spreadsheets_to_display([], offset=0, limit=2)
        spreadsheet_ids = [s["id"] for s in spreadsheets]
        self.assertEqual(spreadsheet_ids, [spreadsheet6.id, spreadsheet5.id])

        # Ordered first by last opened, and then by id.
        spreadsheets = self.env["prescriptions.prescription"].get_spreadsheets_to_display([], offset=2, limit=2)
        spreadsheet_ids = [s["id"] for s in spreadsheets]
        ...
        spreadsheets = self.env["prescriptions.prescription"].get_spreadsheets_to_display([], offset=2)
        spreadsheet_ids = [s["id"] for s in spreadsheets]
        ...
        spreadsheets = self.env["prescriptions.prescription"].get_spreadsheets_to_display([("name", "ilike", "My Spreadsheet")])
        spreadsheet_ids = [s["id"] for s in spreadsheets]
        self.assertEqual(spreadsheet_ids, [spreadsheet3.id, spreadsheet2.id, spreadsheet1.id])

        # Ordered first by last opened, and then by id without limit with a domain.
        spreadsheets = self.env["prescriptions.prescription"].get_spreadsheets_to_display([("name", "ilike", "SP ")])
        spreadsheet_ids = [s["id"] for s in spreadsheets]
        ...
        # Only the last opened spreadsheet.
        spreadsheets = self.env["prescriptions.prescription"].with_user(user).get_spreadsheets_to_display([], offset=0, limit=1)
        spreadsheet_ids = [s["id"] for s in spreadsheets]
        self.assertEqual(spreadsheet_ids, [spreadsheet4.id])

        # Two last opened spreadsheets.
        spreadsheets = self.env["prescriptions.prescription"].with_user(user).get_spreadsheets_to_display([], offset=0, limit=2)
        spreadsheet_ids = [s["id"] for s in spreadsheets]
        self.assertEqual(spreadsheet_ids, [spreadsheet4.id, spreadsheet2.id])

        # Ordered first by last opened, and then by id.
        spreadsheets = self.env["prescriptions.prescription"].with_user(user).get_spreadsheets_to_display([], offset=2, limit=2)
        spreadsheet_ids = [s["id"] for s in spreadsheets]
        self.assertEqual(spreadsheet_ids, [spreadsheet1.id, spreadsheet6.id])

        # Ordered first by last opened, and then by id without limit.
        spreadsheets = self.env["prescriptions.prescription"].with_user(user).get_spreadsheets_to_display([], offset=2)
        spreadsheet_ids = [s["id"] for s in spreadsheets]
        self.assertEqual(spreadsheet_ids, [spreadsheet1.id, spreadsheet6.id, spreadsheet5.id, spreadsheet3.id])

        # Ordered first by last opened, and then by id without limit with a domain.
        spreadsheets = self.env["prescriptions.prescription"].with_user(user).get_spreadsheets_to_display([("name", "ilike", "My Spreadsheet")])
        spreadsheet_ids = [s["id"] for s in spreadsheets]
        self.assertEqual(spreadsheet_ids, [spreadsheet2.id, spreadsheet1.id, spreadsheet3.id])

        # Ordered first by last opened, and then by id without limit with a domain.
        spreadsheets = self.env["prescriptions.prescription"].with_user(user).get_spreadsheets_to_display([("name", "ilike", "SP ")])
        spreadsheet_ids = [s["id"] for s in spreadsheets]
    ...
    def test_spreadsheet_to_display(self):
        ...
        archived_prescription = self.env["prescriptions.prescription"].create(
            {
                "spreadsheet_data": r"{}",
                "folder_id": self.folder.id,
                "active": False,
        ...
        spreadsheets = self.env["prescriptions.prescription"].get_spreadsheets_to_display([])
        spreadsheet_ids = [s["id"] for s in spreadsheets]
        self.assertTrue(
            prescription.id in spreadsheet_ids, "It should contain the new prescription"
        )
        self.assertFalse(
            archived_prescription.id in spreadsheet_ids,
            "It should not contain the archived prescription",
        )
    ...
    def test_spreadsheet_to_display_without_contrib(self):
        ...
        with freeze_time("2020-02-15 18:00"):
            spreadsheet2 = self.create_spreadsheet()
        spreadsheets = (
            self.env["prescriptions.prescription"].with_user(user).get_spreadsheets_to_display([])
        )
        spreadsheet_ids = [s["id"] for s in spreadsheets]
        self.assertEqual(spreadsheet_ids, [spreadsheet1.id, spreadsheet2.id])
    ...
    def test_spreadsheet_to_display_access_portal(self):
        ...
        with self.assertRaises(
            ...
            self.env["prescriptions.prescription"].with_user(
                portal
            ).get_spreadsheets_to_display([])

    def test_spreadsheet_to_display_access_ir_rule(self):
        user = new_test_user(
            self.env, "Test user", groups="prescriptions.group_prescriptions_manager"
        )

        model = self.env.ref("prescriptions.model_prescriptions_prescription")
        group = self.env.ref("prescriptions.group_prescriptions_manager")
    ...
    def test_spreadsheet_to_display_access_field_groups(self):
        existing_groups = self.env["prescriptions.prescription"]._fields["name"].groups
        self.env["prescriptions.prescription"]._fields["name"].groups = "base.group_system"
        user = new_test_user(
            self.env, "Test user", groups="prescriptions.group_prescriptions_manager"
        )
        ...
        with self.assertRaises(AccessError, msg="field should be protected"):
            self.env["prescriptions.prescription"].with_user(user).get_spreadsheets_to_display([])
        self.env["prescriptions.prescription"]._fields["name"].groups = existing_groups
    ...
    def test_user_right_own_template(self):
        user = new_test_user(
            self.env, "Test user", groups="prescriptions.group_prescriptions_user"
        )
        template = (
    ...
    def test_user_right_not_own_template(self):
        manager = new_test_user(
            self.env, "Test manager", groups="prescriptions.group_prescriptions_manager"
        )
        user = new_test_user(
            self.env, "Test user", groups="prescriptions.group_prescriptions_user"
        )
    ...
    def test_contributor_write_spreadsheet_data(self):
        ...
        prescription.with_user(user).write({"spreadsheet_data": r"{}"})
    ...
    def test_contributor_move_workspace(self):
        prescription = self.create_spreadsheet()
        new_folder = self.env["prescriptions.folder"].create({"name": "New folder"})
        user = new_test_user(
            self.env, "Test Manager", groups="prescriptions.group_prescriptions_manager"
        )
        prescription.with_user(user).write({"folder_id": new_folder.id})
        contributor = self.env["spreadsheet.contributor"].search(
            [("user_id", "=", user.id), ("prescription_id", "=", prescription.id)]
        )
        self.assertEqual(
    ...
    def test_prescription_replacement_with_handler(self):
        prescription = self.env["prescriptions.prescription"].create({
            "spreadsheet_data": r"{}",
            "folder_id": self.folder.id,
            "handler": "spreadsheet",
        ...
        prescription.write(vals)
        self.assertEqual(prescription.handler, "spreadsheet", "The handler must contain the value of the handler mentioned in vals")

    def test_prescription_replacement_data_only(self):
        prescription = self.env["prescriptions.prescription"].create({
            "name": "file",
            "spreadsheet_data": r"{}",
            "folder_id": self.folder.id,
        ...
        prescription.write(vals)
        self.assertEqual(prescription.handler, "spreadsheet", "The handler should not have changed")

    def test_prescription_replacement_with_other_mimetype(self):
        ...
        prescription.write(vals)
        self.assertEqual(prescription.handler, False, "The handler should have been reset")

    def test_prescription_replacement_with_spreadsheet_mimetype(self):
        prescription = self.env["prescriptions.prescription"].create({
            "raw": b'some text',
            "folder_id": self.folder.id,
            "mimetype": "text/plain",
        ...
        prescription.write(vals)
        self.assertEqual(prescription.handler, False, "the file should not be recognized as a spreadsheet")

    def test_prescription_replacement_with_handler_and_other_mimetype(self):
        prescription = self.env["prescriptions.prescription"].create({
            "spreadsheet_data": r"{}",
            "folder_id": self.folder.id,
            "handler": "spreadsheet",
    ...
    def test_create_prescription_with_spreadsheet_mimetype(self):
        prescription = self.env["prescriptions.prescription"].create({
            "spreadsheet_data": r"{}",
            "folder_id": self.folder.id,
            "mimetype": "application/o-spreadsheet",
    ...
    def test_read_spreadsheet_data(self):
        ...
        self.assertEqual(prescription.spreadsheet_data, "{ sheets: [] }")

    def test_read_spreadsheet_data_bin_size_ctx(self):
        prescription = self.env["prescriptions.prescription"].with_context(bin_size=True).create({
            "name": "test.txt",
        ...
        self.assertEqual(prescription.spreadsheet_data, "{}")
    ...
    def test_read_non_spreadsheet_data(self):
        ...
        self.assertEqual(prescription.spreadsheet_data, False)
    ...
    def test_write_spreadsheet_data(self):
        ...
        data = b"{ sheets: [] }"
        prescription.spreadsheet_data = data
        self.assertEqual(prescription.datas, base64.b64encode(data))
    ...
    def test_autovacuum_remove_old_empty_spreadsheet(self):
        self.env["prescriptions.prescription"].search([('handler', '=', 'spreadsheet')]).unlink()

        self.create_spreadsheet(name="Spreadsheet", values={
            "create_date": datetime(2023, 5, 15, 18)
        })
        self.assertEqual(len(self.env["prescriptions.prescription"].search([('handler', '=', 'spreadsheet')])), 1)

        with freeze_time("2023-05-16 19:00"):
            self.env["prescriptions.prescription"]._gc_spreadsheet()
        self.assertEqual(len(self.env["prescriptions.prescription"].search([('handler', '=', 'spreadsheet')])), 0)
