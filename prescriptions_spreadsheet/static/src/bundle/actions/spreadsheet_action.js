...
import { convertFromSpreadsheetTemplate } from "@prescriptions_spreadsheet/bundle/helpers";
import { AbstractSpreadsheetAction } from "@spreadsheet_edition/bundle/actions/abstract_spreadsheet_action";
import { PrescriptionsSpreadsheetControlPanel } from "../components/control_panel/spreadsheet_control_panel";
...
export class SpreadsheetAction extends AbstractSpreadsheetAction {
    ...
    setup() {
        super.setup();
        this.notificationMessage = _t("New spreadsheet created in Prescriptions");
        this.state = useState({
            isFavorited: false,
    ...
    async _fetchData() {
        ...
        if (this.params.convert_from_template) {
            ...
            await this.orm.write("prescriptions.prescription", [this.resId], {
    ...
    async _onSpreadSheetFavoriteToggled(ev) {
        ...
        return await this.orm.call("prescriptions.prescription", "toggle_favorited", [[this.resId]]);
    }
    ...
    async createNewSpreadsheet() {
        const action = await this.orm.call("prescriptions.prescription", "action_open_new_spreadsheet");
    ...
    async _onSpreadSheetNameChanged(detail) {
        ...
        const { name } = detail;
        return this.orm.write("prescriptions.prescription", [this.resId], {
            name,
        });
    }
    ...
    async saveAsTemplate() {
        ...
        this.actionService.doAction("prescriptions_spreadsheet.save_spreadsheet_template_action", {
            additionalContext: {
    ...
    async shareSpreadsheet(data, excelExport) {
        const vals = {
            prescription_ids: [x2ManyCommands.set([this.resId])],
            folder_id: this.record.folder_id,
            type: "ids",
            spreadsheet_shares: [
                {
                    prescription_id: this.resId,
                    spreadsheet_data: JSON.stringify(data),
                    excel_files: excelExport.files,
                },
            ],
        };
        const url = await this.orm.call("prescriptions.share", "action_get_share_url", [vals]);
        return url;
    }
}

SpreadsheetAction.template = "prescriptions_spreadsheet.SpreadsheetAction";
SpreadsheetAction.components = {
    SpreadsheetComponent,
    PrescriptionsSpreadsheetControlPanel,
    SpreadsheetName,
};
