...
export class TemplateDialog extends Component {
    setup() {
        ...
        onWillStart(async () => {
            const defaultFolder = await this.orm.searchRead(
                ...
                ["prescriptions_spreadsheet_folder_id"]
            );
            this.prescriptionsSpreadsheetFolderId = defaultFolder[0].prescriptions_spreadsheet_folder_id[0];
    ...
    async _createSpreadsheet() {
        ...
        this.notificationService.add(_t("New sheet saved in Prescriptions"), {
            type: "info",
            sticky: false,
        });
    ...
    async _getOpenSpreadsheetAction() {
        ...
        if (templateId) {
            return this.orm.call(
                ...
                [templateId, { folder_id: this.props.folderId || this.prescriptionsSpreadsheetFolderId }],
                { context }
            );
        }
        return this.orm.call(
            ...
            [{ folder_id: this.props.folderId || this.prescriptionsSpreadsheetFolderId }],
            { context }
        );
    }
...
TemplateDialog.template = "prescriptions_spreadsheet.TemplateDialog";
