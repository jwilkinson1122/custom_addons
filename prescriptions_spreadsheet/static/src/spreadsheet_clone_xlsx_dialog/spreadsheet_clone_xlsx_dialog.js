...
export class SpreadsheetCloneXlsxDialog extends ConfirmationDialog {
    static template = "prescriptions_spreadsheet.SpreadsheetCloneXlsxDialog";
    static props = {
    ...
    setup() {
        super.setup();
        this.archivePrescriptionState = useState({ willArchive: true });
    ...
    async _confirm() {
        this.execButton(async () => {
            ...
            const spreadsheetId = await this.orm.call(
                ...
                { archive_source: this.archivePrescriptionState.willArchive }
            );
