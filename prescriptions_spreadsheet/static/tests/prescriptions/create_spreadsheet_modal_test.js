...
async function getDocumentBasicData(views = {}) {
    ...
    pyEnv["prescriptions.share"].create({
        name: "Share1",
...
async function initTestEnvWithBlankSpreadsheet(params = {}) {
    const pyEnv = await startServer();
    const prescriptionsFolderId1 = pyEnv["prescriptions.folder"].create({ has_write_access: true });
...
QUnit.module(
    "prescriptions_spreadsheet > create spreadsheet from template modal",
    {
        beforeEach() {
            setupViewRegistries();
            target = getFixture();
            ...
            patchWithCleanup(session.user_companies.allowed_companies[1], {
                prescriptions_spreadsheet_folder_id: 1,
            });
    ...
    () => {
        QUnit.test("Create spreadsheet from kanban view opens a modal", async function (assert) {
            ...
            await click(menu, ".o_prescriptions_kanban_spreadsheet");
            assert.containsOnce(
                target,
        ...
        QUnit.test("Can fetch next templates", async function (assert) {
            ...
            await click(menu, ".dropdown-toggle");
            await click(menu, ".o_prescriptions_kanban_spreadsheet");
            const dialog = prescription.querySelector(".o-spreadsheet-templates-dialog");
        ...
        QUnit.test("Can create a blank spreadsheet from template dialog", async function (assert) {
            ...
            await click(menu, ".dropdown-toggle");
            await click(menu, ".o_prescriptions_kanban_spreadsheet");
            let dialog = prescription.querySelector(".o-spreadsheet-templates-dialog");
            ...
            await click(menu, ".dropdown-toggle");
            await click(menu, ".o_prescriptions_kanban_spreadsheet");
            dialog = prescription.querySelector(".o-spreadsheet-templates-dialog");
            await triggerEvent(dialog.querySelectorAll(".o-template-image")[0], null, "focus");
