...
import { SpreadsheetTemplateAction } from "@prescriptions_spreadsheet/bundle/actions/spreadsheet_template/spreadsheet_template_action";
...
export async function createSpreadsheet(params = {}) {
    ...
    if (!params.spreadsheetId) {
        const prescriptions = params.serverData.models["prescriptions.prescription"].records;
        const spreadsheetId = Math.max(...prescriptions.map((d) => d.id)) + 1;
        prescriptions.push({
            id: spreadsheetId,
