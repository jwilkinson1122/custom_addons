...
QUnit.module(
    "prescriptions_spreadsheet > global_filters side panel",
    {
        beforeEach: function () {
            target = getFixture();
    ...
    () => {
        ...
        QUnit.test("Create a new many2many relational global filter", async function (assert) {
            ...
            serverData.models["custom"] = {
                fields: {},
                records: [],
            };
            serverData.models["partner"].fields.custom_ids = {
                relation: "custom",
                string: "Custom",
                type: "many2many",
                searchable: true,
            ...
            await openGlobalFilterSidePanel();
            await clickCreateFilter("relation");
            await selectModelForRelation("custom");
            assert.strictEqual(
            ...
            const [globalFilter] = model.getters.getGlobalFilters();
            assert.strictEqual(globalFilter.label, "Custom");
            assert.deepEqual(globalFilter.defaultValue, []);
            assert.deepEqual(model.getters.getPivotFieldMatching("1", globalFilter.id), {
                chain: "custom_ids",
                type: "many2many",
        ...
        QUnit.test("Only related models can be selected", async function (assert) {
            ...
            data["partner"].fields.prescription = {
                relation: "prescriptions.prescription",
                string: "Prescription",
                type: "many2one",
            };
            data["partner"].fields.custom_ids = {
                relation: "custom",
                string: "Custom",
                type: "many2many",
            };
            ...
            assert.equal(model3.innerText, "Prescription");
            assert.equal(model4.innerText, "Custom");
        ...
        QUnit.test(
            ...
            async function (assert) {
                ...
                await contains(".o_notification.border-info", {
                    text: "New spreadsheet created in Prescriptions",
                });
        ...
        QUnit.test(
            ...
            async function (assert) {
                ...
                const defaultUserOption = prescription.querySelector("#user_automatic_filter");
        ...
        QUnit.test("Readonly user can update relation filter values", async function (assert) {
            ...
            await click(prescription, "ul.ui-autocomplete li:first-child");
