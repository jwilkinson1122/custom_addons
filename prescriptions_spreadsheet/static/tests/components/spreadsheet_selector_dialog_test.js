...
QUnit.module("prescriptions_spreadsheet > Spreadsheet Selector Dialog", { beforeEach }, () => {
    ...
    QUnit.test("Pager is limited to 9 elements", async (assert) => {
        ...
        data.models["prescriptions.prescription"].records = [];
        // Insert 20 elements
        for (let i = 1; i <= 20; i++) {
            data.models["prescriptions.prescription"].records.push({
                folder_id: 1,
                id: i,
                handler: "spreadsheet",
                name: `Spreadsheet_${i}`,
