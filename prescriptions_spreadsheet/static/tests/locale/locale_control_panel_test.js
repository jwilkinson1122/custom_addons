...
QUnit.module("prescriptions_spreadsheet > Locale Control Panel", {}, function () {
    QUnit.test("No locale icon if user locale matched spreadsheet locale", async function (assert) {
        ...
        const icon = prescription.querySelector(".o_spreadsheet_status .fa-globe");
        assert.notOk(icon);
    });
    ...
    QUnit.test("changing spreadsheet locale to user locale: remove icon", async function (assert) {
        ...
        await nextTick();
        assert.notOk(prescription.querySelector(".o_spreadsheet_status .fa-globe"));
    });
});
