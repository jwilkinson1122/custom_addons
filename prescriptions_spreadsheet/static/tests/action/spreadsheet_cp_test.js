...
QUnit.module(
    "prescriptions_spreadsheet > Spreadsheet Control Panel",
    {
        beforeEach: function () {
            target = getFixture();
        },
    ...
    function () {
        ...
        QUnit.test("changing the input saves the name", async function (assert) {
            ...
            assert.equal(
                serverData.models["prescriptions.prescription"].records[1].name,
                "My spreadsheet",
        ...
        QUnit.test("share spreadsheet from control panel", async function (assert) {
            ...
            const serverData = getBasicServerData();
            serverData.models["prescriptions.prescription"].records = [
                {
                    name: "My spreadsheet",
                    id: spreadsheetId,
                ...
                mockRPC: async function (route, args) {
                    if (args.method === "action_get_share_url") {
                        ...
                        assert.deepEqual(shareVals, {
                            prescription_ids: [x2ManyCommands.set([spreadsheetId])],
                            folder_id: 465,
                            type: "ids",
                            spreadsheet_shares: [
                                {
                                    spreadsheet_data: JSON.stringify(model.exportData()),
                                    prescription_id: spreadsheetId,
                                    excel_files: excel,
                                },
                            ],
                        });
        ...
        QUnit.test("toggle favorite", async function (assert) {
            ...
            await createSpreadsheet({
                spreadsheetId: 1,
                mockRPC: async function (route, args) {
                    if (args.method === "toggle_favorited" && args.model === "prescriptions.prescription") {
                        assert.step("favorite_toggled");
                        assert.deepEqual(args.args[0], [1], "It should write the correct prescription");
                        return true;
                    }
