...
QUnit.module(
    "prescriptions_spreadsheet > Spreadsheet Client Action",
    {
        beforeEach: function () {
            target = getFixture();
        },
    ...
    function () {
        ...
        QUnit.test("breadcrumb is rendered in control panel", async function (assert) {
            ...
            const views = {
                "prescriptions.prescription,false,list": '<tree><field name="name"/></tree>',
                "prescriptions.prescription,false,search": "<search></search>",
            };
        ...
        QUnit.test("dialog window not normally displayed", async function (assert) {
            ...
            const dialog = prescription.querySelector(".o_dialog");
        ...
        QUnit.test("raise error window", async function (assert) {
            ...
            assert.equal(
                prescription.querySelector(".modal-body p").textContent,
                "this is a message in an opened window that requests user action",
                "Can set dialog content"
            );
            assert.equal(
                prescription.querySelector(".o_dialog_error_text"),
                null,
                "NotifyUser have no error text"
            );
            assert.equal(
                prescription.querySelectorAll(".modal-footer button").length,
                1,
        ...
        QUnit.test("ask confirmation when merging", async function (assert) {
            ...
            await click(prescription, ".o_dialog .btn-secondary"); // cancel
            assert.equal(model.getters.isSingleCellOrMerge(sheetId, toZone("A1:A2")), false);

            await click(fixture, ".o-menu-item-button[title='Merge cells']");
            await click(prescription, ".o_dialog .btn-primary"); // confirm
        ...
        QUnit.test("Grid has still the focus after a dialog", async function (assert) {
            ...
            await nextTick();
            await click(prescription, ".modal-footer .btn-primary");
            await nextTick();
            assert.strictEqual(prescription.activeElement.tagName, "INPUT");
            assert.ok([...prescription.activeElement.parentElement.classList].includes("o-grid"));
        });
        ...
        QUnit.test("convert data from template", async function (assert) {
            ...
            serverData.models["prescriptions.prescription"].records.push({
                id: 3000,
            ...
            const { model } = await createSpreadsheet({
                ...
                mockRPC: function (route, { method, model, args }) {
                    if (model === "prescriptions.prescription" && method === "write") {
        ...
        QUnit.test("menu > download as json", async function (assert) {
            ...
            const serverData = getBasicServerData();
            const spreadsheet = serverData.models["prescriptions.prescription"].records[1];
        ...
        QUnit.test("Odoo locales are displayed in setting side panel", async function (assert) {
            ...
            const loadedLocales = [];
            const options = prescription.querySelectorAll(".o-settings-panel select option");
