...
import { SpreadsheetAction } from "@prescriptions_spreadsheet/bundle/actions/spreadsheet_action";
import {
...
QUnit.module("spreadsheet pivot view", {}, () => {
    ...
    QUnit.test("Insert in spreadsheet is disabled when data is empty", async (assert) => {
        ...
        assert.ok(prescription.body.querySelector("button.o_pivot_add_spreadsheet").disabled);
    });
    ...
    QUnit.test("Can save a pivot in a new spreadsheet", async (assert) => {
        ...
        await click(prescription.querySelector(".modal-content > .modal-footer > .btn-primary"));
    ...
    QUnit.test("Can save a pivot in existing spreadsheet", async (assert) => {
        ...
        const webClient = await createWebClient({
            ...
            mockRPC: function (route, args) {
                ...
                if (args.model === "prescriptions.prescription") {
                    switch (args.method) {
    ...
    QUnit.test("Add pivot sheet at the end of existing sheets", async (assert) => {
        ...
        models["prescriptions.prescription"].records = [
            {
    ...
    QUnit.test("Pivot name can be changed from the dialog", async (assert) => {
        ...
        await click(prescription.body.querySelector(".o_pivot_add_spreadsheet"));
        /** @type {HTMLInputElement} */
        const name = prescription.body.querySelector(".o_spreadsheet_name");
        name.value = "New name";
    ...
    QUnit.test("Pivot name is not changed if the name is empty", async (assert) => {
        ...
        await click(prescription.body.querySelector(".o_pivot_add_spreadsheet"));
        prescription.body.querySelector(".o_spreadsheet_name").value = "";
        await click(prescription.querySelector(".modal-content > .modal-footer > .btn-primary"));
        await nextTick();
        const model = getSpreadsheetActionModel(spreadsheetAction);
        await waitForDataSourcesLoaded(model);
