...
QUnit.module("prescriptions_spreadsheet > Topbar Menu Items", {}, function () {
    ...
    QUnit.test("Can make a copy", async function (assert) {
        ...
        const { env, model } = await createSpreadsheet({
            ...
            serverData,
            mockRPC: async function (route, args) {
                if (args.method === "copy" && args.model === "prescriptions.prescription") {
                    assert.step("copy");
                    assert.equal(
    ...
    QUnit.test("Lazy load currencies", async function (assert) {
        ...
        await nextTick();
        await click(prescription.querySelector(".o-sidePanelClose"));
        await doMenuAction(topbarMenuRegistry, menuPath, env);
        await nextTick();
    ...
    QUnit.test("Can Insert odoo formulas from Insert > Functions > Odoo", async function (assert) {
        ...
        await click(prescription.querySelector(".o-topbar-menu[data-id='insert']"));
        await click(prescription.querySelector(".o-menu-item[data-name='insert_function']"));
        await click(prescription.querySelector(".o-menu-root[title='Odoo']"));
        await click(prescription.querySelector(".o-menu-item[title='ODOO.CURRENCY.RATE']"));

        await triggerEvent(prescription.activeElement, null, "keydown", { key: "Enter" });
