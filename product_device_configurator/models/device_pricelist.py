...
class DevicePricelist(models.Model):
    _name = 'device.pricelist'
    _description = "Device Pricelist"

    name = fields.Char(required=True)
    item_ids = fields.One2many(
        'device.pricelist.item',
        'device_pricelist_id',
        string="Items",
    )
...
class DevicePricelistItem(models.Model):
    _name = 'device.pricelist.item'
    _description = "Device Pricelist Item"

    device_pricelist_id = fields.Many2one('device.pricelist', required=True)
    design_id = fields.Many2one('device.design', required=True)
    price = fields.Float(required=True)
    company_id = fields.Many2one(related='device_pricelist_id.company_id', store=True)
    currency_id = fields.Many2one(related='device_pricelist_id.company_id.currency_id')
