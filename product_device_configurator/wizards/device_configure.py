...
from ..device import code, description, name, parsing, price

# To filter nothing.
ALL_DOMAIN = [(1, '=', 1)]


class DeviceConfigure(models.TransientModel):
    """Model to configure device product using specific device logic."""

    _name = 'device.configure'
    _description = "Configure Device Product"
    ...
    die_id = fields.Many2one('device.die', string="Die Type", required=True)
    ...
    design_id = fields.Many2one('device.design', string="Design Type", required=True)
    is_embossed = fields.Boolean(related='design_id.is_embossed')
    embossed_design_perc = fields.Float("% of embossed design")
    material_id = fields.Many2one('device.material', required=True)
    material_counter_id = fields.Many2one('device.material', "Counter-Die Material")
    finishing_id = fields.Many2one('device.finishing', string="Special Finishing")
    difficulty_id = fields.Many2one(
        'device.difficulty', string="Difficulty Level", required=True
    )
    ...
    category_mold_id = fields.Many2one(
        'product.category', 'Mold Category', domain=[('device_type', '=', 'mold')]
    )
    ...
    def _compute_insert_die_ref(self):
        for rec in self:
            ...
            if code:
                design_codes = self.env['device.design'].get_design_codes(
    ...
    @api.depends('size_length', 'size_width', 'partner_id.property_device_pricelist_id')
    def _compute_area(self):
        for rec in self:
            area = rec.size_length * rec.size_width
            rec.area = area
            min_area = rec.partner_id.property_device_pricelist_id.min_area
    ...
    def prepare_product_insert_die_ref_domain(self):
        self.ensure_one()
        return [('device_type', '=', 'die'), ('is_insert_die', '=', False)]

    def action_configure(self):
        """Create products with details using device configurator."""
        self.ensure_one()
    ...
    def _validate_categories(self):
        self.ensure_one()
        self.design_id.category_id.validate_device_type('die')
        # self.category_counter_die_id.validate_device_type('counter_die')
        # self.category_mold_id.validate_device_type('mold')
    ...
    def _post_product_configurator_message(self, product, data):
        ...
        body = (
            '<strong>Device Configurator Parameters Used:</strong>'
            f'<pre>\n\n{data_str}</pre>'
        )
