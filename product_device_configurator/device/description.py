...
def generate_die_description(
    ...
    difficulty_name = device_cfg.difficulty_id.name
    area_description = _get_area_description(device_cfg)
    price_per_sqm_desc = _get_price_per_scm_description(device_cfg, price, price_digits)
    time_description = _get_engraving_time_description(device_cfg, engraving_digits)
    return (
...
def _get_area_description(device_cfg):
    c = device_cfg
    return f'{c.size_length:g}x{c.size_width:g} {AREA_UOM}'


def _get_price_per_scm_description(device_cfg, price, digits):
    price_per_sqm = calc_price_per_sqm(device_cfg, price, digits=digits)
    price_per_sqm_repr = float_repr(price_per_sqm, precision_digits=digits)
    return f'{price_per_sqm_repr} {PRICE_CM_LABEL}'


def _get_engraving_time_description(device_cfg, digits):
    engraving_time = calc_engraving_time(device_cfg)
