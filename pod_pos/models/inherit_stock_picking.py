    ...
    def _create_move_from_pos_order_lines(self, lines):
        ...
        for product, lines in lines_by_product:
            ...
            if first_line.product_id.bom_product:
                bom_product_list = self.env['bom.product'].search(
                    [('bom_product_id.id', '=', first_line.product_id.id)])
                if bom_product_list.state == 'confirm':
                    for item in bom_product_list.sub_products_ids:
                        Move += Move.create(
                            self._prepare_stock_move_vals_for_sub_product(
                                first_line, item, order_lines)
                        )
