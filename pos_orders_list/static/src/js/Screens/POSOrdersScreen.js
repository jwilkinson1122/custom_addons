odoo.define('pos_orders_list.POSOrdersScreen', function (require) {
	...
	class POSOrdersScreen extends PosComponent {
		...
		async filter_option(){
            var self = this;
            var info_data = rpc.query({
                model: 'ir.model.fields',
                method: 'get_selection_type_data',
                args:[[]],
            }).then(function (order_state) {
                self.props.filter_option.push(order_state)
                if(self.state.filter_option.length == 0){
                    self.state.filter_option.push(order_state)
                }
            });
        }
