...
class PrescriptionDeviceLine(models.Model):
    ...
    device_id = fields.Many2one(
        "podiatry.device", ondelete="restrict", index=True)
...
class PodiatryPrescription(models.Model):
    ...
    def _get_bookin_date(self):
        ...
        bookin_date = fields.Datetime.context_timestamp(
    ...
    podiatry_invoice_id = fields.Many2one(
    ...
    def write(self, vals):
        ...
        for rec in self:
            ...
            device_lst = [
                prescription_rec.product_id.id for prescription_rec in rec.device_line_ids]
            ...
            if len(list(new_devices)) != 0:
                ...
                for rm in device_list:
                    device_obj = podiatry_device_obj.search(
                        [("product_id", "=", rm.id)])
...
class PodiatryPrescriptionLine(models.Model):
    ...
    prescription_id = fields.Many2one(
        "podiatry.prescription", "Prescription", ondelete="cascade")
...
class PodiatryServiceLine(models.Model):
    ...
    prescription_id = fields.Many2one(
        "podiatry.prescription", "Prescription", ondelete="cascade")
    ser_bookin_date = fields.Datetime("From Date")
    ser_bookout_date = fields.Datetime("To Date")
    ...
    @api.onchange("ser_bookin_date", "ser_bookout_date")
    def _on_change_bookin_bookout_dates(self):
        ...
        @param self: object pointer
        """
        if not self.ser_bookin_date:
            time_a = time.strftime(DEFAULT_SERVER_DATETIME_FORMAT)
            self.ser_bookin_date = time_a
        if not self.ser_bookout_date:
            self.ser_bookout_date = time_a
        if self.ser_bookout_date < self.ser_bookin_date:
            raise ValidationError(
                _("Bookout must be greater or equal bookin date"))
        if self.ser_bookin_date and self.ser_bookout_date:
            diffDate = self.ser_bookout_date - self.ser_bookin_date
            qty = diffDate.days + 1
