...
class PodiatryServices(models.Model):

    _name = "podiatry.services"
    _description = "Podiatry Services and its charges"
    ...
    def create(self, vals):
        if "service_categ_id" in vals:
            service_categ = self.env["podiatry.service.type"].browse(
                vals.get("service_categ_id")
            )
            vals.update({"categ_id": service_categ.product_categ_id.id})
        return super(PodiatryServices, self).create(vals)
    ...
    def write(self, vals):
        ...
        """
        if "service_categ_id" in vals:
            service_categ_id = self.env["podiatry.service.type"].browse(
                vals.get("service_categ_id")
            )
            vals.update({"categ_id": service_categ_id.product_categ_id.id})
        return super(PodiatryServices, self).write(vals)
...
class PodiatryServiceType(models.Model):
    ...
    service_id = fields.Many2one("podiatry.service.type", "Service Category")
    child_ids = fields.One2many(
        "podiatry.service.type", "service_id", "Service Child Categories"
    )
    ...
    def create(self, vals):
        ...
        return super(PodiatryServiceType, self).create(vals)
    ...
    def write(self, vals):
        if "service_id" in vals:
            ...
            vals.update({"parent_id": service_categ.product_categ_id.id})
        return super(PodiatryServiceType, self).write(vals)
