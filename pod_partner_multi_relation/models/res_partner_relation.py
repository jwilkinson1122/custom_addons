...
class ResPartnerRelation(models.Model):
    ...
    left_partner_id = fields.Many2one( comodel_name="res.partner", string="Source Partner", required=True, auto_join=True, ondelete="cascade",
    )
    right_partner_id = fields.Many2one( comodel_name="res.partner", string="Destination Partner", required=True, auto_join=True, ondelete="cascade",
    )
    type_id = fields.Many2one( comodel_name="res.partner.relation.type", string="Type", required=True, auto_join=True,
    )
    ...
    def _check_partner(self, side):
        ...
        for record in self:
            ...
            if category and category.id not in partner.category_id.ids:
                raise ValidationError(
                    _(
                        "The {partner} partner does not have category {category}." ).format(partner=side, category=category.name)
                )
