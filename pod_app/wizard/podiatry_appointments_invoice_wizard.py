...
class podiatry_appointments_invoice_wizard(models.TransientModel):
    _name = "podiatry.appointments.invoice.wizard"
    _description = 'podiatry appointments invoice wizard'
        ...
        for active_id in active_ids:
            ...
            if lab_req.no_invoice == False:
                sale_journals = self.env['account.journal'].search(
                    [('type', '=', 'sale')])
                invoice_vals = {
                    'name': self.env['ir.sequence'].next_by_code('podiatry_app_inv_seq'),
                ...
                taxes = lab_req.consultations_id.taxes_id.filtered(
                    lambda r: not lab_req.consultations_id.company_id or r.company_id == lab_req.consultations_id.company_id)
                tax_ids = taxes.ids
                invoice_line_vals = {
                ...
                if list_of_ids:
                    ...
                    lab_req_obj_brw = lab_req_obj.browse(
