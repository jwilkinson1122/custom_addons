...
class podiatry_preinatal(models.Model):

    _name = 'podiatry.preinatal'
    _description = 'podiatry preinatal'

    pregnency_id = fields.Many2one('podiatry.patient.pregnency', 'Pregnancy', )
    gestational_weeks = fields.Integer('Gestational weeks')
    admission_date = fields.Date('Admission Date')
    code = fields.Char('Code')
    labour_mode = fields.Selection(
        [('n', 'Normal'), ('i', 'Induced'), ('c', 'C-Section')], 'Labour Mode')
    ...
    monitor_ids = fields.One2many(
        'podiatry.perinatal.monitor', 'podiatry_perinatal_id')
    ...
    hematoma = fields.Selection(
        [('v', 'Vaginal'), ('vl', 'Vulvar'), ('r', 'Retroperitional')], 'Hematoma')
