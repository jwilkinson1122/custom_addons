...
class podiatry_patient(models.Model):
    ...
    weight = fields.Float(string='Weight (lbs)',
                          help='Weight in lbs', required=True)
    ...
    active_status = fields.Selection(
        [('a', 'Active'), ('x', 'Not Active')], string='Status')
    ...
    podiatry_evaluations_ids=fields.One2many(
        'podiatry.evaluation', 'patient_id', string='Evaluations')
    ...
    @ api.model
    def create(self, val):
        evaluation=self._context.get('evaluation_id')
