...
class podiatry_patient(models.Model):
    ...
    def print_report(self):
        return self.env.ref('pod_app.report_print_patient_card').report_action(self)
    ...
    current_insurance_id = fields.Many2one(
        'podiatry.insurance', string="Insurance")
    partner_address_id = fields.Many2one('res.partner', string="Address", )
    primary_care_physician_id = fields.Many2one(
        'podiatry.physician', string="Primary Care Doctor")
    patient_status = fields.Char(
        string="Care Status", readonly=True)
    patient_condition_ids = fields.One2many(
        'podiatry.patient.condition', 'patient_id')
    patient_psc_ids = fields.One2many('podiatry.patient.psc', 'patient_id')
    ...
    cage_ids = fields.One2many('podiatry.patient.cage', 'patient_id')
    ...
    car_revision = fields.Boolean(
        'Car Revision', help="Maintain the vehicle. Do periodical checks - tires,breaks ...")
    car_seat_belt = fields.Boolean(
        'Seat belt', help="Safety measures when driving : safety belt")
    car_child_safety = fields.Boolean(
        'Car Child Safety', help="Safety measures when driving : child seats, proper seat belting, not seating on the front seat, ....")
    home_safety = fields.Boolean(
        'Home safety', help="Keep safety measures for kids in the kitchen, correct storage of chemicals, ...")
    ...
    menstrual_history_ids = fields.One2many(
        'podiatry.patient.menstrual.history', 'patient_id')
    ...
    mammography_history_ids = fields.One2many(
        'podiatry.patient.mammography.history', 'patient_id')
    pap_history_ids = fields.One2many(
        'podiatry.patient.pap.history', 'patient_id')
    colposcopy_history_ids = fields.One2many(
        'podiatry.patient.colposcopy.history', 'patient_id')
    ...
    pregnancy_history_ids = fields.One2many(
        'podiatry.patient.pregnency', 'patient_id')
    family_history_ids = fields.Many2many(
        'podiatry.family.condition', string="Family Condition Lines")
    perinatal_ids = fields.Many2many('podiatry.preinatal')
    ...
    hours_outside = fields.Integer(
        'Hours outside home', help="Number of hours a day the patient spend outside the house")
    ...
    relative_in_prison = fields.Boolean(
        'Relative in prison', help="Check if someone from the nuclear family - parents sibblings  is or has been in prison")
    ...
    lab_test_ids = fields.One2many('podiatry.patient.lab.test', 'patient_id')
    fertile = fields.Boolean('Fertile')
    ...
    podiatry_vaccination_ids = fields.One2many(
        'podiatry.vaccination', 'podiatry_patient_vaccines_id')
    podiatry_appointments_ids = fields.One2many(
        'podiatry.appointment', 'patient_id', string='Appointments')
