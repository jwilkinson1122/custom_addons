...
class podiatry_icu_ecg(models.Model):
    _name = 'podiatry.icu.ecg'
    _description = 'podiatry icu ecg'
    _rec_name = 'podiatry_inpatient_registration_id'
    ...
    lead = fields.Selection([('i', '|'), ('ii', '||'), ('iii', '|||'), ('avf', 'aVF'), ('avr', 'aVR'), (
        'avl', 'aVL'), ('v1', 'V1'), ('v2', 'V2'), ('v3', 'V3'), ('v4', 'V4'), ('v5', 'V5'), ('v6', 'V6')])
    ...
    pacemaker = fields.Selection([('sa', 'sinus Node'), ('av', 'Atrioventricular'), (
        'pk', 'purkinje')], string="Pacemaker", required=True)
    rhythm = fields.Selection(
        [('regular', 'Regular'), ('irregular', 'Irregular')], string="Rhythm", required=True)
    ...
    st_segment = fields.Selection([('normal', 'Normal'), ('depressed', 'Depressed'), (
        'elevated', 'Elevated')], string="ST Segment", required=True)
    twave_inversion = fields.Boolean(string="T Wave Inversion")
    interpretation = fields.Char(
        string="Interpretation", required=True, size=256)
