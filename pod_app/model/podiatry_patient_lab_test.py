...
class podiatry_patient_lab_test(models.Model):
    ...
    podiatry_test_type_id = fields.Many2one(
        'podiatry.test_type', 'Test Type', required=True)
    ...
    def create_lab_test(self):
        ...
        for browse_record in self:
            ...
            if res_ids:
                ...
                action = self.env.ref('pod_app.action_podiatry_lab_form')
                list_view_id = imd.sudo()._xmlid_to_res_id('pod_app.podiatry_lab_tree_view')
                form_view_id = imd.sudo()._xmlid_to_res_id('pod_app.podiatry_lab_form_view')
                result = {
                    'name': action.name,
                    'help': action.help,
                    'type': action.type,
