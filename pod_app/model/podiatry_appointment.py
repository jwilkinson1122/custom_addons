...
class podiatry_appointment(models.Model):
    ...
    institution_partner_id = fields.Many2one(
        'res.partner', domain=[('is_practice', '=', True)], string="Health Center")
    inpatient_registration_id = fields.Many2one(
        'podiatry.inpatient.registration', string="Inpatient Registration")
    ...
    podiatry_patient_psc_ids = fields.Many2many(
        'podiatry.patient.psc', string='Pediatrics Symptoms Checklist')
    podiatry_prescription_order_ids = fields.One2many(
        'podiatry.prescription.order', 'appointment_id', string='Prescription')
    ...
    def onchange_patient(self):
        if not self.inpatient_registration_id:
            self.patient_id = ""
        inpatient_obj = self.env['podiatry.inpatient.registration'].browse(
