...
class podiatry_puerperium_monitor(models.Model):

    _name = 'podiatry.puerperium.monitor'
    _description = 'podiatry puerperium monitor'

    pregnency_id = fields.Many2one('podiatry.patient.pregnency')
    ...
    lochia_amount = fields.Selection(
        [('n', 'Normal'), ('a', 'Abudant'), ('h', 'Hemorrhage'), ], 'Lochia amount')
    lochia_color = fields.Selection(
        [('r', 'Rubra'), ('s', 'Serosa'), ('a', 'Alba')], 'Loicha Color')
    loicha_order = fields.Selection(
        [('n', 'Normal'), ('o', 'Offensive')], 'Loicha Order')
