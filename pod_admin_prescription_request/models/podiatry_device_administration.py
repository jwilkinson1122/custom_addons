...
class PodiatryDeviceAdministration(models.Model):
    _name = "podiatry.device.administration"
    _inherit = "podiatry.event"
    _description = "podiatry device administration"

    def _default_patient_location(self):
        # return self.env.ref('stock.stock_location_customers')
        return self.env.ref("podiatry_device_request.location_patient")

    internal_identifier = fields.Char(string="Rx administration")
    ...
    def _get_internal_identifier(self, vals):
        return (
            self.env["ir.sequence"].next_by_code(
                "podiatry.device.administration"
            )
            or "Rx"
        )
