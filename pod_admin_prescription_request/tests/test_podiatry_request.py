...
class TestPodiatryRequest(TransactionCase):
    def setUp(self):
        ...
        self.patient = self.env["podiatry.patient"].create(
            {"name": "Test Patient"}
        )
        self.patient2 = self.env["podiatry.patient"].create(
            {"name": "Test Patient2"}
        )
    ...
    def test_constrains(self):
        request = self.env["podiatry.device.request"].create(
            {
        ...
        with self.assertRaises(ValidationError):
            self.env["podiatry.device.request"].create(
                {
    ...
    def test_constrains_administration(self):
        ...
        with self.assertRaises(ValidationError):
            self.env["podiatry.device.administration"].create(
                {
                    "patient_id": self.patient2.id,
                    "device_request_id": request.id,
                    "product_id": self.device.id,
    ...
    def test_views(self):
        # device request
        device_request = self.env["podiatry.device.request"].create(
            {
                "patient_id": self.patient.id,
                "product_id": self.device.id,
                "product_uom_id": self.uom_unit.id,
        ...
        device_request.with_context(
            inverse_id="active_id", model_name="podiatry.device.request"
        ).action_view_request()
        # 1 device request
        device_request2 = self.env["podiatry.device.request"].create(
            {
                "patient_id": self.patient.id,
                "product_id": self.device.id,
                "product_uom_id": self.uom_unit.id,
        ...
        # 2 device request
        device_request3 = self.env["podiatry.device.request"].create(
            {
                "patient_id": self.patient.id,
                "product_id": self.device.id,
                "product_uom_id": self.uom_unit.id,
