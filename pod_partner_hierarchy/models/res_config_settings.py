...
class ResConfigSettings(models.TransientModel):
    ...
    def set_values(self):
        ...
        param.set_param(
            "pod_partner_hierarchy.last_no_of_orders", str(self.last_no_of_orders)
        )
        param.set_param(
            "pod_partner_hierarchy.last_no_of_days_orders", str(self.last_no_of_days_orders)
        )
        param.set_param("pod_partner_hierarchy.stages", self.stages)
        param.set_param(
            "pod_partner_hierarchy.enable_reorder",
    ...
    def get_values(self):
        ...
        res.update(
            last_no_of_orders=int(
                param.get_param("pod_partner_hierarchy.last_no_of_orders", default="0")
            ),
            last_no_of_days_orders=int(
                param.get_param("pod_partner_hierarchy.last_no_of_days_orders", default="0")
            ),
            stages=param.get_param("pod_partner_hierarchy.stages"),
            enable_reorder=param.get_param(
                "pod_partner_hierarchy.enable_reorder", default="False"
            )
            == "True",
        )
        return res
