...
class Service(models.Model):
    _name = 'custom_orthotics.service'
    ...
    order_patient_name = fields.Char(
        related='order.patient_name', string='Patient')
    order_practice_name = fields.Char(
        related='order.practice_name', string='Practice')
    ...
    @api.depends('order_patient_name', 'service_name')
    def _compute_fields_rec_name(self):
        for rec in self:
            rec.rec_name = '{} - {}'.format(rec.order_patient_name,
                                            rec.service_name)
