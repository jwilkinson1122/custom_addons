...
class pod_pos_create_sales_order(models.Model):
    _name = 'pod.pos.create.sales.order'
    ...
    def create_sales_order(self, partner_id,orderlines, cashier_id,terms,state,session_id):
        ...
        order_details=[]
        if state == "confirm":
            order_id = sale_object.sudo().create({'partner_id': partner_id, 'user_id': cashier_id,'note': terms,'pos_session_id':session_id,'create_from_pos':True})
            for dict_line in orderlines:
                ...
                sale_order_line_obj.sudo().create(vals)
...
class ResConfigSettings(models.TransientModel):
    ...
    create_sale_order = fields.Boolean(related="pos_config_id.create_sale_order",readonly=False)
