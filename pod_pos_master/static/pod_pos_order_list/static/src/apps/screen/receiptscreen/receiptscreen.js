...
export class PodReceiptScreen extends ReceiptScreen {
    static template = "PodReceiptScreen";
    ...
    setup(){
        ...
        if (
        config.pod_enable_a3_receipt ||
        config.pod_enable_a4_receipt ||
        config.pod_enable_a5_receipt
        ) {
        if (config.pod_default_receipt) {
            this.receipt_type = config.pod_default_receipt;
        }
        }
    }
    confirm() {
...
registry.category("pos_screens").add("PodReceiptScreen", PodReceiptScreen);
