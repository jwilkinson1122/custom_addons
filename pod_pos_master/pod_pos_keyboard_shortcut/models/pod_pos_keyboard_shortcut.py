...
class PodKeyboardKey(models.Model):
...
class PodKeyboardKeyTemp(models.Model):
    _name = "pod.keyboard.key.temp"
    _description = "keyboard Key Temp"

    pod_key_ids = fields.Many2one("pod.keyboard.key", string="Keys ")
    name = fields.Char(string="Display Key")
    pod_pos_key_ids = fields.Many2one("pod.pos.keyboard.shortcut", string="Keys")
    ...
    def create(self, vals):
        ...
        name = ""
        if res.pod_key_ids:
            for each_key in res.pod_key_ids:
                if name != "":
...
class PodPosKeyboardShortcut(models.Model):
    ...
    pod_key_ids = fields.One2many(
        "pod.keyboard.key.temp", "pod_pos_key_ids", string="Keys"
    )
    pod_payment_shortcut_screen_type = fields.Selection(
        [("payment_screen", "Payment Screen")],
        string="Shortcut Screen Type",
        default="payment_screen",
    )
    pod_shortcut_screen_type = fields.Selection(
        [
            ("payment_screen", "Payment Screen"),
    ...
    pod_shortcut_screen = fields.Selection(
        [
