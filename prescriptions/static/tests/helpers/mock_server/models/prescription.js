...
patch(MockServer.prototype, {
    ...
    async _performRPC(route, args) {
        if (args.model === 'prescriptions.prescription' && args.method === 'get_prescription_max_upload_limit') {
            return Promise.resolve(67000000);
        }
        return super._performRPC(...arguments);
    },
    /**
     * Mocks the '_get_models' method of the model 'prescriptions.prescription'.
     *
     * @param {string} model
     * @param {any[]} domain
     */
    _mockGetModels(model, domain) {
        ...
        for (const group of groups) {
            if (!group.res_model) {
            ...
            } else if (group.res_model === 'prescriptions.prescription') {
                notAttached.push({
                    id: group.res_model,
                ...
                const { res_model_name } = this.models['prescriptions.prescription'].records.find(
    ...
    /**
     * Override to handle the specific case of model 'prescriptions.prescription'.
     *
     * @override
     */
    mockSearchPanelSelectRange(model, [fieldName], kwargs) {
        if (model === 'prescriptions.prescription' && fieldName === 'folder_id') {
            const enableCounters = kwargs.enable_counters || false;
            const fields = ['display_name', 'description', 'parent_folder_id', 'has_write_access'];
            const records = this.mockSearchRead('prescriptions.folder', [[], fields], {});
    ...
    mockSearchPanelSelectMultiRange(model, [fieldName], kwargs) {
        ...
        if (model === 'prescriptions.prescription') {
            if (fieldName === 'tag_ids') {
                ...
                const values = folderId ? this._mockPrescriptionsTag_GetTags(domain, folderId) : [];
                return { values };
