...
export class PdfPage extends Component {
    ...
    static template = "prescriptions.component.PdfPage";
    ...
    renderPage(canvas) {
        ...
        this.canvasWrapperRef.el.querySelector(".o_prescriptions_pdf_canvas")?.remove();
    ...
    onDragStart(ev) {
        ...
        ev.dataTransfer.setData("o_prescriptions_pdf_data", this.props.pageId);
    }
    /**
     * @public
     * @param {MouseEvent} ev
    ...
    onDrop(ev) {
        this.state.isHover = false;
        if (!ev.dataTransfer.types.includes("o_prescriptions_pdf_data")) {
            return;
        }
        const pageId = ev.dataTransfer.getData("o_prescriptions_pdf_data");
