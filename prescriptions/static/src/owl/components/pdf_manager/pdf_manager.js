...
import { PdfGroupName } from '@prescriptions/owl/components/pdf_group_name/pdf_group_name';
import { PdfPage } from '@prescriptions/owl/components/pdf_page/pdf_page';
...
import { ExitSplitToolsDialog } from "@prescriptions/owl/components/pdf_exit_dialog/pdf_exit_dialog";
...
export class PdfManager extends Component {
    ...
    static props = {
        ...
        onProcessPrescriptions: { type: Function },
        close: { type: Function },
    };
    static template = "prescriptions.component.PdfManager";
    ...
    setup() {
        ...
        this.state = useState({
            ...
            // whether to archive the original prescriptions.
            archive: true,
            // Whether to keep the prescription(s) or not.
            keepPrescription: true,
            // Whether there are remaining pages to process.
            remaining: false,
            //name of the opened prescription
            fileName: "",
            // edit on group name
            edit: false,
            isSelecting: false,
        ...
        useEffect(
            () => {
                ...
                if (this.props.prescriptions.length === 1) {
                    this.state.fileName = this._removePdfExtension(this.props.prescriptions[0].name);
                }
                for (const pdf_prescription of this.props.prescriptions) {
                    this._addFile(pdf_prescription.name, {
                        url: `/prescriptions/content/${pdf_prescription.id}`,
                        prescriptionId: pdf_prescription.id,
                    });
                }
                prescription.addEventListener("click", _onOutsideClick, true);
                prescription.addEventListener("mousedown", this._onMouseDown, true);
                prescription.addEventListener("mouseup", this._onMouseUp, true);
                prescription.addEventListener("mousemove", this._onMouseMove, true);
                prescription.addEventListener("keydown", this._onShiftDown, true);
                return () => {
                    prescription.removeEventListener("click", _onOutsideClick, true);
                    prescription.removeEventListener("mousedown", this._onMouseDown, true);
                    prescription.removeEventListener("mouseup", this._onMouseUp, true);
                    prescription.removeEventListener("mousemove", this._onMouseMove, true);
                    prescription.removeEventListener("keydown", this._onShiftDown, true);
                };
            },
            () => []
        );
    ...
    _computeCardsPerLine() {
        const allPages = [...prescription.querySelectorAll(".o_prescriptions_pdf_page_frame")];
    ...
    _splitSelectionHandler() {
        ...
        for (const pageId of selectedPages) {
            ...
            if (
                ...
                const parent = prescription
                    .querySelector(`[data-id=${pageId}]`)
                    .closest(".o_prescriptions_pdf_page_frame");
     ...
     * @param {String} name
     * @param {Object} param1
     * @param {number} [param1.prescriptionId] the id of the `prescriptions.prescription` record.
     * @param {Object} [param1.file]
     * @param {String} [param1.url]
     */
    async _addFile(name, { prescriptionId, file, url }) {
        if (!url) {
        ...
        if (file) {
            ...
            this._newFiles[fileId] = { type: "prescription", prescriptionId };
        }
     ...
     * @param {number} number
     */
    _displayNumberCreatedPrescriptions(number) {
        this.notification.add(_t("%s new prescription(s) created", number), { type: "success" });
    }
    /**
     * @private
     * @param {number} number
    ...
    async _applyChanges(ruleId) {
        ...
        if (processedPageIds.length === 0 && !this.state.focusedPage) {
            this._displayErrorNotification(_t("No prescription has been selected"));
            return;
        }
        ...
        try {
            const prescriptionIds = await this._sendChanges();
            this.props.onProcessPrescriptions({ prescriptionIds, ruleId, exit });
            this._displayNumberCreatedPrescriptions(prescriptionIds.length);
            if (!exit) {
    ...
    _createPages({ fileId, name, pageCount }) {
        let groupId;
        let groupName = name;
        const multipleFiles = this.props.prescriptions.length > 1;
        let groupLock = false;
        const pageIds = [];
        const newPages = {};
    ...
    async _loadAssets() {
        try {
            await loadBundle("prescriptions.pdf_js_assets");
        } catch {
    ...
    async _renderCanvas(page, { width, height }) {
        ...
        const canvas = prescription.createElement("canvas");
        canvas.className = "o_prescriptions_pdf_canvas";
        canvas.width = width;
    ...
    async _sendChanges() {
        ...
        for (const newFile of newFiles) {
            ...
            for (const pageId of newFile.pageIds) {
                ...
                const old_file_index =
                    old_file_type === "file" ? fileIds.indexOf(fileId) : file.prescriptionId;
        ...
        newFiles.reverse();
        // Http request
        const prescription = this.props.prescriptions[0];
        ...
        data.append(
            "vals",
            JSON.stringify({
                folder_id: prescription.folder_id[0],
                tag_ids: prescription.tag_ids.currentIds,
                owner_id: prescription.owner_id[0],
                partner_id: prescription.partner_id[0],
                active: this.state.keepPrescription,
            })
        );
        const response = await fetch("/prescriptions/pdf_split", {
            method: "post",
            body: data,
        });
    ...
    /**
     * Gather the remaining pages so they are kept into one prescription that is sent to the backend
     * @private
     */
    ...
    _keepFocusedPageInScreen() {
        const card = prescription.querySelector(`[data-id=${this.state.focusedPage}]`);
     ...
     */
    _onShiftDown(ev) {
        if (prescription.activeElement.classList.contains("o_pdf_name_input")) {
            ev.stopPropagation();
            return;
        }
        if (
    ...
    _onMouseDown(ev) {
        if (
            ...
            ev.target.closest(".o_prescriptions_pdf_manager_top_bar") ||
            ev.target.closest(".o_main_navbar") ||
            ev.target.closest(".o_prescriptions_pdf_page_preview") ||
    ...
    _onMouseMove(ev) {
        ...
        const cards = prescription.querySelectorAll(".o_pdf_page");
     ...
     */
    onClickSplit() {
        this.state.keepPrescription = true;
        this._applyChanges();
    }
    /**
     * @public
        ...
        this.dialog.add(ConfirmationDialog, {
            ...
            confirm: async () => {
                ...
                if (this.state.numberOfPages === 0) {
                    this.props.onProcessPrescriptions({ isForcingDelete: true });
                    await this.props.close();
                }
    ...
    _onOutsideClick(ev) {
        if (
            (!this._exitSplitToolsClick &&
                ...
                ev.target.closest(".dropdown-item")?.dataset.menu !== "prescriptionation" &&
    ...
    async onClickPage(pageId) {
        ...
        const previewWrapper = prescription.querySelector(".o_prescriptions_pdf_page_preview");
        const ratio = 18 / 13;
    ...
    onClickExitPreview(ev) {
        if (
            ev.target.classList.contains("o_prescriptions_pdf_page_preview") ||
