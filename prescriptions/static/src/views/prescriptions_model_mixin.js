...
import { inspectorFields } from "./inspector/prescriptions_inspector";
import { makeActiveField } from "@web/model/relational_model/utils";
import { ConfirmationDialog } from "@web/core/confirmation_dialog/confirmation_dialog";

export const PrescriptionsModelMixin = (component) =>
    class extends component {
        ...
        setup(params) {
            ...
            if (this.config.resModel === "prescriptions.prescription") {
        ...
        async load() {
            ...
            this.env.bus.trigger("prescriptions-close-preview");
...
export const PrescriptionsRecordMixin = (component) => class extends component {
    ...
    onRecordClick(ev, options = {}) {
        ...
        const anchor = root._prescriptionsAnchor;
        if (!isRangeSelection || root.selection.length === 0) {
            root._prescriptionsAnchor = this;
        }
    ...
    async onDragStart(ev) {
        ...
        if (draggableRecords.length === 1) {
            ...
            dragText = _t("%s Prescriptions (%s locked)", draggableRecords.length, lockedCount);
        } else {
            dragText = _t("%s Prescriptions", draggableRecords.length);
        }
        const newElement = prescription.createElement("span");
        newElement.classList.add("o_prescriptions_drag_icon");
        newElement.innerText = dragText;
        prescription.body.append(newElement);
    ...
    async openDeleteConfirmationDialog(root, callback, isPermanent) {
        const dialogProps = {
            ...
            body: isPermanent ? root.isDomainSelected || root.selection.length > 1
                ? _t("Are you sure you want to permanently erase the prescriptions?")
                : _t("Are you sure you want to permanently erase the prescription?")
            ...
            confirm: async () => {
                await callback();
                await this.model.env.prescriptionsView.bus.trigger("prescriptions-close-preview");
            },
            cancel: () => {},
        };
