...
import { PrescriptionsKanbanCompiler } from "./prescriptions_kanban_compiler";
...
export class PrescriptionsKanbanRecord extends KanbanRecord {
    setup() {
        super.setup();
        // File upload
        ...
        useBus(bus, "FILE_UPLOAD_ADDED", (ev) => {
            if (ev.detail.upload.data.get("prescription_id") == this.props.record.resId) {
                this.render(true);
            }
        });
    ...
    getFileUpload() {
        return Object.values(this.prescriptionUploads).find(
            (upload) => upload.data.get("prescription_id") == this.props.record.resId
        );
    }
    ...
    onGlobalClick(ev) {
        ...
        // Preview is clicked
        if (ev.target.closest("div[name='prescription_preview']")) {
...
PrescriptionsKanbanRecord.components = {
    ...KanbanRecord.components,
    FileUploadProgressBar,
};
PrescriptionsKanbanRecord.defaultProps = {
    ...KanbanRecord.defaultProps,
    Compiler: PrescriptionsKanbanCompiler,
};

PrescriptionsKanbanRecord.template = xml`
    <div
        role="article"
