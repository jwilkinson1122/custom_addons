...
export class PrescriptionsInspectorField extends Field {
    get fieldComponentProps() {
        ...
        if (
            ...
            new Set(
                this.props.prescriptions.map((rec) => (rec.data[this.props.name] ? rec.data[this.props.name][0] : false))
            ).size > 1
        ) {
        ...
        if (!record.isPrescriptionsInspector) {
            record.isPrescriptionsInspector = true;
