...
import { PrescriptionsInspectorField } from "./prescriptions_inspector_field";
import { download } from "@web/core/network/download";
import { onNewPdfThumbnail } from "../helper/prescriptions_pdf_thumbnail_service";
import dUtils from "@prescriptions/views/helper/prescriptions_utils";
import { useTriggerRule, toggleArchive } from "@prescriptions/views/hooks";
...
export class PrescriptionsInspector extends Component {
    static props = [
        ...
        "prescriptions", // Array of records
        "fields",
    ];
    ...
    setup() {
        ...
        this.prescriptionsReplaceInput = useRef("replaceFileInput");
        ...
        useBus(fileUploadBus, "FILE_UPLOAD_LOADED", (ev) => {
            const prescriptionId = ev.detail.upload.data.get("prescription_id");
            if (prescriptionId && this.resIds.includes(Number.parseInt(prescriptionId))) {
        ...
        const updateLockedState = (props) => {
            this.isLocked =
                (props.prescriptions.find(
                    (rec) => rec.data.lock_uid && rec.data.lock_uid[0] !== session.uid
                ) &&
                    true) ||
                false;
            const folderIds = props.prescriptions.map((rec) => rec.data.folder_id[0]);
        ...
        const chatterReloadHandler = async () => {
            const record = this.props.prescriptions[0];
            if (!record) {
                return;
            }
            await record.load();
        ...
        // Pdf thumbnails
        this.pdfService = useService("prescriptions_pdf_thumbnail");
        onWillStart(() => {
            this.pdfService.enqueueRecords(this.props.prescriptions);
        });
        onWillUpdateProps((nextProps) => {
            this.pdfService.enqueueRecords(nextProps.prescriptions);
        });
        onNewPdfThumbnail(({ detail }) => {
            if (this.props.prescriptions.find((rec) => rec.resId === detail.record.resId)) {
                this.render(true);
            }
        });
        ...
        onWillUpdateProps((nextProps) => {
            ...
            this.shouldOpenInspector = nextProps.prescriptions.length === 1;
        });
        onPatched(() => {
    ...
    get resIds() {
        return this.props.prescriptions.map((rec) => rec.resId);
    }
    ...
    updateAttachmentHistory(nextProps) {
        ...
        const record = props.prescriptions[0];
        if (props.prescriptions.length !== 1) {
            this.state.showChatter = this.isMobile;
        }
        if (!record || props.prescriptions.length !== 1 || !record.data.previous_attachment_ids.count) {
            this.keepLast.add(Promise.resolve());
            this.state.previousAttachmentData = null;
            return;
        }
        const previousRecord = this.props.prescriptions.length === 1 && this.props.prescriptions[0];
        if (
            nextProps &&
            previousRecord &&
    ...
    getPreviewClasses(record, additionalData) {
        const nbPreviews = this.props.prescriptions.length;
        const classes = ["o_prescription_preview"];
        if (record.data.type === "empty") {
            classes.push("o_prescription_request_preview");
        }
        if (nbPreviews === 1) {
            classes.push("o_prescriptions_single_preview");
        }
        if (
            ...
            classes.push("o_prescriptions_preview_image");
        } else {
            classes.push("o_prescriptions_preview_mimetype");
        }
    ...
    isPdfOnly() {
        return this.props.prescriptions.every((record) => record.isPdf());
    }
    ...
    download(records) {
        if (records.length === 1) {
            download({
                data: {},
                url: `/prescriptions/content/${records[0].resId}`,
            });
        } else {
            download({
                data: {
                    file_ids: records.map((rec) => rec.resId),
                    zip_name: `prescriptions-${serializeDate(DateTime.now())}.zip`,
                },
                url: "/prescription/zip",
            });
        }
    }

    onDownload() {
        const prescriptions = this.props.prescriptions.filter((rec) => rec.data.type !== "empty");
        if (!prescriptions.length) {
            return;
        }
        const linkPrescriptions = prescriptions.filter((el) => el.data.type === "url");
        const noLinkPrescriptions = prescriptions.filter((el) => el.data.type !== "url");
        // Manage link prescriptions
        if (prescriptions.length === 1 && linkPrescriptions.length) {
            // Redirect to the link
            let url = linkPrescriptions[0].data.url;
            url = /^(https?|ftp):\/\//.test(url) ? url : "http://" + url;
            window.open(url, "_blank");
        } else if (noLinkPrescriptions.length) {
            // Download all prescriptions which are not links
            this.download(noLinkPrescriptions);
        }
    }

    async onShare() {
        const resIds = this.props.prescriptions
    ...
    async createShareVals() {
        const resIds = this.props.prescriptions
            ...
            .map((rec) => rec.resId);
        return {
            prescription_ids: [x2ManyCommands.set(resIds)],
    ...
    async onReplace(ev) {
        ...
        const record = this.props.prescriptions[index];

        await this.env.prescriptionsView.bus.trigger("prescriptions-upload-files", {
    ...
    async onLock() {
        await this.doLockAction(async () => {
            const record = this.props.prescriptions[0];
            await this.orm.call("prescriptions.prescription", "toggle_lock", this.resIds);
            await record.load();
        });
    }
    ...
    onDelete() {
        const records = this.props.prescriptions;
    ...
    getFieldProps(fieldName, additionalProps) {
        // `prescriptions` might come from a state, and can be a Proxy object at this point
        ...
        const rec = this.props.prescriptions[0];
    ...
    _getCommonM2M(field) {
        ...
        let commonData = prescriptions[0].data[field].records.map((rec) => rec.resId);
        for (let idx = 1; idx < prescriptions.length; idx++) {
            if (commonData.length === 0) {
                break;
            }
            commonData = intersection(
                commonData,
                prescriptions[idx].data[field].records.map((rec) => rec.resId)
            );
        }
        return commonData.map((id) =>
            prescriptions[0].data[field].records.find((data) => data.resId === id)
        );
    }
    ...
    async triggerRule(rule) {
        await this._triggerRule(
            this.props.prescriptions.map((rec) => rec.resId),
            rule.resId
        );
    }
    ...
    async onDeletePreviousAttachment(attachmentId) {
        ...
        await this.doLockAction(async () => {
            ...
            const record = this.props.prescriptions[0];
            const model = this.props.prescriptions[0].model;
            await record.load();
            this.state.previousAttachmentDirty = true;
            await model.notify();
            this.deleting = false;
    ...
    async onRestorePreviousAttachment(attachmentId) {
        const record = this.props.prescriptions[0];
        await this.doLockAction(async () => {
            await this.orm.write("prescriptions.prescription", [record.resId], {
    ...
    openPreview(mainPrescription = false, isPdfSplit = false) {
        if ((isPdfSplit && !this.isPdfOnly()) || this.previewLockCount) {
            return;
        }
        const prescriptions = this.props.prescriptions.filter((rec) => rec.isViewable());
        if (!prescriptions.length) {
            return;
        }
        this.env.prescriptionsView.bus.trigger("prescriptions-open-preview", {
            prescriptions: prescriptions,
            mainPrescription: mainPrescription || prescriptions[0],
            isPdfSplit,
        ...
        this.action.doAction(
            {
                ...
                target: "new",
                context: {
                    default_prescription_ids: [record.resId],
    ...
    onDeleteModel() {
        const recordId = this.props.prescriptions[0].resId;
        const model = this.props.prescriptions[0].model;
        this.dialogService.add(ConfirmationDialog, {
            body: _t("Do you really want to unlink this record?"),
            confirm: async () => {
                await this.orm.call("prescriptions.workflow.rule", "unlink_record", [[recordId]]);
                await model.load();
                model.notify();
            },
        });
...
if (uiUtils.isSmall()) {
    PrescriptionsInspector.template = "prescriptions.PrescriptionsInspectorMobile";
} else {
    PrescriptionsInspector.template = "prescriptions.PrescriptionsInspector";
}
