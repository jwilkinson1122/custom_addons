...
export const prescriptionsPdfThumbnailService = {
    dependencies: ["orm"],
    start(env, { orm }) {
        let enabled = true;
        ...
        const checkForThumbnail = async (record) => {
            ...
            try {
                ...
                if (Object.keys(writeData).length) {
                    await orm.write("prescriptions.prescription", [record.resId], writeData);
                    record.data.thumbnail_status = writeData.thumbnail_status;
                    if (writeData.thumbnail) {
                        env.bus.trigger("prescriptions-new-pdf-thumbnail", { record });
                    }
                }
...
export function onNewPdfThumbnail(callback) {
    ...
    useBus(component.env.bus, "prescriptions-new-pdf-thumbnail", callback);
}

registry.category("services").add("prescriptions_pdf_thumbnail", prescriptionsPdfThumbnailService);
