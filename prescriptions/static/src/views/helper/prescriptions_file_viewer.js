...
import { FileViewer } from "@prescriptions/attachments/prescription_file_viewer";
import { Component, useEffect, useRef, useState } from "@odoo/owl";

export class PrescriptionsFileViewer extends Component {
    static props = [
    ...
    setup() {
        ...
        useEffect(
            (iframe) => {
                ...
                const onLoad = () => {
                    // In case of youtube links contentPrescription might be null.
                    if (!iframe.contentPrescription) {
                        return;
                    }
                    iframe.contentPrescription.addEventListener("keydown", onKeydown);
                };
    ...
    onIframeKeydown(ev) {
        if (ev.key === "Escape") {
            this.env.model.env.prescriptionsView.bus.trigger("prescriptions-close-preview");
        }
    }
}
PrescriptionsFileViewer.components = {
    FileViewer,
};
PrescriptionsFileViewer.template = "prescriptions.PrescriptionsFileViewer";
