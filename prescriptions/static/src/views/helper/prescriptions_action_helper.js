...
export class PrescriptionsActionHelper extends Component {
    static props = [
                ...
                `<p class='o_view_nocontent_smiling_face'>
                    ${escape(
                        this.selectedFolderId === "TRASH"
                            ? _t("Deleted prescriptions will show up here")
                            : _t("Select a workspace to upload a prescription")
                    )}
                </p>`
            );
        }
    ...
    async updateShareInformation() {
        ...
        const shares = await this.orm.searchRead("prescriptions.share", domain, ["id", "alias_id"], {
            limit: 1,
        });
        if (shares.length) {
...
PrescriptionsActionHelper.template = "prescriptions.PrescriptionsActionHelper";
