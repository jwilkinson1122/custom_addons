...
export class PrescriptionsDropZone extends Component {
    static props = [
    ...
    get rootDropOverClass() {
        return this.isFolder ? "o_prescriptions_drop_over" : "o_prescriptions_drop_over_unauthorized";
    }

    onDragOver(ev) {
        if (
            !ev.dataTransfer.types.includes("Files") ||
            ev.dataTransfer.types.includes("o_prescriptions_data")
        ) {
            return;
        }
        ev.stopPropagation();
    ...
    onDrop(ev) {
        ...
        this.state.dragOver = false;
        if (this.isFolder) {
            this.env.prescriptionsView.bus.trigger("prescriptions-upload-files", {
...
PrescriptionsDropZone.template = "prescriptions.PrescriptionsDropZone";
