...
export class PrescriptionsSearchModel extends SearchModel {
    ...
    async load(config) {
        ...
        this.deletionDelay = await this.orm.call("prescriptions.prescription", "get_deletion_delay", [[]]);
    ...
    /**
     * Returns a description of each folder (record of prescriptions.folder).
     * @returns {Object[]}
     */
    getFolders() {
    ...
    /**
     * Returns a description of each tag (record of prescriptions.tag).
     * @returns {Object[]}
     */
    getTags() {
        const { values } = this.getSections(isTagFilter)[0];
    ...
    async updateRecordFolderId(recordIds, valueId) {
        await this.orm.write("prescriptions.prescription", recordIds, {
            folder_id: valueId,
        });
        this.trigger("update");
    }
    ...
    /**
     * Adds a new fake folder to see all currently archived prescriptions.
     * @override
     */
