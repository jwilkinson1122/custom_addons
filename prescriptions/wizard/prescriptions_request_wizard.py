...
class RequestWizard(models.TransientModel):
    _name = "prescriptions.request_wizard"
    ...
    activity_type_id = fields.Many2one('mail.activity.type',
                                       ...
                                       default=lambda self: self.env.ref('prescriptions.mail_prescriptions_activity_data_md',
    ...
    def request_prescription(self):
        ...
        if request_by_mail:
            share_vals = {
                ...
                'prescription_ids': [(4, prescription.id)],
                'activity_note': self.activity_note,
            }
            if deadline:
                share_vals['date_deadline'] = deadline
            share = self.env['prescriptions.share'].create(share_vals)
            share.with_context(clean_context(self.env.context)).send_share_by_mail('prescriptions.mail_template_prescription_request')
            prescription.create_share_id = share

        activity = prescription.with_context(mail_activity_quick_update=request_by_mail).activity_schedule(**activity_vals)
        prescription.request_activity_id = activity
        return prescription
