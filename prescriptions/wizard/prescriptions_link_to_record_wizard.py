...
class LinkToRecordWizard(models.TransientModel):
    _name = "prescriptions.link_to_record_wizard"
    _description = "Prescriptions Link to Record"

    def _get_model_domain(self):
        models = self.env['ir.model.access']._get_allowed_models() - {'prescriptions.prescription'}
    ...
    def _compute_accessible_model_ids(self):
        model_ids = self.env['ir.model'].sudo().search([('model', '!=', 'prescriptions.prescription'), ('is_mail_thread', '=', 'True')])
    ...
    def link_to(self):
        self.ensure_one()
        self.prescription_ids.with_company(self.env.company).write({
