...
class TestTags(TransactionCase):
    ...
    def test_create_tag(self):
        marketing_assets = self.ref('prescriptions.prescriptions_marketing_assets')
        tag = self.env['prescriptions.tag'].create({
            'name': 'Foo',
    ...
    def test_get_tags(self):
        folder_id = self.ref('prescriptions.prescriptions_marketing_folder')
        facet_assets = self.env['prescriptions.facet'].browse(self.ref('prescriptions.prescriptions_marketing_assets'))
        tag_assets_ads = self.env['prescriptions.tag'].browse(self.ref('prescriptions.prescriptions_marketing_assets_ads'))
        tag_assets_videos = self.env['prescriptions.tag'].browse(self.ref('prescriptions.prescriptions_marketing_assets_Videos'))

        domain = [('folder_id', '=', folder_id)]
        folder_ids = self.env['prescriptions.folder'].search([('parent_folder_id', 'parent_of', folder_id)]).ids
        tags = self.env['prescriptions.tag']._get_tags(domain, folder_ids)
        processed_tags = self.env['prescriptions.prescription']._get_processed_tags(domain, folder_ids)
    ...
    def test_get_tags_reordered(self):
        folder_id = self.ref('prescriptions.prescriptions_marketing_folder')
        facet_assets = self.env['prescriptions.facet'].browse(self.ref('prescriptions.prescriptions_marketing_assets'))
        tag_assets_images = self.env['prescriptions.tag'].browse(self.ref('prescriptions.prescriptions_marketing_assets_images'))
        tag_assets_videos = self.env['prescriptions.tag'].browse(self.ref('prescriptions.prescriptions_marketing_assets_Videos'))
    ...
    def test_get_tags_empty_folder(self):
        empty_folder_id = self.env['prescriptions.folder'].create({
            'name': 'Empty Folder',
            'parent_folder_id': self.env.ref('prescriptions.prescriptions_marketing_folder').id,
        }).id
        facet_assets = self.env['prescriptions.facet'].browse(self.ref('prescriptions.prescriptions_marketing_assets'))
        tag_assets_ads = self.env['prescriptions.tag'].browse(self.ref('prescriptions.prescriptions_marketing_assets_ads'))
        tag_assets_videos = self.env['prescriptions.tag'].browse(self.ref('prescriptions.prescriptions_marketing_assets_Videos'))
        ...
        folder_ids = self.env['prescriptions.folder'].search([('parent_folder_id', 'parent_of', empty_folder_id)]).ids
        tags = self.env['prescriptions.tag']._get_tags(domain, folder_ids)
