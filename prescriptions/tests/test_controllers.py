# -*- coding: utf-8 -*-

import io
import zipfile
import base64

from odoo import http, fields
from odoo.tests.common import HttpCase
from odoo.tools import mute_logger


class TestPrescriptionsRoutes(HttpCase):
    def setUp(self):
        super().setUp()
        self.folder_a, self.folder_b = self.env['prescriptions.folder'].create([
            {'name': 'folder A'},
            {'name': 'folder B'},
        ])
        self.prescription_txt = self.env['prescriptions.prescription'].create({
            'raw': b'TEST',
            'name': 'file.txt',
            'mimetype': 'text/plain',
            'folder_id': self.folder_a.id,
        })
        self.share_folder_b = self.env['prescriptions.share'].create(
            {
                'folder_id': self.folder_b.id,
                'type': 'domain',
                'action': 'downloadupload',
            }
        )

    def test_prescriptions_content(self):
        self.authenticate('admin', 'admin')
        response = self.url_open('/prescriptions/content/%s' % self.prescription_txt.id)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.content, b'TEST')

    def test_prescriptions_zip(self):
        self.authenticate('admin', 'admin')
        response = self.url_open('/prescription/zip', data={
            'file_ids': [self.prescription_txt.id],
            'zip_name': 'testZip.zip',
            'csrf_token': http.Request.csrf_token(self),
        })
        self.assertEqual(response.status_code, 200)
        with io.BytesIO(response.content) as buffer, zipfile.ZipFile(buffer) as zipfile_obj:
            self.assertEqual(zipfile_obj.read(self.prescription_txt.name), b'TEST')

    def test_prescriptions_zip_authentification(self):

        self.authenticate('admin', 'admin')

        vals = {
            "name": "private_folder",
            "parent_folder_id": False,
            "company_id": False,
            "facet_ids": [],
            "group_ids": [self.env.ref('base.group_system').id],
            "user_specific_write": True,
            "read_group_ids": [self.env.ref('base.group_system').id],
            "user_specific": True,
            "description": False
        }
        workspace_private = self.env['prescriptions.folder'].create(vals)

        vals = {
            "name": "public_folder",
        }
        workspace_public = self.env['prescriptions.folder'].create(vals)

        rawpdf_base64 = '' + 682*'ICAg' + ''
        rawpdf = base64.b64decode(rawpdf_base64.encode())

        private_doc = self.env['prescriptions.prescription'].create({
            'raw': rawpdf,
            'name': 'private_file.pdf',
            'mimetype': 'application/pdf',
            'folder_id': workspace_private.id,
        })

        prescription_group_id_manager = self.env.ref('prescriptions.group_prescriptions_manager')
        prescription_group_id_user = self.env.ref('prescriptions.group_prescriptions_user')

        user = self.env.user.create({'name': 'test_doc', 'login': 'test_doc', 'password': 'test_doc'})   # User with explicitely removed manager right and given user right on the prescription app
        user.groups_id -= prescription_group_id_manager
        user.groups_id += prescription_group_id_user

        self.assertNotIn(private_doc, self.env['prescriptions.prescription'].with_user(user.id).search([]))   # Our new user should not be able to find the private doc

        prescription_public = self.env['prescriptions.prescription'].create({
            'raw': rawpdf,
            'name': 'public.pdf',
            'mimetype': 'application/pdf',
            'folder_id': workspace_public.id,
        })

        self.assertIn(prescription_public, self.env['prescriptions.prescription'].with_user(user.id).search([])) # but he should see the public prescription

        self.authenticate('test_doc', 'test_doc')

        with mute_logger('odoo.http'):
            response = self.url_open('/prescription/zip', data={
                'file_ids': f'{private_doc.id},{prescription_public.id}',
                'zip_name': 'testZip.zip',
                'csrf_token': http.Request.csrf_token(self),
            })
            self.assertNotEqual(response.status_code, 200)

        with mute_logger('odoo.http'):
            response = self.url_open('/prescription/zip', data={
                'file_ids': f'{prescription_public.id},{private_doc.id}',
                'zip_name': 'testZip.zip',
                'csrf_token': http.Request.csrf_token(self),
            })
            self.assertNotEqual(response.status_code, 200)

    def test_prescriptions_from_web(self):
        self.authenticate('admin', 'admin')
        raw_gif = b"R0lGODdhAQABAIAAAP///////ywAAAAAAQABAAACAkQBADs="
        prescription_gif = self.env['prescriptions.prescription'].create({
            'raw': raw_gif,
            'name': 'file.gif',
            'mimetype': 'image/gif',
            'folder_id': self.folder_a.id,
        })
        response = self.url_open('/web/image/%s?model=prescriptions.prescription' % prescription_gif.id)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.content, raw_gif)

    def test_prescriptions_share_expired_link(self):
        self.authenticate('admin', 'admin')
        # Test on available link
        tomorrow = fields.Date.from_string(fields.Date.add(fields.Date.today(), days=1))
        vals = {
            'prescription_ids': [(6, 0, [self.prescription_txt.id])],
            'folder_id': self.folder_a.id,
            'date_deadline': tomorrow,
            'type': 'ids',
        }
        self.result_share_prescriptions_act = self.env['prescriptions.share'].create(vals)
        response = self.url_open(self.result_share_prescriptions_act.full_url)
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'is sharing content with you', response.content, "Failed route test on available link")

        # Test on expired link
        vals = {
            'prescription_ids': [(6, 0, [self.prescription_txt.id])],
            'folder_id': self.folder_a.id,
            'date_deadline': '2001-11-05',
            'type': 'ids',
        }
        self.result_share_prescriptions_act = self.env['prescriptions.share'].create(vals)
        response = self.url_open(self.result_share_prescriptions_act.full_url)
        self.assertEqual(response.status_code, 200)
        self.assertTrue(b'Sorry, this link is no longer valid.' in response.content, "Failed route test on expired link")

    def test_download_all_prescriptions_with_url(self):
        prescription_url = self.env['prescriptions.prescription'].create({
            'name': 'file.txt',
            'type': 'url',
            'folder_id': self.folder_a.id,
        })
        vals = {
            'prescription_ids': [(6, 0, [self.prescription_txt.id, prescription_url.id])],
            'folder_id': self.folder_a.id,
            'type': 'ids',
        }
        share = self.env['prescriptions.share'].create(vals)
        response = self.url_open(f"/prescription/download/all/{share.id}/{share.access_token}")
        self.assertEqual(response.status_code, 200)

    def test_upload_attachment_public(self):
        """Check the upload and notifications for public users."""
        files = [('files', ('test.txt', b'test', 'plain/text'))]
        response = self.url_open(
            f'/prescription/upload/{self.share_folder_b.id}/{self.share_folder_b.access_token}', files=files
        )
        prescription = self.env['prescriptions.prescription'].search([('folder_id', '=', self.folder_b.id)])
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(prescription), 1)
        self.assertEqual(prescription.name, 'test.txt')
        self.assertEqual(prescription.raw, b'test')
        self.assertEqual(prescription.mimetype, 'plain/text')

        file_uploaded_note = prescription.message_ids[0]
        self.assertEqual(file_uploaded_note.author_id, self.share_folder_b.create_uid.partner_id)
        self.assertIn('<b>File uploaded by:</b> Public user <br>', file_uploaded_note.body)
        self.assertIn(f'<b>Link created by:</b> {self.share_folder_b.create_uid.name}', file_uploaded_note.body)

    def test_upload_attachment_user(self):
        """Check that logged user's name is used in notification."""
        files = [('files', ('test.txt', b'test', 'plain/text'))]
        self.authenticate('demo', 'demo')
        self.url_open(f'/prescription/upload/{self.share_folder_b.id}/{self.share_folder_b.access_token}', files=files)
        prescription = self.env['prescriptions.prescription'].search([('folder_id', '=', self.folder_b.id)])

        file_uploaded_note = prescription.message_ids[0]
        self.assertEqual(file_uploaded_note.author_id, self.share_folder_b.create_uid.partner_id)
        self.assertIn('<b>File uploaded by:</b> Marc Demo <br>', file_uploaded_note.body)
        self.assertIn(f'<b>Link created by:</b> {self.share_folder_b.create_uid.name}', file_uploaded_note.body)
