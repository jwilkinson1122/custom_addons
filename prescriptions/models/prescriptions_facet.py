...
class TagsCategories(models.Model):
    _name = "prescriptions.facet"
    _description = "Category"
    _order = "sequence, name"

    folder_id = fields.Many2one('prescriptions.folder', string="Workspace", ondelete="cascade")
    name = fields.Char(required=True, translate=True)
    tag_ids = fields.One2many('prescriptions.tag', 'facet_id', copy=True)
