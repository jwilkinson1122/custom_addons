...
class PrescriptionFolder(models.Model):
    _name = 'prescriptions.folder'
    _description = 'Prescriptions Workspace'
    ...
    _sql_constraints = [
        ('check_user_specific', 'CHECK(not ((NOT user_specific OR user_specific IS NULL) and user_specific_write))',
            'Own Prescriptions Only may not be enabled for write groups if it is not enabled for read groups.')
    ]
    ...
    def default_get(self, fields):
        res = super(PrescriptionFolder, self).default_get(fields)
    ...
    parent_folder_id = fields.Many2one('prescriptions.folder',
    ...
    children_folder_ids = fields.One2many('prescriptions.folder', 'parent_folder_id', string="Sub workspaces")
    prescription_ids = fields.One2many('prescriptions.prescription', 'folder_id', string="Prescriptions")
    sequence = fields.Integer('Sequence', default=10)
    share_link_ids = fields.One2many('prescriptions.share', 'folder_id', string="Share Links")
    is_shared = fields.Boolean(compute='_compute_is_shared')
    facet_ids = fields.One2many('prescriptions.facet', 'folder_id', copy=True,
                                string="Tag Categories",
                                help="Tag categories defined for this workspace")
    group_ids = fields.Many2many('res.groups', copy=True,
        string="Write Groups", help='Groups able to see the workspace and read/create/edit its prescriptions.')
    read_group_ids = fields.Many2many('res.groups', 'prescriptions_folder_read_groups', copy=True,
        string="Read Groups", help='Groups able to see the workspace and read its prescriptions without create/edit rights.')

    user_specific = fields.Boolean(string="Own Prescriptions Only",
                                   help="Limit Read Groups to the prescriptions of which they are owner.")
    user_specific_write = fields.Boolean(string="Own Prescriptions Only (Write)",
                                    compute='_compute_user_specific_write', store=True, readonly=False,
                                    help="Limit Write Groups to the prescriptions of which they are owner.")
    has_write_access = fields.Boolean('Prescription User Upload Rights', compute="_compute_has_write_access")
    ...
    def _compute_deletion_delay(self):
        self.deletion_delay = self.env['prescriptions.prescription'].get_deletion_delay()
    ...
    def _compute_is_shared(self):
        ...
        doc_share_read_group = self.env['prescriptions.share']._read_group(
            search_domain,
    ...
    def _compute_has_write_access(self):
        ...
        has_write_access = self.user_has_groups('prescriptions.group_prescriptions_manager')
        if has_write_access:
    ...
    def _compute_action_count(self):
        read_group_var = self.env['prescriptions.workflow.rule']._read_group(
    ...
    def _get_old_id_to_new_id_maps(self, copied_folder):
        ...
        copied_folder_facet_map, copied_folder_tag_map = [get_old_id_to_new_id_map(self.id, copied_folder.id, table) for table in ('prescriptions_facet', 'prescriptions_tag')]
    ...
    def _copy_workflow_rules_and_actions(self, copied_folder, old_facet_id_to_new_facet_id, old_tag_id_to_new_tag_id):
        ...
        for workflow_rule in self.env['prescriptions.workflow.rule'].search([('domain_folder_id', '=', self.id)]):
        ...
        old_workflow_actions = self.env['prescriptions.workflow.action'].search([
    ...
    def copy(self, default=None):
        ...
        self.env['prescriptions.tag'].flush_model(['folder_id'])
    ...
    def action_see_actions(self):
        return {
            'name': _('Actions'),
            'res_model': 'prescriptions.workflow.rule',
    ...
    def _compute_prescription_count(self):
        ...
        prescription_count_dict = {folder.id: count for folder, count in read_group_var}
        for record in self:
    ...
    def _get_max_sequence_inside_parent(self, parent_folder_id):
        result = self.env['prescriptions.folder'].sudo().search_read(
    ...
    def action_unarchive(self):
        ...
        # If at least one prescription linked to that folder is active, It means that the unarchived function is triggered
        # from the prescription.prescription model. No other linked prescription or child folder is unarchived.
        if self.prescription_ids.filtered(self._active_name):
            return res
        ...
        # Unarchive prescriptions linked to the folder
        self.with_context({'active_test': False}).prescription_ids.action_unarchive()
    ...
    def is_folder_containing_prescription(self):
        return self.prescription_ids or (
            self.children_folder_ids and self.children_folder_ids.is_folder_containing_prescription()
        )

    def action_delete_folder(self):
        if self.is_folder_containing_prescription():
            return {
                'name': _('Move to trash?'),
                'type': 'ir.actions.act_window',
                'res_model': 'prescriptions.folder',
                'res_id': self.id,
                'views': [(self.env.ref('prescriptions.folder_deletion_form', False).id, 'form')],
                'target': 'new',
            }
        self.action_archive()
    ...
    def action_archive(self):
        if not self:
            return
        self.prescription_ids.action_archive()
        # Recursive call of the function to the children folders
        self.children_folder_ids.action_archive()
        # If no prescription or child folder is linked to the folder, unlink it. Otherwise, archive it.
        removable_folders = self.with_context({'active_test': False}).filtered(
            lambda folder: not folder.prescription_ids and not folder.children_folder_ids
        )
        removable_folders.unlink()
        return super(PrescriptionFolder, self - removable_folders).action_archive()
    ...
    def unlink(self):
        ...
        removable_parent_folders = self.parent_folder_id.with_context({'active_test': False}).filtered(
            lambda folder: (not folder.prescription_ids and len(folder.children_folder_ids) == 1 and not folder.active)
        )
        res = super().unlink()
