...
from .prescriptions_facet import N_FACET_COLORS
...
class Prescription(models.Model):
    ...
    thumbnail_status = fields.Selection([
            ('present', 'Present'), # Prescription has a thumbnail
    ...
    tag_ids = fields.Many2many('prescriptions.tag', 'prescription_tag_rel', string="Tags")
    partner_id = fields.Many2one('res.partner', string="Contact", tracking=True)
    owner_id = fields.Many2one('res.users', default=lambda self: self.env.user.id, string="Owner",
                               tracking=True)
    available_rule_ids = fields.Many2many('prescriptions.workflow.rule', compute='_compute_available_rules',
    ...
    create_share_id = fields.Many2one('prescriptions.share', help='Share used to create this prescription')
    ...
    _sql_constraints = [
        ('attachment_unique', 'unique (attachment_id)', "This attachment is already a prescription"),
    ]
    ...
    def _compute_is_multipage(self):
        for prescription in self:
            ...
            prescription.is_multipage = bool(prescription._get_is_multipage())  # None => False
    ...
    def _inverse_res_model(self):
        for record in self:
            attachment = record.attachment_id.with_context(no_prescription=True)
            if attachment:
    ...
    def _compute_thumbnail_status(self):
        ...
        prescriptions_with_thumbnail = set(res['res_id'] for res in self.env['ir.attachment'].sudo().search_read(domain, ['res_id']))
        for prescription in self:
            if prescription.mimetype == 'application/pdf':
                ...
                prescription.thumbnail_status = False
            else:
                prescription.thumbnail_status = prescription.id in prescriptions_with_thumbnail and 'present'
    ...
    def get_deletion_delay(self):
        return int(self.env['ir.config_parameter'].sudo().get_param('prescriptions.deletion_delay', '30'))

    def _get_is_multipage(self):
        """
        :return: Whether the prescription can be considered multipage or `None` if unable determine
        :rtype: bool | None
        """
    ...
    def _get_models(self, domain):
        ...
        :param domain: the domain of the _read_group on prescriptions.
        ...
        for res_model, count in groups:
            ...
            elif res_model == 'prescriptions.prescription':
                not_attached.append({
                    'id': res_model,
    ...
    def _inverse_is_favorited(self):
        unfavorited_prescriptions = favorited_prescriptions = self.env['prescriptions.prescription'].sudo()
        for prescription in self:
            if self.env.user in prescription.favorited_ids:
                unfavorited_prescriptions |= prescription
            else:
                favorited_prescriptions |= prescription
        favorited_prescriptions.write({'favorited_ids': [(4, self.env.uid)]})
        unfavorited_prescriptions.write({'favorited_ids': [(3, self.env.uid)]})
    ...
    def _compute_available_rules(self):
        ...
        rules = self.env['prescriptions.workflow.rule'].sudo().search(rule_domain)
        for rule in rules:
            domain = []
            ...
            prescription_ids = self.env['prescriptions.prescription'].search(subset)
            for prescription in prescription_ids:
                prescription.available_rule_ids = [(4, rule.id, False)]

    @api.constrains('url')
    def _check_url(self):
        for prescription in self.filtered("url"):
            if not prescription.url.startswith(('https://', 'http://', 'ftp://')):
                raise ValidationError(_('URL %s does not seem complete, as it does not begin with http(s):// or ftp://', prescription.url))
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        # Remove non existing tags to allow saving prescription with the mail alias
        ...
        if tags and isinstance(tags, (list, tuple)) and isinstance(tags[0], (list, tuple)):
            custom_values['tag_ids'] = [(tags[0][0], tags[0][1],
                                             self.env['prescriptions.tag'].browse(tags[0][2]).exists().ids)]
        ...
        return super(Prescription, self).message_new(msg_dict, defaults)

    @api.returns('mail.message', lambda value: value.id)
    def message_post(self, *, message_type='notification', **kwargs):
        if message_type == 'email' and self.create_share_id:
            self = self.with_context(no_prescription=True)
        return super(Prescription, self).message_post(message_type=message_type, **kwargs)
    ...
    def _message_post_after_hook(self, message, msg_vals):
        ...
        if share and not self.env.context.get("no_prescription") or message.message_type == 'email':
            ...
            prescriptions = self.env['prescriptions.prescription'].create([{
            ...
            for (attachment, prescription) in zip(attachments, prescriptions):
                attachment.write({
                    'res_model': 'prescriptions.prescription',
                    'res_id': prescription.id,
                })
                prescription.message_post(body=msg_vals.get('body', ''), subject=self.name)
                if share.activity_option:
                    prescription.prescriptions_set_activity(settings_record=share)

        return super(Prescription, self)._message_post_after_hook(message, msg_vals)

    def prescriptions_set_activity(self, settings_record=None):
        """
    ...
    def access_content(self):
        ...
        if self.url:
            ...
            action['url'] = '/prescriptions/content/%s' % self.id
        return action
    ...
    def create_share(self):
        ...
        vals = {
            'type': 'ids',
            'prescription_ids': [(6, 0, self.ids)],
            'folder_id': self.folder_id.id,
        }
        return self.env['prescriptions.share'].open_share_popup(vals)
    ...
    def toggle_lock(self):
        ...
        Members of the group prescriptions.group_prescription_manager and the superuser can unlock the file regardless.
        """
        self.ensure_one()
        if self.lock_uid:
            if self.env.user == self.lock_uid or self.env.is_admin() or self.user_has_groups(
                    'prescriptions.group_prescription_manager'):
                self.lock_uid = False
        else:
    ...
    def _compute_is_locked(self):
        for record in self:
            record.is_locked = record.lock_uid and not (
                    ...
                    self.user_has_groups('prescriptions.group_prescription_manager'))

    def action_archive(self):
        if not self:
            return
        active_prescriptions = self.filtered(self._active_name)
        ...
        active_prescriptions._message_log_batch(bodies={doc.id: log_message for doc in active_prescriptions})
        return super().action_archive()

    def action_unarchive(self):
        if not self:
            return
        archived_prescriptions = self.filtered(lambda record: not record[self._active_name])
        log_message = _("This file has been restored")
        archived_prescriptions._message_log_batch(bodies={doc.id: log_message for doc in archived_prescriptions})
        # Unarchive folders linked to archived prescriptions
        archived_prescriptions.folder_id.filtered(lambda folder: not folder[self._active_name]).action_unarchive()
    ...
    def create(self, vals_list):
        ...
        for prescription, attachment in zip(prescriptions, attachments):
            if (attachment and not attachment.res_id and (not attachment.res_model or attachment.res_model == 'prescriptions.prescription')):
                attachment.with_context(no_prescription=True).write({
                    'res_model': 'prescriptions.prescription',
                    'res_id': prescription.id})
    ...
    def write(self, vals):
        if vals.get('folder_id') and not self.env.is_superuser():
            folder = self.env['prescriptions.folder'].browse(vals.get('folder_id'))
            if not folder.has_write_access:
                raise AccessError(_("You don't have the right to move prescriptions to that workspace."))
        ...
        for record in self:
            ...
            if record.type == 'empty' and ('datas' in vals or 'url' in vals):
                body = _("Prescription Request: %s Uploaded by: %s", record.name, self.env.user.name)
                record.with_context(no_prescription=True).message_post(body=body)
            ...
            if record.attachment_id:
                ...
                if 'datas' in vals:
                    old_attachment = record.attachment_id.with_context(no_prescription=True).copy()
                ...
                res_model = vals.get('res_model', record.res_model or 'prescriptions.prescription')
                res_id = vals.get('res_id') if vals.get('res_model') else record.res_id if record.res_model else record.id
                if res_model and res_model != 'prescriptions.prescription' and not self.env[res_model].browse(res_id).exists():
                    record.res_model = res_model = 'prescriptions.prescription'
                    record.res_id = res_id = record.id
                attachment = self.env['ir.attachment'].with_context(no_prescription=True).create({
                    'name': vals.get('name', record.name),
                    'res_model': res_model,
                    'res_id': res_id
                })
                record.attachment_id = attachment.id
                record.with_context(no_prescription=True)._process_activities(attachment.id)
        ...
        write_result = super(Prescription, self).write(vals)
        if attachment_dict:
    ...
    def _process_activities(self, attachment_id):
        ...
        if attachment_id and self.request_activity_id:
            feedback = _("Prescription Request: %s Uploaded by: %s", self.name, self.env.user.name)
    ...
    def search_panel_select_range(self, field_name, **kwargs):
        if field_name == 'folder_id':
            ...
            available_folders = self.env['prescriptions.folder'].search([])
            ...
            PrescriptionFolder = self.env['prescriptions.folder'].sudo().with_context(hierarchical_naming=False)
            records = PrescriptionFolder.search_read(folder_domain, fields)
        ...
        return super(Prescription, self).search_panel_select_range(field_name)
    ...
    def _get_processed_tags(self, domain, folder_id):
        ...
        """
        tags = self.env['prescriptions.tag']._get_tags(domain, folder_id)
    ...
    def search_panel_select_multi_range(self, field_name, **kwargs):
        ...
        return super(Prescription, self).search_panel_select_multi_range(field_name, **kwargs)

    @api.model
    def get_prescription_max_upload_limit(self):
        try:
            return int(self.env['ir.config_parameter'].sudo().get_param('prescription.max_fileupload_size', default=0))
        except Exception:
            return False

    def unlink(self):
        """Remove its folder when deleting a prescription to ensure we don't retain unnecessary folders in the database.
        ...
        removable_folders = self.folder_id.with_context({'active_test': False}).filtered(
            lambda folder: len(folder.prescription_ids) == 1
