...
class DeviceCustomLogContract(models.Model):
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _name = 'device.custom.log.contract'
    _description = 'Custom Contract'
    ...
    custom_id = fields.Many2one('device.custom', 'Custom', required=True, check_company=True)
    cost_subtype_id = fields.Many2one('device.service.type', 'Type', help='Cost type purchased with this cost', domain=[('category', '=', 'contract')])
    ...
    purchaser_id = fields.Many2one(related='custom_id.driver_id', string='Driver')
    ...
    service_ids = fields.Many2many('device.service.type', string="Included Services")

    @api.depends('custom_id.name', 'cost_subtype_id')
    ...
    def write(self, vals):
        res = super(DeviceCustomLogContract, self).write(vals)
        ...
        if vals.get('expiration_date') or vals.get('user_id'):
            self.activity_reschedule(['device.mail_act_device_contract_to_renew'], date_deadline=vals.get('expiration_date'), new_user_id=vals.get('user_id'))
        return res
    ...
    def scheduler_manage_contract_expiration(self):
        ...
        # It manages the state of a contract, possibly by posting a message on the custom concerned and updating its status
        ...
        reminder_activity_type = self.env.ref('device.mail_act_device_contract_to_renew', raise_if_not_found=False) or self.env['mail.activity.type']
        ...
        for contract in nearly_expired_contracts:
            contract.activity_schedule(
                'device.mail_act_device_contract_to_renew', contract.expiration_date,
