...
class FileStorage(models.Model):
    ...
    __slots__ = ("__file", "__odoo_storage_path")
    ...
    code = fields.Char(
        ...
        "'ir_attachment.storage.force.database' when the module 'file_attachment' "
        "is installed.",
    )
    ...
    @api.model
    def get_by_code(self, code) -> FileStorage:
    ...
    def get_file_by_code(self, code):
        ...
        """
        file = None
        file_storage = self.get_by_code(code)
        if file_storage:
            file = pod_file_storage.file
        return file
    ...
    def _get_protocols(self) -> list[tuple[str, str]]:
        protocol = [("odoofile", "Odoo's FileSystem")]
    ...
    def _get_marker_file_name(self):
        return ".odoo_file_storage_%s.marker" % self.id

    def _marker_file_check_connection(self, file):
        marker_file_name = self._get_marker_file_name()
        try:
            file.info(marker_file_name)
        except FileNotFoundError:
            file.touch(marker_file_name)

    def _ls_check_connection(self, file):
        file.ls("", detail=False)

    def _check_connection(self, file, check_connection_method):
        if check_connection_method == "marker_file":
            self._marker_file_check_connection(file)
        elif check_connection_method == "ls":
            self._ls_check_connection(file)
        return True

    @property
    def file(self) -> fsspec.AbstractFileSystem:
        """Get the fsspec filesystem for this backend."""
        self.ensure_one()
        if not self.__file:
            self.__file = self.sudo()._get_filesystem()
        if not tools.config["test_enable"]:
            # Check whether we need to invalidate File cache or not.
            # Use a marker file to limit the scope of the LS command for performance.
            try:
                self._check_connection(self.__file, self.check_connection_method)
            except Exception as e:
                self.__file.clear_instance_cache()
                self.__file = None
                raise e
        return self.__file
    ...
    def _recursive_add_odoo_storage_path(self, options: dict) -> dict:
        ...
        if "target_protocol" in options:
            ...
            if options["target_protocol"] == "odoofile":
        ...
        for key, value in options.items():
            if isinstance(value, dict):
                ...
                if env_variable_value is not None:
                    ...
                    values[key] = value
                    _logger.warning(
                        "Environment variable %s is not set for file_storage %s.",
                        env_variable_name,
                        self.display_name,
                    )
            else:
    ...
    def _get_filesystem(self) -> fsspec.AbstractFileSystem:
        ...
        file = fsspec.filesystem(self.protocol, **options)
        directory_path = self.directory_path
        if directory_path:
            file = fsspec.filesystem("rooted_dir", path=directory_path, file=file)
        return file

    # Deprecated methods used to ease the migration from the storage_backend addons
    # to the file_storage addons. These methods will be removed in the future (Odoo 18)
    ...
    def add(self, relative_path, data, binary=True, **kwargs) -> None:
        ...
        path = relative_path.split(self.file.sep)[:-1]
        if not self.file.exists(self.file.sep.join(path)):
            self.file.makedirs(self.file.sep.join(path))
        with self.file.open(relative_path, "wb", **kwargs) as f:
            f.write(data)

    @deprecated("Please use _get_filesystem() instead and the fsspec API directly.")
    def get(self, relative_path, binary=True, **kwargs) -> AnyStr:
        data = self.file.read_bytes(relative_path, **kwargs)
    ...
    def list_files(self, relative_path="", pattern=False) -> list[str]:
        relative_path = relative_path or self.file.root_marker
        if not self.file.exists(relative_path):
            return []
        if pattern:
            relative_path = self.file.sep.join([relative_path, pattern])
            return self.file.glob(relative_path)
        return self.file.ls(relative_path, detail=False)
    ...
    def find_files(self, pattern, relative_path="", **kw) -> list[str]:
        ...
        for file_path in self.file.ls(relative_path, detail=False):
            # file.ls returns a relative path
    ...
    def move_files(self, files, destination_path, **kw) -> None:
        ...
        for file_path in files:
            self.file.move(
                file_path,
                self.file.sep.join([destination_path, os.path.basename(file_path)]),
                **kw,
            )

    @deprecated("Please use _get_filesystem() instead and the fsspec API directly.")
    def delete(self, relative_path) -> None:
        self.file.rm_file(relative_path)
    ...
    def action_test_config(self):
        ...
        if self.check_connection_method:
            ...
            action = self.env["ir.actions.actions"]._for_xml_id(
                "pod_file_storage.act_open_file_test_connection_view"
            )
            action["context"] = {"active_model": "file.storage", "active_id": self.id}
            return action

    def _test_config(self, connection_method):
        try:
            self._check_connection(self.file, connection_method)
