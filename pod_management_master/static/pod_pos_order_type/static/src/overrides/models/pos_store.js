...
import { OrderTypePopup } from "@pod_management_master/static/pod_pos_order_type/apps/popups/order_type_popup/order_type_popup"

patch(PosStore.prototype, {
    async _processData(loadedData) {
        await super._processData(...arguments);
        if (this.config.enable_order_type) {
            this.order_types = loadedData['pod.order.type'];
        }
    }
});
...
patch(Order.prototype, {
    ...
    init_from_JSON(json){
        ...
        if (json && json.pod_order_type_id) {
            const ordertype = this.pos.order_types.filter(type => type.id == json.pod_order_type_id)[0];
    ...
    export_as_JSON() {
        ...
        if (this.current_order_type) {
            json.pod_order_type_id = this.get_order_type().id 
        }
        return json;
    }, 
    export_for_printing(){
