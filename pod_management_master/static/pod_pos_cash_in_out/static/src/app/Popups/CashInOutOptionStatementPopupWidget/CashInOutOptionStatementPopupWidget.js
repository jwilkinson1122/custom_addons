...
export class CashInOutOptionStatementPopupWidget extends AbstractAwaitablePopup {
    static template = "pod_pos_cash_in_out.CashInOutOptionStatementPopupWidget";
        setup() {
            super.setup();
        ...
        async print() {
            ...
            if (statementValue && statementPrintValue) {
                let all_cash_in_out_statement =  await this.orm.call("pod.cash.in.out", "search_read", [
                    [['pod_session', '=', self.pos.pos_session.id]]
                ])
                if (statementValue == "current_session" && statementPrintValue == "pdf") {
                    if(all_cash_in_out_statement) {
                        ...
                        if (all_cash_in_out_statement && all_cash_in_out_statement.length > 0) {
                            for(let each_cash_in_out_statement of all_cash_in_out_statement){   
                                if (
                                    ...
                                    each_cash_in_out_statement.pod_session &&
                                    each_cash_in_out_statement.pod_session[0] &&
                                    each_cash_in_out_statement.pod_session[0] == self.pos.pos_session.id
                                ) {
                        ...
                        if (self.pos.db.all_cash_in_out_statement_id && self.pos.db.all_cash_in_out_statement_id.length > 0) {
                            self.report.doAction("pod_management_master.pod_pos_cash_in_out_report", [self.pos.db.all_cash_in_out_statement_id
                            ]
                            );
                        } else {
                    ...
                    if (all_cash_in_out_statement && all_cash_in_out_statement.length) {
                        ...
                        for(let each_cash_in_out_statement of all_cash_in_out_statement){
                            if (
                                ...
                                each_cash_in_out_statement.pod_session &&
                                each_cash_in_out_statement.pod_session &&
                                each_cash_in_out_statement.pod_session == self.pos.pos_session.id
                                ) {
                                    self.pos.display_cash_in_out_statement.push(each_cash_in_out_statement);
                            }
                        };
                    ...
                    if ($(".start_date").val() && $(".end_date").val()) {
                        if ($(".start_date").val() > $(".end_date").val()) {
                            ...
                            let all_cash_in_out_statement =await this.orm.call("pod.cash.in.out", "search_read", [
                                [['pod_date','>=',start_date],["pod_date", "<=", end_date]]
                            ])
                           if(all_cash_in_out_statement) {
                                self.pos.db.all_cash_in_out_statement_id = [];
                                if (all_cash_in_out_statement && all_cash_in_out_statement.length > 0) {
                                    ...
                                    if (self.pos.db.all_cash_in_out_statement_id && self.pos.db.all_cash_in_out_statement_id.length > 0) {
                                        self.report.doAction("pod_management_master.pod_pos_cash_in_out_date_wise_report", [ self.pos.db.all_cash_in_out_statement_id
                                            ]
                                        );
                                    }
                                    this.props.close({ confirmed: true, payload: await this.getPayload() });
                    ...
                    if ($(".start_date").val() && $(".end_date").val()) {
                        if ($(".start_date").val() > $(".end_date").val()) {
                            ...
                            if (all_cash_in_out_statement && all_cash_in_out_statement.length > 0) {
                                self.pos.display_cash_in_out_statement = [];
                                for(let each_cash_in_out_statement of all_cash_in_out_statement){
                                    if (each_cash_in_out_statement.pod_date && each_cash_in_out_statement.pod_date >= start_date && each_cash_in_out_statement.pod_date <= end_date) {
                                        self.pos.display_cash_in_out_statement.push(each_cash_in_out_statement);
                                    }
                                };
                                if (self.pos.display_cash_in_out_statement && self.pos.display_cash_in_out_statement.length > 0) {
        ...
        changeStatementOption(event){
            if ($(event.target).val() == "current_session") {
                $(".pod_statement_date").removeClass("show");
            } else if ($(event.target).val() == "date_wise") {
                $(".pod_statement_date").addClass("show");
            }
        }
    }
