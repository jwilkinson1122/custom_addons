...
export class GlobalDiscountPopupWidget extends AbstractAwaitablePopup {
  static template = "pod_management_master.GlobalDiscountPopupWidget";
  setup() {
    super.setup();
    this.pos = usePos();
    $(".pod_discount_value").focus();
  }
  async confirm() {
    var self = this;
    this.props.resolve({ confirmed: true, payload: await this.getPayload() });

    if (!$(".pod_discount_value").val()) {
      alert("Enter amount of discount.");
      $(".pod_discount_value").addClass("invalid_number");
    } else if (
      ($(".pod_discount_value").val() &&
        parseFloat($(".pod_discount_value").val()) > 100 &&
        document.getElementById("discount_percentage_radio") &&
        document.getElementById("discount_percentage_radio").checked) ||
      ($(".pod_discount_value").val() &&
        parseFloat($(".pod_discount_value").val()) < 0) ||
      !/^\d*\.?\d*$/.test(parseFloat($(".pod_discount_value").val()))
    ) {
      $(".pod_discount_value").addClass("invalid_number");
      $(".pod_discount_value").val(" ");
      $(".pod_discount_value").focus();
    } else {
      var value = $(".pod_discount_value").val();
      value = !isNaN(value)
        ? value
