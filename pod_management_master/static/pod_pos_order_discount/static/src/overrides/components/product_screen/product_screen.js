...
import { GlobalDiscountPopupWidget } from "@pod_management_master/static/pod_pos_order_discount/apps/popups/global_discount_popup/global_discount_popup";
...
patch(ProductScreen.prototype, {
  ...
  onNumpadClick(buttonValue) {
    ...
    if (
      ...
      self.pos.config.pod_allow_order_line_discount
    ) {
  ...
  _setValue(val) {
    ...
    if (order.get_selected_orderline()) {
        if (mode == "discount") {
            ...
            var pod_dic =order.get_selected_orderline().get_global_discount()
            pod_dic = parseFloat(pod_dic).toFixed(2)
            order.get_selected_orderline().set_discount(pod_dic);
