...
import { ProductStockRestrict } from "@pod_management_master/static/pod_pos_wh_stock/app/popups/restrict_sale_popup/restrict_sale_popup";
    ...
    async addProductToCurrentOrder(product, options = {}) {
        ...
        if (this.config.pod_show_qty_location && this.config.pod_display_stock && product.type == "product") {
            ...
            if (line && line.length) {
                ...
                for (let ol of line) {
                    qty += ol.quantity
                    if (ol && ol.product.id == product.id && (stock_qty - qty) <= pod_min_qty) {
                        restrict_popup = true
                    }
                }
                if (restrict_popup) {
