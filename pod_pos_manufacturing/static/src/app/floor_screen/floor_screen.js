...
import { EditSection } from "@pod_pos_manufacturing/app/floor_screen/edit_section";
import { EditLine } from "@pod_pos_manufacturing/app/floor_screen/edit_line";
import { Section } from "@pod_pos_manufacturing/app/floor_screen/section";
...
export class FloorScreen extends Component {
    ...
    static template = "pod_pos_manufacturing.FloorScreen";
    ...
    async _save(section) {
        ...
        if (section.id) {
            await this.orm.write("manufacturing.section", [section.id], sectionCopy);
        } else {
            const sectionId = await this.orm.create("manufacturing.section", [sectionCopy]);
    ...
    async _renameFloor(floorId, newName) {
        await this.orm.call("manufacturing.floor", "rename_floor", [floorId, newName]);
    }
    get activeFloor() {
    ...
    async addFloor() {
        ...
        const floor = await this.orm.call("manufacturing.floor", "create_from_ui", [
            newName,
            "#ACADAD",
            this.pos.config.id,
        ]);
    ...
    async duplicateSectionOrFloor() {
        if (this.selectedSections.length == 0) {
            ...
            const newFloor = await this.orm.call("manufacturing.floor", "create_from_ui", [
                newFloorName,
    ...
    async setFloorColor(color) {
        ...
        await this.orm.write("manufacturing.floor", [this.activeFloor.id], {
    ...
    async deleteFloorOrSection() {
        if (this.selectedSections.length == 0) {
            ...
            await this.orm.call("manufacturing.floor", "deactivate_floor", [
        ...
        const response = await this.orm.call("manufacturing.section", "are_orders_still_in_draft", [
        ...
        if (!response) {
            for (const id of originalSelectedSectionIds) {
                ...
                this.orm.write("manufacturing.section", [id], { active: false });
