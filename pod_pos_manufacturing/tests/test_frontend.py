...
class TestFrontend(odoo.tests.HttpCase):
    def setUp(self):
        ...
        section_05 = self.env['manufacturing.section'].create({
            'name': '5',
        ...
        section_04 = self.env['manufacturing.section'].create({
            'name': '4',
            'floor_id': main_floor.id,
            'seats': 4,
            'shape': 'square',
        ...
        section_02 = self.env['manufacturing.section'].create({
            'name': '2',
            'floor_id': main_floor.id,
            'seats': 4,
            'position_h': 250,
        ...
        second_floor = self.env['manufacturing.floor'].create({
            'name': 'Second Floor',
            'pos_config_ids': [(4, pos_config.id)],
        })

        section_01 = self.env['manufacturing.section'].create({
            'name': '1',
        ...
        section_03 = self.env['manufacturing.section'].create({
            'name': '3',
            'floor_id': second_floor.id,
            'seats': 4,
            'position_h': 100,
        ...
        pricelist = self.env['product.pricelist'].create({'name': 'Manufacturing Pricelist'})
        pos_config.write({'pricelist_id': pricelist.id})

        self.pos_config = pos_config

    def test_01_pos_manufacturing(self):

        self.pos_config.with_user(self.env.ref('base.user_demo')).open_ui()

        self.start_tour("/pos/ui?config_id=%d" % self.pos_config.id, 'pos_manufacturing_sync', login="demo")
        ...
        self.start_tour("/pos/ui?config_id=%d" % self.pos_config.id, 'pos_manufacturing_sync_second_login', login="demo")
    ...
    def test_02_others(self):
        self.pos_config.with_user(self.env.ref('base.user_demo')).open_ui()
        self.start_tour("/pos/ui?config_id=%d" % self.pos_config.id, 'SplitInvoiceScreenTour', login="demo")
    ...
    def test_06_split_invoice_screen(self):
        self.pos_config.with_user(self.env.ref('base.user_demo')).open_ui()
        self.start_tour("/pos/ui?config_id=%d" % self.pos_config.id, 'SplitInvoiceScreenTour2', login="demo")

    def test_07_split_invoice_screen(self):
        self.pos_config.with_user(self.env.ref('base.user_demo')).open_ui()
        self.start_tour("/pos/ui?config_id=%d" % self.pos_config.id, 'SplitInvoiceScreenTour3', login="demo")

    def test_08_refund_stay_current_section(self):
        self.pos_config.with_user(self.env.ref('base.user_demo')).open_ui()
        self.start_tour("/pos/ui?config_id=%d" % self.pos_config.id, 'RefundStayCurrentSectionTour', login="demo")

    def test_09_combo_split_invoice(self):
        combo_product = setup_pos_combo_items(self)
        combo_product.write({'lst_price': 40})
        self.pos_config.with_user(self.env.ref('base.user_demo')).open_ui()
        self.start_tour(f"/pos/ui?config_id={self.pos_config.id}", 'SplitInvoiceScreenTour4PosCombo', login="demo")
