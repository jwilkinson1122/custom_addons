...
class PosOrder(models.Model):
    ...
    section_id = fields.Many2one('manufacturing.section', string='Section', help='The section where this order was served', index='btree_not_null', readonly=True)
    ...
    def remove_from_ui(self, server_ids):
        sections = self.env['pos.order'].search([('id', 'in', server_ids)]).section_id
        order_ids = super().remove_from_ui(server_ids)
        self.send_section_count_notification(sections)
        return order_ids

    def _process_saved_order(self, draft):
        order_id = super()._process_saved_order(draft)
        self.send_section_count_notification(self.section_id)
        return order_id

    def send_section_count_notification(self, section_ids):
        messages = []
        for config in self.env['pos.config'].search([('floor_ids', 'in', section_ids.floor_id.ids)]):
            config_cur_session = config.current_session_id
            if config_cur_session:
                order_count = config.get_sections_order_count_and_printing_changes()
    ...
    def _order_fields(self, ui_order):
        ...
        order_fields['section_id'] = ui_order.get('section_id', False)
    ...
    @api.model
    def export_for_ui_section_draft(self, section_ids):
        orders = self.env['pos.order'].search([('state', '=', 'draft'), ('section_id', 'in', section_ids)])
