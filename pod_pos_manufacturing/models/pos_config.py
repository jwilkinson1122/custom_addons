...
class PosConfig(models.Model):
    ...
    floor_ids = fields.Many2many('manufacturing.floor', string='Manufacturing Floors', help='The manufacturing manufacturing floors served by this point of sale.')
    ...
    module_pos_manufacturing = fields.Boolean(default=True)
    module_pos_manufacturing_booking = fields.Boolean("Section Booking")

    def get_sections_order_count_and_printing_changes(self):
        self.ensure_one()
        sections = self.env['manufacturing.section'].search([('floor_id.pos_config_ids', '=', self.id)])
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            if is_manufacturing and 'iface_splitinvoice' not in vals:
    ...
    def setup_defaults(self, company):
        main_manufacturing = self.env.ref('pod_pos_manufacturing.pos_config_main_manufacturing', raise_if_not_found=False)
    ...
    def _setup_default_floor(self, pos_config):
        if not pos_config.floor_ids:
            ...
            self.env['manufacturing.section'].create({
                'name': '1',
