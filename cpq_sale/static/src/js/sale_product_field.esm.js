        ...
        try {
            ...
            if (!activeId || isVirtual) {
                const createdId = await this.orm.call("sale.order.line", "create", [{
                    ...
                    product_id: (await safeRead("product.template", productTmplId, ["product_variant_id"])).product_variant_id[0],
                    product_uom: (await safeRead("product.template", productTmplId, ["uom_id"])).uom_id[0],
                    product_uom_qty: 1,
