...
class PrescriptionOrderOption(models.Model):
    ...
    prescription_order_id = fields.Many2one('prescription.order', 'Prescription Order Reference', ondelete='cascade', index=True)
    ...
    def _compute_name(self):
        for option in self:
            if not option.product_id:
                continue
            product_lang = option.product_id.with_context(lang=option.prescription_order_id.partner_id.lang)
    ...
    def _compute_price_unit(self):
        for option in self:
            ...
            new_rxl = self.env['prescription.order.line'].new(values)
            ...
            new_rxl.prescription_order_id = False
    ...
    @api.depends('line_id', 'prescription_order_id.prescription_order_line', 'product_id')
    def _compute_is_present(self):
        ...
        for option in self:
            option.is_present = bool(option.prescription_order_id.prescription_order_line.filtered(lambda l: l.product_id == option.product_id))
    ...
    def add_option_to_prescription(self):
        ...
        prescription_order_line = self.env['prescription.order.line'].create(values)

        self.write({'line_id': prescription_order_line.id})
        if prescription_order:
            prescription_order.add_option_to_prescription_with_taxcloud()

        return prescription_order_line
