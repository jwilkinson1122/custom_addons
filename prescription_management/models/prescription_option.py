...
class PrescriptionOption(models.Model):
    ...
    order_id = fields.Many2one('prescription', 'Prescription Order Reference', ondelete='cascade', index=True)
    ...
    def _compute_price_unit(self):
        for option in self:
            ...
            new_sol = self.env['prescription.line'].new(values)
    ...
    def add_option_to_order(self):
        ...
        prescription = self.order_id

        if not prescription._can_be_edited_on_portal():
            raise UserError(_('You cannot add options to a confirmed order.'))

        values = self._get_values_to_add_to_order()
        order_line = self.env['prescription.line'].create(values)

        self.write({'line_id': order_line.id})
        if prescription:
            prescription.add_option_to_order_with_taxcloud()
