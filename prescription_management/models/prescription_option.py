...
class PrescriptionOption(models.Model):
    ...
    prescription_id = fields.Many2one('prescription', 'Prescription Order Reference', ondelete='cascade', index=True)
    ...
    line_id = fields.Many2one(
        comodel_name='prescription.line', ondelete='set null', copy=False)
    ...
    def _compute_name(self):
        for option in self:
            if not option.product_id:
                continue
            product_lang = option.product_id.with_context(lang=option.prescription_id.partner_id.lang)
    ...
    def _compute_price_unit(self):
        for option in self:
            ...
            values = option._get_values_to_add_to_prescription()
            new_rxl = self.env['prescription.line'].new(values)
    ...
    def _get_values_to_add_to_prescription(self):
        self.ensure_one()
        return {
    ...
    @api.depends('line_id', 'prescription_id.prescription_line', 'product_id')
    def _compute_is_present(self):
        ...
        for option in self:
            option.is_present = bool(option.prescription_id.prescription_line.filtered(lambda l: l.product_id == option.product_id))
    ...
    def _product_id_domain(self):
        """ Returns the domain of the products that can be added as a prescription option. """
        return [('sale_ok', '=', True)]

    #=== ACTION METHODS ===#

    def button_add_to_prescription(self):
        self.add_option_to_prescription()

    def add_option_to_prescription(self):
        self.ensure_one()

        prescription = self.prescription_id

        if not prescription._can_be_edited_on_portal():
            raise UserError(_('You cannot add options to a confirmed order.'))

        values = self._get_values_to_add_to_prescription()
        prescription_line = self.env['prescription.line'].create(values)

        self.write({'line_id': prescription_line.id})
        if prescription:
            prescription.add_option_to_prescription_with_taxcloud()
