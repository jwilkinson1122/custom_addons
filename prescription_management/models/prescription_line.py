...
class PrescriptionLine(models.Model):
    ...
    def _compute_name(self):
        ...
        super()._compute_name()
        for line in self:
            if line.product_id and line.prescription_id.prescription_template_id and line._use_template_name():
                for template_line in line.prescription_id.prescription_template_id.prescription_template_line_ids:
                    if line.product_id == template_line.product_id:
                        lang = line.prescription_id.partner_id.lang
    ...
    def _use_template_name(self):
        """ Allows overriding to avoid using the template lines descriptions for the prescription lines descriptions.
    ...
    def _can_be_edited_on_portal(self):
        return self.prescription_id._can_be_edited_on_portal() and (
            self.prescription_option_ids
            or self.product_id in self.prescription_id.prescription_option_ids.product_id
        )
