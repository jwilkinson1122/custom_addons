...
from odoo.addons.prescription_management.tests.common import PrescriptionManagementCommon


@tagged('-at_install', 'post_install')
class TestPrescription(PrescriptionManagementCommon):
    ...
    def setUpClass(cls):
        ...
        cls.quotation_template_no_discount = cls.env['prescription.template'].create({
            'name': 'A quotation template',
            'prescription_template_line_ids': [
                Command.create({
                    'product_id': cls.product_1.id,
                }),
            ],
            'prescription_template_option_ids': [
                Command.create({
        ...
        cls.prescription = cls.empty_order

    def test_01_template_without_pricelist(self):
        """
        This test checks that without any rule in the pricelist, the public price
        of the product is used in the prescription after selecting a
        quotation template.
        """
        # first case, without discount in the quotation template
        self.prescription.write({
            'prescription_template_id': self.quotation_template_no_discount.id
        })
        self.prescription._onchange_prescription_template_id()

        self.assertEqual(
            len(self.prescription.order_line),
            1,
            "The prescription shall contains the same number of products as"
            "the quotation template.")

        self.assertEqual(
            self.prescription.order_line[0].product_id.id,
            self.product_1.id,
            "The prescription shall contains the same products as the"
            "quotation template.")

        self.assertEqual(
            self.prescription.order_line[0].price_unit,
            self.pub_product_price,
            "Without any price list and discount, the public price of"
            "the product shall be used.")

        self.assertEqual(
            len(self.prescription.prescription_option_ids),
            1,
            "The prescription shall contains the same number of optional products as"
            "the quotation template.")

        self.assertEqual(
            self.prescription.prescription_option_ids[0].product_id.id,
            self.optional_product.id,
            "The prescription shall contains the same optional products as the"
            "quotation template.")

        self.assertEqual(
            self.prescription.prescription_option_ids[0].price_unit,
            self.pub_option_price,
            "Without any price list and discount, the public price of"
            "the optional product shall be used.")

        # add the option to the order
        self.prescription.prescription_option_ids[0].button_add_to_prescription()
        ...
        self.assertEqual(
            self.prescription.order_line[1].product_id.id,
            self.optional_product.id,
            "The prescription shall contains the same products as the"
            "quotation template.")

        self.assertEqual(
            self.prescription.order_line[1].price_unit,
            self.pub_option_price,
            "Without any price list and discount, the public price of"
            "the optional product shall be used.")
    ...
    def test_02_template_with_discount_included_pricelist(self):
        ...
        the price used in the prescription is computed according to the
        price list.
        """
    ...
    def test_03_template_with_discount_excluded_pricelist(self):
        ...
        the price used in the prescription is the product public price and
        ...
        self.assertEqual(
            self.prescription.order_line[0].price_subtotal,
            self.pl_product_price,
        ...
        self.assertEqual(
            self.prescription.order_line[0].discount,
            self.pl_discount,
        ...
        self.assertEqual(
            self.prescription.prescription_option_ids[0].discount,
        ...
        self.assertEqual(
            self.prescription.order_line[1].price_subtotal,
            self.pl_option_price,
            "If a pricelist is set without discount included, the subtotal "
            "price shall be the price computed according to the price list.")

        self.assertEqual(
            self.prescription.order_line[1].discount,
            self.pl_option_discount,
            "If a pricelist is set without discount included, the discount "
            "shall be computed according to the price unit and the subtotal."
            "price")
    ...
    def test_04_update_pricelist_option_line(self):
        ...
        self.assertEqual(
            self.prescription.prescription_option_ids[0].discount, 0,
            "If no pricelist is set, the discount should be 0.")

        self.prescription.write({
            'pricelist_id': self.discount_included_price_list.id,
        })
        self.prescription._recompute_prices()
    ...
    def test_option_creation(self):
        ...
        order_form = Form(self.prescription)
        with order_form.prescription_option_ids.new() as option:
            option.product_id = self.product_1
        order = order_form.save()
        self.assertTrue(bool(order.prescription_option_ids.uom_id))
    ...
    def test_option_price_unit_is_not_recomputed(self):
        ...
        prescription_with_option = self.env['prescription'].create({
            'partner_id': self.partner.id,
            'prescription_option_ids': [Command.create({
                'product_id': self.optional_product.id,
                'price_unit': 10,
            })],
        })
        prescription_with_option.prescription_option_ids.add_option_to_prescription()

        # after changing the quantity of the product, the price unit should not be recomputed
        prescription_with_option.order_line.product_uom_qty = 10
        self.assertEqual(prescription_with_option.prescription_option_ids.price_unit, 10)
