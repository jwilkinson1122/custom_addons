...
class Prescription(models.Model):
    ...
    def _init_column(self, column_name):
        ...
        if column_name != "warehouse_id":
            return super(Prescription, self)._init_column(column_name)
    ...
    def _compute_expected_date(self):
        super(Prescription, self)._compute_expected_date()
        for order in self:
            dates_list = []
    ...
    def write(self, values):
        ...
        res = super(Prescription, self).write(values)
        if values.get('order_line') and self.state == 'prescription':
    ...
    def _compute_json_popover(self):
        for order in self:
            ...
            order.json_popover = json.dumps({
                'popoverTemplate': 'prescription_stock.DelayAlertWidget',
                'late_elements': [{
                        'id': late_move.id,
    ...
    def _action_confirm(self):
        ...
        return super(Prescription, self)._action_confirm()
    ...
    def _compute_warehouse_id(self):
        for order in self:
            default_warehouse_id = self.env['ir.default'].with_company(
                order.company_id.id)._get_model_defaults('prescription').get('warehouse_id')
    ...
    def _action_cancel(self):
        ...
        for prescription in self:
            if prescription.state == 'prescription' and prescription_line:
                prescription_lines_quantities = {order_line: (order_line.product_uom_qty, 0) for order_line in prescription_line}
                documents = self.env['stock.picking'].with_context(include_draft_documents=True)._log_activity_get_documents(prescription_lines_quantities, 'move_ids', 'UP')
    ...
    def _prepare_invoice(self):
        invoice_vals = super(Prescription, self)._prepare_invoice()
    ...
    def _log_decrease_ordered_quantity(self, documents, cancel=False):
        ...
        def _render_note_exception_quantity_so(rendering_context):
            ...
            order_line_ids = self.env['prescription.line'].browse([order_line.id for order in order_exceptions.values() for order_line in order[0]])
            prescription_ids = order_line_ids.mapped('order_id')
            ...
            return self.env['ir.qweb']._render('prescription_stock.exception_on_so', values)
