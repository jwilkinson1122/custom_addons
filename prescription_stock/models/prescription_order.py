...
class PrescriptionOrder(models.Model):
    ...
    expected_date = fields.Datetime( help="Delivery date you can promise to the customer, computed from the minimum lead time of "
                                          ...
                                          "the order lines.")
    json_popover = fields.Char('JRXN data for the popover widget', compute='_compute_json_popover')
    ...
    def _action_confirm(self):
        ...
        return super(PrescriptionOrder, self)._action_confirm()
    ...
    def _prepare_invoice(self):
        invoice_vals = super(PrescriptionOrder, self)._prepare_invoice()
    ...
    def _log_decrease_ordered_quantity(self, documents, cancel=False):
        ...
        def _render_note_exception_quantity_rx(rendering_context):
            ...
            order_lines = self.env['prescription.order.line'].browse([order_line.id for order in order_exceptions.values() for order_line in order[0]])
            prescription_order_ids = order_lines.mapped('order_id')
            ...
            return self.env['ir.qweb']._render('prescription_stock.exception_on_rx', values)
