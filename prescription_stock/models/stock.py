...
class StockMove(models.Model):
    _inherit = "stock.move"
    prescription_line_id = fields.Many2one('prescription.line', 'Prescription Line', index='btree_not_null')
    ...
    def _assign_picking_post_process(self, new=False):
        ...
        if new:
            ...
            prescription_ids = self.mapped('prescription_line_id.order_id')
            for prescription_id in prescription_ids:
                picking_id.message_post_with_source(
                    'mail.message_origin_link',
                    render_values={'self': picking_id, 'origin': prescription_id},
...
class ProcurementGroup(models.Model):
    ...
    prescription_id = fields.Many2one('prescription', 'Prescription Order')
...
class StockPicking(models.Model):
    ...
    def _action_done(self):
        ...
        prescription_lines_vals = []
        for move in self.move_ids:
            prescription = move.picking_id.prescription_id
            ...
            if not prescription or move.location_dest_id.usage != 'customer' or move.prescription_line_id or not move.picked:
                continue
            ...
            if product.invoice_policy == 'delivery':
                ...
                so_line = prescription_line.filtered(lambda sol: sol.product_id == product)
                if so_line:
            ...
            prescription_lines_vals.append(so_line_vals)

        if prescription_lines_vals:
            self.env['prescription.line'].with_context(skip_procurement=True).create(prescription_lines_vals)
        return res
    ...
    def _log_less_quantities_than_expected(self, moves):
        ...
        def _render_note_exception_quantity(moves_information):
            ...
            {'move_id': ['prescription_line_id', (new_qty, old_qty)], ..}
            ...
            return self.env['ir.qweb']._render('prescription_stock.exception_on_picking', values)
...
class StockLot(models.Model):
    ...
    prescription_ids = fields.Many2many('prescription', string="Prescription Orders", compute='_compute_prescription_ids')
    prescription_count = fields.Integer('Prescription order count', compute='_compute_prescription_ids')

    @api.depends('name')
    def _compute_prescription_ids(self):
        prescriptions = defaultdict(lambda: self.env['prescription'])
        for move_line in self.env['stock.move.line'].search([('lot_id', 'in', self.ids), ('state', '=', 'done')]):
            ...
            if move.picking_id.location_dest_id.usage == 'customer' and move.prescription_line_id.order_id:
                prescriptions[move_line.lot_id.id] |= move.prescription_line_id.order_id
        for lot in self:
            lot.prescription_ids = prescriptions[lot.id]
            lot.prescription_count = len(lot.prescription_ids)

    def action_view_so(self):
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("prescription.action_prescription")
        action['domain'] = [('id', 'in', self.mapped('prescription_ids.id'))]
