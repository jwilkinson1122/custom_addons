...
class StockPicking(models.Model):
    ...
    def _log_less_quantities_than_expected(self, moves):
        ...
        def _keys_in_groupby(prescription_line):
            """ group by order_id and the prescription_user on the order """
        ...
        def _render_note_exception_quantity(moves_information):
            ...
            {'move_id': ['prescription_order_line', (new_qty, old_qty)], ..}
...
class StockLot(models.Model):
    ...
    def action_view_rx(self):
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("prescription.action_orders")
