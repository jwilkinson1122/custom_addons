...
class TestPrescriptionStock(TestPrescriptionCommon, ValuationReconciliationTestCommon):
    ...
    def test_00_prescription_stock_invoice(self):
        ...
        self.assertEqual(self.rx.invoice_status, 'no', 'Prescription Stock: so invoice_status should be "nothing to invoice" after invoicing')
        pick = self.rx.picking_ids
        pick.move_ids.write({'quantity': 1, 'picked': True})
        wiz_act = pick.button_validate()
        wiz = Form(self.env[wiz_act['res_model']].with_context(wiz_act['context'])).save()
        wiz.process()
        self.assertEqual(self.rx.invoice_status, 'to invoice', 'Prescription Stock: so invoice_status should be "to invoice" after partial delivery')
        ...
        self.assertEqual(self.rx.invoice_status, 'to invoice', 'Prescription Stock: so invoice_status should be "to invoice" after complete delivery')
        del_qties = [rxl.qty_delivered for rxl in self.rx.order_line]
    ...
    def test_01_prescription_stock_order(self):
        ...
        adv_wiz = self.env['prescription.advance.payment.inv'].with_context(active_ids=[self.rx.id]).create({
        ...
        self.assertEqual(self.rx.invoice_status, 'to invoice', 'Prescription Stock: so should be to invoice after invoicing deposit')
        # invoice on order: everything should be invoiced
        self.rx._create_invoices(final=True)
        self.assertEqual(self.rx.invoice_status, 'invoiced', 'Prescription Stock: so should be fully invoiced after second invoice')
    ...
    def test_02_prescription_stock_return(self):
        ...
        self.rx = self.env['prescription.order'].create(rx_vals)
        ...
        self.assertEqual(self.rx.invoice_status, 'no', 'Prescription Stock: so invoice_status should be "no" instead of "%s".' % self.rx.invoice_status)
        ...
        # Check invoice
        self.assertEqual(self.rx.invoice_status, 'to invoice', 'Prescription Stock: so invoice_status should be "to invoice" instead of "%s" before invoicing' % self.rx.invoice_status)
        self.inv_1 = self.rx._create_invoices()
        self.assertEqual(self.rx.invoice_status, 'invoiced', 'Prescription Stock: so invoice_status should be "invoiced" instead of "%s" after invoicing' % self.rx.invoice_status)
        ...
        # Check invoice
        self.assertEqual(self.rx.invoice_status, 'to invoice', 'Prescription Stock: so invoice_status should be "to invoice" instead of "%s" after picking return' % self.rx.invoice_status)
        ...
        self.assertEqual(self.rx.invoice_status, 'no', 'Prescription Stock: so invoice_status should be "no" instead of "%s" after invoicing the return' % self.rx.invoice_status)
    ...
    def test_05_confirm_cancel_confirm(self):
        ...
        rx1 = self.env['prescription.order'].create({
            'partner_id': partner1,
            'order_line': [(0, 0, {
                'name': item1.name,
                'product_id': item1.id,
    ...
    def test_07_forced_qties(self):
        ...
        rx1.picking_ids.button_validate()
        self.assertEqual(rx1.picking_ids.state, 'done')
    ...
    def test_08_prescription_return_qty_and_cancel(self):
        ...
        rx = self.env['prescription.order'].create(rx_vals)
        ...
        rx._action_cancel()
        wizard = self.env['prescription.order.cancel'].with_context({'order_id': rx.id}).create({'order_id': rx.id})
    ...
    def test_15_cancel_delivery(self):
        ...
        rx.action_confirm()
        self.assertEqual(rx.state, 'prescription')
    ...
    def test_18_deliver_more_and_multi_uom(self):
        ...
        rx = self._get_new_prescription_order(product=self.product_a)
        rx.action_confirm()
    ...
    def test_19_deliver_update_rx_line_qty(self):
        ...
        with Form(prescription_order.with_context(import_file=True)) as rx_form:
    ...
    def test_backorder_and_decrease_rxl_qty(self):
        ...
        rx = self._get_new_prescription_order()
        rx.action_confirm()
    ...
    def test_incoterm_in_advance_payment(self):
        ...
        adv_wiz = self.env['prescription.advance.payment.inv'].with_context(active_ids=[rx.id]).create({
            'advance_payment_method': 'percentage',
            'amount': 5.0,
            'product_id': advance_product.id,
        })
    ...
    def test_exception_delivery_partial_multi(self):
        ...
        rx_1 = self._get_new_prescription_order()
        rx_1.action_confirm()
        picking_1 = rx_1.picking_ids
        picking_1.move_ids.write({'quantity': 1, 'picked': True})

        rx_2 = self._get_new_prescription_order()
        rx_2.action_confirm()
        ...
        activity = self.env['mail.activity'].search([('res_id', '=', rx_2.id), ('res_model', '=', 'prescription.order')])
    ...
    def test_3_steps_and_unpack(self):
        ...
        rx_1 = self._get_new_prescription_order(product=self.test_product_delivery)
        rx_1.action_confirm()
        ...
        pick_picking_2 = rx_1.picking_ids.filtered(lambda x: x.picking_type_id == warehouse.pick_type_id and x.state != 'done')
        ...
        pack_picking_2 = rx_1.picking_ids.filtered(lambda p: p.picking_type_id == warehouse.pack_type_id and p.state != 'done')
    ...
    def test_inventory_admin_no_backorder_not_own_prescription_order(self):
        ...
        inventory_admin_user = self.env['res.users'].create({
            ...
            'groups_id': [(6, 0, [
                ...
                self.ref('prescription_team.group_prescription_prescriptionman')])]
        })
    ...
    def test_reduce_qty_ordered_no_backorder(self):
        ...
        rx_1 = self._get_new_prescription_order(amount=3, product=self.test_product_delivery)
        rx_1.action_confirm()
