...
class TestPrescriptionStock(TestPrescriptionCommon, ValuationReconciliationTestCommon):
    ...
    def _get_new_prescription(self, amount=10.0, product=False):
    ...
    def test_00_prescription_stock_invoice(self):
        ...
        """
        self.so = self.env['prescription'].create({
            'partner_id': self.partner_a.id,
            'partner_invoice_id': self.partner_a.id,
            'partner_shipping_id': self.partner_a.id,
            'order_line': [
        ...
        self.assertTrue(self.so.picking_ids, 'Prescription Stock: no picking created for "invoice on delivery" storable products')
        # invoice on order
        self.so._create_invoices()

        # deliver partially, check the so's invoice_status and delivered quantities
        self.assertEqual(self.so.invoice_status, 'no', 'Prescription Stock: so invoice_status should be "nothing to invoice" after invoicing')
        pick = self.so.picking_ids
        pick.move_ids.write({'quantity': 1, 'picked': True})
        wiz_act = pick.button_validate()
        wiz = Form(self.env[wiz_act['res_model']].with_context(wiz_act['context'])).save()
        wiz.process()
        self.assertEqual(self.so.invoice_status, 'to invoice', 'Prescription Stock: so invoice_status should be "to invoice" after partial delivery')
        ...
        self.assertEqual(self.so.invoice_status, 'no',
                         'Prescription Stock: so invoice_status should be "nothing to invoice" after partial delivery and invoicing')
        self.assertEqual(len(self.so.picking_ids), 2, 'Prescription Stock: number of pickings should be 2')
        ...
        self.assertEqual(self.so.invoice_status, 'to invoice', 'Prescription Stock: so invoice_status should be "to invoice" after complete delivery')
        del_qties = [sol.qty_delivered for sol in self.so.order_line]
    ...
    def test_01_prescription_stock_order(self):
        ...
        self.assertTrue(self.so.picking_ids, 'Prescription Stock: no picking created for "invoice on order" storable products')
        ...
        self.assertEqual(inv.amount_untaxed, self.so.amount_untaxed * 5.0 / 100.0, 'Prescription Stock: deposit invoice is wrong')
        self.assertEqual(self.so.invoice_status, 'to invoice', 'Prescription Stock: so should be to invoice after invoicing deposit')
        # invoice on order: everything should be invoiced
        self.so._create_invoices(final=True)
        self.assertEqual(self.so.invoice_status, 'invoiced', 'Prescription Stock: so should be fully invoiced after second invoice')
    ...
    def test_02_prescription_stock_return(self):
        ...
        self.so = self.env['prescription'].create(so_vals)
        ...
        self.assertEqual(self.so.invoice_status, 'no', 'Prescription Stock: so invoice_status should be "no" instead of "%s".' % self.so.invoice_status)
        ...
        # Check invoice
        self.assertEqual(self.so.invoice_status, 'to invoice', 'Prescription Stock: so invoice_status should be "to invoice" instead of "%s" before invoicing' % self.so.invoice_status)
        self.inv_1 = self.so._create_invoices()
        self.assertEqual(self.so.invoice_status, 'invoiced', 'Prescription Stock: so invoice_status should be "invoiced" instead of "%s" after invoicing' % self.so.invoice_status)
        self.assertEqual(len(self.inv_1), 1, 'Prescription Stock: only one invoice instead of "%s" should be created' % len(self.inv_1))
        self.assertEqual(self.inv_1.amount_untaxed, self.inv_1.amount_untaxed, 'Prescription Stock: amount in SO and invoice should be the same')
        ...
        self.assertEqual(self.so.invoice_status, 'no', 'Prescription Stock: so invoice_status should be "no" instead of "%s" after invoicing the return' % self.so.invoice_status)

    def test_04_create_picking_update_prescriptionline(self):
        """
    ...
    def test_05_create_picking_update_prescriptionline(self):
    ...
    def test_05_confirm_cancel_confirm(self):
        ...
        so1 = self.env['prescription'].create({
            'partner_id': partner1,
            'order_line': [(0, 0, {
                'name': item1.name,
                'product_id': item1.id,
    ...
    def test_11_return_with_refund(self):
        ...
        prescription = self._get_new_prescription()
        # Valids the prescription order, then valids the delivery.
        prescription.action_confirm()
        self.assertTrue(prescription.picking_ids)
        self.assertEqual(prescription_line.qty_delivered, 0)
        picking = prescription.picking_ids
        picking.move_ids.write({'quantity': 10, 'picked': True})
        picking.button_validate()

        # Checks the delivery amount (must be 10).
        self.assertEqual(prescription_line.qty_delivered, 10)
    ...
    def test_13_delivered_qty(self):
        ...
        prescription = self._get_new_prescription(amount=3)
        # Confirms the prescription order, then increases the delivered qty., adds a new
        # line and valids the delivery.
        prescription.action_confirm()
        self.assertTrue(prescription.picking_ids)
        self.assertEqual(len(prescription_line), 1)
        self.assertEqual(prescription_line.qty_delivered, 0)
        picking = prescription.picking_ids
        initial_product = prescription_line.product_id
        ...
        self.assertEqual(len(prescription_line), 2)
        so_line_1 = prescription_line[0]
        so_line_2 = prescription_line[1]
        ...
        self.assertRecordValues(prescription.picking_ids.move_ids, [
        ...
        prescription = self._get_new_prescription(product=product_inv_on_order, amount=3)
        # Confirms the prescription order, then increases the delivered qty., adds a new
        # line and valids the delivery.
        prescription.action_confirm()
        self.assertTrue(prescription.picking_ids)
    ...
    def test_14_delivered_qty_in_multistep(self):
        ...
        self.assertEqual(len(prescription_line), 1)
        self.assertEqual(prescription_line.qty_delivered, 0)
        pick = prescription.picking_ids.filtered(lambda p: p.picking_type_code == 'internal')
        delivery = prescription.picking_ids.filtered(lambda p: p.picking_type_code == 'outgoing')
        ...
        self.assertEqual(len(prescription_line), 1)
        self.assertEqual(prescription_line.product_uom_qty, 10)
        self.assertEqual(prescription_line.qty_delivered, 10)
        self.assertEqual(prescription_line.price_unit, 70.0)
    ...
    def test_08_prescription_return_qty_and_cancel(self):
        ...
        so = self.env['prescription'].create(so_vals)
        ...
        so._action_cancel()
        wizard = self.env['prescription.cancel'].with_context({'order_id': so.id}).create({'order_id': so.id})
    ...
    def test_reservation_method_w_prescription(self):
        ...
        prescription1 = self._get_new_prescription(amount=10.0)
        # Validate the prescription order, picking should automatically assign stock
        prescription1.action_confirm()
        picking1 = prescription1.picking_ids
        ...
        prescription2 = self._get_new_prescription(amount=10.0)
        # Validate the prescription order, picking should not automatically assign stock
        prescription2.action_confirm()
        picking2 = prescription2.picking_ids
        ...
        prescription3 = self._get_new_prescription(amount=10.0)
        prescription3.commitment_date = datetime.now() + timedelta(days=10)
        prescription3.action_confirm()
        picking3 = prescription3.picking_ids
        ...
        prescription4 = self._get_new_prescription(amount=10.0)
        prescription4.commitment_date = datetime.now() + timedelta(days=1)
        prescription4.action_confirm()
        self.assertTrue(prescription4.picking_ids)
        self.assertEqual(prescription4.picking_ids.state, 'assigned')
    ...
    def test_15_cancel_delivery(self):
        ...
        """
        group_auto_done = self.env.ref('prescription.group_auto_done_setting')
    ...
    def test_17_qty_update_propagation(self):
        ...
        self.assertEqual(len(prescription.picking_ids), 2, 'A picking and a delivery picking should have been created.')
        customer_location = self.env.ref('stock.stock_location_customers')
        move_pick = prescription.picking_ids.filtered(lambda p: p.location_dest_id.id != customer_location.id).move_ids
        move_out = prescription.picking_ids.filtered(lambda p: p.location_dest_id.id == customer_location.id).move_ids
        ...
        prescription.write({
            'order_line': [
                (1, prescription_line.id, {'product_uom_qty': 30}),
            ]
        })
        ...
        self.assertEqual(len(prescription.picking_ids), 2, 'No additionnal picking should have been created.')

        # Increase the quantity in the prescription order and check the move has been updated.
        prescription.write({
            'order_line': [
                (1, prescription_line.id, {'product_uom_qty': 40})
            ]
        })
    ...
    def test_18_deliver_more_and_multi_uom(self):
        ...
        so = self._get_new_prescription(product=self.product_a)
        so.action_confirm()
    ...
    def test_19_deliver_update_so_line_qty(self):
        ...
        with Form(prescription.with_context(import_file=True)) as so_form:
            with so_form.order_line.edit(0) as line:
                line.product_uom_qty = 777

        self.assertEqual(len(prescription.picking_ids), 2)
    ...
    def test_exception_delivery_partial_multi(self):
        ...
        activity = self.env['mail.activity'].search([('res_id', '=', so_2.id), ('res_model', '=', 'prescription')])
    ...
    def test_3_steps_and_unpack(self):
        ...
        so_1 = self._get_new_prescription(product=self.test_product_delivery)
        so_1.action_confirm()
    ...
    def test_inventory_admin_no_backorder_not_own_prescription(self):
        prescription = self._get_new_prescription()
        prescription.action_confirm()
        pick = prescription.picking_ids
        inventory_admin_user = self.env['res.users'].create({
            ...
            'groups_id': [(6, 0, [
                ...
                self.ref('prescription_team.group_prescription_personnel')])]
        })
    ...
    def test_reduce_qty_ordered_no_backorder(self):
        ...
        so_1 = self._get_new_prescription(amount=3, product=self.test_product_delivery)
        so_1.action_confirm()
