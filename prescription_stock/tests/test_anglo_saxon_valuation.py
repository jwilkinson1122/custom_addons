...
class TestAngloSaxonValuation(ValuationReconciliationTestCommon):
    ...
    def test_standard_ordered_invoice_pre_delivery(self):
        ...
        prescription = self._so_and_confirm_two_units()
    ...
    def test_standard_ordered_invoice_post_partial_delivery_1(self):
        ...
        # Deliver one.
        prescription.picking_ids.move_ids.write({'quantity': 1, 'picked': True})
        wiz = prescription.picking_ids.button_validate()
        ...
        prescription.picking_ids[0].move_ids.write({'quantity': 1, 'picked': True})
        prescription.picking_ids[0].button_validate()

        # change the standard price to 16
        self.product.standard_price = 16.0

        # invoice 1
        invoice2 = prescription._create_invoices()
    ...
    def test_standard_ordered_invoice_post_delivery(self):
        ...
        # deliver the backorder
        prescription.picking_ids.filtered('backorder_id').move_ids.write({'quantity': 1, 'picked': True})
        prescription.picking_ids.filtered('backorder_id').button_validate()
    ...
    def test_avco_ordered_invoice_post_delivery(self):
        ...
        # Deliver one.
        prescription.picking_ids.move_ids.write({'quantity': 2, 'picked': True})
        prescription.picking_ids.button_validate()
    ...
    def test_avco_partially_owned_and_delivered_invoice_post_delivery(self):
        ...
        prescription.picking_ids.move_line_ids.write({'quantity': 1, 'picked': True})
        prescription.picking_ids.button_validate()

        # Invoice one by one
        invoice01 = prescription._create_invoices()
        with Form(invoice01) as invoice_form:
            with invoice_form.invoice_line_ids.edit(0) as line_form:
                line_form.quantity = 1
        invoice01.action_post()

        invoice02 = prescription._create_invoices()
    ...
    def test_avco_fully_owned_and_delivered_invoice_post_delivery(self):
        ...
        prescription = self._so_and_confirm_two_units()
        prescription.picking_ids.move_line_ids.write({'quantity': 2, 'picked': True})
        prescription.picking_ids.button_validate()
    ...
    def test_fifo_delivered_invoice_pre_delivery(self):
        ...
        # Nothing delivered = nothing to invoice.
        with self.assertRaises(UserError):
            invoice_id = prescription._create_invoices()
    ...
    def test_fifo_delivered_invoice_post_delivery_2(self):
        ...
        # Deliver 10
        prescription.picking_ids.move_ids.write({'quantity': 10, 'picked': True})
        prescription.picking_ids.button_validate()
        ...
        self.assertEqual(prescription_line.move_ids.stock_valuation_layer_ids[-1].quantity, 0)
    ...
    def test_fifo_delivered_invoice_post_delivery_3(self):
        """Receive 5@8, receive 8@12, prescription 1@20, deliver, prescription 6@20, deliver. Make sure no rouding
        ...
        # prescription 1@20, deliver, invoice
        prescription = self.env['prescription'].create({
            'partner_id': self.partner_a.id,
            'order_line': [
                (0, 0, {
        ...
        # prescription 6@20, deliver, invoice
        prescription = self.env['prescription'].create({
            'partner_id': self.partner_a.id,
            'order_line': [
                (0, 0, {
        ...
        prescription.action_confirm()
        prescription.picking_ids.move_ids.write({'quantity': 6, 'picked': True})
        prescription.picking_ids.button_validate()
        invoice = prescription._create_invoices()
        invoice.action_post()
    ...
    def test_fifo_delivered_invoice_post_delivery_4(self):
        ...
        # Create and confirm a prescription order for 10@12
        prescription = self.env['prescription'].create({
            'partner_id': self.partner_a.id,
            'order_line': [
                (0, 0, {
        ...
        revalued_anglo_expense_amls = prescription.picking_ids.move_ids.stock_valuation_layer_ids[-1].stock_move_id.account_move_ids[-1].line_ids
    ...
    def test_fifo_delivered_invoice_post_delivery_with_return(self):
        ...
        so_2 = self.env['prescription'].create({
            'partner_id': self.partner_a.id,
            'order_line': [
                (0, 0, {
                    'name': self.product.name,
    ...
    def test_fifo_uom_computation(self):
        ...
        so_1 = self.env['prescription'].create({
            'partner_id': self.partner_a.id,
            'order_line': [
                (0, 0, {
                    'name': self.product.name,
        ...
        # Product Prescription                    0.00$      18.00$
        ...
        # Product Prescription                    0.00$       18.0$
    ...
    def test_fifo_return_and_create_invoice(self):
        ...
        ctx = {'active_model': 'prescription', 'active_ids': so.ids}
    ...
    def test_fifo_edit_svl_without_reinvoice(self):
        ...
        self.assertEqual(prescription_line.product_uom_qty, 2.0)

        # Deliver one.
        prescription.picking_ids.move_ids.write({'quantity': 2, 'picked': True})
        prescription.picking_ids.button_validate()
        svl_am = prescription_line.move_ids.stock_valuation_layer_ids.account_move_id
        svl_am.button_draft()
        svl_line = svl_am.line_ids.filtered(lambda aml: aml.account_id == self.company_data['default_account_stock_out'])

        svl_line.write({'analytic_distribution': {prescription.analytic_account_id.id: 100}})
        svl_am.action_post()

        # Check no reinvoice line addded to the prescription order
        self.assertEqual(len(prescription_line), 1)
        self.assertEqual(prescription_line.product_uom_qty, 2.0)
