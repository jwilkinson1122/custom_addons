...
class TestPERF(common.TransactionCase):
    ...
    @warmup
    @prepare
    def test_empty_prescription_creation_perf(self):
        with self.assertQueryCount(admin=33):
            self.env['prescription'].create({
    ...
    @warmup
    @prepare
    def test_empty_prescriptions_batch_creation_perf(self):
        # + 1 SO insert
        # + 1 SO sequence fetch
        # + 1 warehouse fetch
    ...
    @warmup
    @prepare
    def test_dummy_prescriptions_batch_creation_perf(self):
    ...
    @warmup
    @prepare
    def test_light_prescriptions_batch_creation_perf_without_taxes(self):
    ...
    @users('admin')
    @warmup
    def __test_light_prescriptions_batch_creation_perf(self):
    ...
    @users('admin')
    @warmup
    def __test_complex_prescriptions_batch_creation_perf(self):
        ...
        self._test_complex_prescriptions_batch_creation_perf(1504)

    @users('admin')
    @warmup
    def ___test_complex_prescriptions_batch_creation_perf_with_discount_computation(self):
        ...
        self._test_complex_prescriptions_batch_creation_perf(1546)

    def _test_complex_prescriptions_batch_creation_perf(self, query_count):
        ...
        with self.assertQueryCount(admin=query_count):
            ...
            t1 = time.time()
            _logger.info(MSG, 'prescription', self.ENTITIES, "BATCH", t1 - t0)
            self.env.cr.flush()
            _logger.info(MSG, 'prescription', self.ENTITIES, "FLUSH", time.time() - t1)
