...
class TestPERF(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.prescriptionmans = cls.env.ref('base.user_admin') | cls.env.ref('base.user_demo')
    ...
    @warmup
    @prepare
    def test_empty_prescription_order_creation_perf(self):
        with self.assertQueryCount(admin=33):
            self.env['prescription.order'].create({
                'partner_id': self.partners[0].id,
                'user_id': self.prescriptionmans[0].id,
            })
    ...
    @warmup
    @prepare
    def test_light_prescription_orders_batch_creation_perf_without_taxes(self):
    ...
    @users('admin')
    @warmup
    def __test_light_prescription_orders_batch_creation_perf(self):
    ...
    @users('admin')
    @warmup
    def __test_complex_prescription_orders_batch_creation_perf(self):
        ...
        self._test_complex_prescription_orders_batch_creation_perf(1504)

    @users('admin')
    @warmup
    def ___test_complex_prescription_orders_batch_creation_perf_with_discount_computation(self):
        ...
        self.env.user.groups_id += self.env.ref('product.group_discount_per_rx_line')

        # Verify any modification to this count on nightly runbot builds
        self._test_complex_prescription_orders_batch_creation_perf(1546)

    def _test_complex_prescription_orders_batch_creation_perf(self, query_count):
        MSG = "Model %s, %i records, %s, time %.2f"

        vals_list = [{
            "partner_id": self.partners[i].id,
            "user_id": self.prescriptionmans[i % 2].id,
            "order_line": [
                (0, 0, {"display_type": "line_note", "name": "NOTE"})
            ] + [
        ...
        with self.assertQueryCount(admin=query_count):
            t0 = time.time()
            self.env["prescription.order"].create(vals_list)
            t1 = time.time()
            _logger.info(MSG, 'prescription.order', self.ENTITIES, "BATCH", t1 - t0)
            self.env.cr.flush()
            _logger.info(MSG, 'prescription.order', self.ENTITIES, "FLUSH", time.time() - t1)
