...
class TestPrescriptionExpectedDate(ValuationReconciliationTestCommon):
    ...
    def test_prescription_expected_date(self):
        ...
        product_A = Product.create({
            'name': 'Product A',
            'type': 'product',
            'prescription_delay': 5,
            'uom_id': 1,
        })
        product_B = Product.create({
            'name': 'Product B',
            'type': 'product',
            'prescription_delay': 10,
            'uom_id': 1,
        })
        product_C = Product.create({
            'name': 'Product C',
            'type': 'product',
            'prescription_delay': 15,
            'uom_id': 1,
        })
        ...
        prescription = self.env['prescription'].create({
            ...
            'order_line': [
                (0, 0, {'name': product_A.name, 'product_id': product_A.id, 'customer_lead': product_A.prescription_delay, 'product_uom_qty': 5}),
                (0, 0, {'name': product_B.name, 'product_id': product_B.id, 'customer_lead': product_B.prescription_delay, 'product_uom_qty': 5}),
                (0, 0, {'name': product_C.name, 'product_id': product_C.id, 'customer_lead': product_C.prescription_delay, 'product_uom_qty': 5})
            ],
        })
        ...
        self.assertAlmostEqual(expected_date, prescription.expected_date,
        ...
        prescription.write({'picking_policy': 'one'})
        ...
        prescription.write({'date_order': confirm_date})
        ...
        prescription.write({'picking_policy': 'direct'})
        ...
        picking = prescription.picking_ids[0]
        picking.move_ids.picked = True
        picking._action_done()
        self.assertEqual(picking.state, 'done', "Picking not processed correctly!")
        self.assertEqual(fields.Date.today(), prescription.effective_date.date(), "Wrong effective date on prescription order!")

    def test_prescription_commitment_date(self):
        ...
        new_order = self.env['prescription'].create({
