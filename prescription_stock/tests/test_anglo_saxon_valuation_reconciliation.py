...
class TestValuationReconciliationCommon(ValuationReconciliationTestCommon):
    ...
    def _create_prescription(self, product, date, quantity=1.0):
        rslt = self.env['prescription'].create({
    ...
    def _create_invoice_for_so(self, prescription, product, date, quantity=1.0):
        rslt = self.env['account.move'].create({
            ...
            'invoice_line_ids': [(0, 0, {
                ...
                'product_id': product.id,
                'prescription_line_ids': [(6, 0, prescription_line.ids)],
            })],
        })

        prescription.invoice_ids += rslt
        return rslt
...
class TestValuationReconciliation(TestValuationReconciliationCommon):
    def test_shipment_invoice(self):
        ...
        prescription = self._create_prescription(test_product, '2108-01-01')
        self._process_pickings(prescription.picking_ids)

        invoice = self._create_invoice_for_so(prescription, test_product, '2018-02-12')
        invoice.action_post()
        picking = self.env['stock.picking'].search([('prescription_id', '=', prescription.id)])
    ...
    def test_invoice_shipment(self):
        ...
        prescription = self._create_prescription(test_product, '2018-01-01')

        invoice = self._create_invoice_for_so(prescription, test_product, '2018-02-03')
        invoice.action_post()
    ...
    def test_multiple_shipments_invoices(self):
        ...
        prescription = self._create_prescription(test_product, '2018-01-01', quantity=5)

        self._process_pickings(prescription.picking_ids, quantity=2.0)
        picking = self.env['stock.picking'].search([('prescription_id', '=', prescription.id)], order="id asc", limit=1)

        invoice = self._create_invoice_for_so(prescription, test_product, '2018-02-03', quantity=3)
        invoice.action_post()
        self.check_reconciliation(invoice, picking, full_reconcile=False, operation='prescription')

        invoice2 = self._create_invoice_for_so(prescription, test_product, '2018-03-12', quantity=2)
        invoice2.action_post()
        self.check_reconciliation(invoice2, picking, full_reconcile=False, operation='prescription')

        self._process_pickings(prescription.picking_ids.filtered(lambda x: x.state != 'done'), quantity=3.0)
        picking = self.env['stock.picking'].search([('prescription_id', '=', prescription.id)], order='id desc', limit=1)
