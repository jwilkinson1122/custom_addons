...
class ConfigurationCreate(TransactionCase):
    def setUp(self):
        ...
        self.config_product = self.env.ref("hotel.bmw_2_series")
        self.product_category = self.env.ref("product.product_category_5")

        # attributes
        self.attr_fuel = self.env.ref("hotel.product_attribute_fuel")
        self.attr_engine = self.env.ref("hotel.product_attribute_engine")
        self.attr_color = self.env.ref("hotel.product_attribute_color")
        self.attr_rims = self.env.ref("hotel.product_attribute_rims")
        self.attr_model_line = self.env.ref(
            "hotel.product_attribute_model_line"
        )
        self.attr_tapistry = self.env.ref(
            "hotel.product_attribute_tapistry"
        )
        self.attr_transmission = self.env.ref(
            "hotel.product_attribute_transmission"
        )
        self.attr_options = self.env.ref(
            "hotel.product_attribute_options"
        )

        # values
        self.value_gasoline = self.env.ref(
            "hotel.product_attribute_value_gasoline"
        )
        self.value_218i = self.env.ref(
            "hotel.product_attribute_value_218i"
        )
        self.value_220i = self.env.ref(
            "hotel.product_attribute_value_220i"
        )
        self.value_red = self.env.ref(
            "hotel.product_attribute_value_red"
        )
        self.value_rims_378 = self.env.ref(
            "hotel.product_attribute_value_rims_378"
        )
        self.value_sport_line = self.env.ref(
            "hotel.product_attribute_value_sport_line"
        )
        self.value_model_sport_line = self.env.ref(
            "hotel.product_attribute_value_model_sport_line"
        )
        self.value_tapistry = self.env.ref(
            "hotel.product_attribute_value_tapistry" + "_oyster_black"
        )
        self.value_transmission = self.env.ref(
            "hotel.product_attribute_value_steptronic"
        )
        self.value_options_1 = self.env.ref(
            "hotel.product_attribute_value_smoker_package"
        )
        self.value_options_2 = self.env.ref(
            "hotel.product_attribute_value_sunroof"
        )
