...
class ProductTemplate(models.Model):
    ...
    def get_product_attribute_values_action(self):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id(
            "hotel.product_attribute_value_action"
        )
    ...
    def _check_config_group_rights(self):
        ...
        manager_product_configuration_settings = ICPSudo.get_param(
            "hotel.manager_product_configuration_settings"
        )
    ...
    def check_config_user_access(self):
        ...
        config_manager = self.env.user.has_group(
            "hotel.group_product_configurator_manager"
        )
...
class ProductProduct(models.Model):
    ...
    isdevice = fields.Boolean("Is Device")
    ...
    def _search(
        self,
        args, offset=0, limit=None, order=None, count=False, access_rights_uid=None,
    ):
        args = args or []
    ...
    def check_config_user_access(self, mode):
        ...
        config_user = self.env.user.has_group(
            "hotel.group_product_configurator"
        )
        user_root = self.env.ref("base.user_root")
        user_admin = self.env.ref("base.user_admin")
        if (
