...
class Partner(models.Model):
    ...
    practice_type_id = fields.Many2one(string='Practice Type', comodel_name='contact.practice.type')
    ...
    practitioner_role_ids = fields.Many2many(string="Roles", comodel_name="contact.role")
    ...
    patient_records = fields.One2many('contact.patient', compute='_compute_patient_records', string="Patients")
    ...
    def _compute_patient_records(self):
        for record in self:
            ...
            if record.is_practitioner:
                record.patient_records = self.env['contact.patient'].search([('practitioner_id', '=', record.id)])
            else:
                if not isinstance(record.id, models.NewId) and record.is_company:
                    all_partners = self.env['res.partner'].search([('parent_id', 'child_of', record.id)])
                    all_partners -= record
                    record.patient_records = all_partners.mapped('patient_ids')
            _logger.debug(f"Computed patient records for {record.id}: {record.patient_records.ids}")
    ...
    #         record.patient_records = self.env['contact.patient']
    #         if record.is_practitioner:
    #             record.patient_records = record.patient_ids
    #         elif record.is_company:
    #             patients = self.env['contact.patient']
    #             for child in record.child_ids:
    #                 patients |= child.patient_ids
    #             record.patient_records = patients
    #         _logger.debug(f"Computed patient records for {record.id}: {record.patient_records.ids}")
    ...
    #     return (
    #         self.env["ir.sequence"].sudo().next_by_code("contact.patient")
    #         or "/"
    #     )
    ...
    def _check_contact(self, mode="write"):
        ...
        checks = [
            (self.is_partner, self._check_contact_user, "nwpl_pod_master.group_contacts_user"),
            (self.is_company, self._check_contact_practice, "nwpl_pod_master.group_contacts_configurator"),
            (self.is_practitioner, self._check_contact_practitioner, "nwpl_pod_master.group_contacts_configurator")
        ]
    ...
    def _check_contact_user(self):
        return self.env.user.has_group("nwpl_pod_master.group_contacts_user")
        
    def _check_contact_practice(self):
        return self.env.user.has_group("nwpl_pod_master.group_contacts_configurator")
