...
class ProductConfiguratorSalePrescriptionOrder(models.TransientModel):
    _name = "product.configurator.prescription.order"
    _inherit = "product.configurator"
    _description = "Product Configurator Prescription Order"

    order_id = fields.Many2one(
        comodel_name="prescription.order", required=True, readonly=True
    )
    order_line_id = fields.Many2one(
        comodel_name="prescription.order.line", readonly=True
    )

    domain_attr_ids = fields.Many2many(
        "product.attribute.value",
        "domain_attrs_values_prescription_order_rel",
        "wiz_id",
        "attribute_id",
        string="Domain",
    )
    domain_attr_2_ids = fields.Many2many(
        "product.attribute.value",
        "domain_attrs_2_values_prescription_order_rel",
        "wiz_id",
        "attribute_id",
        string="Domain",
    )
    ...
    def _get_order_line_vals(self, product_id):
        ...
        onchange_fields = ["price_unit", "product_uom", "taxes_id"]
        line = self.env["prescription.order.line"].new(line_vals)
    ...
    def action_config_done(self):
        ...
        if res.get("res_model") == self._name:
            return res
        model_name = "prescription.order.line"
        ...
        if self.order_line_id:
            ...
            self.order_id.write({"line_ids": [(0, 0, values)]})
        return
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if self.env.context.get("default_order_line_id", False):
                sale_line = self.env["prescription.order.line"].browse(
