...
patch(ProductScreen.prototype, {
    async selectLine(orderline) {
        try {
            if (!orderline) {
                throw new Error("Orderline is null or undefined");
            }
            ...
            if (!self.isMobile && searchBox.length && searchBox.val() !== "") {
                ...
                if (searchInput) {
                    allproducts = this.pos.db.search_product_in_category(self.pos.selectedCategoryId, searchInput);
                } else {
            ...
            if (self.pos.config.pod_enable_options) {
                if (Option_products.length > 0) {
                    let { confirmed } = await this.popup.add(OptionsPopup, {
                        'title': 'Options',
    ...
    async _clickRemoveLine({ detail: line_id }) {
        ...
        setTimeout(async () => {
            try {
                ...
                console.error("Error in _clickRemoveLine: ", error);
            }
        }, 100);
    },
});
