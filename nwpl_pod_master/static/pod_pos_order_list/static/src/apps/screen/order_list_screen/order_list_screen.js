...
export class OrderListScreen extends Component {
    ...
    async updateOrderList(event) {
        ...
        if (search) {
            this.isSearch = true
            var Orders = await this.pod_apply_search(search)
    ...
    get allPosOrders() {
        let orders = Object.values(this.pos.db.pos_order_by_id);
        if (this.isSearch) {
            orders = this.subFilterdOrders;
        }
    
        orders = orders.filter(order => {
            const state = order[0].state;
            return state === 'draft' || state === 'quotation' || state === 'paid' || state === 'done'; 
        });
    
        return orders.sort((a, b) => b[0]['id'] - a[0]['id']);
    }
