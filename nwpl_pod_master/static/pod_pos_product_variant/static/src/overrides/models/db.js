...
patch(PosDB.prototype, {
        ...
        for(let product of products){
            if (product){
                if (self.product_by_tmpl_id && self.product_by_tmpl_id[product.product_tmpl_id]){
                    ...
                    if(product.pod_alternative_products){
                        self.alternative_product_by_id[product.product_tmpl_id] = product.pod_alternative_products
                    }
                }
            }
        }
    ...
    search_variants(variants, query) {
        ...
        try {
            ...
            console.error("Error in query processing: ", e);
            return [];
        }

        var results = [];
        for (var i = 0; i < this.limit; i++) {
            var parent_pro = re.exec(this.variant_search_string)
            if (parent_pro) {
                var id = Number(parent_pro[1]);
