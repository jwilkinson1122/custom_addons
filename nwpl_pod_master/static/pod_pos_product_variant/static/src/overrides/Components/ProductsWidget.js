...
import { VariantPopup } from "@nwpl_pod_master/static/pod_pos_product_variant/app/popups/variant_popup/variant_popup"
import { ProductAttributePopup } from "@nwpl_pod_master/static/pod_pos_product_variant/app/popups/product_attribute_popup/product_attribute_popup"
...
patch(ProductsWidget.prototype, { 
    get productsToDisplay() {
        ...
        if (this.searchWord !== '') {
            ...
            if (self.pos.config.pod_pos_enable_product_variants) {
                var tmpl_ids ={}
    ...
    clickVariant:async function( product_tmpl_id ){
        console.log("Variant clicked: ", product_tmpl_id); // Debug log
        var self = this;
        var varaint_ids = this.pos.db.product_by_tmpl_id[product_tmpl_id]
        var variants = []
        var alternative_products = []
        if( self.pos.config.pod_pos_display_alternative_products ){
        ...
        if( this.pos.config.pod_pos_enable_product_variants && this.pos.config.pod_pos_variants_group_by_attribute){
            let attribute_lines = []
            let attribute_by_id = {}
