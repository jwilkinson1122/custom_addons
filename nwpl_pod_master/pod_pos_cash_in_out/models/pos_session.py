...
class PosSessionInherit(models.Model):
    ...
    def _pos_ui_models_to_load(self):
        ...
        # new_model = 'pod.cash.in.out'
        if 'pod.cash.in.out' not in result:
            result.append('pod.cash.in.out')
        if 'pos.payment' not in result:
            result.append('pos.payment')
        return result

    def _loader_params_pod_cash_in_out(self):
        return {'search_params': {'domain': [], 'fields': [], 'load': False}}

    def _get_pos_ui_pod_cash_in_out(self, params):
        return self.env['pod.cash.in.out'].search_read(**params['search_params'])
