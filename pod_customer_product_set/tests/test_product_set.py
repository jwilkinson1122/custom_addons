...
class TestProductSet(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.product_set = cls.env.ref("pod_product_set.product_set_i5_computer")
    ...
    def test_add_set_lines_init(self):
        ...
        # Pass via ctx
        line_ids = self.env.ref("pod_product_set.product_set_line_computer_1").ids
        wiz = self._get_wiz(ctx=dict(product_set_add_set_line_ids=line_ids))
        self.assertEqual(wiz.product_set_line_ids.ids, line_ids)
        self.assertEqual([x.id for x in wiz._get_lines()], line_ids)
        # Pass at create
        line_ids = self.env.ref("pod_product_set.product_set_line_computer_3").ids
        wiz = self._get_wiz()
    ...
    def test_add_set(self):
        ...
        seq_line1 = sequence.pop(
            self.env.ref("pod_product_set.product_set_line_computer_4").product_id.id
        )
        seq_line2 = sequence.pop(
            self.env.ref("pod_product_set.product_set_line_computer_1").product_id.id
        )
        seq_line3 = sequence.pop(
            self.env.ref("pod_product_set.product_set_line_computer_3").product_id.id
        )
        self.assertTrue(
    ...
    def test_add_set_sequence(self):
        ...
        # make sure sale order line sequence keep sequence set on set
        seq_line1 = sequence.pop(
            self.env.ref("pod_product_set.product_set_line_computer_4").product_id
        )
        seq_line2 = sequence.pop(
            self.env.ref("pod_product_set.product_set_line_computer_1").product_id
        )
        seq_line3 = sequence.pop(
            self.env.ref("pod_product_set.product_set_line_computer_3").product_id
        )
        self.assertTrue(
            max(v for k, v in sequence.items()) < seq_line1 < seq_line2 < seq_line3
        )
    ...
    def test_add_set_section(self):
        ...
        count_lines = len(so.order_line)
        product_set_with_section = self.env.ref("pod_product_set.product_set_services")
