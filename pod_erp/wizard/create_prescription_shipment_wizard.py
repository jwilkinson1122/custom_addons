...
class create_prescription_shipment(models.TransientModel):
    ...
    def create_prescription_shipment(self):
        ...
        prescription_obj = self.env['podiatry.prescription.order']
        ...
        if priscription_record.prescription_line_ids:
            for p_line in priscription_record.prescription_line_ids:
                ...
                res1 = sale_order_line_obj.create({'product_id':p_line.orthotic_id.product_id.id ,
                                                 'product_uom': p_line.orthotic_id.product_id.uom_id.id,
                                                 'name': p_line.orthotic_id.product_id.name,
                                                 'product_uom_qty':1,
