...
class create_prescription_invoice(models.TransientModel):
    ...
    def create_prescription_invoice(self):
        ...
        lab_req_obj = self.env['podiatry.prescription.order']
        ...
        for lab_req in lab_reqs:
            ...
            for p_line in lab_req.prescription_line_ids: 
                ...
                if p_line.orthotic_id.product_id.id:
                    invoice_line_account_id = p_line.orthotic_id.product_id.property_account_income_id.id or p_line.orthotic_id.product_id.categ_id.property_account_income_categ_id.id or False 
                ...
                if not invoice_line_account_id:
                    raise UserError(
                        ...
                        (p_line.orthotic_id.product_id.name,))

                tax_ids = []
                taxes = p_line.orthotic_id.product_id.taxes_id.filtered(lambda r: not p_line.orthotic_id.product_id.company_id or r.company_id == p_line.orthotic_id.product_id.company_id)
                tax_ids = taxes.ids
                
                invoice_line_vals = {
                    'name': p_line.orthotic_id.product_id.display_name or '',
                    'move_name': p_line.name or '',
                    'account_id': invoice_line_account_id,
                    'price_unit':p_line.orthotic_id.product_id.lst_price,
                    'product_uom_id': p_line.orthotic_id.product_id.uom_id.id,
                    'quantity': 1,
                    'product_id':p_line.orthotic_id.product_id.id ,
                }
