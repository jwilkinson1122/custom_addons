    ...
    def create_lab_test(self):
        ...
        for browse_record in self:
            ...
            if res_ids:
                ...
                action = self.env.ref('pod_erp.action_medical_lab_form')
                list_view_id = imd.sudo()._xmlid_to_res_id('pod_erp.medical_lab_tree_view')
                form_view_id = imd.sudo()._xmlid_to_res_id('pod_erp.medical_lab_form_view')
                result = {
                    'name': action.name,
                    'help': action.help,
                    'type': action.type,
