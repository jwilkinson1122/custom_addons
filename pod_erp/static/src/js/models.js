odoo.define('pod_erp.models', function (require) {
    ...
    models.load_models([{
        ...
        loaded: function (self, product_templates) {
            self.podiatry.device = [];
            self.podiatry.device_by_id = {};
            self.podiatry.device = product_templates.filter(function (el) { return el.product_variant_count > 1 });
            self.podiatry.device.forEach(function (podiatry_device) {
                self.podiatry.device_by_id[podiatry_device.id] = podiatry_device;
            });
        },
    }, {
