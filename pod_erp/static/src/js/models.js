odoo.define('pod_erp.models', function (require) {
    ...
    models.load_models([{
        ...
        loaded: function (self, product_templates) {
            self.pod.devices = [];
            self.pod.devices_by_id = {};
            self.pod.devices = product_templates.filter(function (el) { return el.product_variant_count > 1 });
            self.pod.devices.forEach(function (pod_device) {
                self.pod.devices_by_id[pod_device.id] = pod_device;
            });
        },
    }, {
