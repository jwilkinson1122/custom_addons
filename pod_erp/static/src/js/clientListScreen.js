        ...
        class extends ClientListScreen {
            ...
            get prescription_count() {
                ...
                if (this.state.selectedClient)
                    return self.env.pos.podiatry.all_orders.filter(function (el) { return el.patient[0] === self.state.selectedClient.id }).length
                else
                    return 0
            }

            prescription_count_click(data) {
                var podiatry_orders = this.env.pos.podiatry.all_orders.filter(function (el) { return el.patient[0] === data.id })
