odoo.define('pod_erp.popups', function (require) {
    ...
    class OrderCreationWidget extends AbstractAwaitablePopup {
        ...
        attribute_variant_onChange() {
            ...
            var variants = []
            $('#devices').html("");
            this.env.pos.pod.devices.forEach(function (pod_device) {
                pod_device.attribute_line_ids.forEach(function (attribute_line_id) {
                    variants.push(self.env.pos.pod.product_attributes_lines_by_id[attribute_line_id].display_name);
                })
                pod_device.product_variant_ids.forEach(function (product_template) {
        ...
        click_confirm() {
            var order = this.env.pos.get_order();
            var id = $('option:selected', $('#devices')).val();
            var found = false;
            if (id !== undefined)
