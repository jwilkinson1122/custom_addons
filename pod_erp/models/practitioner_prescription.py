...
class PractitionerPrescription(models.Model):
    ...
    practitioner = fields.Many2one("pod.practitioner", string="Optometrist", readonly=True)
    ...
    def create(self, vals):
        ...
        result = super(PractitionerPrescription, self).create(vals)
        return result
    ...
    def print_prescription_report_ticket_size(self):
        return self.env.ref(
            "pod_erp.practitioner_prescription_ticket_size2"
        ).report_action(self)

    # def print_opthamologic_prescription_report(self):
    #     return {
    #         'type': 'ir.actions.report',
    #         'report_name': "pod_erp.practitioner_pod_prescription_template",
    #         'report_file': "pod_erp.practitioner_pod_prescription_template",
    #         'report_type': 'qweb-pdf',
    #     }

    def print_opthamologic_prescription_report_ticket_size(self):
        return self.env.ref(
            "pod_erp.practitioner_prescription_pod_ticket_size2"
        ).report_action(self)
