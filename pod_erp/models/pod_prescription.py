...
class PodiatryPrescription(models.Model):
    ...
    _name = "pod.prescription"
    _description = "pod prescription"
    _rec_name = "order_id"
    ...
    option_line_ids = fields.One2many(
        ...
        readonly=True,
        states={"draft": [("readonly", False)], "sent": [("readonly", False)]},
        help="Podiatry options details provided to"
        "Customer and it will included in "
        "the main Invoice.",
    )
    pod_policy = fields.Selection(
        [
            ("prepaid", "On Booking"),
            ("manual", "On Book In"),
            ("picking", "On Book Out"),
    ...
    pod_invoice_id = fields.Many2one("account.move", "Invoice", copy=False)
    ...
    def create(self, vals):
        ...
        @return: new record set for pod prescription.
        """
        if not "option_line_ids" and "prescription_id" in vals:
            tmp_device_lines = vals.get("device_line_ids", [])
            vals["order_policy"] = vals.get("pod_policy", "manual")
            ...
            if not vals:
                vals = {}
            vals["name"] = self.env["ir.sequence"].next_by_code("pod.prescription")
        ...
        for rec in self:
            ...
            if len(list(new_devices)) != 0:
                ...
                for rm in device_list:
                    device_obj = pod_device_obj.search([("product_id", "=", rm.id)])
            ...
            if not len(list(new_devices)):
                ...
                for device in device_list_obj:
                    device_obj = pod_device_obj.search([("product_id", "=", device.id)])
                    device_obj.write({"is_device": False})
                    device_vals = {
                        "device_id": device_obj.id,
                        "book_in": rec.book_in_date,
    ...
    def _onchange_partner_id(self):
        ...
        partner_shipping_id and pricelist_id of the pod prescription as well
        ---------------------------------------------------------------
        @param self: object pointer
        """
        if self.partner_id:
    ...
    def action_cancel(self):
        ...
        for rec in self:
            ...
            for product in rec.device_line_ids.filtered(
                ...
                devices = self.env["pod.device"].search([("product_id", "=", product.id)])
                devices.write({"is_device": True})
...
class PodiatryPrescriptionLine(models.Model):
    ...
    prescription_id = fields.Many2one("pod.prescription", "Prescription", ondelete="cascade")
    ...
    def create(self, vals):
        ...
        @param self: The object pointer
        @param vals: dictionary of fields value.
        @return: new record set for pod prescription line.
        """
        if "prescription_id" in vals:
            prescription = self.env["pod.prescription"].browse(vals["prescription_id"])
    ...
    def _check_dates(self):
        ...
        if self.prescription_id.date_order and self.book_in_date:
            if self.book_in_date.date() < self.prescription_id.date_order.date():
                raise ValidationError(
                    _(
                        """Device line bookin date should be """
    ...
    def unlink(self):
        ...
        for line in self:
            if line.order_line_id:
                devices = self.env["pod.device"].search(
    ...
    def _onchange_book_in_book_out_dates(self):
        """
        When you change book_in_date or book_out_date it will checked it
        and update the qty of pod prescription line
        -----------------------------------------------------------------
        @param self: object pointer
        """
        ...
        if self.book_in_date and self.book_out_date:
            ...
            #            To calculate additional hours in pod device as per minutes
...
class PrescriptionDeviceLine(models.Model):
    ...
    device_id = fields.Many2one("pod.device", ondelete="restrict", index=True)
    book_in = fields.Datetime("Book In Date", required=True)
    book_out = fields.Datetime("Book Out Date", required=True)
    prescription_id = fields.Many2one("pod.prescription", "Prescription Number", ondelete="cascade")
    status = fields.Selection(related="prescription_id.state", string="state")

class PodiatryOptionLine(models.Model):

    _name = "pod.option.line"
    _description = "pod Option line"
    ...
    def copy(self, default=None):
        ...
        @param default: dict of default values to be set
        """
        return super(PodiatryOptionLine, self).copy(default=default)

    option_line_id = fields.Many2one(
        "sale.order.line",
        "Option Line",
        required=True,
        delegate=True,
    ...
    def create(self, vals):
        ...
        @param self: The object pointer
        @param vals: dictionary of fields value.
        @return: new record set for pod option line.
        """
        if "prescription_id" in vals:
            prescription = self.env["pod.prescription"].browse(vals["prescription_id"])
            vals.update({"order_id": prescription.order_id.id})
        return super(PodiatryOptionLine, self).create(vals)
    ...
    def unlink(self):
        ...
        @return: True/False.
        """
        self.mapped("option_line_id").unlink()
        return super().unlink()
    ...
    def _onchange_product_id(self):
        ...
        vals.update(
            name=self.option_line_id.get_sale_order_line_multiline_description_sale(
                product
            )
        )
    ...
    def _on_change_book_in_book_out_dates(self):
        """
        When you change book_in_date or book_out_date it will checked it
        and update the qty of pod option line
        -----------------------------------------------------------------
        @param self: object pointer
        """
        ...
        if self.ser_book_out_date < self.ser_book_in_date:
            raise ValidationError(_("Book-out must be greater or equal book_in date"))
    ...
    def copy_data(self, default=None):
        ...
        @param default: dict of default values to be set
        """
        return self.option_line_id.copy_data(default=default)
