...
class PodiatryDevice(models.Model):
    ...
    product_id = fields.Many2one(
        "product.product",
        "Product_id", required=True, delegate=True, ondelete="cascade",
    )
    floor_id = fields.Many2one(
        "pod.floor",
        "Floor No", help="At which floor the device is located.", ondelete="restrict",
    )
    max_adult = fields.Integer()
    max_child = fields.Integer()
    device_categ_id = fields.Many2one(
        "pod.device.type", "Device Category", required=True, ondelete="restrict"
    )
    device_accommodations_ids = fields.Many2many(
        "pod.device.accommodations", string="Device Accommodations", help="List of device accommodations."
    )
    # status = fields.Selection(
    #     [("available", "Available"), ("occupied", "Occupied")], default="available",
    # )
    ...
    def create(self, vals):
        if "device_categ_id" in vals:
            device_categ = self.env["pod.device.type"].browse(vals.get("device_categ_id"))
    ...
    #     """
    #     This method is used to change the state
    #     to available of the pod device.
    #     ---------------------------------------
    #     @param self: object pointer
    #     """
...
class PodiatryDeviceType(models.Model):
    ...
    _name = "pod.device.type"
    _description = "Device Type"

    categ_id = fields.Many2one("pod.device.type", "Category")
    child_ids = fields.One2many("pod.device.type", "categ_id", "Device Child Categories")
    product_categ_id = fields.Many2one(
        "product.category",
        "Product Category", delegate=True, required=True, copy=False, ondelete="restrict",
    )

    @api.model
    def create(self, vals):
        if "categ_id" in vals:
            device_categ = self.env["pod.device.type"].browse(vals.get("categ_id"))
...
class PodiatryDeviceAccommodationsType(models.Model):
    ...
    _name = "pod.device.accommodations.type"
    _description = "accommodations Type"

    accommodation_id = fields.Many2one("pod.device.accommodations.type", "Category")
    child_ids = fields.One2many(
        "pod.device.accommodations.type", "accommodation_id", "Accommodations Child Categories"
    )
    product_categ_id = fields.Many2one(
        "product.category",
        "Product Category", delegate=True, required=True, copy=False, ondelete="restrict",
    ...
    def create(self, vals):
        if "accommodation_id" in vals:
            accommodation_categ = self.env["pod.device.accommodations.type"].browse(
...
class PodiatryDeviceAccommodations(models.Model):
    ...
    _name = "pod.device.accommodations"
    _description = "Device accommodations"

    product_id = fields.Many2one(
        "product.product",
        "Device Accommodations Product", required=True, delegate=True, ondelete="cascade",
    )
    accommodations_categ_id = fields.Many2one(
        "pod.device.accommodations.type",
        "Accommodations Category", required=True, ondelete="restrict",
    )
    product_manager = fields.Many2one("res.users")
    ...
    def create(self, vals):
        if "accommodations_categ_id" in vals:
            accommodations_categ = self.env["pod.device.accommodations.type"].browse(
