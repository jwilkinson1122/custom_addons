...
class PodiatryReservedTime(models.Model):
    _name = "manager.podiatry.reserved"
    ...
    reserved_obj = fields.Many2one('podiatry.device')


class EmployeePodiatry(models.Model):
    _inherit = 'podiatry.device'

    manager_check_availability = fields.Boolean(default=True, copy=False)
    color = fields.Char(string='Color', default='#FFFFFF')
    manager_reserved_time = fields.One2many('manager.podiatry.reserved', 'reserved_obj', string='Reserved Time', readonly=1,
                                            ondelete='cascade')
    fuel_type = fields.Selection([('gasoline', 'Gasoline'),
                                  ...
                                  ('hybrid', 'Hybrid'),
                                  ('petrol', 'Petrol')],
                                 'Fuel Type', help='Fuel Used by the device')
