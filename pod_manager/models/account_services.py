...
class AccountServices(models.Model):
    ...
    _name = "account.services"
    _description = "Account Services and its charges"
    ...
    def create(self, vals):
        if "service_categ_id" in vals:
            service_categ = self.env["account.service.type"].browse(
                vals.get("service_categ_id")
            )
            vals.update({"categ_id": service_categ.product_categ_id.id})
        return super(AccountServices, self).create(vals)
    ...
    def write(self, vals):
        ...
        """
        if "service_categ_id" in vals:
            service_categ_id = self.env["account.service.type"].browse(
                vals.get("service_categ_id")
            )
            vals.update({"categ_id": service_categ_id.product_categ_id.id})
        return super(AccountServices, self).write(vals)
...
class AccountServiceType(models.Model):
    ...
    service_id = fields.Many2one("account.service.type", "Service Category")
    child_ids = fields.One2many(
        "account.service.type", "service_id", "Service Child Categories"
    )
    ...
    def create(self, vals):
        ...
        return super(AccountServiceType, self).create(vals)
    ...
    def write(self, vals):
        if "service_id" in vals:
            ...
            vals.update({"parent_id": service_categ.product_categ_id.id})
        return super(AccountServiceType, self).write(vals)
