...
class PracticePatient(models.Model):
    ...
    patient_prescription_ids = fields.One2many('pod.manager.prescription', 'patient_id', string="Prescription Count",
                                               readonly=True)
    ...
    def _check_patient_exists(self):
        for record in self:
            patient = self.env['pod.manager.patient'].search(
