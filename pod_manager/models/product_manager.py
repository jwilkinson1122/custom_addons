...
class ProductManagerContract(models.Model):
    _name = 'product.manager.contract'
    _description = 'Podiatry Manager Management'
    _inherit = 'mail.thread'

    @api.onchange('rent_start_date', 'rent_end_date')
    def check_availability(self):
        self.device_id = ''
        podiatry_obj = self.env['podiatry.device'].search([])
        for i in podiatry_obj:
            for each in i.manager_reserved_time:

                if str(each.date_from) <= str(self.rent_start_date) <= str(each.date_to):
                    i.write({'manager_check_availability': False})
                    ...
                    if str(each.date_from) <= str(self.rent_end_date) <= str(each.date_to):
                        ...
                        i.write({'manager_check_availability': True})
                else:
                    i.write({'manager_check_availability': True})

    image = fields.Binary(related='device_id.image_128',
                          string="Image of Device")
    reserved_podiatry_id = fields.Many2one(
        'manager.podiatry.reserved', invisible=True, copy=False)
    name = fields.Char(string="Name", default="Draft Contract",
                       readonly=True, copy=False)
    customer_id = fields.Many2one(
        'res.partner', required=True, string='Customer', help="Customer")
    device_id = fields.Many2one('podiatry.device', string="Device", required=True, help="Device",
                                readonly=True,
                                states={'draft': [('readonly', False)]}
                                )
    product_brand = fields.Many2one('podiatry.device.model.brand', string="Podiatry Brand", size=50,
                                    related='device_id.model_id.brand_id', store=True, readonly=True)
    product_color = fields.Char(string="Podiatry Color", size=50, related='device_id.color', store=True, copy=False,
                                default='#FFFFFF', readonly=True)
    cost = fields.Float(
        string="Rent Cost", help="This fields is to determine the cost of rent", required=True)
    ...
    recurring_line = fields.One2many('podiatry.manager.line', 'manager_number', readonly=True, help="Recurring Invoices",
                                     copy=False)
    ...
    first_invoice_created = fields.Boolean(
        string="First Invoice Created", invisible=True, copy=False)
    attachment_ids = fields.Many2many('ir.attachment', 'product_rent_checklist_ir_attachments_rel',
                                      'manager_id', 'attachment_id', string="Attachments",
                                      help="Images of the device before contract/any attachments")
    checklist_line = fields.One2many('product.manager.checklist', 'checklist_number', string="Checklist",
                                     help="Facilities/Accessories, That should verify when closing the contract.",
                                     states={'invoice': [('readonly', True)],
                                             'done': [('readonly', True)],
                                             'cancel': [('readonly', True)]})
    total = fields.Float(string="Total (Accessories/Tools)",
                         readonly=True, copy=False)
    ...
    damage_cost_sub = fields.Float(
        string="Damage Cost", readonly=True, copy=False)
    ...
    def state_changer(self):
        if self.state == "running":
            state_id = self.env.ref('podiatry_manager.device_state_rent').id
            self.device_id.write({'state_id': state_id})
        elif self.state == "cancel":
            state_id = self.env.ref('podiatry_manager.device_state_active').id
            self.device_id.write({'state_id': state_id})
    ...
    def podiatry_scheduler1(self, rent_date):
        inv_obj = self.env['account.move']
        inv_line_obj = self.env['account.move.line']
        recurring_obj = self.env['podiatry.manager.line']
        start_date = datetime.strptime(
            str(self.rent_start_date), '%Y-%m-%d').date()
        end_date = datetime.strptime(
            str(self.rent_end_date), '%Y-%m-%d').date()
        ...
        mail_content = _(
            ...
            'manager contract has to be done.'
    ...
    def podiatry_scheduler(self):
        ...
        for records in self.search([]):
            start_date = datetime.strptime(
                str(records.rent_start_date), '%Y-%m-%d').date()
            end_date = datetime.strptime(
                str(records.rent_end_date), '%Y-%m-%d').date()
                    ...
                    if product_id.property_account_income_id.id:
                        ...
                        raise UserError(
                            _('Please define income account for this product: "%s" (id:%d).') % (product_id.name,
                                                                                                 product_id.id))
                    recurring_data = {
                        'name': records.device_id.name,
                        'date_today': today,
                        'account_info': income_account.name,
                        'manager_number': records.id,
    ...
    def action_verify(self):
        self.state = "invoice"
        self.reserved_podiatry_id.unlink()
        self.rent_end_date = fields.Date.today()
        if self.total_cost != 0:
            ...
            result = {
                ...
                'name': 'Podiatry Manager Invoices',
                'view_mode': 'form',
    ...
    def action_confirm(self):
        ...
        for each in self.device_id.manager_reserved_time:
        ...
        if check_availability == 0:
            reserved_id = self.device_id.manager_reserved_time.create({'customer_id': self.customer_id.id,
                                                                       'date_from': self.rent_start_date,
                                                                       'date_to': self.rent_end_date,
                                                                       'reserved_obj': self.device_id.id
                                                                       })
            self.write({'reserved_podiatry_id': reserved_id.id})
        else:
            raise Warning(
                'Sorry This device is already booked by another customer')
        self.state = "reserved"
        sequence_code = 'product.manager.sequence'
        ...
        mail_content = _('<h3>Order Confirmed!</h3><br/>Hi %s, <br/> This is to notify that your manager contract has '
                         'been confirmed. <br/><br/>'
                         'Please find the details below:<br/><br/>'
                         '<table><tr><td>Reference Number<td/><td> %s<td/><tr/>'
                         '<tr><td>Time Range <td/><td> %s to %s <td/><tr/><tr><td>Device <td/><td> %s<td/><tr/>'
                         '<tr><td>Point Of Contact<td/><td> %s , %s<td/><tr/><table/>') % \
            (self.customer_id.name, self.name, self.rent_start_date, self.rent_end_date,
             self.device_id.name, self.sales_person.name, self.sales_person.mobile)
        main_content = {
    ...
    def action_invoice_create(self):
        for each in self:
            ...
            if each.cost_frequency != 'no' and rent_date < date.today():
                manager_days = (date.today() - rent_date).days
                if each.cost_frequency == 'weekly':
                    manager_days = int(manager_days / 7)
                if each.cost_frequency == 'monthly':
                    manager_days = int(manager_days / 30)
                for each1 in range(0, manager_days + 1):
                    if rent_date > datetime.strptime(str(each.rent_end_date), "%Y-%m-%d").date():
                        break
                    each.podiatry_scheduler1(rent_date)
...
class PodiatryManagerLine(models.Model):
    _name = 'podiatry.manager.line'

    name = fields.Char('Description')
    date_today = fields.Date('Date')
    account_info = fields.Char('Account')
    recurring_amount = fields.Float('Amount')
    manager_number = fields.Many2one(
        'product.manager.contract', string='Manager Number')
    ...
    date_due = fields.Date(
        string='Due Date', related='invoice_ref.invoice_date_due')
...
class ProductManagerChecklist(models.Model):
    _name = 'product.manager.checklist'

    name = fields.Many2one('product.tools', string="Name")
    checklist_active = fields.Boolean(string="Available", default=True)
    checklist_number = fields.Many2one(
        'product.manager.contract', string="Checklist Number")
...
class ProductTools(models.Model):
    _name = 'product.tools'
