...
class Prescription(models.Model):
    ...
    patient_id = fields.Many2one(
        "podmanager.patient", string='Patient Name', required=True)
    ...
    prescription_device_ids = fields.One2many("podmanager.prescription.device",
                                              "prescription_device_id", string="Prescription Device")
    appointed_doctor_id = fields.Many2one(
        "podmanager.doctor", string="Doctor name", required=True)
    prescription_medical_test_ids = fields.Many2many("podmanager.medicaltest", "medical_test_ids",
    ...
    def create(self, vals):
        ...
        if vals.get('name', _('New')) == _('New'):
            vals['name'] = self.env['ir.sequence'].next_by_code(
                'podmanager.prescription') or _('New')
...
class PrescriptionDevice(models.Model):
    _name = "podmanager.prescription.device"
    ...
    prescription_device_id = fields.Many2one(
        "podmanager.prescription", string="Prescription device")
