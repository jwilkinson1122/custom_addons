...
class KmPod(http.Controller):
    ...
    def create_webpatient(self, **kw):
        # print("\nData Received.....", kw)
        request.env['podmanager.patient'].sudo().create(kw)
    ...
    def view_patient_web(self, **kw):
        patients = request.env['podmanager.patient'].sudo().search([])
    ...
    def prescription_webform(self, **kw):
        patient_rec = request.env['podmanager.patient'].sudo().search([])
        doctor_rec = request.env['podmanager.doctor'].sudo().search([])
    ...
    def create_webprescription(self, **kw):
        print("\nData Received.....", kw)
        request.env['podmanager.prescription'].sudo().create(kw)
