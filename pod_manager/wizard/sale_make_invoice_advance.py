...
class SaleAdvancePaymentInv(models.TransientModel):
    ...
    def create_invoices(self):
        ...
        if self._context.get("active_model") == "account.prescription":

            AccountPrescription = self.env["account.prescription"]
            prescription = AccountPrescription.browse(self._context.get("active_ids", []))
            prescription.device_line_ids.mapped("product_id").write({"is_device": True})
            ctx.update(
                {
