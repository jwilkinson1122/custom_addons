...
class ResPartner(models.Model):
    ...
    lfbb_membership_number = fields.Char(
        "LFBB Membership Number",
        copy=False,
        help="LFBB membership number of the player.\nUsually looks like 300xxxxx.",
    )
    lfbb_uuid = fields.Char(
        "LFBB UUID",
        inverse="_inverse_lfbb_uuid",
        copy=False,
    ...
    lfbb_external_url = fields.Char(
        "LFBB External URL",
        compute="_compute_lfbb_url",
        help="Player/club external link (do not require any authentication).",
    )
    lfbb_internal_url = fields.Char(
        "LFBB Internal URL",
        compute="_compute_lfbb_url",
    ...
    def _compute_lfbb_url(self):
        for record in self:
            if not record.lfbb_uuid:
                ...
                record.lfbb_external_url = (
                    f"{LFBB_ROOT_URL}/{int_mid_url}/{record.lfbb_uuid}"
                )
                ext_id_key, end_url = (
                    ("gid", "group.aspx")
                ...
                record.lfbb_internal_url = (
                    f"{LFBB_ROOT_URL}/organization/{end_url}?{url_params}"
                )
    ...
    def _check_lfbb_membership_number(self):
        ...
        for record in self:
            if (
                record.lfbb_membership_number
                and self.search_count(
                    [("lfbb_membership_number", "=", record.lfbb_membership_number)]
                )
                > 1
            ):
                raise ValidationError(
                    _(
                        "The LFBB membership number must be unique! "
