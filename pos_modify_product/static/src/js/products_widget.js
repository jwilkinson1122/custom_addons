...
import { ProductModifierPopup } from "@pos_modify_product/js/product_modifier_popup";
...
patch(ProductsWidget.prototype,{
    ...
    left_side() {
        ...
        $('.modifiers-product-list').show();
        $('#bilateral_device').hide();
        $('#bilateral_pair_device').hide();
        $('#right_device').hide();
        $('#right_side_device').hide();
        self.modifier_attribute = [];
        var inLeftRadio = document.querySelector('input[name="left"]:checked');
        if (inLeftRadio) {
            $('.modifiers-product-list').show();
        }
    },
    ...
    right_side() {
        ...
        $('#left_side_device').hide();
        self.modifier_attribute = [];
        var inRightRadio = document.querySelector('input[name="right"]:checked');
        if (inRightRadio) {
            $('.modifiers-product-list').show();
        }
    },
	...
	modifierSelection(event) {
        ...
        self.pos.popup.add(ProductModifierPopup, {
	...
	selectSubProduct() {
        ...
        if (self.pos.side_prod_list.length == 0) {
            self.pos.popup.add(ErrorPopup, {
                title: _t('Options'),
                body: _t('Options Are Not Define For This Product.'),
            });
        } else {
            $('.side-prod-info').show();
            $('.side-prod-list').hide();
