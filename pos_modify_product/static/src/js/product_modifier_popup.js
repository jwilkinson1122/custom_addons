...
export class ProductModifierPopup extends AbstractAwaitablePopup {
    static template = "pos_modify_product.ProductModifierPopup";
	...
	click_on_modifier_product(event){
    	...
    	if(product){
        	...
        	vals_list.laterality = self.props.laterality;
        	vals_list.laterality_type = self.props.laterality_type;
    	}
    	if (modifier_list){
			if (modifier_list.length > 0){
	    		if (modifier_list.find((mod) => mod.id === vals_list.id && mod.laterality === vals_list.laterality)){
					var increse = modifier_list.find((mod) => mod.id === vals_list.id && mod.laterality === vals_list.laterality);
		    		if (increse && increse.laterality == vals_list.laterality){
		    			increse.qty += 1; 
		    		}else{
    ...
    //     	vals_list.template_name = product.template_name;
    //     	vals_list.uom_id = product.uom_id;
    //     	vals_list.segment = self.props.segment;
    //     	vals_list.segment_type = self.props.segment_type;
    // 	}
    // 	if (modifier_list){
	// 		if (modifier_list.length > 0){
	//     		if (modifier_list.find((mod) => mod.id === vals_list.id && mod.segment === vals_list.segment)){
	// 				var increse = modifier_list.find((mod) => mod.id === vals_list.id && mod.segment === vals_list.segment);
	// 	    		if (increse && increse.segment == vals_list.segment){
	// 	    			increse.qty += 1; 
	// 	    		}else{
	// 	    			modifier_list.push(vals_list)
	// 	    		}
