...
class ReportBomStructure(models.AbstractModel):
    ...
    def _format_route_info(self, rules, rules_delay, warehouse, product, bom, quantity):
        ...
        if self._is_buy_route(rules, product, bom):
            ...
            prescription_lead = parent_bom.company_id.days_to_prescription + parent_bom.company_id.rx_lead if parent_bom and parent_bom.company_id else 0
            if supplier:
                return {
                    'route_type': 'buy',
                    'route_name': buy_rules[0].route_id.display_name,
                    'route_detail': supplier.display_name,
                    'lead_time': supplier.delay + rules_delay + prescription_lead,
                    'supplier_delay': supplier.delay + rules_delay + prescription_lead,
                    'supplier': supplier,
                }
        return res
