...
class StockMove(models.Model):
    ...
    def _prepare_phantom_move_values(self, bom_line, product_qty, quantity_done):
        ...
        if self.prescription_line_id:
            vals['prescription_line_id'] = self.prescription_line_id.id
        return vals

    def _get_price_unit(self):
        if self.product_id == self.prescription_line_id.product_id or not self.bom_line_id or self._should_ignore_rxl_price():
            return super()._get_price_unit()
        line = self.prescription_line_id
