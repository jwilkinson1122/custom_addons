...
class TestAngloSaxonValuationPrescriptionMRP(TransactionCase):

    @classmethod
    def setUpClass(cls):
        super(TestAngloSaxonValuationPrescriptionMRP, cls).setUpClass()
    ...
    def test_kit_anglo_saxo_price_diff(self):
        ...
        rx_form.partner_id = self.vendor01
        with rx_form.order_line.new() as rxl_form:
            rxl_form.product_id = kit
            rxl_form.price_unit = 100
        rx = rx_form.save()
        rx.button_confirm()

        rx.picking_ids.button_validate()

        action = rx.action_create_invoice()
        invoice = self.env['account.move'].browse(action['res_id'])
        invoice.invoice_date = Date.today()
        invoice.action_post()

        svls = rx.order_line.move_ids.stock_valuation_layer_ids
