...
class TestPrescriptionMrpFlow(TransactionCase):
    ...
    def test_kit_component_cost(self):
        ...
        rx = Form(self.env['prescription.order'])
        rx.partner_id = self.env['res.partner'].create({'name': 'Testy'})
        with rx.order_line.new() as line:
            line.product_id = self.kit_1
            line.product_qty = 120
            line.price_unit = 1260
        rx = rx.save()
        rx.button_confirm()
        rx.picking_ids.button_validate()
    ...
    def test_kit_component_cost_multi_currency(self):
        ...
        layer = rx.picking_ids.move_ids.stock_valuation_layer_ids
    ...
    def test_01_sale_mrp_kit_qty_delivered(self):
        ...
        f = Form(self.env['prescription.order'])
        ...
        # to the RX
        self.assertEqual(len(rx.picking_ids), 1)
        order_line = rx.order_line[0]
        picking_original = rx.picking_ids[0]
        ...
        pick = rx.picking_ids[0]
        res = pick.button_validate()
        Form(self.env[res['res_model']].with_context(res['context'])).save().process()

        # Check that a backorded is created
        self.assertEqual(len(rx.picking_ids), 2)
        backorder_1 = rx.picking_ids - picking_original
        ...
        self.assertEqual(len(rx.picking_ids), 3)
        backorder_2 = rx.picking_ids - picking_original - backorder_1
        ...
        self.assertEqual(len(rx.picking_ids), 4)
        backorder_3 = rx.picking_ids - (picking_original + backorder_1 + backorder_2)
        ...
        self.assertEqual(len(rx.picking_ids), 7)
        backorder_4 = rx.picking_ids - (
    ...
    def test_concurent_procurements(self):
        ...
        # Find RX
        prescription = self.env['prescription.order.line'].search([
            ('product_id', '=', component.id),
        ]).order_id
        self.assertTrue(prescription)
        self.assertEqual(prescription.order_line.product_qty, 5)

    def test_01_prescription_mrp_kit_qty_change(self):
        self.partner = self.env.ref('base.res_partner_1')

        # Create a RX with one unit of the kit product
        self.rx = self.env['prescription.order'].create({
        ...
        # Validate the RX
        self.rx.button_confirm()

        # Check the component qty in the created picking
        self.assertEqual(self.rx.picking_ids.move_ids_without_package[0].product_uom_qty, 2, "The quantity of components must be created according to the BOM")
        self.assertEqual(self.rx.picking_ids.move_ids_without_package[1].product_uom_qty, 1, "The quantity of components must be created according to the BOM")
        self.assertEqual(self.rx.picking_ids.move_ids_without_package[2].product_uom_qty, 3, "The quantity of components must be created according to the BOM")

        # Update the kit quantity in the RX
        self.rx.order_line[0].product_qty = 2
        # Check the component qty after the update
        self.assertEqual(self.rx.picking_ids.move_ids_without_package[0].product_uom_qty, 4, "The amount of the kit components must be updated when changing the quantity of the kit.")
        self.assertEqual(self.rx.picking_ids.move_ids_without_package[1].product_uom_qty, 2, "The amount of the kit components must be updated when changing the quantity of the kit.")
        self.assertEqual(self.rx.picking_ids.move_ids_without_package[2].product_uom_qty, 6, "The amount of the kit components must be updated when changing the quantity of the kit.")
    ...
    def test_procurement_with_preferred_route(self):
        ...
        "Buy". A prescription order should be generated.
        """
        ...
        rx = self.env['prescription.order'].search([('partner_id', '=', vendor.id)])
        self.assertTrue(rx)

        rx.button_confirm()
        move_in = rx.picking_ids.move_ids
    ...
    def test_compute_bom_days_00(self):
        ...
        Security Lead Time and Days to Prescription are set.
        """
        prescription_route = self.env.ref("prescription_stock.route_warehouse0_buy")
        ...
        company_1.rx_lead = 0
        company_1.days_to_prescription = 0
        company_1.manufacturing_lead = 0
        company_2.rx_lead = 0
        company_2.days_to_prescription = 0
        ...
        components.write({
            'route_ids': [(6, 0, prescription_route.ids)],
            'seller_ids': [(0, 0, {
        ...
        # set "Security Lead Time" for Prescription and manufacturing, and "Days to Prescription"
        company_1.rx_lead = 10
        company_1.days_to_prescription = 10
        company_1.manufacturing_lead = 10
        company_2.rx_lead = 20
        company_2.days_to_prescription = 20
        company_2.manufacturing_lead = 20

        # check "Security Lead Time" and "Days to Prescription" will also be included if bom has company_id
        bom_kit_parent.action_compute_bom_days()
        ...
        # check "Security Lead Time" and "Days to Prescription" will won't be included if bom doesn't have company_id
    ...
    def test_orderpoint_with_manufacture_security_lead_time(self):
        ...
        """
        # set prescription security lead time to 20 days
        self.env.company.rx_lead = 20
        ...
        product = self.env['product.product'].create({
            ...
            'route_ids': [
                ...
                (4, self.env.ref('prescription_stock.route_warehouse0_buy').id)
            ],
            'seller_ids': [(0, 0, {
    ...
    def test_bom_report_incoming_rx(self):
        ...
        def create_order(product_id, partner_id, date_order):
            ...
            return f.save()
        partner = self.env['res.partner'].create({'name': 'My Test Partner'})
        # Create and confirm two RXs with 3 component_product at different date
        rx_today = create_order(component_product, partner, fields.Datetime.now())
        rx_5days = create_order(component_product, partner, fields.Datetime.now() + timedelta(days=5))

        rx_today.button_confirm()
        rx_5days.button_confirm()
    ...
    def test_bom_report_incoming_rx2(self):
        """ Test report bom structure with duplicated components
            With an incoming RX for the first and second line.
        """
        uom_unit = self.env.ref('uom.product_uom_unit')
        final_product_tmpl = self.env['product.template'].create({'name': 'Final Product', 'type': 'product'})
        component_product = self.env['product.product'].create({'name': 'Compo 1', 'type': 'product'})
        ...
        partner = self.env['res.partner'].create({'name': 'My Test Partner'})
        # Create and confirm one RX with 6 component_products.
        f = Form(self.env['prescription.order'])
        f.partner_id = partner
        ...
        report_values = self.env['report.mrp.report_bom_structure']._get_report_data(bom_id=bom.id)
        line_values = report_values['lines']['components'][0]
        self.assertEqual(line_values['availability_state'], 'expected', 'The first component should be expected as there is an incoming RX.')
