...
class FileStream(Stream):
    ...
    @classmethod
    def from_file_attachment(cls, attachment: IrAttachment) -> FileStream:
    ...
    def read(self):
        if self.type == "file":
            with self.pod_file_attachment.open("rb") as f:
                return f.read()
        return super().read()
    ...
    def get_response(self, as_attachment=None, immutable=None, **send_file_kwargs):
        ...
        use_x_sendfile = self._file_use_x_sendfile
        # The file will be closed by werkzeug...
        send_file_kwargs["use_x_sendfile"] = use_x_sendfile
        if not use_x_sendfile:
            f = self.pod_file_attachment.open("rb")
            res = _send_file(f, **send_file_kwargs)
        else:
            x_accel_redirect = f"/{self.pod_file_attachment.file_storage_code}{self.pod_file_attachment.file_url_path}"
    ...
    def _check_use_x_sendfile(cls, attachment: IrAttachment) -> bool:
        return (
            config["x_sendfile"]
            and attachment.file_url
            and attachment.file_storage_id.use_x_sendfile_to_serve_internal_url
        )

    @property
    def _file_use_x_sendfile(self) -> bool:
        """Return True if x-sendfile should be used to serve the file"""
        return self._check_use_x_sendfile(self.file_attachment)
