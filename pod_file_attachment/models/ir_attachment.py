...
File_FILENAME_RE_PARSER = re.compile(
...
class IrAttachment(models.Model):
    ...
    file_url_path = fields.Char(
        "Filesystem URL Path",
        compute="_compute_file_url_path",
        help="The path to access the file from the filesystem storage.",
    )
    file_storage_code = fields.Char(
        "Filesystem Storage Code",
        related="file_storage_id.code",
        store=True,
    )
    file_storage_id = fields.Many2one(
        "file.storage",
        "Filesystem Storage",
        compute="_compute_file_storage_id",
    ...
    def _compute_file_url(self) -> None:
        for rec in self:
            new_url = None
            actual_url = rec.file_url or None
            if rec.filename:
                new_url = self.env["file.storage"]._get_url_for_attachment(rec)
            # ensure we compare value of same type and not None with False
            new_url = new_url or None
            if new_url != actual_url:
                rec.file_url = new_url

    @api.depends("filename")
    def _compute_file_url_path(self) -> None:
        for rec in self:
            rec.file_url_path = None
            if rec.filename:
                rec.file_url_path = self.env["file.storage"]._get_url_for_attachment(
                    rec, exclude_base_url=True
                )

    @api.depends("filename")
    def _compute_file_storage_id(self):
        for rec in self:
            if rec.store_fname:
                code = rec.store_fname.partition("://")[0]
                file_storage = self.env["file.storage"].sudo().get_by_code(code)
                if file_storage != rec.file_storage_id:
                    rec.file_storage_id = file_storage
            elif rec.file_storage_id:
                rec.file_storage_id = None
    ...
    def _get_storage_force_db_config(self):
        return self.env["file.storage"].get_force_db_for_default_attachment_rules(
            self._storage()
        )
    ...
    def _store_in_db_instead_of_object_storage(self, data, mimetype):
        ...
        * better portability of a database: when replicating a production
        ...
        The configuration can be modified on the file.storage record, in the
    ...
    def _storage(self):
        ...
        storage = self.env["file.storage"].get_default_storage_code_for_attachments()
        if not storage:
    ...
    def _storage_file_read(self, fname: str) -> bytes | None:
        ...
        file, _storage, fname = self._file_parse_store_fname(fname)
        try:
            with file.open(fname, "rb") as f:
                return f.read()
        except IOError:
            _logger.info(
    ...
    def _storage_file_write(self, bin_data: bytes) -> str:
        ...
        storage = self.env.context.get("storage_location") or self._storage()
        file = self._get_file_storage_for_code(storage)
        path = self._get_file_path(storage, bin_data)
        dirname = os.path.dirname(path)
        if not file.exists(dirname):
            file.makedirs(dirname)
        fname = f"{storage}://{path}"
        with file.open(path, "wb") as f:
            f.write(bin_data)
        self._file_mark_for_gc(fname)
        return fname
    ...
    @api.model
    def _get_file_path(self, storage_code: str, bin_data: bytes) -> str:
        """Compute the path to store the file in the filesystem storage"""
        key = self.env.context.get("force_storage_key")
        if not key:
            key = self._compute_checksum(bin_data)
        if self.env["file.storage"]._must_optimize_directory_path(storage_code):
    ...
    def _build_filename(self):
        ...
        the filename field is set. If it is set, we increment the version
        ...
        version = 0
        if self.filename:
            parsed = self._parse_filename(self.filename)
            if parsed:
                version = parsed[2] + 1
        return "{}{}".format(
            slugify(
    ...
    def _enforce_meaningful_storage_filename(self) -> None:
        ...
        for attachment in self:
            if not self._is_file_from_a_storage(attachment.store_fname):
                continue
            file, storage, filename = attachment._get_file_parts()

            if self.env["file.storage"]._must_use_filename_obfuscation(storage):
                attachment.filename = filename
                continue
            new_filename = attachment._build_filename()
            ...
            file.rename(filename, new_filename_with_path)
            ...
            self._file_mark_for_gc(attachment.store_fname)
    ...
    def _file_parse_store_fname(
        ...
        file = self._get_file_storage_for_code(storage_code)
        fname = partition[2]
        return file, storage_code, fname

    @api.model
    def _parse_filename(self, filename: str) -> tuple[str, int, int, str] | None:
        ...
        match = File_FILENAME_RE_PARSER.match(filename)
        if not match:
            return None
    ...
    @api.model
    def _file_mark_for_gc(self, fname):
        ...
        """
        self.env["file.gc"]._mark_for_gc(fname)
    ...
    def _get_file_parts(
        ...
        return self._file_parse_store_fname(self.store_fname)
    ...
    def _get_storage_codes(self):
        ...
        return self.env["file.storage"].sudo().get_storage_codes()
        ...
        * _new_store_fname: the new store_fname if the file is
        ...
        if (
            ...
            if self.attachment._is_file_from_a_storage(self.attachment.store_fname):
                file, _storage, fname = self.attachment._get_file_parts()
                filepath = fname
                filesystem = file
            ...
            if self.attachment._is_file_from_a_storage(new_store_fname):
                (
                    ...
                    _storage,
                    new_filepath,
                ) = self.attachment._file_parse_store_fname(new_store_fname)
                _file, _storage, old_filepath = self.attachment._get_file_parts()
            else:
