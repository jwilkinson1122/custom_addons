...
class FileGC(models.Model):

    _name = "file.gc"
    _description = "Filesystem storage file garbage collector"

    store_fname = fields.Char("Stored Filename")
    file_storage_code = fields.Char("Storage Code")
    ...
    def _gc_files(self) -> None:
        ...
        # from the module pod_file_attachment. The only difference is that the list
        # of files to delete is retrieved from the table file_gc instead
        ...
        # prevent all concurrent updates on ir_attachment and file_gc
        ...
        cr.execute("LOCK file_gc IN SHARE MODE")
    ...
    def _gc_files_unsafe(self) -> None:
        # get the list of file.storage codes that must be autovacuumed
        codes = (
            self.env["file.storage"].search([]).filtered("autovacuum_gc").mapped("code")
        )
        if not codes:
            return
        ...
        self._cr.execute(
            ...
            FROM
                file_gc
            WHERE
                file_storage_code IN %s
                AND NOT EXISTS (
                    SELECT 1
                    FROM ir_attachment
                    WHERE store_fname = file_gc.store_fname
                )
            GROUP BY
                file_storage_code
            """,
        ...
        for code, store_fnames in self._cr.fetchall():
            self.env["file.storage"].get_by_code(code)
        ...
        # delete the records from the table file_gc
        self._cr.execute(
            """
            DELETE FROM
                file_gc
