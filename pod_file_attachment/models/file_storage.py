...
class FileStorage(models.Model):
    ...
    autovacuum_gc = fields.Boolean(
        ...
        "by other systems (like a website). In such case, records in the file.gc "
    ...
    use_x_sendfile_to_serve_internal_url = fields.Boolean(
        ...
        "by the proxy using the file_url if defined. If not, the file will be "
    ...
    field_xmlids = fields.Char(
        ...
        "is linked to a field that is in one File storage, and the related "
        "model is in another File storage, we will store it into"
    ...
    def _get_url_for_attachment(
        ...
        :return: the URL to access the attachment
        """
        file_storage = self.sudo().get_by_code(attachment.file_storage_code)
        if not file_storage:
            return None
        base_url = pod_file_storage.base_url_for_files
        if not base_url:
            return None
        if exclude_base_url:
            base_url = (
                base_url.replace(pod_file_storage.base_url.rstrip("/"), "") or "/"
            )
        # always remove the directory_path from the filename
        # only if it's at the start of the filename
        filename = attachment.filename
        if pod_file_storage.directory_path and filename.startswith(
            pod_file_storage.directory_path
        ):
            filename = filename.replace(pod_file_storage.directory_path, "")
    ...
    def recompute_urls(self) -> None:
        ...
        domain = [
            ("file_storage_id", "in", self.ids),
            "|",
            ("res_field", "=", False),
            ("res_field", "!=", False),
        ]
        attachments = self.env["ir.attachment"].search(domain)
        attachments._compute_file_url()
        attachments._compute_file_url_path()
