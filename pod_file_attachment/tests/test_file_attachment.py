...
from .common import MyException, TestFileAttachmentCommon


class TestFileAttachment(TestFileAttachmentCommon):
    ...
    def test_default_attachment_store_in_file(self):
    ...
    def test_file_update_transactionnal(self):
        ...
        self.assertEqual(attachment.filename, initial_filename)
        self.assertEqual(
        ...
        try:
            with self.env.cr.savepoint():
                ...
                self.assertEqual(attachment.filename, new_filename)
    ...
    def test_file_create_transactional(self):
    ...
    def test_file_no_delete_if_not_in_current_directory_path(self):
    ...
    def test_attachment_file_url(self):
        ...
        self.assertEqual(
            attachment.file_url, f"https://acsone.eu/media{attachment_path}"
        )
        self.assertEqual(attachment.file_url_path, attachment_path)
    ...
    @mute_logger("odoo.addons.pod_file_attachment.models.ir_attachment")
    def test_force_storage_to_file(self):
        attachment = self.ir_attachment_model.create(
            {"name": "test.txt", "raw": b"content"}
        )
        self.env.flush_all()
        file_path = self.ir_attachment_model._filestore() + "/" + attachment.store_fname
        self.assertTrue(os.path.exists(file_path))
        self.assertEqual(os.listdir(self.temp_dir), [])
        # we decide to force the storage in the filestore
        self.temp_backend.use_as_default_for_attachments = True
        with mock.patch.object(self.env.cr, "commit"), mock.patch(
            "odoo.addons.pod_file_attachment.models.ir_attachment.clean_file"
        ) as clean_file:
            self.ir_attachment_model.force_storage()
            clean_file.assert_called_once()
    ...
    def test_storage_use_filename_obfuscation(self):
        ...
        self.assertEqual(attachment.checksum, attachment.file_url.split("/")[-1])
        self.assertEqual(attachment.mimetype, "text/plain")
    ...
    def test_write_name(self):
        ...
        self.assertTrue(attachment.filename.startswith("file-"))
        self.assertTrue(attachment.filename.endswith(".bin"))
        attachment.write({"name": "file2.txt"})
        self.assertTrue(attachment.filename.startswith("file2-"))
