...
class TestFileAttachmentInternalUrl(HttpCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.env = cls.env(context=dict(cls.env.context, tracking_disable=True))
        temp_dir = tempfile.mkdtemp()
        cls.temp_backend = cls.env["file.storage"].create(
            {
                "name": "Temp File Storage",
                "protocol": "file",
                "code": "tmp_dir",
        ...
        cls.gc_file_model = cls.env["file.gc"]
    ...
    def test_file_attachment_internal_url(self):
    ...
    def test_file_attachment_internal_url_x_sendfile(self):
        self.authenticate("admin", "admin")
