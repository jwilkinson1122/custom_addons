...
class StockMove(models.Model):
    ...
    def _get_new_picking_values(self):
        ...
        return {
            ...
            "partner_id": self.sale_line_id.delivery_addr_id.id
            or self.sale_line_id.order_id.partner_id.id,
    ...
    def _assign_picking(self):
        ...
        for group, moves in grouped_moves:
            ...
            if picking:
                ...
                orders = self.env["sale.order"].search([("name", "=", origins)])
                for rec in orders:
                    addr = [rec.partner_id.id]
                    for line in rec.order_line:
                        if (
                            line.delivery_addr_id
                            and line.delivery_addr_id.id not in addr
                        ):
                            addr.append(line.delivery_addr_id.id)
                        elif line.delivery_addr_id and rec.partner_id.id not in addr:
                    ...
                    if len(addr) <= 1:
                        ...
                        for mov in moves:
                            mov.write(
                                {
                                    "partner_id": mov.sale_line_id.delivery_addr_id.id
