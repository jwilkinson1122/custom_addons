...
class ProductTemplate(models.Model):
    ...
    def fetch_taxed_price(self, price, qty=1, company=False):
        ...
        taxes = partner.property_account_position_id.map_tax(
            self.sudo().taxes_id.filtered(lambda x: x.company_id == company)
        )
    ...
    def get_option_data(self, pricelist, company, price_with_pricelist=True):
        ...
        optionData = {}
        for option in self.product_option_ids:
            if toCurrency != self.currency_id:
                price = self.currency_id._convert(
                    option.price, toCurrency, company, fields.Date.today()
                )
            else:
                price = option.price
        ...
        optionValueData = {}
        for optionValue in self.product_option_ids.mapped("product_options_value_ids"):
            if toCurrency != self.currency_id:
                price = self.currency_id._convert(
                    optionValue.price, toCurrency, company, fields.Date.today()
                )
            else:
        ...
        if price_with_pricelist:
            ...
            optionValueData = self._get_pricelist_based_price_data(
                optionValueData, pricelistId
            )
