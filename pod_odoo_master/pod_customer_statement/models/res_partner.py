...
class ResPartner(models.Model):
    ...
    def action_send_filter_customer_whatsapp(self):
        ...
        template = self.env.ref(
            "pod_odoo_master.send_customer_filter_whatsapp_email_template"
        )
        ctx = {
    ...
    def action_send_customer_whatsapp(self):
        ...
        if not self.mobile:
            raise UserError(_("Partner Mobile Number Not Exist !"))
        template = self.env.ref(
            "pod_odoo_master.send_customer_whatsapp_email_template"
        )
        ctx = {
            "default_model": "res.partner",
            "default_res_id": self.ids[0],
    ...
    def action_send_due_customer_whatsapp(self):
        ...
        if not self.mobile:
            raise UserError(_("Partner Mobile Number Not Exist !"))
        template = self.env.ref(
            "pod_odoo_master.send_customer_due_whatsapp_email_template"
        )
        ctx = {
            "default_model": "res.partner",
            "default_res_id": self.ids[0],
    ...
    def update_statement_config_manually_(self):
        view = self.env.ref("pod_odoo_master.update_customers_statement_config_wizard")
        return {
            "type": "ir.actions.act_window",
    ...
    def send_customer_statement(self):
        for rec in self:
            if rec.customer_rank > 0 and rec.customer_statement_ids:
                template = self.env.ref(
                    "pod_odoo_master.customer_statement_mail_template"
                )
                if template:
    ...
    def send_customer_overdue_statement(self):
        for rec in self:
            if rec.customer_rank > 0 and rec.customer_due_statement_ids:
                template = self.env.ref(
                    "pod_odoo_master.customer_due_statement_mail_template"
                )
                if template:
                    mail = template.sudo().send_mail(rec.id, force_send=True)
                    mail_id = self.env["mail.mail"].sudo().browse(mail)
    ...
    def action_print_customer_statement(self):
        return self.env.ref(
            "pod_odoo_master.action_report_customer_statement"
        ).report_action(self)

    def action_send_customer_statement(self):
        self.ensure_one()
        template = self.env.ref("pod_odoo_master.customer_statement_mail_template")
        if template:
    ...
    def action_print_customer_due_statement(self):
        return self.env.ref(
            "pod_odoo_master.action_report_customer_due_statement"
        ).report_action(self)

    def action_send_customer_due_statement(self):
        self.ensure_one()
        template = self.env.ref("pod_odoo_master.customer_due_statement_mail_template")
        if template:
            mail = template.sudo().send_mail(self.id, force_send=True)
            mail_id = self.env["mail.mail"].sudo().browse(mail)
            if mail_id:
    ...
    def action_print_filter_customer_statement(self):
        return self.env.ref(
            "pod_odoo_master.action_report_customer_filtered_statement"
        ).report_action(self)

    def action_send_filter_customer_statement(self):
        self.ensure_one()
        template = self.env.ref(
            "pod_odoo_master.customer_filter_statement_mail_template"
        )
        if template:
            mail = template.sudo().send_mail(self.id, force_send=True)
            mail_id = self.env["mail.mail"].sudo().browse(mail)
