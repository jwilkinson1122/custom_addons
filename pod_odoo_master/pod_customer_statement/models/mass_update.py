...
class MassActionWizard(models.TransientModel):
    ...
    customer_update = fields.Selection(
...
class MassActionPartnerWizard(models.TransientModel):
    ...
    customer_config_update = fields.Selection(
        [("add", "Add"), ("remove", "Remove")],
        string="Customer Overdue Statement Action",
        default="add",
    )
    update_config_ids = fields.Many2many(
        "customer.statement.config", string="Config", required="1"
    )
