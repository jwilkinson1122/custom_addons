        ...
        if self.work_process_order_id:
            ...
            for line in self.order_line:
                line.product_id.invoice_policy = (
                    self.work_process_order_id.invoicing_policy
                )
            ...
            if self.work_process_order_id.validation_order == True:
                ...
                for order in self:
                    if (
                        self.work_process_order_id.validation_picking == True
                        or self.work_process_order_id.force_transfer == True
                    ):
                        ...
                        for pick in picking_obj:
                            for qty in pick.move_lines:
                                ...
                                qty.write({"quantity_done": qty.product_uom_qty})
            ...
            if self.work_process_order_id.create_invoice == True:
                ...
                if (
                    self.work_process_order_id.register_payment == True
                    and self.work_process_order_id.validate_invoice == True
                ):
                        ...
                        payment_order.write(
                            {
                                ...
                                .with_context(ir_sequence_date=payment_order.date)
                                .next_by_code(sequence_code),
                                "currency_id": payment_order.company_id.currency_id,
                            }
                        )
                        inv.action_invoice_paid()
                        pay_confirm = payment = self.env["account.payment"].search(
                            [("ref", "=", inv.name)]
                        )
                    ...
                    self.work_process_order_id.validate_invoice == True
                    or self.work_process_order_id.force_invoice == True
                ):
                    validate = invoice_obj.action_post()
            ...
            raise ValidationError(
                ("Workflow Process is not given,Please give the Workflow process.")
            )
