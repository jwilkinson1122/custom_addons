...
class SaleOrder(models.Model):
    ...
    def _compute_limited_order_history(self):
        ...
        param_value = self.env["ir.config_parameter"].sudo().get_param(
            "pod_odoo_master.last_no_of_days_orders", "7"
        )
        ...
        stages = self.env["ir.config_parameter"].sudo().get_param("pod_odoo_master.stages", "all")
        last_orders = int(
            self.env["ir.config_parameter"].sudo().get_param("pod_odoo_master.last_no_of_orders", "10")
        )
