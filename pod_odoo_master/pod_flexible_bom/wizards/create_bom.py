...
class CreateMRPBOMWizard(models.TransientModel):
    ...
    bom_line_ids = fields.One2many(
        "create.mrp.bom.wizard.line", "bom_id", "BoM Lines", copy=True
    )
    ...
    operation_ids = fields.One2many(
        "mrp.routing.workcenter.wizard", "wizard_id", "Operations", copy=True
    )
    ...
    def create_bom(self):
        ...
        if sale_order_line.product_id.product_tmpl_id.margin:
            sale_order_line.price_unit = total_cost / (
                1 - sale_order_line.product_id.product_tmpl_id.margin
            )
...
class CreateMRPBOMWizardLine(models.TransientModel):
    ...
    product_cost = fields.Float(
        string="Unit cost", related="product_id.product_tmpl_id.standard_price"
    )
    product_total_cost = fields.Float(
        string="Total cost", compute="_compute_product_total_cost"
    )
    ...
    allowed_operation_ids = fields.One2many(
        "mrp.routing.workcenter.wizard", related="bom_id.operation_ids"
    )
    operation_id = fields.Many2one(
        "mrp.routing.workcenter.wizard",
    ...
    def _compute_product_total_cost(self):
        for record_id in self:
            record_id.product_total_cost = (
                record_id.product_qty * record_id.product_cost
            )
...
class MrpRoutingWorkcenterWizard(models.TransientModel):
    ...
    _description = "Routing Workcenter Wizard"
    ...
    employee_ratio = fields.Float(
        "Employee Capacity",
        default=1,
        help="Number of employees needed to complete operation.",
    )
