...
class SaleOrder(models.Model):
    ...
    configurator_id = fields.Many2one("product.configurator", string="Configurator", ondelete="cascade")
    config_session_id = fields.Many2one("product.config.session", string="Configuration Session", ondelete="cascade") 


    def action_config_start(self):
        """Start or reset the product configurator for a sale order, ensuring session, template, and attributes persist correctly."""

        _logger.info(f"🔄 [CONFIG START] Initializing Configurator for Order ID {self.id}")

        config_session_obj = self.env["product.config.session"]
        configurator_obj = self.env["product.configurator.sale"]

        # ✅ Fetch a default product template to prevent validation errors
        product_tmpl = self.env["product.template"].search([("config_ok", "=", True)], limit=1)
        
        if not product_tmpl:
            raise ValidationError(_("No configurable product template found. Please configure a product before proceeding."))

        # ✅ Ensure this order gets a new session if none exists
        if not self.config_session_id or not self.config_session_id.exists():
            _logger.warning(f"⚠️ No active configuration session found for Order ID {self.id}. Creating a new session.")

            # ✅ Force a new session for fresh configurations
            self.config_session_id = config_session_obj.create({
                "user_id": self.env.uid,
                "state": "draft",
                "product_tmpl_id": product_tmpl.id,  # ✅ Ensure a template is assigned
                "config_step": "select",   # ✅ Start fresh at template selection
            })

        # ✅ Ensure the configurator exists for this order
        configurator = configurator_obj.search(
            [("order_id", "=", self.id), ("config_session_id", "=", self.config_session_id.id)], limit=1
        )

        if not configurator:
            _logger.info(f"✅ Creating a new configurator for Order ID {self.id}")
            configurator = configurator_obj.create({
                "order_id": self.id,
                "config_session_id": self.config_session_id.id,
                "product_tmpl_id": self.config_session_id.product_tmpl_id.id,  # ✅ Restore product template
                "state": "select",  # ✅ Force reset to "Select Template" step
            })
        else:
            _logger.info(f"✅ Reusing existing configurator ID {configurator.id}")

        # ✅ Ensure configurator always starts at "Select Template" for a new order
        if not self.config_session_id.product_tmpl_id:
            _logger.info(f"🔄 Resetting configurator step to 'Select Template' for Order ID {self.id}")
            configurator.state = "select"
            configurator.write({"state": "select"})  # ✅ Force-write to avoid incorrect step persistence

        _logger.info(f"✅ [CONFIG START] Opening Configurator ID {configurator.id} for Order ID {self.id}")

        # ✅ Set up the correct context for the configurator wizard
        ctx = dict(
            self.env.context,
            default_order_id=self.id,
            default_config_session_id=self.config_session_id.id,
            default_configurator_id=configurator.id,
            wizard_model="product.configurator.sale",
            allow_preset_selection=True,
        )

        return configurator_obj.with_context(**ctx).get_wizard_action(wizard=configurator)


    def unlink(self):
        """Ensure configurator sessions and configurators are removed before deleting the order."""
        
        for order in self:
            _logger.info(f"🗑️ [DELETE ORDER] Deleting Order ID {order.id}, ensuring related records are removed.")

            # Find and delete linked configurators
            configurators = self.env["product.configurator.sale"].search([("order_id", "=", order.id)])
            
            if configurators:
                _logger.warning(f"⚠️ [DELETE ORDER] Found {len(configurators)} linked configurators. Deleting them.")
                configurators.unlink()  # ✅ Remove linked configurators first

            # Find and delete related sessions
            config_sessions = self.env["product.config.session"].search([("id", "=", order.config_session_id.id)])

            for session in config_sessions:
                if session.state == "draft":
                    _logger.warning(f"⚠️ Prevented deletion of session {session.id} in 'draft' state.")
                else:
                    _logger.info(f"✅ Deleting Configuration Session {session.id}")
                    session.unlink()
...
class SaleOrderLine(models.Model):
    ...
    template_id = fields.Many2one("product.configurator.template", string="Configuration Template", help="Select a previously saved configuration template.")
    custom_value_ids = fields.One2many("product.config.session.custom.value", "cfg_session_id", related="config_session_id.custom_value_ids", string="Configurator Custom Values")
    ...
    laterality = fields.Selection(selection=[("left", "Left Only"), ("right", "Right Only"), ("bilateral", "Bilateral")], string="Laterality", required=True, default="bilateral", help="Specifies whether this order line is for the left foot, right foot, or both.")
    ...
    previous_config_comparison = fields.Text(compute="_compute_previous_config_comparison", store=False, string="Previous Configuration Comparison")
    exclude_from_bulk_copy = fields.Boolean(string="Exclude from Bulk Copy", help="Check this box if you do not want this product to be included in bulk copying of configurations.")
    ...
    # --- helper: find “latest” matching line without dotted order
    @api.model
    def _find_previous_line(self, partner_id, product_id, laterality, exclude_order_id=None, lookback=50):
        if not partner_id or not product_id:
            return self.browse()
        order_domain = [('partner_id', '=', partner_id)]
        if exclude_order_id:
            order_domain.append(('id', '!=', exclude_order_id))
        # recent orders by date (safe)
        prev_orders = self.env['sale.order'].search(order_domain, order='date_order desc, id desc', limit=lookback)
        for so in prev_orders:
            line = self.search([
                ('order_id', '=', so.id),
                ('product_id', '=', product_id),
                ('laterality', '=', laterality),
            ], order='id desc', limit=1)  # local fields only
            if line:
                return line
        return self.browse()
