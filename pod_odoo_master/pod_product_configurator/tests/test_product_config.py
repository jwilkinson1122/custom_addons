...
class ProductConfig(ProductConfiguratorTestCases):
    def setUp(self):
        ...
        self.config_product = self.env.ref("pod_odoo_master.product_template_superglass_flex")
        self.attr_engine = self.env.ref("pod_odoo_master.product_attribute_engine")
        self.config_step_engine = self.env.ref(
            "pod_odoo_master.config_step_engine"
        )
        self.config_product_1 = self.env.ref(
            "pod_odoo_master.product_config_line_gasoline_engines"
        )
        self.config_product_2 = self.env.ref(
            "pod_odoo_master.2_series_config_step_body"
        )
        # domain
        self.domain_gasolin = self.env.ref(
            "pod_odoo_master.product_config_domain_gasoline"
        )
        self.domain_engine = self.env.ref(
            "pod_odoo_master.product_config_domain_diesel"
        )
        self.config_image_red = self.env.ref("pod_odoo_master.config_image_1")
        # value
        self.value_gasoline = self.env.ref(
            "pod_odoo_master.pav_fuel_gasoline"
        )
        self.value_diesel = self.env.ref(
            "pod_odoo_master.pav_fuel_diesel"
        )
        self.value_red = self.env.ref(
            "pod_odoo_master.pav_color_red"
        )
        # config_step
        self.config_step_engine = self.env.ref(
            "pod_odoo_master.config_step_engine"
        ...
        self.attribute_line = self.env.ref(
            "pod_odoo_master.ptal_superglass_flex_engine_types"
        )
        self.value_silver = self.env.ref(
            "pod_odoo_master.pav_color_silver"
        )
        self.value_rims_387 = self.env.ref(
            "pod_odoo_master.pav_rims_v_spoke_18"
        )
        # attribute line
        self.attribute_line_2_series_rims = self.env.ref(
            "pod_odoo_master.ptal_superglass_flex_rim_types"
        )
        self.attribute_line_2_series_tapistry = self.env.ref(
            "pod_odoo_master.ptal_superglass_flex_tapistry_types"
        )
        self.attribute_value_tapistry_oyster_black = self.env.ref(
            "pod_odoo_master." + "pav_tapistry_oyster_black"
        )
        self.attribute_line_2_series_transmission = self.env.ref(
            "pod_odoo_master.ptal_superglass_flex_transmission_types"
        )

        # attribute value
        self.attribute_rims = self.env.ref(
            "pod_odoo_master.product_attribute_rims"
        )
        self.attribute_tapistry = self.env.ref(
            "pod_odoo_master.product_attribute_tapistry"
        )
        self.attribute_transmission = self.env.ref(
            "pod_odoo_master.product_attribute_transmission"
        )
