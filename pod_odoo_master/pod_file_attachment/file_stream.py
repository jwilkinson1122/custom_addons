...
class FileStream(Stream):
    ...
    def read(self):
        if self.type == "file":
            with self.pod_odoo_master.open("rb") as f:
                return f.read()
        return super().read()
    ...
    def get_response(self, as_attachment=None, immutable=None, **send_file_kwargs):
        ...
        if not use_x_sendfile:
            f = self.pod_odoo_master.open("rb")
            res = _send_file(f, **send_file_kwargs)
        else:
            x_accel_redirect = f"/{self.pod_odoo_master.file_storage_code}{self.pod_odoo_master.file_url_path}"
