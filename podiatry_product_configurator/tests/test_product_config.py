...
class ProductConfig(ProductConfiguratorTestCases):
    def setUp(self):
        ...
        self.config_product = self.env.ref("podiatry_product_configurator.bmw_2_series")
        self.attr_engine = self.env.ref("podiatry_product_configurator.product_attribute_engine")
        self.config_step_engine = self.env.ref(
            "podiatry_product_configurator.config_step_engine"
        )
        self.config_product_1 = self.env.ref(
            "podiatry_product_configurator.product_config_line_gasoline_engines"
        )
        self.config_product_2 = self.env.ref(
            "podiatry_product_configurator.2_series_config_step_body"
        )
        # domain
        self.domain_gasolin = self.env.ref(
            "podiatry_product_configurator.product_config_domain_gasoline"
        )
        self.domain_engine = self.env.ref(
            "podiatry_product_configurator.product_config_domain_diesel"
        )
        self.config_image_red = self.env.ref("podiatry_product_configurator.config_image_1")
        # value
        self.value_gasoline = self.env.ref(
            "podiatry_product_configurator.product_attribute_value_gasoline"
        )
        self.value_diesel = self.env.ref(
            "podiatry_product_configurator.product_attribute_value_diesel"
        )
        self.value_red = self.env.ref(
            "podiatry_product_configurator.product_attribute_value_red"
        )
        # config_step
        self.config_step_engine = self.env.ref(
            "podiatry_product_configurator.config_step_engine"
        ...
        self.attribute_line = self.env.ref(
            "podiatry_product_configurator.product_attribute_line_2_series_engine"
        )
        self.value_silver = self.env.ref(
            "podiatry_product_configurator.product_attribute_value_silver"
        )
        self.value_rims_387 = self.env.ref(
            "podiatry_product_configurator.product_attribute_value_rims_387"
        )
        # attribute line
        self.attribute_line_2_series_rims = self.env.ref(
            "podiatry_product_configurator.product_attribute_line_2_series_rims"
        )
        self.attribute_line_2_series_tapistry = self.env.ref(
            "podiatry_product_configurator.product_attribute_line_2_series_tapistry"
        )
        self.attribute_value_tapistry_oyster_black = self.env.ref(
            "podiatry_product_configurator." + "product_attribute_value_tapistry_oyster_black"
        )
        self.attribute_line_2_series_transmission = self.env.ref(
            "podiatry_product_configurator.product_attribute_line_2_series_transmission"
        )

        # attribute value
        self.attribute_rims = self.env.ref(
            "podiatry_product_configurator.product_attribute_rims"
        )
        self.attribute_tapistry = self.env.ref(
            "podiatry_product_configurator.product_attribute_tapistry"
        )
        self.attribute_transmission = self.env.ref(
            "podiatry_product_configurator.product_attribute_transmission"
        )
