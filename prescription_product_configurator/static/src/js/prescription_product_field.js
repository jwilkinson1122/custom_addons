    ...
    async _onProductTemplateUpdate() {
        ...
        if(result && result.product_id) {
            ...
            if (!result.mode || result.mode === 'configurator') {
                ...
                // only triggered when prescription_product_matrix is installed.
    ...
    async _openProductConfigurator(edit=false) {
        ...
        this.dialog.add(ProductConfiguratorDialog, {
            ...
            save: async (mainProduct, optionalProducts) => {
                ...
                this._onProductUpdate();
                prescriptionOrderRecord.data.order_line.leaveEditMode();
                for (const optionalProduct of optionalProducts) {
                    const line = await prescriptionOrderRecord.data.order_line.addNewRecord({
                        position: 'bottom',
                        mode: "readonly",
                    });
