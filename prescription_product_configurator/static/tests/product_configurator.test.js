odoo.define('prescription.product.configurator.tests', function (require) {
"use strict";

var FormView = require('web.FormView');
var ProductConfiguratorFormView = require('prescription_product_configurator.ProductConfiguratorFormView');
...
QUnit.module('Product Configurator', {
        ...
        var form = await createView({
            ...
            intercepts: {
                do_action: function (ev) {
                    if (ev.data.action === 'prescription_product_configurator.prescription_product_configurator_action') {
    ...
    QUnit.test('Select a product in the list and check for template loading', async function (assert) {
        ...
        var product_configurator_form = await createView({
            ...
            model: 'prescription_product_configurator',
            data: this.data,
            arch:
                '<form js_class="product_configurator_form">' +
                    ...
                    '</group>' +
                    '<footer>' +
                        '<button string="Add" class="btn-primary o_prescription_product_configurator_add disabled"/>' +
                        '<button string="Cancel" class="btn-secondary" special="cancel"/>' +
                    '</footer>' +
                '</form>',
                mockRPC: function (route) {
                    if (route === '/prescription_product_configurator/configure') {
                        assert.ok(true);
