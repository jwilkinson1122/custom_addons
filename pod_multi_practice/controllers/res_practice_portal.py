...
class PracticePortal(CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        rtn["practices_count"] = http.request.env["res.practice"].search_count(
            practices_domain
        )
        rtn["patients_count"] = http.request.env["res.patient"].search_count(
            patients_domain
        )
        return rtn
    ...
    def _prepare_practices_domain(cls):
        ...
        return [
            ("contact_ids.user_ids", "in", user.id),
        ]

    @classmethod
    def _prepare_patients_domain(cls, practices_domain):
        practice_ids = http.request.env["res.practice"].search(practices_domain).ids
        return [
    ...
    def view_practices(self, page=0, **kw):
        ...
        Practices = http.request.env["res.practice"]
        ...
        practices = http.request.env["res.practice"].search(
            self._prepare_practices_domain(),
            offset=pgr["offset"],
            limit=practices_count,
        )
        return http.request.render(
            template="pod_multi_practice.portal_my_practices",
            qcontext={
    ...
    def view_practice(self, practice_id, page=0, **kw):
        ...
        practice = http.request.env["res.practice"].browse(practice_id)
        if not practice:
        ...
        patients = http.request.env["res.patient"].search(
            [
        ...
        return http.request.render(
            template="pod_multi_practice.portal_my_practice_patients",
            qcontext={
                "practice": practice,
    ...
    def view_patients(self, page=0, **kw):
        ...
        practices_domain = self._prepare_practices_domain()
        patients_domain = self._prepare_patients_domain(practices_domain)
        patients_count = http.request.env["res.patient"].search_count(patients_domain)
        pgr = pager(
        ...
        return http.request.render(
            template="pod_multi_practice.portal_my_patients",
            qcontext={
                "patients_count": patients_count,
                "patients": patients,
                "pager": pgr,
    ...
    def view_patient(self, patient_id, practice_id=None, **kw):
        ...
        patient = http.request.env["res.patient"].browse(patient_id)
        practice = practice_id and http.request.env["res.practice"].browse(practice_id)
        if not patient:
            raise UserError(_("This patient could not be found."))
        pathologies = patient.pathology_ids.filtered(lambda r: r.status == "active")
        return http.request.render(
            template="pod_multi_practice.portal_my_patient_pathologies",
            qcontext={
                "patient": patient,
