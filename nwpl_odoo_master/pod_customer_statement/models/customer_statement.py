...
class CustomerStatementConfig(models.Model):
    ...
    customer_statement_auto_send = fields.Boolean("Statement Auto Send", readonly=False)
    filter_only_unpaid_and_send_that = fields.Boolean(string="Filter/Send Only Unpaid")
    ...
    cust_monthly_date = fields.Integer("Monthly  Day", readonly=False, default=1)
    cust_monthly_template_id = fields.Many2one(
        "mail.template", string="Monthly Mail Template", readonly=False
    )
    cust_yearly_date = fields.Integer(" Yearly day ", readonly=False, default=1)
    cust_monthly_end = fields.Boolean("End of  month", readonly=False)
    ...
    cust_create_log_history = fields.Boolean("Mail Statement History", readonly=False)

    customer_due_statement_auto_send = fields.Boolean("Overdue Statement Auto Send")
    ...
    cust_due_monthly_date = fields.Integer("Monthly Day    ", readonly=False, default=1)
    cust_due_monthly_end = fields.Boolean("End of month", readonly=False)
    ...
    cust_due_create_log_history = fields.Boolean("Mail Overdue History", readonly=False)
    ...
    def mail_history(self):
        search = (
            self.env["customer.mail.history"]
            .search([("partner_id", "in", self.partner_ids.ids)])
            .ids
        )
        return {
    ...
    def add_replace_customer_manually_(self):
        view = self.env.ref("nwpl_odoo_master.update_customers_statement_wizard")
        return {
            "name": "Mass Update",
            "type": "ir.actions.act_window",
            "view_mode": "form",
                    ...
                    if partner.customer_rank > 0:
                        ...
                        if not partner.dont_send_due_customer_statement_auto:
                            if statement.customer_statement_auto_send:
                                if (
                                    statement.filter_only_unpaid_and_send_that
                                ...
                                if statement.customer_statement_action == "daily":
                                    if statement.cus_daily_statement_template_id:
                                        mail = statement.cus_daily_statement_template_id.sudo().send_mail(
                                        ...
                                        if (
                                            mail_id
                                            and statement.cust_create_log_history
                                        ):
                                            self.env[
                                    ...
                                    if int(statement.cust_week_day) == today:
                                        if statement.cust_weekly_statement_template_id:
                                            mail = statement.cust_weekly_statement_template_id.sudo().send_mail(
                                                partner.id, force_send=True
                                            )
                                            mail_id = (
                                                self.env["mail.mail"]
                                    ...
                                    if statement.cust_monthly_end:
                                        ...
                                        if today_date == last_day:
                                            if statement.cust_monthly_template_id:
                                                mail = statement.cust_monthly_template_id.sudo().send_mail(
                                                    partner.id, force_send=True
                                                )
                                                mail_id = (
                                                    self.env["mail.mail"]
                                    ...
                                    if (
                                        statement.cust_yearly_date == today_date
                                        and statement.cust_yearly_month == today_month
                                    ):
                                        if statement.cust_yearly_template_id:
                                            mail = statement.cust_yearly_template_id.sudo().send_mail(
                                                partner.id, force_send=True
                                            )
                                            mail_id = (
                                                self.env["mail.mail"]
                            ...
                            if statement.customer_due_statement_auto_send:
                                ...
                                if statement.customer_due_statement_action == "daily":
                                    if statement.cus_due_daily_statement_template_id:
                                        mail = statement.cus_due_daily_statement_template_id.sudo().send_mail(
                                            partner.id, force_send=True
                                        )
                                        mail_id = (
                                            self.env["mail.mail"].sudo().browse(mail)
                                        ...
                                        if (
                                            mail_id
                                            and statement.cust_due_create_log_history
                                        ):
                                            self.env[
                                                "customer.mail.history"
                                            ].sudo().create(
                                    ...
                                    statement.customer_due_statement_action == "weekly"
                                ):
                                    today = fields.Date.today().weekday()
                                    if int(statement.cust_due_week_day) == today:
                                        if (
                                            statement.cust_due_weekly_statement_template_id
                                        ):
                                            mail = statement.cust_due_weekly_statement_template_id.sudo().send_mail(
                                                partner.id, force_send=True
                                            )
                                            mail_id = (
                                                self.env["mail.mail"]
                                    ...
                                    statement.customer_due_statement_action == "monthly"
                                ):
                                    ...
                                    if statement.cust_due_monthly_end:
                                        ...
                                        if today_date == last_day:
                                            if statement.cust_due_monthly_template_id:
                                                mail = statement.cust_due_monthly_template_id.sudo().send_mail(
                                                    partner.id, force_send=True
                                                )
                                                mail_id = (
                                                    self.env["mail.mail"]
                                    ...
                                    statement.customer_due_statement_action == "yearly"
                                ):
                                    today = fields.Date.today()
                                    today_date = today.day
                                    today_month = today.strftime("%B").lower()
                                    if (
                                        statement.cust_due_yearly_date == today_date
                                        and statement.cust_due_yearly_month
                                        == today_month
                                    ):
                                        if statement.cust_due_yearly_template_id:
                                            mail = statement.cust_due_yearly_template_id.sudo().send_mail(
                                                partner.id, force_send=True
                                            )
                                            mail_id = (
                                                self.env["mail.mail"]
