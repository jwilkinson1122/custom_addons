...
class ResPartner(models.Model):
    ...
    customer_statement_config = fields.Many2many(
        "customer.statement.config",
    ...
    def _compute_cust_due_report_url(self):
        for rec in self:
            ...
            if rec.company_id.statement_pdf_in_message:
                ...
                if rec.customer_rank > 0:
                    rec.cust_due_statement_report_url = (
    ...
    def action_send_filter_customer_whatsapp(self):
        ...
        template = self.env.ref(
            "nwpl_odoo_master.send_customer_filter_whatsapp_email_template"
        )
        ctx = {
    ...
    def action_send_customer_whatsapp(self):
        ...
        if not self.mobile:
            raise UserError(_("Partner Mobile Number Not Exist !"))
        template = self.env.ref(
            "nwpl_odoo_master.send_customer_whatsapp_email_template"
        )
        ctx = {
            "default_model": "res.partner",
            "default_res_id": self.ids[0],
    ...
    def action_send_due_customer_whatsapp(self):
        ...
        if not self.mobile:
            raise UserError(_("Partner Mobile Number Not Exist !"))
        template = self.env.ref(
            "nwpl_odoo_master.send_customer_due_whatsapp_email_template"
        )
        ctx = {
            "default_model": "res.partner",
            "default_res_id": self.ids[0],
    ...
    def update_statement_config_manually_(self):
        view = self.env.ref("nwpl_odoo_master.update_customers_statement_config_wizard")
        return {
            "type": "ir.actions.act_window",
        ...
        for rec in self:
            ...
            if rec.customer_rank > 0:
                ...
                if moves:
                            ...
                            statement_vals.update(
                                {
                                    ...
                                    "customer_balance": move.amount_total
                    ...
                    rec.customer_total = (
                        rec.customer_zero_to_thiry
                        + rec.customer_thirty_to_sixty
                        + rec.customer_sixty_to_ninety
                        + rec.customer_ninety_plus
                    )
                    ...
                    for advance_payment in advanced_payments_inbound:
                        ...
                        if advance_payment.reconciled_invoice_ids:
                            ...
                            if total_paid_amount < advance_payment_amount:
                                statement_vals = {
                                    "account": advance_payment.destination_account_id.name,
                            ...
                            overdue_statement_vals.update(
                                {
                                    ...
                                    "due_customer_balance": overdue.amount_total
    ...
    def send_customer_statement(self):
        for rec in self:
            if rec.customer_rank > 0 and rec.customer_statement_ids:
                template = self.env.ref(
                    "nwpl_odoo_master.customer_statement_mail_template"
                )
                if template:
    ...
    def send_customer_overdue_statement(self):
        for rec in self:
            if rec.customer_rank > 0 and rec.customer_due_statement_ids:
                template = self.env.ref(
                    "nwpl_odoo_master.customer_due_statement_mail_template"
                )
                if template:
                    mail = template.sudo().send_mail(rec.id, force_send=True)
                    mail_id = self.env["mail.mail"].sudo().browse(mail)
    ...
    def action_print_customer_statement(self):
        return self.env.ref(
            "nwpl_odoo_master.action_report_customer_statement"
        ).report_action(self)

    def action_send_customer_statement(self):
        self.ensure_one()
        template = self.env.ref("nwpl_odoo_master.customer_statement_mail_template")
        if template:
    ...
    def action_print_customer_due_statement(self):
        return self.env.ref(
            "nwpl_odoo_master.action_report_customer_due_statement"
        ).report_action(self)

    def action_send_customer_due_statement(self):
        self.ensure_one()
        template = self.env.ref("nwpl_odoo_master.customer_due_statement_mail_template")
        if template:
            mail = template.sudo().send_mail(self.id, force_send=True)
            mail_id = self.env["mail.mail"].sudo().browse(mail)
            if mail_id:
    ...
    def action_get_customer_statement(self):
        ...
        if self.date_filter == "this_quarter":
            ...
            self.end_date = datetime(today.year, 3 * currQuarter + 1, 1) + timedelta(
                days=-1
            )
        ...
        if self.date_filter == "last_quarter":
            ...
            self.start_date = datetime(
                today.year, current_quar_start.month, 1
                        ...
                        statement_vals.update(
                            {
                                ...
                                - move.amount_residual,
                                "filter_balance": move.amount_total
                                - (move.amount_total - move.amount_residual),
                            }
                        )
                    elif move.move_type == "out_refund":
    ...
    def action_print_filter_customer_statement(self):
        return self.env.ref(
            "nwpl_odoo_master.action_report_customer_filtered_statement"
        ).report_action(self)

    def action_send_filter_customer_statement(self):
        self.ensure_one()
        template = self.env.ref(
            "nwpl_odoo_master.customer_filter_statement_mail_template"
        )
        if template:
            mail = template.sudo().send_mail(self.id, force_send=True)
            mail_id = self.env["mail.mail"].sudo().browse(mail)
            ...
            for i in self.filter_customer_statement_ids:
                for j in i:
                    ...
                    if j.filter_amount == j.filter_balance:
                        ...
                        worksheet.write(k, 2, str(j.filter_invoice_date), cyan_text)
                        if j.filter_due_date:
                            worksheet.write(k, 3, str(j.filter_due_date), cyan_text)
                        else:
                        ...
                        worksheet.write(
                            ...
                            5,
                            str(i.currency_id.symbol)
                            + str("{:.2f}".format(j.filter_paid_amount)),
                            cyan_text,
                        )
                        worksheet.write(
                            k,
                            6,
                            str(i.currency_id.symbol)
                            + str("{:.2f}".format(j.filter_balance)),
                            cyan_text,
                        )
                    elif j.filter_balance == 0:
                        worksheet.write(k, 0, j.name, green_text)
                        worksheet.write(k, 1, j.account, green_text)
                        worksheet.write(k, 2, str(j.filter_invoice_date), green_text)
                        if j.filter_due_date:
                            worksheet.write(k, 3, str(j.filter_due_date), green_text)
                        else:
                        ...
                        worksheet.write(k, 2, str(j.filter_invoice_date), red_text)
                        if j.filter_due_date:
                            worksheet.write(k, 3, str(j.filter_due_date), red_text)
                        else:
    ...
    def action_print_customer_statement_xls(self):
            ...
            for i in self.customer_statement_ids:
                for j in i:
                    ...
                    if j.customer_amount == j.customer_balance:
                        worksheet.write(k, 0, j.name, cyan_text)
                        worksheet.write(k, 1, j.account, cyan_text)
                        worksheet.write(k, 2, str(j.customer_invoice_date), cyan_text)
                        if j.customer_due_date:
                            worksheet.write(k, 3, str(j.customer_due_date), cyan_text)
                        else:
                            worksheet.write(k, 3, "", cyan_text)
                        worksheet.write(
                            k,
                            4,
                            str(i.currency_id.symbol)
                            + str("{:.2f}".format(j.customer_amount)),
                            cyan_text,
                        )
                        worksheet.write(
                            k,
                            5,
                            str(i.currency_id.symbol)
                            + str("{:.2f}".format(j.customer_paid_amount)),
                            cyan_text,
                        )
                        worksheet.write(
                            k,
                            6,
                            str(i.currency_id.symbol)
                            + str("{:.2f}".format(j.customer_balance)),
                            cyan_text,
                        )
                    elif j.customer_balance == 0:
                        worksheet.write(k, 0, j.name, green_text)
                        worksheet.write(k, 1, j.account, green_text)
                        worksheet.write(k, 2, str(j.customer_invoice_date), green_text)
                        if j.customer_due_date:
                            worksheet.write(k, 3, str(j.customer_due_date), green_text)
                        else:
                            worksheet.write(k, 3, "", green_text)
                        worksheet.write(
                            k,
                        ...
                        worksheet.write(k, 0, j.name, red_text)
                        worksheet.write(k, 1, j.account, red_text)
                        worksheet.write(k, 2, str(j.customer_invoice_date), red_text)
                        if j.customer_due_date:
                            worksheet.write(k, 3, str(j.customer_due_date), red_text)
                        else:
                            worksheet.write(k, 3, "", red_text)
                        worksheet.write(
                            k,
                ...
                total_paid_amount = total_paid_amount + float(
                    "{:.2f}".format(j.customer_paid_amount)
                )
        ...
        if self.customer_statement_ids:
            worksheet.write(
                k + 4, 1, str("{:.2f}".format(self.customer_zero_to_thiry)), normal
            )
            worksheet.write(
                k + 4, 2, str("{:.2f}".format(self.customer_thirty_to_sixty)), normal
            )
            worksheet.write(
                k + 4, 3, str("{:.2f}".format(self.customer_sixty_to_ninety)), normal
            )
            worksheet.write(
                k + 4, 4, str("{:.2f}".format(self.customer_ninety_plus)), normal
            )
            worksheet.write(k + 4, 5, str("{:.2f}".format(self.customer_total)), normal)
    ...
    def action_print_customer_due_statement_xls(self):
        ...
        if self.customer_due_statement_ids:
            for i in self.customer_due_statement_ids:
                ...
                for j in i:
                    if (
                        j.due_customer_due_date
                        and j.today
                        and j.due_customer_due_date < j.today
                    ):
                        worksheet.write(k, 0, j.name, red_text)
                        worksheet.write(k, 1, j.account, red_text)
                        worksheet.write(
                            k, 2, str(j.due_customer_invoice_date), red_text
                        )
                        if j.due_customer_due_date:
                            worksheet.write(
                                k, 3, str(j.due_customer_due_date), red_text
                            )
                        else:
                            worksheet.write(k, 3, "", red_text)
                        worksheet.write(
                            ...
                            4,
                            str(i.currency_id.symbol)
                            + str("{:.2f}".format(j.due_customer_amount)),
                            red_text,
                        )
                        worksheet.write(
                            k,
                            5,
                            str(i.currency_id.symbol)
                            + str("{:.2f}".format(j.due_customer_paid_amount)),
                            red_text,
                        )
                        worksheet.write(
                            k,
                            6,
                            str(i.currency_id.symbol)
                            + str("{:.2f}".format(j.due_customer_balance)),
                            red_text,
                        )
                    else:
                        ...
                        worksheet.write(
                            k, 2, str(j.due_customer_invoice_date), center_text
                        )
                        if j.due_customer_due_date:
                            worksheet.write(
                                k, 3, str(j.due_customer_due_date), center_text
                            )
                        else:
            ...
            try:
                ...
                if partner.customer_rank > 0:
                    ...
                    if not partner.dont_send_customer_statement_auto:
                        if (
                            self.env.company.customer_statement_auto_send
                            and partner.customer_statement_ids
                        ):
                            if (
                                self.env.company.filter_only_unpaid_and_send_that
                                and not partner.customer_statement_ids.filtered(
                                    lambda x: x.filter_balance > 0
                                )
                            ):
                                return
                            ...
                            if self.env.company.customer_statement_action == "daily":
                                if self.env.company.cus_daily_statement_template_id:
                                    mail = self.env.company.cus_daily_statement_template_id.sudo().send_mail(
                                        partner.id, force_send=True
                                    )
                                    mail_id = self.env["mail.mail"].sudo().browse(mail)
                                    if (
                                        mail_id
                                        and self.env.company.cust_create_log_history
                                    ):
                                        self.env["customer.mail.history"].sudo().create(
                                            {
                                                "name": "Customer Account Statement",
                                ...
                                if int(self.env.company.cust_week_day) == today:
                                    if (
                                        self.env.company.cust_weekly_statement_template_id
                                    ):
                                        mail = self.env.company.cust_weekly_statement_template_id.sudo().send_mail(
                                            partner.id, force_send=True
                                        )
                                        mail_id = (
                                ...
                                self.env.company.customer_statement_action == "monthly"
                            ):
                                ...
                                if self.env.company.cust_monthly_end:
                                    ...
                                    if today_date == last_day:
                                        if self.env.company.cust_monthly_template_id:
                                            mail = self.env.company.cust_monthly_template_id.sudo().send_mail(
                                                partner.id, force_send=True
                                            )
                                            mail_id = (
                                                self.env["mail.mail"]
                                ...
                                if (
                                    self.env.company.cust_yearly_date == today_date
                                    and self.env.company.cust_yearly_month
                                    == today_month
                                ):
                                    if self.env.company.cust_yearly_template_id:
                                        mail = self.env.company.cust_yearly_template_id.sudo().send_mail(
                                            partner.id, force_send=True
                                        )
                                        mail_id = (
                                            self.env["mail.mail"].sudo().browse(mail)
                    ...
                    if not partner.dont_send_due_customer_statement_auto:
                        if (
                            self.env.company.customer_due_statement_auto_send
                            and partner.customer_due_statement_ids
                        ):
                            if (
                                self.env.company.filter_only_unpaid_and_send_that
                                and not partner.customer_due_statement_ids.filtered(
                                    lambda x: x.filter_balance > 0
                                )
                            ):
                                return
                            ...
                            if (
                                self.env.company.customer_due_statement_action
                                == "daily"
                            ):
                                if self.env.company.cus_due_daily_statement_template_id:
                                    mail = self.env.company.cus_due_daily_statement_template_id.sudo().send_mail(
                                        partner.id, force_send=True
                                    )
                                    mail_id = self.env["mail.mail"].sudo().browse(mail)
                                    if (
                                        mail_id
                                        and self.env.company.cust_due_create_log_history
                                    ):
                                        self.env["customer.mail.history"].sudo().create(
                                            {
                                                "name": "Customer Account Overdue Statement",
                                ...
                                if int(self.env.company.cust_due_week_day) == today:
                                    if (
                                        self.env.company.cust_due_weekly_statement_template_id
                                    ):
                                        mail = self.env.company.cust_due_weekly_statement_template_id.sudo().send_mail(
                                            partner.id, force_send=True
                                        )
                                        mail_id = (
                                            self.env["mail.mail"].sudo().browse(mail)
                                ...
                                if self.env.company.cust_due_monthly_end:
                                    ...
                                    if today_date == last_day:
                                        if (
                                            self.env.company.cust_due_monthly_template_id
                                        ):
                                            mail = self.env.company.cust_due_monthly_template_id.sudo().send_mail(
                                                partner.id, force_send=True
                                            )
                                            mail_id = (
                                                self.env["mail.mail"]
                                ...
                                today_month = today.strftime("%B").lower()
                                if (
                                    self.env.company.cust_due_yearly_date == today_date
                                    and self.env.company.cust_due_yearly_month
                                    == today_month
                                ):
                                    if self.env.company.cust_due_yearly_template_id:
                                        mail = self.env.company.cust_due_yearly_template_id.sudo().send_mail(
                                            partner.id, force_send=True
                                        )
                                        mail_id = (
                                            self.env["mail.mail"].sudo().browse(mail)
