...
class CustomerStatementPortal(CustomerPortal):
    ...
    def _prepare_portal_layout_values(self):
        values = super(CustomerStatementPortal, self)._prepare_portal_layout_values()
        values.update(
            {
                "page_name": "customer_statement_portal",
    ...
    def portal_my_customer_statements(self, **kw):
        ...
        customer_statement_ids = partner_id.customer_statement_ids
        overdue_statement_ids = partner_id.customer_due_statement_ids
        filtered_statement_ids = partner_id.filter_customer_statement_ids
        values.update(
            {
        ...
        return request.render("nwpl_odoo_master.customer_statement_portal", values)
    ...
    def filter_customer_statement_get(self, **post):
        ...
        if post.get("start_date") and post.get("end_date"):
            ...
            request.env.user.partner_id.filter_customer_statement_ids.unlink()
            ...
            if moves:
                ...
                for move in moves:
                    statement_vals = {
                        "account": request.env.user.partner_id.property_account_receivable_id.name,
                        "name": move.name,
            ...
            if statement_lines:
                request.env.user.partner_id.filter_customer_statement_ids = (
                    statement_lines
                )
