...
class BlanketOrderWizard(models.TransientModel):
    _name = "cpq.blanket.order.wizard"
    _description = "Blanket order wizard"

    @api.model
    def _default_order(self):
        # in case the cron hasn't run
        self.env["cpq.blanket.order"].expire_orders()
        if not self.env.context.get("active_id"):
            return False
        blanket_order = self.env["cpq.blanket.order"].search(
    ...
    def _default_lines(self):
        blanket_order_line_obj = self.env["cpq.blanket.order.line"]
        blanket_order_line_ids = self.env.context.get("active_ids", False)
        active_model = self.env.context.get("active_model", False)

        if active_model == "cpq.blanket.order":
    ...
    blanket_order_id = fields.Many2one(
        comodel_name="cpq.blanket.order",
        readonly=True,
...
class BlanketOrderWizardLine(models.TransientModel):
    ...
    _name = "cpq.blanket.order.wizard.line"
    _description = "Blanket order wizard line"

    wizard_id = fields.Many2one("cpq.blanket.order.wizard")
    blanket_line_id = fields.Many2one("cpq.blanket.order.line")
