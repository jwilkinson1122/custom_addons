...
class TestSaleBlanketOrders(common.TransactionCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.blanket_order_obj = cls.env["cpq.blanket.order"]
        cls.blanket_order_line_obj = cls.env["cpq.blanket.order.line"]
        cls.blanket_order_wiz_obj = cls.env["cpq.blanket.order.wizard"]
    ...
    def test_02_create_sale_orders_from_blanket_order(self):
        ...
        wizard1 = self.blanket_order_wiz_obj.with_context(
            active_id=blanket_order.id, active_model="cpq.blanket.order"
        ).create({})
