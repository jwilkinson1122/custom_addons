...
class BlanketOrder(models.Model):
    ...
    line_ids = fields.One2many(
        "cpq.blanket.order.line", "order_id", string="Order lines", copy=True
    )
    ...
    def action_confirm(self):
        ...
        for order in self:
            ...
            name = sequence_obj.next_by_code("cpq.blanket.order")
    ...
    def action_view_cpq_blanket_order_line(self):
        action = self.env["ir.actions.act_window"]._for_xml_id(
            "nwpl_odoo_master.act_open_cpq_blanket_order_lines_view_tree"
        )
    ...
    def _search_original_uom_qty(self, operator, value):
        bo_line_obj = self.env["cpq.blanket.order.line"]
        res = []
...
class BlanketOrderLine(models.Model):
    ...
    order_id = fields.Many2one("cpq.blanket.order", required=True, ondelete="cascade")
    product_id = fields.Many2one(
        "product.product",
        string="Product",
        domain=[("sale_ok", "=", True)],
