...
class ContactPatient(models.Model):
    ...
    partner_relation_label = fields.Char(
        "Partner relation label", translate=True, default="Responsible:", readonly=True
    )
    ...
    # Flags
    patient_flag_ids = fields.One2many("contact.flag", inverse_name="patient_id")
    ...
    def unlink(self):
        ...
        try:
            return super(ContactPatient, self).unlink()
        except Exception as e:
            _logger.error(
                f"An error occurred while trying to delete the patient records: {e}"
            )
    ...
    def delete_patient_by_partner_id(self, partner_id):
        ...
        if patient_to_delete:
            try:
                ...
                _logger.error(
                    f"An error occurred while trying to delete the patient record: {e}"
                )
                raise UserError(
                    _("An error occurred while trying to delete the patient record.")
                )
        else:
