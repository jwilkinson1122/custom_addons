from odoo import _, api, fields, models
from odoo.exceptions import UserError


class ContactAbstract(models.AbstractModel):
    # FHIR Entity: default entity, as all models have partner_identifiers
    _name = "contact.abstract"
    _description = "Default FHIR entity"

    partner_identifier = fields.Char(
        name="Identifier",
        help="Internal identifier used to identify this record",
        readonly=True,
        default="New",
        copy=False,
    )  # FHIR Field: identifier

    @api.model
    def create(self, vals):
        vals_upd = vals.copy()
        if vals_upd.get("partner_identifier", "New") == "New":
            vals_upd["partner_identifier"] = self._get_partner_identifier(vals_upd)
            return super().create(vals_upd)
        # return super(ContactAbstract, self).create(vals_upd)

    def _get_partner_identifier(self, vals):
        # It should be rewritten for each element
        raise UserError(_("Function is not defined"))
