...
class ResPartnerContactPoint(models.Model):
    ...
    def _get_default_tag(self):
        contact_point_type = self._context.get("default_contact_point_type", False)
    ...
    def _compute_contact_point(self):
        for record in self:
            for cptype, label in CONTACT_POINT_TYPES:
                record[cptype] = (
                    record.name if record.contact_point_type == cptype else False
                )
    ...
    def _get_is_default_domain(self):
        self.ensure_one()
        return [("partner_id", "=", self.partner_id.id)] + super(
            ResPartnerContactPoint, self
        )._get_is_default_domain()
    ...
    def _complete_vals(self, vals):
        if not vals.get("tag_id"):
            vals["tag_id"] = (
                self.env["res.partner.contact_point.tag"]
                .search(
                    [
        ...
        if "country_id" not in vals and vals.get("contact_point_type") != "email":
            vals["country_id"] = (
                self.env["res.partner"].browse(vals.get("partner_id")).country_id.id
            )
