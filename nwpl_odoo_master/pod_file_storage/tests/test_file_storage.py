...
from .common import TestFileStorageCase


class TestFileStorage(TestFileStorageCase):
    ...
    def test_ensure_one_file_by_record(self):
        # in this test we ensure that we've one file by record
        backend_ids = []
        for i in range(4):
            backend_ids.append(
                self.backend.create(
        ...
        file = None
        for rec in records:
            self.assertNotEqual(file, rec.file)
    ...
    def test_relative_access(self):
        ...
        with self.assertRaises(PermissionError), self.env.cr.savepoint():
            ...
            backend2.file.ls(f"../{self.case_with_subdirectory}")
        self.backend.file.rm_file(self.filename)
        backend2.file.rm_file(self.filename)
    ...
    def test_interface_values(self):
        ...
        with Form(self.env["file.storage"]) as new_storage:
    ...
    def test_options_env(self):
        ...
        options = self.backend._get_file_options()
        ...
        self.assertIn(
            (
                ...
                f"file_storage {self.backend.display_name}."
            ),
            log.output[0],
        )
