...
class FileStorage(models.Model):
    ...
    def _get_url_for_attachment(
        ...
        if not file_storage:
            return None
        base_url = nwpl_odoo_master.base_url_for_files
        if not base_url:
            return None
        if exclude_base_url:
            base_url = (
                base_url.replace(nwpl_odoo_master.base_url.rstrip("/"), "") or "/"
            )
        # always remove the directory_path from the filename
        # only if it's at the start of the filename
        filename = attachment.filename
        if nwpl_odoo_master.directory_path and filename.startswith(
            nwpl_odoo_master.directory_path
        ):
            filename = filename.replace(nwpl_odoo_master.directory_path, "")
