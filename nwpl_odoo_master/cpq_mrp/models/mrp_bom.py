...
class MrpBom(models.Model):
    ...
    cpq_config_hash = fields.Char(index=True)
    ...
    def _get_component_template_product(
        ...
        if bom_line.component_template_id:
            ...
            ptav_ids = self.env.context.get("cpq_ptav_ids")
            if not ptav_ids:
                _logger.warning("No PTAVs in context for BOM explosion on %s", bom_product_id)
            ptavs = self.env['product.template.attribute.value'].browse(ptav_ids) if ptav_ids else bom_product_id.product_template_attribute_value_ids
...
class CPQMrpBomBuilder(models.AbstractModel):
    _name = 'cpq.mrp.bom.builder'
    _description = 'Build/ensure a normal BoM from a CPQ config'

    def ensure_bom_for_sol(self, product, sol):
        """Return an mrp.bom for this SOL/config; create if missing."""
        Bom = self.env['mrp.bom']
        # 1) If a real normal BoM already exists on the product/template, use it.
        existing = Bom.search([
            ('type', '=', 'normal'),
        ...
        # 2) Reuse a per-config BoM if one was created before.
        if sol.cpq_config_hash:
            reused = Bom.search([('cpq_config_hash', '=', sol.cpq_config_hash),
                                 ('product_tmpl_id', '=', product.product_tmpl_id.id)], limit=1)
            if reused:
                return reused

        # 3) Build a new BoM from the CPQ config
        utils = self.env['cpq.mrp.component.utils']
        comp_lines = utils.get_components_from_cpq_config(sol.cpq_configuration_json)  # returns (0,0,{ product_id, uom, qty, name })
        bom_line_vals = []
        for _, _, comp in comp_lines:
            bom_line_vals.append((0, 0, {
                'product_id': comp['product_id'],
                'product_uom_id': comp['product_uom_id'],
        ...
        bom = Bom.create({
            ...
            'product_tmpl_id': product.product_tmpl_id.id,
            'product_qty': 1.0,
            'code': f"CPQ-{sol.order_id.id}-{sol.id}",
            'cpq_config_hash': sol.cpq_config_hash or False,
            'sale_line_id': sol.id,
            'bom_line_ids': bom_line_vals,
            # Optional: add operation_ids here if you want work orders
            # 'operation_ids': [(0,0,{ 'name': 'Cutting', 'workcenter_id': wc.id, 'time_cycle_manual': 10 })]
        })
        return bom
