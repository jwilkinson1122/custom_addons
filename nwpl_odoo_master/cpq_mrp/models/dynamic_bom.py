...
class DynamicBomOperation(models.Model):
    _name = "cpq.dynamic.bom.operation"
    _description = "Dynamic BoM Operation"
    _order = "sequence, id"

    bom_id = fields.Many2one("cpq.dynamic.bom", required=True, ondelete="cascade")
    ...
    duration_expected = fields.Float(
        string="Duration (min)",
        help="Planned time in minutes for this operation."
    )
...
class DynamicBom(models.Model):
    ...
    operation_ids = fields.One2many(
        "cpq.dynamic.bom.operation", "bom_id", string="Operations"
    )
    ...
    def _default_mrp_picking_type(self):
    ...
    picking_type_id = fields.Many2one(
        ...
        domain="[('code', '=', 'mrp_operation'), ('company_id', '=', company_id)]",
        check_company=True,
        default=_default_mrp_picking_type,   
        help="When a procurement has a ‘produce’ route with a operation type"
        " set, it will try to create a Manufacturing Order for that product"
        " using a BoM of the same operation type. That allows to define stock"
        " rules which trigger different manufacturing orders with different BoMs.",
    ...
    @api.constrains("type", "picking_type_id", "company_id")
    def _ensure_manufacture_has_picking_type_id(self):
        for rec in self:
            if rec.type != "normal":
                continue
            has_mrp_type = bool(self.env['stock.picking.type'].search(
                [('code', '=', 'mrp_operation'), ('company_id', '=', rec.company_id.id)],
                limit=1
            ))
            if has_mrp_type and not rec.picking_type_id and rec.user_has_groups("stock.group_adv_location"):
                raise ValidationError(_("Manufactured dynamic BoMs must have an operation type set"))
...
class DynamicBomLine(models.Model):
    ...
    def _onchange_type_company(self):
        for rec in self:
            if rec.type == "normal" and not rec.picking_type_id:
                rec.picking_type_id = rec._default_mrp_picking_type()
