...
def post_init_hook(env):
    ...
    # ─────────────────────────────────────
    # try:
    #     _logger.info("[post_init] Running CPQ setup...")
    ...
    #     _logger.info("[post_init] CPQ setup complete.")
    # except Exception as e:
    #     _logger.error("[post_init] CPQ setup failed: %s", e)

    # ─────────────────────────────────────
    # 7. Shopify Sync Tracking
    # ─────────────────────────────────────
    try:
...
def uninstall_hook(env):
    ...
    try:
        keys_to_delete = [
            "nwpl_odoo_master.amazon_access_key",
            "nwpl_odoo_master.amazon_secret_key",
            "nwpl_odoo_master.amazon_bucket_name",
            "nwpl_odoo_master.amazon_connector",
        ]
...
# >>> from nwpl_odoo_master import hooks_dev_safe
...
def auto_fix_manifest_references(addons_path="custom_addons", dry_run=True):
    ...
    for module_path in base_path.iterdir():
        ...
        required_files = {
            "nwpl_odoo_master/cpq/security/ir.model.access.csv": module_path / "nwpl_odoo_master/cpq/security/ir.model.access.csv",
            "nwpl_odoo_master/cpq_sale/security/ir.model.access.csv": module_path / "nwpl_odoo_master/cpq_sale/security/ir.model.access.csv",
            "nwpl_odoo_master/pod_contacts/security/ir.model.access.csv": module_path / "nwpl_odoo_master/pod_contacts/security/ir.model.access.csv",
        }
