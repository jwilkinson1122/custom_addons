...
class SaleOrderLine(models.Model):
    ...
    bom_id = fields.Many2one(
        "mrp.bom", string="BOM", domain="[('id', 'in', allowed_bom_ids)]"
    )
    flexible_bom = fields.Boolean(
        string="Flexible bom", related="product_id.product_tmpl_id.flexible_bom"
    )
    ...
    def _compute_allowed_bom_ids(self):
        for record_id in self:
            ...
            if record_id.product_id:
                allowed_bom_ids = allowed_bom_ids.filtered(
                    lambda m: m.product_tmpl_id == record_id.product_id.product_tmpl_id
                )
    ...
    def onchange_product_id(self):
        for record_id in self:
            if record_id.product_id:
                record_id.allowed_bom_ids = (
                    self.env["mrp.bom"]
                    .search([])
                    .filtered(
                        lambda m: m.product_tmpl_id
                        == record_id.product_id.product_tmpl_id
                    )
                )
    ...
    #         total_cost = product_id._compute_bom_price(self.bom_id)
    #         if total_cost:
    #             self.price_unit = total_cost
    #         else:
    #             self.price_unit = self.product_id.standard_price
    #         if product_tmpl_id.margin:
    #             self.price_unit = self.price_unit / (1 - product_tmpl_id.margin)
    #     else:
    #         self.price_unit = self.product_id.list_price
    ...
    def action_set_bom(self):
        ...
        bom_ids = (
            self.env["mrp.bom"]
            .search([("product_tmpl_id", "=", self.product_id.product_tmpl_id.id)])
            .sorted("sequence")
        )
        bom_id = None
        if not bom_ids:
