...
class SaleOrder(TC.ProductConfiguratorTestCases):
    def setUp(self):
        super().setUp()
        self.SalePrescriptionOrderId = self.env["prescription.order"]
        ...
        self.ProductConfWizard = self.env["product.configurator.prescription.order"]
    ...
    def test_00_reconfigure_product(self):
        ...
        prescription_order_id = self.SalePrescriptionOrderId.create(
            {
        ...
        self.ProductConfWizard = self.env[
            "product.configurator.prescription.order"
        ].with_context(**context)
        prescription_order_id.action_config_start()
        self._configure_product_nxt_step()
        prescription_order_id.line_ids.reconfigure_product()
        product_tmpl = prescription_order_id.line_ids.product_id.product_tmpl_id
        self.assertEqual(
            product_tmpl.id,
