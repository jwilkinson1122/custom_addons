...
class TestSaleOrder(common.TransactionCase):
    def setUp(self):
        ...
        self.prescription_order_line_obj = self.env["prescription.order.line"]
    ...
    def create_prescription_order_01(self):
    ...
    def create_prescription_order_02(self):
        prescription_order = self.prescription_order_obj.create(
            {
                "partner_id": self.partner.id,
                "validity_date": fields.Date.to_string(self.validity),
    ...
    def test_01_create_sale_order(self):
        prescription_order = self.create_prescription_order_01()
        prescription_order.sudo().action_confirm()
        rx_lines = self.prescription_order_line_obj.search(
        ...
        self.assertEqual(so_line._get_eligible_rx_lines(), rx_lines)
        rx_line_assigned = self.prescription_order_line_obj.search(
            [("date_schedule", "=", fields.Date.to_string(self.date_schedule_1))]
        )
        self.assertEqual(so_line.prescription_order_line, rx_line_assigned)

    def test_02_create_sale_order(self):
        prescription_order = self.create_prescription_order_02()
        prescription_order.sudo().action_confirm()
        rx_lines = self.prescription_order_line_obj.search(
            [("order_id", "=", prescription_order.id)]
        )
        ...
        so_line.onchange_product_id()
        self.assertEqual(
            so_line._get_eligible_rx_lines(),
            rx_lines.filtered(lambda rx_line: rx_line.product_id == self.product),
        )
        rx_line_assigned = self.prescription_order_line_obj.search(
            [
                ("order_id", "=", prescription_order.id),
