odoo.define('product_multi_variant.ProductsPopup', function(require) {
    'use strict';
    ...
    const { configureGui } = require('podiatry_manager.Gui');
    const {
        useListener
    } = require('web.custom_hooks');
    const ProductItem = require('podiatry_manager.ProductItem');
    ...
    class ProductsPopup extends ProductItem {
        ...
        {   var price = 0.00
            ...
            price += parseFloat(add)
                if (order.selected_orderline.product.product_variants){
                    variant.push({
                        'extra_price': add,
                        'type': $(this).data('type'),
                    })
