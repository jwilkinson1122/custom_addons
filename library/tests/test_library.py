...
class TestLibrary(common.TransactionCase):
    def setUp(self):
        ...
        self.student_id = self.env.ref("podiatry.demo_student_student_5")
        self.podiatry_standard = self.env.ref("podiatry.demo_podiatry_standard_2")
        self.standard = self.env.ref("podiatry.demo_standard_standard_2")
        ...
        self.company_id = self.env.ref("podiatry.demo_podiatry_1")
        ...
        self.library_rack = self.library_rack_obj.create(
            {"name": "Rack34", "code": "rack34", "active": True}
        )
        # Create product language
        self.product_lang = self.product_lang_obj.create(
            {"code": "LG4", "name": "Hindi"}
        )
        # Create library author
        self.library_author = self.library_author_obj.create({"name": "NCERT"})
        # Create product
        categ = self.env["product.category"].search(
            [("book_categ", "=", True)], limit=1
        )
        self.product_product = self.product_product_obj.create(
            {
                "name": "Java",
                "categ_id": categ.id,
                "type": "product",
        ...
        # Create editior
        self.book_editor = self.book_editor_obj.create(
            {"name": "S.S Prasad", "book_id": self.product_product.id}
        )
        # Create library card
        self.library_card = self.library_card_obj.create(
            {
                "code": "C0056",
                "user": "student",
                "book_limit": 10,
                ...
                "roll_no": 2,
                "standard_id": self.podiatry_standard.id,
            }
        )
        ...
        self.purchase_order = self.purchase_order_obj.create(
            {
                ...
                "date_order": "2020-06-29 16:58:10",
                "date_planned": "2020-06-29 16:58:10",
            }
        )
        # Create purchase order line
        self.purchase_order_line = self.purchase_order_line_obj.create(
            {
        ...
        self.imm = self.immideate_transfer.create(
            {"pick_ids": [(4, self.stock_picking.id)]}
        )
        for rec in self.imm:
            rec.process()
        # Book request created
        self.library_book_request = self.library_book_request_obj.create(
            {
                "req_id": "New",
                "type": "existing",
        ...
        # Book issue created
        self.library_book_issue = self.library_book_issue_obj.create(
            {
                "issue_code": "L097",
                "name": self.product_product.id,
                "card_id": self.library_card.id,
                ...
                "roll_no": 2,
                "actual_return_date": "2017-06-29 16:58:10",
            }
        )
