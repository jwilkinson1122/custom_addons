...
class ProductSetAdd(models.TransientModel):
    ...
    prescription_order_id = fields.Many2one(
        ...
        if self.env.context.get("active_model") == "prescription.order"
        else None,
        ondelete="cascade",
    )
    ...
    prescription_partner_id = fields.Many2one(related="prescription_order_id.partner_id", ondelete="cascade")
    ...
    def _check_partner(self):
        ...
        if not self.product_set_id.partner_id or not self.product_set_id.prescription_partner_id or self.env.context.get(
            "product_set_add_skip_validation"
        ):
            return

        allowed_partners = self._allowed_order_partners()
        if self.order_id.partner_id or self.prescription_order_id.prescription_partner_id not in allowed_partners:
            raise exceptions.ValidationError(
                _(
                    "You can use an order assigned "
                    "only to following partner(s): {}"
                ).format(", ".join(allowed_partners.mapped("name")))
            )

    def _allowed_order_partners(self):
        """Product sets' partners allowed for current order."""
    ...
    def add_set(self):
        """Add product set, multiplied by quantity in order line"""
        self._check_partner()
        order_lines = self._prepare_order_lines()
        if order_lines:
            self.order_id.write({"order_line": order_lines})
        else: 
            self.prescription_order_id.write({"order_line": order_lines})
        return order_lines
    ...
    def _prepare_order_lines(self):
        ...
        for seq, set_line in enumerate(self._get_lines(), start=1):
            if order_lines:
                order_values = self.prepare_sale_order_line_data(set_line)
                order_values.update({"sequence": max_sequence + seq})
                order_lines.append((0, 0, order_values))
            else:
                prescription_values = self.prepare_prescription_order_line_data(set_line)
                prescription_values.update({"sequence": max_sequence + seq})
                order_lines.append((0, 0, prescription_values))
        return order_lines
    ...
    def _get_max_sequence(self):
        ...
        if self.prescription_order_id.order_line:
            max_sequence = max(line.sequence for line in self.prescription_order_id.order_line)
        return max_sequence

    def _get_lines(self):
        # hook here to take control on used lines
        rx_product_ids = self.prescription_order_id.order_line.mapped("product_id").ids
        so_product_ids = self.order_id.order_line.mapped("product_id").ids
        for set_line in self.product_set_line_ids:
            if self.skip_existing_products and set_line.product_id.id in so_product_ids and rx_product_ids:
                continue
            yield set_line

    def prepare_sale_order_line_data(self, set_line, max_sequence=0):
        self.ensure_one()
        line_values = set_line.prepare_order_line_values(
            self.order_id, self.quantity, max_sequence=max_sequence
        )
        return line_values
    
    def prepare_prescription_order_line_data(self, set_line, max_sequence=0):
        self.ensure_one()
        line_values = set_line.prepare_prescription_order_line_values(
            self.prescription_order_id, self.quantity, max_sequence=max_sequence
        )
        return line_values
