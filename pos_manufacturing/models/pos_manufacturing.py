...
class ManufacturingFloor(models.Model):

    _name = 'manufacturing.floor'
    _description = 'Manufacturing Floor'
    _order = "sequence, name"

    name = fields.Char('Floor Name', required=True)
    pos_config_ids = fields.Many2many('pos.config', string='Point of Sales', domain="[('module_pos_manufacturing', '=', True)]")
    ...
    table_ids = fields.One2many('manufacturing.table', 'floor_id', string='Tables')
    sequence = fields.Integer('Sequence', default=1)
    active = fields.Boolean(default=True)

    @api.ondelete(at_uninstall=False)
    def _unlink_except_active_pos_session(self):
        confs = self.mapped('pos_config_ids').filtered(lambda c: c.module_pos_manufacturing)
    ...
    def write(self, vals):
        ...
        return super(ManufacturingFloor, self).write(vals)
...
class ManufacturingTable(models.Model):

    _name = 'manufacturing.table'
    _description = 'Manufacturing Table'

    name = fields.Char('Table Name', required=True, help='An internal identification of a table')
    floor_id = fields.Many2one('manufacturing.floor', string='Floor')
    ...
    @api.ondelete(at_uninstall=False)
    def _unlink_except_active_pos_session(self):
        confs = self.mapped('floor_id.pos_config_ids').filtered(lambda c: c.module_pos_manufacturing)
        opened_session = self.env['pos.session'].search([('config_id', 'in', confs.ids), ('state', '!=', 'closed')])
        if opened_session:
