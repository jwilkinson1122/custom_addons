...
import { EditableTable } from "@pos_manufacturing/app/floor_screen/editable_table";
import { EditBar } from "@pos_manufacturing/app/floor_screen/edit_bar";
import { Table } from "@pos_manufacturing/app/floor_screen/table";
...
export class FloorScreen extends Component {
    ...
    static template = "pos_manufacturing.FloorScreen";
    ...
    async _save(table) {
        ...
        if (table.id) {
            await this.orm.write("manufacturing.table", [table.id], tableCopy);
        } else {
            const tableId = await this.orm.create("manufacturing.table", [tableCopy]);
    ...
    async deleteFloorOrTable() {
        ...
        const response = await this.orm.call("manufacturing.table", "are_orders_still_in_draft", [
        ...
        if (!response) {
            for (const id of originalSelectedTableIds) {
                ...
                this.orm.write("manufacturing.table", [id], { active: false });
