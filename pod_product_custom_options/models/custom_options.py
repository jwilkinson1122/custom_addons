...
class ProductCustomOptions(models.Model):
    ...
    input_type = fields.Selection(INPUT_TYPES, required=True)
    is_required = fields.Boolean(default=False)
    prod_tmpl_id = fields.Many2one('product.template', required=True, ondelete='cascade')
    active = fields.Boolean(default=True)
    sequence = fields.Integer(default=10)
    price = fields.Float(digits='Product Price')
    custom_options_value_ids = fields.One2many('product.custom.options.value', 'custom_option_id')
    allowed_file_extension = fields.Char()
    image_size_length = fields.Integer()
    image_size_width = fields.Integer()

    _sql_constraints = [
        ('name_tmpl_uniq', 'unique(name, prod_tmpl_id, input_type)', 'Option names must be unique per product & option type!'),
    ]
...
class ProductCustomOptionsValue(models.Model):
    ...
    name = fields.Char(required=True, translate=True)
    price = fields.Float(digits='Product Price')
    custom_option_id = fields.Many2one('product.custom.options', required=True, ondelete='cascade')
    is_default = fields.Boolean()
    sequence = fields.Integer(default=10)

    _sql_constraints = [
        ('name_option_uniq', 'unique(name, custom_option_id)', 'Option value names must be unique per option!'),
    ]
...
class PrescriptionCustomOptions(models.Model):
    ...
    custom_option_id = fields.Many2one('product.custom.options', required=True, ondelete='restrict')
    order_line_id = fields.Many2one('prescription.order.line', required=True, ondelete='cascade')
    price = fields.Float(digits='Product Price')
