...
class PaymentLinkWizard(models.TransientModel):
    ...
    def _compute_confirmation_message(self):
        ...
        for wizard in self.filtered(lambda w: w.res_model == 'prescription.order'):
            prescription_order = wizard.env['prescription.order'].sudo().browse(wizard.res_id)
            if prescription_order.state in ('draft', 'sent') and prescription_order.require_payment:
                remaining_amount = prescription_order._get_prepayment_required_amount() - prescription_order.amount_paid
    ...
    def _get_additional_link_values(self):
        """ Override of `payment` to add `prescription_order_id` to the payment link values.
        ...
        if self.res_model != 'prescription.order':
            return res

        # Order-related fields are retrieved in the controller
        return {
            'prescription_order_id': self.res_id,
        }
