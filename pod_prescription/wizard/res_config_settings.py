...
class ResConfigSettings(models.TransientModel):
    ...
    group_auto_done_setting = fields.Boolean(
        string="Lock Confirmed Prescription", implied_group='pod_prescription.group_auto_done_setting')
    # group_proforma_prescription = fields.Boolean(
    #     string="Pro-Forma Invoice", implied_group='pod_prescription.group_proforma_prescription',
    #     help="Allows you to send pro-forma invoice.")
    group_warning_prescription = fields.Boolean(
        string="Prescription Order Warnings", implied_group='pod_prescription.group_warning_prescription')
    ...
    automatic_invoice = fields.Boolean(
        ...
        config_parameter='pod_prescription.automatic_invoice',
    )
    deposit_default_product_id = fields.Many2one(
        related='company_id.prescription_down_payment_product_id',
        readonly=False,
        # previously config_parameter='pod_prescription.default_deposit_product_id',
    )
    ...
    invoice_mail_template_id = fields.Many2one(
        ...
        config_parameter='pod_prescription.default_invoice_email_template',
    ...
    def set_values(self):
        ...
        if self.default_invoice_policy != 'order':
            self.env['ir.config_parameter'].set_param('pod_prescription.automatic_invoice', False)
