...
class TestPrescriptionOrderDownPayment(TestPrescriptionCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        PrescriptionOrder = cls.env['prescription.order'].with_context(tracking_disable=True)
        ...
        cls.prescription_order = PrescriptionOrder.create({
    ...
    def test_tax_breakdown(self):
        self.prescription_order.order_line[0].tax_id = self.tax_15 + self.tax_10
        self.prescription_order.order_line[1].tax_id = self.tax_10
        self.prescription_order.order_line[2].tax_id = self.tax_10
        self.make_downpayment()
        invoice = self.prescription_order.invoice_ids
        down_pay_amt = self.prescription_order.amount_total / 2
    ...
    def test_tax_breakdown_other_currency(self):
        self.prescription_order.currency_id = self.currency_data['currency']  # rate = 2.0
        self.prescription_order.order_line[0].tax_id = self.tax_15 + self.tax_10
        self.prescription_order.order_line[1].tax_id = self.tax_10
        self.prescription_order.order_line[2].tax_id = self.tax_10
        self.make_downpayment()
    ...
    def test_tax_breakdown_fixed_payment_method_with_taxes_on_all_lines(self):
        self.prescription_order.order_line[0].tax_id = self.tax_15
        self.prescription_order.order_line[1].tax_id = self.tax_10
        self.prescription_order.order_line[2].tax_id = self.tax_10
        self.prescription_order.order_line[3].tax_id = self.tax_10
        self.make_downpayment(advance_payment_method='fixed', fixed_amount=222.5, amount=0)
        invoice = self.prescription_order.invoice_ids
        down_pay_amt = 222.5
        # pylint: disable=C0326
    ...
    def test_tax_price_include_breakdown(self):
        ...
        self.prescription_order.order_line[0].tax_id = tax_10_incl + self.tax_10
        self.prescription_order.order_line[1].tax_id = self.tax_10
        self.prescription_order.order_line[2].tax_id = self.tax_10
        self.make_downpayment()
        invoice = self.prescription_order.invoice_ids
    ...
    def test_tax_price_include_include_base_amount_breakdown(self):
        ...
        self.prescription_order.order_line[0].tax_id = tax_10_pi_ba + self.tax_10
        self.prescription_order.order_line[1].tax_id = self.tax_10
        self.prescription_order.order_line[2].tax_id = self.tax_10
        self.make_downpayment()
        invoice = self.prescription_order.invoice_ids
    ...
    def test_tax_breakdown_with_discount(self):
        self.prescription_order.order_line[0].tax_id = self.tax_10
        self.prescription_order.order_line[1].tax_id = self.tax_10
        self.prescription_order.order_line[1].discount = 25.0
        self.prescription_order.order_line[2].tax_id = self.tax_15
        self.make_downpayment()
        invoice = self.prescription_order.invoice_ids
        down_pay_amt = self.prescription_order.amount_total / 2
        # pylint: disable=C0326
    ...
    def test_tax_price_include_include_base_amount_breakdown_with_discount(self):
        ...
        self.tax_10.sequence = 2
        self.prescription_order.order_line[0].tax_id = tax_10_pi_ba + self.tax_10
        self.prescription_order.order_line[0].discount = 25.0
        self.prescription_order.order_line[1].tax_id = self.tax_10
        self.prescription_order.order_line[2].tax_id = self.tax_10
        self.make_downpayment()
        invoice = self.prescription_order.invoice_ids
    ...
    def test_tax_fixed_amount_breakdown(self):
        ...
        self.prescription_order.order_line[0].tax_id = tax_group_1
        self.prescription_order.order_line[1].tax_id = tax_group_2
        self.prescription_order.order_line[2].tax_id = tax_10_a
        self.make_downpayment()
        invoice = self.prescription_order.invoice_ids
        down_pay_amt = self.prescription_order.amount_total / 2
        # pylint: disable=C0326
    ...
    def test_analytic_distribution(self):
        ...
        self.prescription_order.order_line[0].tax_id = self.tax_15 + self.tax_10
        self.prescription_order.order_line[0].analytic_distribution = {an_acc_01: 100}
        self.prescription_order.order_line[1].tax_id = self.tax_10
        self.prescription_order.order_line[1].analytic_distribution = {an_acc_01: 50, an_acc_02: 50}
        self.prescription_order.order_line[2].tax_id = self.tax_10
        self.prescription_order.order_line[2].analytic_distribution = {an_acc_01: 100}
        self.make_downpayment()
        invoice = self.prescription_order.invoice_ids
        down_pay_amt = self.prescription_order.amount_total / 2
        # pylint: disable=C0326
    ...
    def test_tax_fixed_amount_analytic_distribution(self):
        ...
        self.prescription_order.order_line[0].tax_id = tax_group_1
        self.prescription_order.order_line[0].analytic_distribution = {an_acc_01: 50, an_acc_02: 50}
        self.prescription_order.order_line[1].tax_id = tax_group_2
        self.prescription_order.order_line[2].tax_id = tax_10_a
        self.make_downpayment()
        invoice = self.prescription_order.invoice_ids
    ...
    def test_tax_price_include_amount_rounding(self):
        ...
        self.prescription_order.order_line[0].product_uom_qty = 1
        self.prescription_order.order_line[0].tax_id = tax_21

        self.prescription_order.order_line[1].price_unit = 90
        self.prescription_order.order_line[1].product_uom_qty = 2
        self.prescription_order.order_line[1].tax_id = tax_21

        self.prescription_order.order_line[2].price_unit = 49
        self.prescription_order.order_line[2].product_uom_qty = 4
        self.prescription_order.order_line[2].tax_id = tax_21
    ...
    def test_tax_price_include_negative_amount_rounding_final_invoice(self):
        ...
        self.prescription_order.order_line[0].product_id = self.company_data['product_delivery_no'].id,
        self.prescription_order.order_line[0].product_uom_qty = 1
        self.prescription_order.order_line[0].qty_delivered = 0
        self.prescription_order.order_line[0].tax_id = tax_21_a
        self.prescription_order.order_line[0].price_unit = 1000

        self.prescription_order.order_line[1].product_id = self.company_data['product_delivery_no'].id,
        self.prescription_order.order_line[1].product_uom_qty = 1
        self.prescription_order.order_line[1].qty_delivered = 0
        self.prescription_order.order_line[1].tax_id = tax_21_b
        self.prescription_order.order_line[1].price_unit = 1000

        self.prescription_order.order_line[2:].unlink()
    ...
    def test_tax_price_include_positive_amount_rounding_final_invoice(self):
        ...
        self.prescription_order.order_line[0].product_uom_qty = 1
        self.prescription_order.order_line[0].qty_delivered = 0
        self.prescription_order.order_line[0].tax_id = tax_24_a
        self.prescription_order.order_line[0].price_unit = 1000

        self.prescription_order.order_line[1].product_id = self.company_data['product_delivery_no'].id,
        self.prescription_order.order_line[1].product_uom_qty = 1
        self.prescription_order.order_line[1].qty_delivered = 0
        self.prescription_order.order_line[1].tax_id = tax_24_b
        self.prescription_order.order_line[1].price_unit = 1000
    ...
    def test_tax_price_include_small_amount_rounding_final_invoice(self):
        ...
        self.prescription_order.order_line[2].product_id = self.company_data['product_delivery_no'].id,
        self.prescription_order.order_line[2].product_uom_qty = 1
        self.prescription_order.order_line[2].qty_delivered = 1
        self.prescription_order.order_line[2].tax_id = tax_25_a
        self.prescription_order.order_line[2].price_unit = 968

        self.prescription_order.order_line[3].product_id = self.company_data['product_delivery_no'].id,
        self.prescription_order.order_line[3].product_uom_qty = 1
        self.prescription_order.order_line[3].qty_delivered = 1
        self.prescription_order.order_line[3].tax_id = tax_25_b
        self.prescription_order.order_line[3].price_unit = 968

        self.prescription_order.order_line[3].copy({
            'order_id':self.prescription_order.id,
            'tax_id': tax_25_c,
            'qty_delivered': 1,
        })

        self.prescription_order.order_line.qty_delivered_method = 'manual'
