...
class TestPrescriptionOrderDownPayment(TestPrescriptionCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.sol_product_order = cls.env['prescriptions.order.line'].create({
        ...
        cls.sol_serv_deliver = cls.env['prescriptions.order.line'].create({
        ...
        cls.sol_serv_order = cls.env['prescriptions.order.line'].create({
        ...
        cls.sol_product_deliver = cls.env['prescriptions.order.line'].create({
    ...
    def make_downpayment(cls, **kwargs):
        ...
        downpayment = cls.env['prescriptions.advance.payment.inv'].with_context(so_context).create(payment_params)
    ...
    def test_tax_price_include_amount_rounding(self):
        ...
        downpayment = self.env['prescriptions.advance.payment.inv'].with_context(so_context).create(payment_params)
        downpayment.create_invoices()
        invoice = self.prescriptions_order.invoice_ids
        # pylint: disable=C0326
        expected = [
    ...
    def test_tax_price_include_negative_amount_rounding_final_invoice(self):
        ...
        downpayment = self.env['prescriptions.advance.payment.inv'].with_context({**so_context, 'raise_if_nothing_to_invoice': False}).create(payment_params)
        action = downpayment.create_invoices()
        invoice = self.env['account.move'].browse(action['res_id'])
        # pylint: disable=C0326
        expected = [
