...
from odoo.addons.pod_prescription.controllers.portal import PaymentPortal
...
class TestPrescriptionPayment(AccountPaymentCommon, PrescriptionCommon, PaymentHttpCommon):
    ...
    def test_11_rx_payment_link(self):
        ...
        route_values['prescription_order_id'] = self.prescription_order.id
        ...
        with patch(
            ...
            patched.assert_called_once_with(ANY, prescription_order_id=ANY)

        self.assertEqual(tx_context['currency_id'], self.prescription_order.currency_id.id)
        self.assertEqual(tx_context['partner_id'], self.prescription_order.partner_invoice_id.id)
        self.assertEqual(tx_context['amount'], self.prescription_order.amount_total)
        ...
        self.assertEqual(tx_sudo.prescription_order_ids, self.prescription_order)
        self.assertEqual(tx_sudo.amount, self.amount)
        self.assertEqual(tx_sudo.partner_id, self.prescription_order.partner_invoice_id)
        self.assertEqual(tx_sudo.company_id, self.prescription_order.company_id)
    ...
    def test_12_rx_partial_payment_link(self):
        ...
        pay_route_values['prescription_order_id'] = self.prescription_order.id
        ...
        tx_sudo._set_done()
        with mute_logger('odoo.addons.pod_prescription.models.payment_transaction'):
            tx_sudo._finalize_post_processing()
        self.assertEqual(self.prescription_order.state, 'draft') # Only a partial amount was paid
        ...
        self.assertEqual(tx2_sudo.prescription_order_ids, self.prescription_order)
        self.assertEqual(tx2_sudo.amount, self.amount)
        self.assertEqual(tx2_sudo.partner_id, self.prescription_order.partner_invoice_id)
        self.assertEqual(tx2_sudo.company_id, self.prescription_order.company_id)
        self.assertEqual(tx2_sudo.currency_id, self.prescription_order.currency_id)

        self.assertEqual(self.prescription_order.state, 'draft')
        self.assertEqual(self.prescription_order.transaction_ids, tx_sudo + tx2_sudo)
    ...
    def test_confirmed_transactions_comfirms_rx_with_multiple_transaction(self):
        ...
        self._create_transaction(
            flow='redirect',
            prescription_order_ids=[self.prescription_order.id],
            state='draft',
        ...
        tx = self._create_transaction(flow='redirect', prescription_order_ids=[self.prescription_order.id], state='done')
    ...
    def test_rx_partial_payment_no_invoice(self):
        ...
        # Create the payment
        self.amount = self.prescription_order.amount_total / 10.
        tx = self._create_transaction(flow='redirect', prescription_order_ids=[self.prescription_order.id], state='done')
        with mute_logger('odoo.addons.pod_prescription.models.payment_transaction'):
            tx._reconcile_after_done()
    ...
    def test_invoice_is_final(self):
        ...
        with mute_logger('odoo.addons.pod_prescription.models.payment_transaction'), patch(
            'odoo.addons.pod_prescription.models.prescription_order.PrescriptionOrder._create_invoices',
    ...
    def test_downpayment_confirm_prescription_order_sufficient_amount(self):
        """Paying down payments can confirm an order if amount is enough."""
        self.prescription_order.require_payment = True
        self.prescription_order.prepayment_percent = 0.1
        order_amount = self.prescription_order.amount_total

        tx = self._create_transaction(
            flow='direct',
            amount=order_amount * self.prescription_order.prepayment_percent,
            prescription_order_ids=[self.prescription_order.id],
            state='done',
        )
        with mute_logger('odoo.addons.pod_prescription.models.payment_transaction'):
    ...
    def test_downpayment_confirm_prescription_order_insufficient_amount(self):
        """Confirmation cannot occur if amount is not enough."""

        self.prescription_order.require_payment = True
        self.prescription_order.prepayment_percent = 0.2
        order_amount = self.prescription_order.amount_total
        ...
        self.assertTrue(self.prescription_order.state == 'draft')

    def test_downpayment_confirm_prescription_order_several_payments(self):
        """
    ...
    def test_downpayment_automatic_invoice(self):
        ...
        tx = self._create_transaction(
            flow='direct',
            amount=self.prescription_order.amount_total * self.prescription_order.prepayment_percent,
            prescription_order_ids=[self.prescription_order.id],
            state='done')
    ...
    def test_check_portal_access_token_before_rerouting_flow(self):
        ...
        with patch.object(CustomerPortal, '_document_check_access') as mock:
            ...
            self.assertEqual(
                mock.call_count, 0, msg="No check should be made when prescription_order_id is not provided."
            )

            mock.reset_mock()

            payment_portal_controller._get_extra_payment_form_values(
                prescription_order_id=self.prescription_order.id, access_token='whatever'
            )
            self.assertEqual(
                mock.call_count, 1, msg="The check should be made as prescription_order_id is provided."
            )
    ...
    def test_transaction_route_rejects_unexpected_kwarg(self):
        url = self._build_url(f'/my/orders/{self.prescription_order.id}/transaction')
        route_kwargs = {
            'access_token': self.prescription_order._portal_ensure_token(),
