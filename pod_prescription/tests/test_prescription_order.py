...
class TestPrescriptionOrder(PrescriptionCommon):
    ...
    def test_invoice_state_when_ordered_quantity_is_negative(self):
        ...
        self.assertTrue(prescriptions_order.invoice_status == 'invoiced', 'Prescription: The invoicing status of the SO should be "invoiced"')
    ...
    def test_prescriptions_sequence(self):
        ...
        self.assertTrue(prescriptions_order.name.startswith('SO/2019/'))
        prescriptions_order = self.prescriptions_order.copy({'date_order': '2020-01-01'})
        self.assertTrue(prescriptions_order.name.startswith('SO/2020/'))
    ...
    def test_unlink_cancel(self):
        ...
        so_copy = self.prescriptions_order.copy()
        with self.assertRaises(AccessError):
            so_copy.with_user(self.prescriptions_user).unlink()
        ...
        self.assertTrue(so_copy.state == 'prescriptions', 'Prescription: SO should be in state "prescriptions"')
        ...
        # SO in state 'prescriptions' cannot be deleted
        self.prescriptions_order.action_confirm()
        self.assertTrue(self.prescriptions_order.state == 'prescriptions', 'Prescription: SO should be in state "prescriptions"')
    ...
    def test_compute_packaging_00(self):
        ...
        so2 = self.env['prescriptions.order'].create({
            'partner_id': self.partner.id,
        })
        so2_form = Form(so2)
    ...
    def test_compute_packaging_01(self):
        ...
        so2 = self.env['prescriptions.order'].with_company(company2).create({
            'partner_id': self.partner.id,
        })
        so2_form = Form(so2)
        with so2_form.order_line.new() as line:
    ...
    def test_sol_name_search(self):
        ...
        self.assertEqual(list(sol_ids_found), self.prescriptions_order.order_line.ids)
...
class TestPrescriptionsTeam(PrescriptionCommon):
    ...
    def test_prescriptions_order_analytic_distribution_change(self):
        ...
        so_no_analytic_account = self.env['prescriptions.order'].create({
            'partner_id': self.env.ref('base.res_partner_1').id,
        })
        sol_no_analytic_account = self.env['prescriptions.order.line'].create({
            'name': super_product.name,
            'product_id': super_product.id,
        ...
        prescriptions_order.action_confirm()
        sol_on_confirmed_order = self.env['prescriptions.order.line'].create({
            'name': super_product.name,
            'product_id': super_product.id,
            'order_id': prescriptions_order.id,
        })
    ...
    def test_cannot_assign_tax_of_mismatch_company(self):
        """ Test that sol cannot have assigned tax belonging to a different company from that of the prescriptions order. """
        ...
        sol = self.env['prescriptions.order.line'].sudo().create({
            'name': product.name,
