...
class TestReInvoice(TestPrescriptionCommon):
    ...
    def test_at_cost(self):
        ...
        prescription_order_line3 = self.prescription_order.order_line.filtered(lambda sol: sol != prescription_order_line1 and sol.product_id == self.company_data['product_order_cost'])
        prescription_order_line4 = self.prescription_order.order_line.filtered(lambda sol: sol != prescription_order_line2 and sol.product_id == self.company_data['product_delivery_cost'])
        ...
        self.assertEqual(len(self.prescription_order.order_line), 4, "There should be 4 lines on the SO (2 vendor bill lines created)")
        self.assertEqual(len(self.prescription_order.order_line.filtered(lambda sol: sol.is_expense)), 2, "There should be 4 lines on the SO (2 vendor bill lines created)")
        ...
        self.assertEqual(prescription_order_line3.qty_delivered_method, 'analytic', "Delivered quantity of 'expense' SO line should be computed by analytic amount")
        self.assertEqual(prescription_order_line4.qty_delivered_method, 'analytic', "Delivered quantity of 'expense' SO line should be computed by analytic amount")
        ...
        prescription_order_line5 = self.prescription_order.order_line.filtered(lambda sol: sol != prescription_order_line1 and sol != prescription_order_line3 and sol.product_id == self.company_data['product_order_cost'])
        prescription_order_line6 = self.prescription_order.order_line.filtered(lambda sol: sol != prescription_order_line2 and sol != prescription_order_line4 and sol.product_id == self.company_data['product_delivery_cost'])
        ...
        self.assertEqual(len(self.prescription_order.order_line), 6, "There should be still 4 lines on the SO, no new created")
        self.assertEqual(len(self.prescription_order.order_line.filtered(lambda sol: sol.is_expense)), 4, "There should be still 2 expenses lines on the SO")
    ...
    def test_prescription_price(self):
        ...
        prescription_order_line3 = self.prescription_order.order_line.filtered(lambda sol: sol != prescription_order_line1 and sol.product_id == self.company_data['product_delivery_prescription_price'])
        prescription_order_line4 = self.prescription_order.order_line.filtered(lambda sol: sol != prescription_order_line2 and sol.product_id == self.company_data['product_order_prescription_price'])
        ...
        self.assertEqual(prescription_order_line3.qty_delivered_method, 'analytic', "Delivered quantity of 'expense' SO line 3 should be computed by analytic amount")
        self.assertEqual(prescription_order_line4.qty_delivered_method, 'analytic', "Delivered quantity of 'expense' SO line 4 should be computed by analytic amount")
        ...
        prescription_order_line5 = self.prescription_order.order_line.filtered(lambda sol: sol != prescription_order_line1 and sol != prescription_order_line3 and sol.product_id == self.company_data['product_delivery_prescription_price'])
        prescription_order_line6 = self.prescription_order.order_line.filtered(lambda sol: sol != prescription_order_line2 and sol != prescription_order_line4 and sol.product_id == self.company_data['product_order_prescription_price'])
        ...
        self.assertEqual(len(self.prescription_order.order_line), 5, "There should be 5 lines on the SO, 1 new created and 1 incremented")
        self.assertEqual(len(self.prescription_order.order_line.filtered(lambda sol: sol.is_expense)), 3, "There should be 3 expenses lines on the SO")
    ...
    def test_no_expense(self):
        ...
        self.assertEqual(len(self.prescription_order.order_line), 1, "No SO line should have been created (or removed) when validating vendor bill")
    ...
    def test_not_reinvoicing_invoiced_so_lines(self):
        ...
        so_line1 = self.env['prescription.order.line'].create({
            'product_id': self.company_data['product_delivery_cost'].id,
            'discount': 100.00,
            'order_id': self.prescription_order.id,
        })
        so_line2 = self.env['prescription.order.line'].create({
            'product_id': self.company_data['product_delivery_prescription_price'].id,
            'discount': 100.00,
            'order_id': self.prescription_order.id,
        })
        ...
        so_line3 = self.prescription_order.order_line.filtered(lambda sol: sol != so_line1 and sol.product_id == self.company_data['product_delivery_cost'])
        so_line4 = self.prescription_order.order_line.filtered(lambda sol: sol != so_line2 and sol.product_id == self.company_data['product_delivery_prescription_price'])
