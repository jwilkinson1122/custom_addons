...
class CrmTeam(models.Model):
    ...
    invoiced = fields.Float(
        ...
        help="Invoice revenue for the current month. This is the amount the prescriptions "
    ...
    prescriptions_to_invoice_count = fields.Integer(
        compute='_compute_prescriptions_to_invoice',
        string='Number of prescriptions to invoice', readonly=True)
    prescriptions_order_count = fields.Integer(compute='_compute_prescriptions_order_count', string='# Prescription Orders')

    def _compute_quotations_to_invoice(self):
        query = self.env['prescriptions.order']._where_calc([
            ('team_id', 'in', self.ids),
            ('state', 'in', ['draft', 'sent']),
        ])
        self.env['prescriptions.order']._apply_ir_rules(query, 'read')
    ...
    def _compute_prescriptions_to_invoice(self):
        prescriptions_order_data = self.env['prescriptions.order']._read_group([
            ('team_id', 'in', self.ids),
            ('invoice_status','=','to invoice'),
        ], ['team_id'], ['__count'])
        data_map = {team.id: count for team, count in prescriptions_order_data}
        for team in self:
            team.prescriptions_to_invoice_count = data_map.get(team.id,0.0)
    ...
    def _compute_prescriptions_order_count(self):
        ...
        data_map = {team.id: count for team, count in prescriptions_order_data}
        for team in self:
            team.prescriptions_order_count = data_map.get(team.id, 0)

    def _in_prescriptions_scope(self):
        return self.env.context.get('in_prescriptions_app')

    def _graph_get_model(self):
        if self._in_prescriptions_scope():
    ...
    def _graph_get_table(self, GraphModel):
        if self._in_prescriptions_scope():
            ...
            # as the amounts of the prescriptions report are converted in the currency
    ...
    def _extra_sql_conditions(self):
        if self._in_prescriptions_scope():
            return "AND state = 'prescriptions'"
        return super()._extra_sql_conditions()

    def _graph_title_and_key(self):
        if self._in_prescriptions_scope():
            return ['', _('Prescriptions: Untaxed Total')] # no more title
        return super()._graph_title_and_key()

    def _compute_dashboard_button_name(self):
        super(CrmTeam,self)._compute_dashboard_button_name()
        if self._in_prescriptions_scope():
            self.dashboard_button_name = _("Prescriptions Analysis")

    def action_primary_channel_button(self):
        if self._in_prescriptions_scope():
            return self.env["ir.actions.actions"]._for_xml_id("pod_prescriptions.action_order_report_so_prescriptions_team")
    ...
    def _unlink_except_used_for_prescriptions(self):
        ...
        SO_COUNT_TRIGGER = 5
        for team in self:
            if team.prescriptions_order_count >= SO_COUNT_TRIGGER:
                raise UserError(
                    _('Team %(team_name)s has %(prescriptions_order_count)s active prescriptions orders. Consider canceling them or archiving the team instead.',
                      team_name=team.name,
                      prescriptions_order_count=team.prescriptions_order_count
                      ))
