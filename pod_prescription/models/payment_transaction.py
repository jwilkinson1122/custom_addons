...
class PaymentTransaction(models.Model):
    ...
    prescription_order_ids = fields.Many2many('prescription.order', 'prescription_order_transaction_rel', 'transaction_id', 'prescription_order_id',
                                      string='Prescription Orders', copy=False, readonly=True)
    prescription_order_ids_nbr = fields.Integer(compute='_compute_prescription_order_ids_nbr', string='# of Prescription Orders')
    ...
    def _compute_prescription_order_reference(self, order):
        ...
        if invoice_journal:
            order_reference = invoice_journal._process_reference_for_prescription_order(order_reference)

        return order_reference

    @api.depends('prescription_order_ids')
    def _compute_prescription_order_ids_nbr(self):
        for trans in self:
            trans.prescription_order_ids_nbr = len(trans.prescription_order_ids)
    ...
    def _check_amount_and_confirm_order(self):
        ...
        :rtype: a `prescription.order` recordset
        """
        confirmed_orders = self.env['prescription.order']
        for tx in self:
    ...
    def _set_authorized(self, state_message=None, **kwargs):
        ...
        (self.prescription_order_ids - confirmed_orders)._send_payment_succeeded_for_order_mail()
    ...
    def _log_message_on_linked_documents(self, message):
        ...
        for order in self.prescription_order_ids or self.source_transaction_id.prescription_order_ids:
    ...
    def _reconcile_after_done(self):
        ...
        auto_invoice = str2bool(
            self.env['ir.config_parameter'].sudo().get_param('pod_prescription.automatic_invoice'))
        if auto_invoice:
            ...
            self._invoice_prescription_orders()
    ...
    def _send_invoice(self):
        template_id = int(self.env['ir.config_parameter'].sudo().get_param(
            'pod_prescription.default_invoice_email_template',
            default=0
        ))
        if not template_id:
            return
    ...
    def _cron_send_invoice(self):
        ...
        """
        if not self.env['ir.config_parameter'].sudo().get_param('pod_prescription.automatic_invoice'):
            return
        ...
        self.search([
            ...
            ('prescription_order_ids.state', '=', 'prescription'),
            ('last_state_change', '>=', retry_limit_date),
        ])._send_invoice()

    def _invoice_prescription_orders(self):
    ...
    def _compute_reference_prefix(self, provider_code, separator, **values):
        ...
        If the `values` parameter has an entry with 'prescription_order_ids' as key and a list of (4, id, O)
        ...
        :param dict values: The transaction values used to compute the reference prefix. It should
                            have the structure {'prescription_order_ids': [(X2M command), ...], ...}.
        :return: The computed reference prefix if order ids are found, the one of `super` otherwise
        :rtype: str
        """
        command_list = values.get('prescription_order_ids')
        if command_list:
            # Extract prescription order id(s) from the X2M commands
            order_ids = self._fields['prescription_order_ids'].convert_to_cache(command_list, self)
            orders = self.env['prescription.order'].browse(order_ids).exists()
    ...
    def action_view_prescription_orders(self):
        ...
        prescription_order_ids = self.prescription_order_ids.ids
        if len(prescription_order_ids) == 1:
            action['res_id'] = prescription_order_ids[0]
            action['view_mode'] = 'form'
        else:
            action['view_mode'] = 'tree,form'
            action['domain'] = [('id', 'in', prescription_order_ids)]
        return action
