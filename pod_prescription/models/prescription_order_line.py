...
class PrescriptionOrderLine(models.Model):
 ...
 # @api.onchange('product_id')
    ...
    def add_button(self):
        existing_wizard = self.env['creating.wizard'].search([('prescription_order_id','=', self.id)], limit=1)
        existing_wizard.prescription_order_id = self.id
        if existing_wizard:
            return {
            ...
            new_wizard.prescription_order_id = self.id
...
class CreatingWizard(models.Model):
    ...
    prescription_order_id = fields.Many2one('prescription.order.line', string='Prescription Order')
    data_ids = fields.One2many('another.one.two.many', 'one_id', string='Data')

    def add_descript(self):
        self.prescription_order_id.name = self.prescription_order_id.product_template_id.name # we are taking product_template_id bcz we can't use for loop under the description field that's wy we are useing for loop under the product_template_id
        print(self.prescription_order_id.name,'kkkkkkkkkkkkkkkkkkk')
        print(self.prescription_order_id.product_template_id.name,'nnnnnnnnnnnnnnnnnnnn')
        print(self.prescription_order_id.product_template_id,'222222222222222222')
        for record in self.data_ids:
            self.prescription_order_id.name = self.prescription_order_id.name + '\n' + str(record.date) + ':' + str(record.qty_id)
