...
class PodiatryPrescriptionItem(models.Model):
    _name = "pod.prescription.item"
    _description = "pod.prescription.item"
    ...
    def _to_prescription_request(self, data):
        ...
        # We are adding the information on the first prescription request that
        # is not sellable
        ...
        for request in requests.filtered(
            ...
            request._add_prescription_item(self)
            return product.id, self.location_id.location_type_id.id, request
        # If no prescriptions are found, we are returning an error
