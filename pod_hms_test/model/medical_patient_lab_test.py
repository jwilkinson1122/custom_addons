...
class medical_patient_lab_test(models.Model):
    ...
    medical_test_type_id = fields.Many2one(
        'medical.test_type', 'Test Type', required=True)
    ...
    def create_lab_test(self):
        ...
        for browse_record in self:
            ...
            if res_ids:
                ...
                action = self.env.ref('pod_hms_test.action_medical_lab_form')
                list_view_id = imd.sudo()._xmlid_to_res_id('pod_hms_test.medical_lab_tree_view')
                form_view_id = imd.sudo()._xmlid_to_res_id('pod_hms_test.medical_lab_form_view')
                result = {
                    'name': action.name,
                    'help': action.help,
                    'type': action.type,
