...
class AccountMove(models.Model):
    ...
    def _stock_account_get_last_step_stock_moves(self):
        ...
        for invoice in self.filtered(lambda x: x.move_type == 'out_invoice'):
            rslt += invoice.mapped('invoice_line_ids.prescriptions_line_ids.move_ids').filtered(lambda x: x.state == 'sales' and x.location_dest_id.usage == 'customer')
        for invoice in self.filtered(lambda x: x.move_type == 'out_refund'):
            rslt += invoice.mapped('reversed_entry_id.invoice_line_ids.prescriptions_line_ids.move_ids').filtered(lambda x: x.state == 'sales' and x.location_id.usage == 'customer')
            # Add refunds generated from the RX
            rslt += invoice.mapped('invoice_line_ids.prescriptions_line_ids.move_ids').filtered(lambda x: x.state == 'sales' and x.location_id.usage == 'customer')
        return rslt
    ...
    def _get_invoiced_lot_values(self):
        ...
        all_invoices_amls = current_invoice_amls.prescriptions_line_ids.invoice_lines.filtered(lambda aml: aml.move_id.state == 'posted').sorted(lambda aml: (aml.date, aml.move_name, aml.id))
        ...
        stock_move_lines = current_invoice_amls.prescriptions_line_ids.move_ids.move_line_ids.filtered(lambda sml: sml.state == 'sales' and sml.lot_id).sorted(lambda sml: (sml.date, sml.id))
    ...
    @api.depends('line_ids.prescriptions_line_ids.order_id')
    def _compute_delivery_date(self):
        # EXTENDS 'account'
        super()._compute_delivery_date()
        for move in self:
            prescriptions_order_effective_date = list(filter(None, move.line_ids.prescriptions_line_ids.order_id.mapped('effective_date')))
            effective_date_res = max(prescriptions_order_effective_date) if prescriptions_order_effective_date else False
            # if multiple prescriptions order we take the bigger effective_date
    ...
    def _compute_incoterm_location(self):
        ...
        for move in self:
            prescriptions_locations = move.line_ids.prescriptions_line_ids.order_id.mapped('incoterm_location')
            incoterm_res = next((incoterm for incoterm in prescriptions_locations if incoterm), False)
...
class AccountMoveLine(models.Model):
    ...
    def _prescriptions_can_be_reinvoice(self):
        self.ensure_one()
        return self.move_type != 'entry' and self.display_type != 'cogs' and super(AccountMoveLine, self)._prescriptions_can_be_reinvoice()

    def _stock_account_get_anglo_saxon_price_unit(self):
        self.ensure_one()
        price_unit = super(AccountMoveLine, self)._stock_account_get_anglo_saxon_price_unit()

        rx_line = self.prescriptions_line_ids and self.prescriptions_line_ids[-1] or False
        if rx_line:
