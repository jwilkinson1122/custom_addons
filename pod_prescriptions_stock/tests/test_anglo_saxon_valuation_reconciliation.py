...
class TestValuationReconciliationCommon(ValuationReconciliationTestCommon):
    ...
    def _create_invoice_for_rx(self, prescriptions_order, product, date, quantity=1.0):
...
class TestValuationReconciliation(TestValuationReconciliationCommon):
    def test_shipment_invoice(self):
        ...
        invoice = self._create_invoice_for_rx(prescriptions_order, test_product, '2018-02-12')
        invoice.action_post()
        picking = self.env['stock.picking'].search([('prescriptions_id', '=', prescriptions_order.id)])
        self.check_reconciliation(invoice, picking, operation='sales')
    ...
    def test_invoice_shipment(self):
        ...
        invoice = self._create_invoice_for_rx(prescriptions_order, test_product, '2018-02-03')
        invoice.action_post()
        ...
        self.check_reconciliation(invoice.reversal_move_id, return_pick, operation='sales')
    ...
    def test_multiple_shipments_invoices(self):
        ...
        invoice = self._create_invoice_for_rx(prescriptions_order, test_product, '2018-02-03', quantity=3)
        invoice.action_post()
        self.check_reconciliation(invoice, picking, full_reconcile=False, operation='sales')

        invoice2 = self._create_invoice_for_rx(prescriptions_order, test_product, '2018-03-12', quantity=2)
        invoice2.action_post()
        self.check_reconciliation(invoice2, picking, full_reconcile=False, operation='sales')

        self._process_pickings(prescriptions_order.picking_ids.filtered(lambda x: x.state != 'sales'), quantity=3.0)
        picking = self.env['stock.picking'].search([('prescriptions_id', '=', prescriptions_order.id)], order='id desc', limit=1)
        self.check_reconciliation(invoice2, picking, operation='sales')
