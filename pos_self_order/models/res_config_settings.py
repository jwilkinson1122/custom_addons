...
class ResConfigSettings(models.TransientModel):
    ...
    @api.onchange("pos_self_ordering_mode", "pos_module_pos_manufacturing")
    def _onchange_pos_self_order_kiosk(self):
        if self.pos_self_ordering_mode == 'kiosk':
            self.is_kiosk_mode = True
            self.pos_module_pos_manufacturing = False
            self.pos_self_ordering_pay_after = "each"
        else:
            self.is_kiosk_mode = False

            if not self.pos_module_pos_manufacturing:
    ...
    def generate_qr_codes_zip(self):
        ...
        if self.pos_module_pos_manufacturing:
    ...
    def generate_qr_codes_page(self):
        ...
        """
        if self.pos_self_ordering_mode == 'mobile' and self.pos_module_pos_manufacturing:
            table_ids = self.pos_config_id.floor_ids.table_ids
        ...
        return self.env.ref("pos_self_order.report_self_order_qr_codes_page").report_action(
            [], data={
                ...
                'table_mode': self.pos_self_ordering_mode and self.pos_module_pos_manufacturing and self.pos_self_ordering_service_mode == 'table',
