...
class PosConfig(models.Model):
    ...
    self_ordering_alternative_fp_id = fields.Many2one(
        ...
        help='This is useful for manufacturings with onsite and take-away services that imply specific tax rates.',
    )
    ...
    def create(self, vals_list):
        ...
        for pos_config_id in pos_config_ids:
            ...
            if pos_config_id.module_pos_manufacturing:
    ...
    def write(self, vals):
        for record in self:
            ...
            if (not vals.get('module_pos_manufacturing') and not record.module_pos_manufacturing) and vals.get('self_ordering_mode') == 'mobile':
                vals['self_ordering_pay_after'] = 'each'
    ...
    @api.depends("module_pos_manufacturing")
    def _compute_self_order(self):
        for record in self:
            if not record.module_pos_manufacturing and record.self_ordering_mode != 'kiosk':
    ...
    def _get_qr_code_data(self):
        ...
        table_qr_code = []
        if self.self_ordering_mode == 'mobile' and self.module_pos_manufacturing and self.self_ordering_service_mode == 'table':
    ...
    def _get_self_order_route(self, table_id: Optional[int] = None) -> str:
        ...
        if self.self_ordering_mode == 'mobile':
            table = self.env["manufacturing.table"].search(
    ...
    def _get_self_order_custom_links(self):
        ...
        that are defined by the manufacturing employee in the backend.
