...
class PosSelfOrderController(http.Controller):
    ...
    def _verify_authorization(self, access_token, table_identifier, take_away):
        """
        Similar to _verify_pos_config but also looks for the manufacturing.table of the given identifier.
        The manufacturing.table record is also returned with reduced privileges.
        """
        pos_config = self._verify_pos_config(access_token)
        table_sudo = request.env["manufacturing.table"].sudo().search([('identifier', '=', table_identifier)], limit=1)
