...
class PosQuotation(models.Model):
    ...
    name = fields.Char(string='Booking Ref', required=True,
    ...
    amount_tax = fields.Float(
        compute='_compute_amount_all', string='Taxes', digits=0, default=1.2)
    amount_total = fields.Float(
        compute='_compute_amount_all', string='Total', digits=0)
    lines = fields.One2many('book.order.line', 'order_id',
    ...
    book_order_ref = fields.Char(
        string='Booked Order Ref', readonly=True, copy=False)
    ...
    delivery_address = fields.Char(
        'Delivery Address', help='Address of customer for delivery')
...
class PosQuotationLine(models.Model):
    ...
    order_id = fields.Many2one(
        'book.order', string='Order Ref', ondelete='cascade')
