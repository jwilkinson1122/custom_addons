...
class PodiatryProcedure(models.Model):
    ...
    procedure_request_id = fields.Many2one( comodel_name="pod.procedure.request", string="Procedure request", ondelete="restrict", index=True, readonly=True,
    )  # FHIR Field: BasedOn
    performed_initial_date = fields.Datetime( string="Initial date", readonly=True, states={"draft": [("readonly", False)]},
    )  # FHIR Field: performed/performedPeriod
    performed_end_date = fields.Datetime( string="End date", readonly=True, states={"draft": [("readonly", False)]},
    )  # FHIR Field: performed/performedPeriod
    location_id = fields.Many2one( comodel_name="res.partner", domain=[("is_location", "=", True)], ondelete="restrict", index=True, string="Location", readonly=True, states={"draft": [("readonly", False)]},
    )  # FHIR Field: location
