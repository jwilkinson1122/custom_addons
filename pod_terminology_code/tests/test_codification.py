...
class TestCodification(TransactionCase):
    def test_codification(self):
        code = self.env["code.concept"].create(
            {"code": "001", "name": "Test"}
        )
        self.assertEqual(code.display_name, "[001] Test")

    def test_search(self):
        code = self.env["code.concept"].name_search("138875005")
        self.assertTrue(code)

    def test_multiparents(self):
        sct = self.env["code.concept"]
        root = self.browse_ref("pod_terminology_code.code_138875005")
        parent = self.browse_ref("pod_terminology_code.code_105590001")
        code = sct.create(
            {"code": "Test", "name": "Name",
