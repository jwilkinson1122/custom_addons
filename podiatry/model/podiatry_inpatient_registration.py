...
class podiatry_inpatient_registration(models.Model):
    ...
    practice_date = fields.Datetime(string="Practice date",required=True)
    discharge_date = fields.Datetime(string="Expected Discharge date",required=True)
    attending_physician_id = fields.Many2one('podiatry.physician',string="Attending Physician")
    operating_physician_id = fields.Many2one('podiatry.physician',string="Operating Physician")
    ...
    podiatry_pathology_id = fields.Many2one('podiatry.pathology',string="Reason for Admission")
    info = fields.Text(string="Extra Info")
    bed_transfers_ids = fields.One2many('bed.transfer','inpatient_id',string='Transfer Bed')
    podiatry_diet_belief_id = fields.Many2one('podiatry.diet.belief',string='Belief')
    therapeutic_diets_ids = fields.One2many('podiatry.inpatient.diet','podiatry_inpatient_registration_id',string='Therapeutic_diets')
    ...
    state = fields.Selection([('free','Free'),('confirmed','Confirmed'),('practiceized','Practiceized'),('cancel','Cancel'),('done','Done')],string="State",default="free")
    ...
    medication_ids = fields.One2many('podiatry.inpatient.medication','podiatry_inpatient_registration_id',string='Medication')

    @api.model
    def default_get(self, fields):
        result = super(podiatry_inpatient_registration, self).default_get(fields)
        patient_id  = self.env['ir.sequence'].next_by_code('podiatry.inpatient.registration')
        if patient_id:
            result.update({
                        'name':patient_id,
                       })
