...
class podiatry_patient_evaluation(models.Model):
	...
	podiatry_patient_id = fields.Many2one('podiatry.patient',string="Patient",required=True)
	...
	urgency  = fields.Selection([('a', 'Normal'), ('b', 'Urgent'), ('c', 'Podiatry Emergency')],string='Urgency')
	systolic = fields.Integer('Systolic Pressure')
	diastolic = fields.Integer('Diastolic Pressure')
	respiratory_rate = fields.Integer('Respiratory Rate')
	signs_and_symptoms_ids = fields.One2many('podiatry.signs.and.sympotoms','patient_evaluation_id','Signs and Symptoms')
	...
	podiatry_appointment_date_id = fields.Many2one('podiatry.appointment','Appointment Date')
	next_appointment_id = fields.Many2one('podiatry.appointment','Next Appointment')
	derived_from_physician_id = fields.Many2one('podiatry.physician','Derived from Doctor')
	derived_to_physician_id = fields.Many2one('podiatry.physician','Derived to Doctor')
	secondary_conditions_ids = fields.One2many('podiatry.secondary_condition','patient_evaluation_id','Secondary Conditions')
	diagnostic_hypothesis_ids = fields.One2many('podiatry.diagnostic_hypotesis','patient_evaluation_id','Procedures')
	procedure_ids = fields.One2many('podiatry.directions','patient_evaluation_id','Procedures')
