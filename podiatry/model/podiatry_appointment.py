...
class podiatry_appointment(models.Model):
	_name = "podiatry.appointment"
	_description = "Podiatry Appointment"
	...
	inpatient_registration_id = fields.Many2one('podiatry.inpatient.registration',string="Inpatient Registration")
	...
	doctor_id = fields.Many2one('podiatry.physician','Physician',required=True)
	...
	podiatry_patient_psc_ids = fields.Many2many('podiatry.patient.psc',string='Pediatrics Symptoms Checklist')
	podiatry_prescription_order_ids = fields.One2many('podiatry.prescription.order','appointment_id',string='Prescription')
	...
	def onchange_name(self):
		...
		ins_record = ins_obj.search([('podiatry_insurance_partner_id', '=', self.patient_id.patient_id.id)])
		if len(ins_record)>=1:
	...
	def create(self, vals):
		vals['name'] = self.env['ir.sequence'].next_by_code('podiatry.appointment') or 'APT'
		msg_body = 'Appointment created'
		for msg in self:
			msg.message_post(body=msg_body)
		result = super(podiatry_appointment, self).create(vals)
		return result
	...
	def onchange_patient(self):
		if not self.inpatient_registration_id:
			self.patient_id = ""
		inpatient_obj = self.env['podiatry.inpatient.registration'].browse(self.inpatient_registration_id.id)
	...
	def create_invoice(self):
		lab_req_obj = self.env['podiatry.appointment']
