...
class Prescription(models.Model):
    ...
    # defaultuomname	        name	                        categoryname	        price_base	        productnumber	        producttypecodename

    # EACH	                FREE RUSH UPGRADE           	    Miscellaneous	        0	                RUSH_ORDER_PROMOTION	    Sales Inventory
    # EACH	                SUPER RUSH-WALK THRU	            Miscellaneous	        25	                SUPER_RUSH-WALK_THRU	    Sales Inventory
    # EACH	                RUSH ONE DAY	                    Miscellaneous	        65	                RUSH_ONE_DAY_2	            Sales Inventory
    # EACH	                RUSH ONE DAY	                    Miscellaneous	        53	                RUSH_ONE_DAY	            Sales Inventory
    # PAIR	                RUSH	                            Miscellaneous	        26	                SUPER_RUSH-WALK_THRU_1	    Sales Inventory
    # PAIR	                RUSH	                            Miscellaneous	        29	                RUSH	                    Sales Inventory
    ...
    num_prescriptions = fields.Integer(compute="_compute_num_prescriptions", store=True)

    @api.depends("prescription_line")
    def _compute_num_prescriptions(self):
        for prescription in self:
            prescription.num_prescriptions = len(prescription.prescription_line)
    ...
    shell_foundation = fields.Selection(
        [
            ("cataract_eye", "Cataract Eye"),
            ("pterygium", "Pterygium"),
            ("glaucoma", "Glaucoma"),
    ...
    def print_prescription_report_ticket_size(self):
        return self.env.ref(
            "podiatry.practitioner_prescription_ticket_size2"
        ).report_action(self)
