...
class FolioRoomLine(models.Model):
    ...
    _description = "Podiatry Room Reservation"
    _rec_name = "room_id"

    room_id = fields.Many2one("podiatry.room", ondelete="restrict", index=True)
    check_in = fields.Datetime("Check In Date", required=True)
    check_out = fields.Datetime("Check Out Date", required=True)
    folio_id = fields.Many2one("podiatry.folio", "Folio Number", ondelete="cascade")
    status = fields.Selection(related="folio_id.state", string="state")


class PodiatryFolio(models.Model):

    _name = "podiatry.folio"
    _description = "podiatry folio"
    _rec_name = "order_id"
    ...
    room_line_ids = fields.One2many(
        ...
        help="Podiatry room reservation detail.",
    )
    ...
    def _update_folio_line(self, folio_id):
        ...
        podiatry_room_obj = self.env["podiatry.room"]
        for rec in folio_id:
            for room_rec in rec.room_line_ids:
                room = podiatry_room_obj.search(
    ...
    def create(self, vals):
        ...
        @return: new record set for podiatry folio.
        """
        if not "service_line_ids" and "folio_id" in vals:
            ...
            folio_id = super(PodiatryFolio, self).create(vals)
            ...
            if not vals:
                vals = {}
            vals["name"] = self.env["ir.sequence"].next_by_code("podiatry.folio")
    ...
    def write(self, vals):
        ...
        for rec in self:
            ...
            if len(list(new_rooms)) != 0:
                ...
                for rm in room_list:
                    room_obj = podiatry_room_obj.search([("product_id", "=", rm.id)])
            ...
            if not len(list(new_rooms)):
                ...
                for room in room_list_obj:
                    room_obj = podiatry_room_obj.search([("product_id", "=", room.id)])
                    room_obj.write({"isroom": False})
                    room_vals = {
                        "room_id": room_obj.id,
                        "check_in": rec.checkin_date,
        ...
        return super(PodiatryFolio, self).write(vals)

    @api.onchange("partner_id")
    def _onchange_partner_id(self):
        """
        When you change partner_id it will update the partner_invoice_id,
        partner_shipping_id and pricelist_id of the podiatry folio as well
        ---------------------------------------------------------------
        @param self: object pointer
        """
        if self.partner_id:
    ...
    def action_cancel(self):
        ...
        for rec in self:
            ...
            for product in rec.room_line_ids.filtered(
                ...
                rooms = self.env["podiatry.room"].search([("product_id", "=", product.id)])
...
class PodiatryFolioLine(models.Model):

    _name = "podiatry.folio.line"
    _description = "Podiatry Folio Line"
    ...
    folio_id = fields.Many2one("podiatry.folio", "Folio", ondelete="cascade")
    ...
    def create(self, vals):
        ...
        @param self: The object pointer
        @param vals: dictionary of fields value.
        @return: new record set for podiatry folio line.
        """
        if "folio_id" in vals:
            folio = self.env["podiatry.folio"].browse(vals["folio_id"])
            vals.update({"order_id": folio.order_id.id})
        return super(PodiatryFolioLine, self).create(vals)
    ...
    def unlink(self):
        ...
        for line in self:
            if line.order_line_id:
                rooms = self.env["podiatry.room"].search(
        ...
        return super(PodiatryFolioLine, self).unlink()
    ...
    def _onchange_checkin_checkout_dates(self):
        ...
        and update the qty of podiatry folio line
        -----------------------------------------------------------------
        @param self: object pointer
        """
        ...
        if self.checkin_date and self.checkout_date:
            ...
            #            To calculate additional hours in podiatry room as per minutes
