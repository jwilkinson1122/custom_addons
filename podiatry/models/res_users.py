...
class ResUsers(models.Model):
    ...
    def create(self, vals):
        ...
        if self._context.get('teacher_create', False):
            ...
            res.write({'groups_id': [(6, 0, teacher_group_ids)],
                    'company_id': self._context.get('clinic_id'),
                    'company_ids': [(4, self._context.get('clinic_id'))]})
        return res
    ...
    #             self.env.ref('base.group_partner_manager').id]
    #         res.write({'groups_id': [(6, 0, teacher_group_ids)],
    #                 'partner_id': self._context.get('clinic_id'),
    #                 'partner_ids': [(4, self._context.get('clinic_id'))]})
    #     return res
