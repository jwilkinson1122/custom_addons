...
class PatientPatient(models.Model):
    ...
    def _search(self, args, offset=0, limit=None, order=None, count=False,
        ...
        '''Method to get patient of doctor having group hcp'''
        hcp_group = self.env.user.has_group(
            'podiatry.group_podiatry_hcp')
    ...
    @api.depends('state')
    def _compute_hcp_user(self):
        '''Compute hcp boolean field if user form hcp group'''
        hcp = self.env.user.has_group("podiatry.group_podiatry_hcp")
        for rec in self:
            rec.hcp_user_grp = False
    ...
    hcp_user_grp = fields.Boolean("HCP Group",
                                  compute="_compute_hcp_user",
                                  help='Activate/Deactivate hcp group')
    ...
    def create(self, vals):
        ...
        hcp = self.env['podiatry.hcp']
        for data in res.doctor_id:
            for record in hcp.search([('pt_doctor_id', '=', data.id)]):
    ...
    def write(self, vals):
        ...
        patient to their respective hcp'''
        hcp = self.env['podiatry.hcp']
        if vals.get('doctor_id'):
            for doctor in vals.get('doctor_id')[0][2]:
                for data in hcp.search([('pt_doctor_id', '=', doctor)]):
