...
class Practitioner(models.Model):
            ...
            if delta.months > 0 and delta.years < 9:
                result.append(
                    "{months} {practitioner}".format(
                        months=delta.months,
                        practitioner=_("month") if delta.months == 1 else _("months"),
                    )
                )
    ...
    def _compute_same_reference_practitioner_id(self):
        for practitioner in self:
            ...
            practitioner.same_reference_practitioner_id = bool(
    ...
    def _add_followers(self):
        for practitioner in self:
            partner_ids = (
                practitioner.user_id.partner_id | practitioner.responsible_id.partner_id
            ).ids
