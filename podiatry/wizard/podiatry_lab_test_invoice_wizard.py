...
class podiatry_lab_test_invoice(models.TransientModel):
    
    _name = 'podiatry.lab.test.invoice'
    _description='podiatry lab test invoice'
    ...
    def create_lab_invoice(self):
        ...
        list_of_ids  = []
        lab_req_obj = self.env['podiatry.patient.lab.test']
        lab_result_obj = self.env['podiatry.lab']
        account_invoice_obj = self.env['account.move']
        account_invoice_line_obj = self.env['account.move.line']
        ir_property_obj = self.env['ir.property']
        for active_id in active_ids:
            if self._context['active_model'] == 'podiatry.patient.lab.test':
                ...
                invoice_vals = {
                'name': self.env['ir.sequence'].next_by_code('podiatry_lab_test_inv_seq'),
                'invoice_origin': lab_req.podiatry_test_type_id.name or '',
                ...
                product = lab_req.podiatry_test_type_id.service_product_id
                ...
                tax_ids = taxes.ids
                invoice_line_vals = {
                'name': lab_req.podiatry_test_type_id.service_product_id.name or '',
                'account_id': invoice_line_account_id,
                'price_unit': lab_req.podiatry_test_type_id.service_product_id.lst_price,
                'product_uom_id': lab_req.podiatry_test_type_id.service_product_id.uom_id.id,
                'quantity': 1,
                'product_id':lab_req.podiatry_test_type_id.service_product_id.id ,
                    }
                res1 = res.write({'invoice_line_ids' :([(0,0,invoice_line_vals)]) })
                list_of_ids.append(res.id)
            if self._context['active_model'] == 'podiatry.lab':
