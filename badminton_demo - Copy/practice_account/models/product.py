    ...
    def _check_membership_product(self):
        ...
        for prod_tmpl in self:
            if prod_tmpl.membership_ok:
                if (
                    self.search_count(
                        [("membership_ok", "=", True), ("name", "=", prod_tmpl.name)]
                    )
                    > 1
                ):
                ...
                if prod_tmpl.attribute_line_ids.filtered(
                ...
                if prod_tmpl.attribute_line_ids.filtered(
                    ...
                    raise ValidationError(
                        _("You cannot use attribute '{}' for this product.").format(
                            membership_attr.name
                        )
                    )
...
class ProductProduct(models.Model):
    ...
    period_category_id = fields.Many2one(
        "period.category", string="Period Category", compute="_compute_period_category"
    )

    @api.depends("period_category_ids")
    def _compute_period_category(self):
        for product in self:
            product.period_category_id = (
                self.period_category_ids[0] if product.period_category_ids else False
            )
    ...
    def _check_period_category_ids(self):
        for product in self:
            if not product.membership_ok and product.period_category_ids:
                raise ValidationError(
                    _(
                        "You cannot have period categories for a non 'membership' product."
                    )
                )
            elif len(product.period_category_ids) > 1:
                raise ValidationError(
                    _("You cannot have multiple period categories for this product.")
                )
