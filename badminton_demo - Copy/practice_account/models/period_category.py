...
class PeriodCategory(models.Model):
    ...
    def _create_product_product(self, period=None, category=None):
        ...
        membership_attr = self.env.ref(
            "practice_account.product_attribute_membership_category"
        )
        if category is None:
            category = self.category_id
        prod_attr_val = category.product_attribute_value_id

        ptal = prod_tmpl.attribute_line_ids.filtered(
            lambda ptal: ptal.attribute_id == membership_attr
        )
        if not ptal:
            prod_tmpl.write(
                {
            ...
            ptal.write({"value_ids": [(4, prod_attr_val.id)]})
        if len(ptal) != 1:
            raise ValidationError(
                _(
                    "Product template must have only 1 attribute line assigned (currently: '%s').",
                    ptal,
                )
            )
        ...
        def get_prod_prod(prod_prod):
            return (
                prod_attr_val
                in prod_prod.product_template_attribute_value_ids.mapped(
    ...
    def _get_expected_product_product(self):
        ...
        return self.period_id.product_tmpl_id.product_variant_ids.filtered(
            lambda pp: pav
            in pp.product_template_attribute_value_ids.mapped(
                "product_attribute_value_id"
            )
        )
    ...
    def unlink(self):
        for per_cat in self:
            ...
            ptal = per_cat.product_id.product_tmpl_id.attribute_line_ids.filtered(
                lambda ptal: prod_attr_val in ptal.value_ids
            )
    ...
    def _check_product_id(self):
        ...
        for per_cat in self:
            if self.search_count([("product_id", "=", per_cat.product_id.id)]) > 1:
                raise ValidationError(
                    _(
                        "The product of the period category must be unique! "
            ...
            if expected_prod_prod and per_cat.product_id != expected_prod_prod:
                raise ValidationError(
                    _(
                        "The expected product of the period category does not match the assigned one."
                    )
                )
