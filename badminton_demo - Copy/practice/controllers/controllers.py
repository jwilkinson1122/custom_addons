...
class Practice(AuthSignupHome):
    ...
    def request_membership(self):
        consumed_period_ids = (
            ...
            .mapped("period_id")
            .ids
        )  # TODO Test 'request.env.user.partner_id.id'
        ...
        return request.render(
            "practice.website_membership_request",
            {
    ...
    #     return request.render('practice.website_member_info', {
    #         'member': member
    #     })

    @route("/aboutus", auth="public", website=True)
    def info_practice(self):
        ...
        return request.render(
            "practice.website_info_practice",
            {
                "committee": committee,
            },
        )

    # @route('%s/info_practice/' % _URL_ROOT, auth='public', website=True)
    # def info_practice(self):
    #     committee = (
    ...
    #     )
    #     return request.render('practice.website_info_practice', {
    #         'committee': committee,
    #     })

    @route(
        "%s/my/membership/<string:action>" % _URL_ROOT,
        type="http",
        auth="public",
        website=True,
    )
    def membership_invitation_response(self, action, token):
        ...
        if not membership.token_is_valid and action != "view":
            messages["warning"] = (
                "You cannot perform this action because the token has expired."
            )

        if membership.state == "member":
            messages["info"] = (
                "Congratulations, your membership has already been approved by the committee."
            )
        elif membership.token_is_valid:
            if action == "accept":
                membership.do_accept()
                messages["success"] = (
                    "You have accepted the invitation. Your membership must now be approved by the committee."
                )
            elif action == "decline":
                ...
                messages["info"] = (
                    "Your membership request is going to be examined by the committee. \
                    Please ensure to pay the amount due first."
                )

        response_content = request.env["ir.ui.view"]._render_template(
            "practice.membership_affiliation_page_anonymous",
            {
