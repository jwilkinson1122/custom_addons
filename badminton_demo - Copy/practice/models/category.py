...
class Category(models.Model):
    ...
    period_ids = fields.Many2many(
        "period", string="Periods", compute="_compute_period_ids"
    )
    ...
    need_responsible = fields.Boolean(
        "Need Responsible",
    ...
    def _compute_membership_ids(self):
        for record in self:
            record.membership_ids = self.env["membership"].search(
                [
                    ("period_category_id.category_id.id", "=", record.id or False),
                ]
            )
        ...
        for category in self:
            if self.search_count([("name", "=", category.name)]) > 1:
                raise ValidationError(
                    _(
                        "The name of the category must be unique! Please change it accordingly."
                    )
                )
