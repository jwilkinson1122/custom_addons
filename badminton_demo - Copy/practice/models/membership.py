...
class Membership(models.Model):
    ...
    member_tag_ids = fields.Many2many(
    ...
    member_user_id = fields.Many2one(
        "res.users", compute="_compute_member_user_id", index=True
    )
    ...
    contact_person_id = fields.Many2one(
        ...
        domain=CONTACT_DOMAIN,
        tracking=True,
        help="Contact with which all communication will happen.",
    )
    ...
    token_validity = fields.Datetime(
        "Token Validity", readonly=True
    )  # , groups='base.group_user')
    token_is_valid = fields.Boolean(
        "Token Is Valid", compute="_compute_token_is_valid", readonly=True
    )
    ...
    def write(self, vals):
        ...
        if vals.get("state") == "member":
            ...
            self.member_id.practice_id = self.company_id.partner_id
    ...
    def _send_email(self, template_xmlid, composer_title, **composer_ctx):
        ...
        return {
            ...
            "params": {
                ...
                "message": _(
                    "Only '%s' will be used for emails.", self._get_closest_lang()
                ),
                "sticky": False,
                "next": action,
            },
    ...
    def send_email_invitation(self):
        ...
        if not memberships:
            raise UserError(
                _(
                    "The membership(s) must have state 'Unknown' or 'Old Member' in order "
                    "to send an invitation email."
                )
            )
        return memberships._send_email(
            template_xmlid="practice.email_template_membership_affiliation_invitation",
    ...
    def send_email_confirmation(self):
        ...
        if not memberships:
            raise UserError(
                _(
                    "The membership(s) must have state 'Member' in order "
                    "to send a confirmation email."
                )
            )
        return memberships._send_email(
            template_xmlid="practice.email_template_membership_affiliation_confirmation",
    ...
    def send_email_payment_due(self):
        return self._send_email(
            template_xmlid="practice.email_template_membership_payment_due",
    ...
    def do_accept(self):
        ...
        for membership in self:
            membership.message_post(
                body=_(
                    "%s has accepted the invitation. His status has been changed to prevalidated."
                )
    ...
    def do_decline(self):
        ...
        for membership in self:
            membership.message_post(
                body=_(
                    "%s has declined the invitation. His status has been changed to Rejected."
                )
                % (membership.member_id.name)
            )
        return res
    ...
    def message_new(self, msg, custom_values=None):
        ...
        if not period_category:
            period_category = self.env["period.category"].search(
                ...
                order="create_date asc, category_id asc",
                limit=1,
            )
        ...
        email = (
            ...
            or tools.email_split(msg.get("email_from"))
            or [None]
        )[0]
        name = (
            msg.get("from").split("<")[0].strip()
            or msg.get("email_from").split("<")[0].strip()
            or None
        )
    ...
    def _modify_period_category_vals(self, vals, at_creation=False):
        ...
        if "period_category_id" in vals or "period_id" in vals or "category_id" in vals:
            if vals.get("period_category_id"):
                period_category = self.env["period.category"].browse(
                    vals["period_category_id"]
                )
            else:
                period_category = self.env["period.category"].search(
                    [
            ...
            if not period_category:
                raise ValidationError(
                    _(
                        'Fields "Period" and "Category" cannot be set together with such values.'
                    )
                )
    ...
    def _onchange_period_or_category(self):
        ...
        return {
            "warning": {
                "title": _("Message"),
                "message": _("Price Due has changed"),
                "type": "notification",
            },
        }
    ...
    def _onchange_token(self):
        for record in self:
            if (
                record.token
                and record.env["membership"].search_count(
                    [
                        ("token", "=", record.token),
                    ]
                )
    ...
    def _compute_member_user_id(self):
        ...
        for record in self:
            record.member_user_id = ResUsers.search(
                [("partner_id", "=", record.member_id.id)], limit=1
            )
    ...
    def _inverse_contact_person_id(self):
        for record in self:
            if (
                ...
                and record.member_id
                and not record.member_id.responsible_id
            ):
        ...
        if self.contact_person_id and not self.member_id.responsible_id:
            return {
                "warning": {
                    "title": _("Message"),
                    "message": _(
                        "The contact person will also be set as responsible of the player!"
                    ),
                    "type": "notification",
                },
            }
    ...
    def _compute_token_is_valid(self):
        for record in self:
            # TODO 30 should be in practice parameters
            record.token_is_valid = (
                record.token_validity
                and datetime.now()
                <= fields.Datetime.from_string(record.token_validity)
                + timedelta(days=30)
            )
    ...
    def _check_user_period(self):
        ...
        for membership in self:
            if (
                ...
                raise ValidationError(
                    _(
                        "The user '%s' has already a membership for this period (%s). Please change accordingly."
                    )
                    % (membership.member_id.name, membership.period_id.name)
                )
    ...
    def _read_group_fill_results(
        ...
        res = super()._read_group_fill_results(
            domain,
            groupby,
            remaining_groupbys,
            aggregated_fields,
        ...
        for d in res:
            d["__fold"] = (
                True if d.get("state") in ("old_member", "rejected") else False
            )
        return res
