...
class MailComposer(models.TransientModel):
    ...
    def _default_season_id(self):
        if self._context.get("for_new_season_mail"):
            return self.env["period"].search(
                [("current", "=", True)], limit=1
            ) or self.env["period"].search([("upcoming", "=", True)], limit=1)
        return False

    template_id = fields.Many2one(
        "mail.template", domain="[('id', 'in', allowed_template_ids)]"
    )
    allowed_template_ids = fields.One2many(
        "mail.template", compute="_compute_allowed_template_ids"
    )
    ...
    def _onchange_season_id(self):
        ...
        if self.season_id:
            if self.season_id.alias_name:
                email = "%s@%s" % (
                    self.season_id.alias_name,
                    self.season_id.alias_domain,
                )
                self.reply_to = tools.formataddr(
                    (_("New Season") or "False", email or "False")
                )
            if not self.template_id:
                # should fetch 'practice.email_template_new_season' record if no other template has been created afterwards
                tmpl = self.env["mail.template"].search(
                    [("kind", "=", "new_season")], order="id desc", limit=1
                )
    ...
    def _compute_allowed_template_ids(self):
        ...
        for rec in self:
            allowed_templates = MailTemplate.search_read(
                [("model", "=", rec.model)] + base_domain, ["id"]
            )
    ...
    def _onchange_template_id(self, template_id, composition_mode, model, res_id):
        ...
        return super(MailComposer, self)._onchange_template_id(
    ...
    def action_send_mail(self):
        ...
        if (
            self.env.context.get("open_records_view")
            or "active_domain" in self._context
        ):
...
class MailTemplate(models.Model):
    ...
    kind = fields.Selection(
        ...
        " * 'Affiliate': emails related to the affiliates (application 'Affiliates' needs to be installed).\n"
    ...
    def _get_sent_mail_message(self, contacts):
        ...
        return _(
            "Email <i>%(template)s</i> has been sent to the following contacts:<ul>%(contacts)s</ul>",
            template=mail_link,
