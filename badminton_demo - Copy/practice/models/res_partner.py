...
class ResPartner(models.Model):
    ...
    company_type = fields.Selection(
        string="Company Type",
        selection=[("person", "Individual"), ("company", "Practice")],
    )
    ...
    player_ids = fields.One2many("res.partner", "practice_id", string="Players")
    responsible_id = fields.Many2one(
        ...
        help="Contact responsible of current contact.",
    )
    dependent_ids = fields.One2many(
        "res.partner", "responsible_id", string="Dependents"
    )
    membership_ids = fields.One2many("membership", "member_id", string="Memberships")
    membership_count = fields.Integer(
        "Count Memberships", compute="_compute_memberships"
    )
    ...
    state_id = fields.Many2one(
        "res.country.state", inverse="_inverse_address", tracking=True
    )
    zip = fields.Char(inverse="_inverse_address", tracking=True)
    country_id = fields.Many2one(
        "res.country", inverse="_inverse_address", tracking=True
    )
    ...
    def _get_hierarchical_contacts(self, existing_contacts=None, exclude_contact=None):
        ...
        new_contacts = (
            self.responsible_id._origin | self.dependent_ids._origin
        ) - existing_contacts
        if new_contacts:
            return new_contacts._get_hierarchical_contacts(
                existing_contacts | new_contacts, exclude_contact
            )
        else:
    ...
    def _inverse_address(self):
        for record in self:
            ...
            related_contacts = self._get_hierarchical_contacts().filtered(
                has_different_address
            )
            if related_contacts:
                related_contacts.write(
                    {field: record[field] for field in ADDRESS_FIELDS}
                )

    def _inverse_responsible_id(self):
        non_address_fields = [
            field for field in ("email", "phone", "mobile") if field in ADDRESS_FIELDS
        ]
        ...
        address_fields = [
            field for field in ADDRESS_FIELDS if field not in non_address_fields
        ]
        for member in self:
            ...
            member_fields = [
                field
                for field in non_address_fields
                if member[field] and field not in resp_fields
            ]
            member_address_ok = not resp_address_ok and any(
                member[field] for field in address_fields
            )
    ...
    def _onchange_responsible_id(self):
        if self.responsible_id and any(
            self[field] != self.responsible_id[field] for field in ADDRESS_FIELDS
        ):
            return {
                "warning": {
                    "title": _("Message"),
                    "message": _(
                        "Current data will be changed based on new responsible!"
                    ),
                    "type": "notification",
                },
            }
    ...
    def _onchange_address(self):
        ...
        if contacts:
            return {
                "warning": {
                    "title": _("Message"),
                    "message": _("Related data of %s will also be modified!")
                    % ", ".join(contacts.mapped("name")),
                    "type": "notification",
                },
            }
