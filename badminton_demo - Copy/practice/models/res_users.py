...
class ResUsers(models.Model):
    ...
    president = fields.Boolean(
        "Is President", compute="_compute_role", store=True, readonly=True
    )
    secretary = fields.Boolean(
        "Is Secretary", compute="_compute_role", store=True, readonly=True
    )
    treasurer = fields.Boolean(
        "Is Treasurer", compute="_compute_role", store=True, readonly=True
    )
    committee_user = fields.Boolean(
        "Is Committee User",
        compute="_compute_committee_user",
        inverse="_inverse_committee_user",
        help='If checked, the user will have a read access to the application "Practice".',
    )
    committee_manager = fields.Boolean(
        "Is Committee Manager",
        compute="_compute_committee_manager",
        inverse="_inverse_committee_manager",
        help='If checked, the user will have a write access to the application "Practice".\n\
    ...
    def modify_role(self):
        ...
        current_role = self.role_ids.filtered(
            lambda r: r.current and r.name == role_name
        )
        if (action == "stop" and not current_role) or (
            action == "start" and current_role
        ):
            raise ValidationError(
                _(
                    "Error while trying to %s role '%s': maybe no current role has been found!"
                )
                % (action, role_name)
            )
    ...
    def validate_email(self):
        ...
        ):  # TODO Mettre ça une fois qu'on sauve?
            raise ValidationError(
                _("This email already exists. Please enter another one.")
            )
    ...
    def _get_group_vals(self, vals):
        ...
        new_groups = []
        status_groups = (
            ("president", "practice.group_practice_president"),
            ("secretary", "practice.group_practice_secretary"),
            ("treasurer", "practice.group_practice_treasurer"),
        )
        ...
        if "add" in committee_manager_group_todo:
            new_groups.append(
                (4, self.env.ref("practice.group_practice_committee_manager").id)
            )
        elif "nothing" in committee_manager_group_todo:
            pass
        elif "delete" in committee_manager_group_todo:
            new_groups.append(
                (3, self.env.ref("practice.group_practice_committee_manager").id)
            )
        return {"groups_id": new_groups} if new_groups else {}

    def _compute_committee_user(self):
        for user in self:
            user.committee_user = user.has_group(
                "practice.group_practice_committee_user"
            )
            ...
            user.write(
                {
                    "groups_id": [
                        (
                            4 if user.committee_user else 3,
                            self.env.ref("practice.group_practice_committee_user").id,
                        )
                    ]
                }
            )
    ...
    def _compute_committee_manager(self):
        for user in self:
            user.committee_manager = user.has_group(
                "practice.group_practice_committee_manager"
            )
            ...
            user.write(
                {
                    "groups_id": [
                        (
                            4 if user.committee_manager else 3,
                            self.env.ref(
                                "practice.group_practice_committee_manager"
                            ).id,
                        )
    ...
    def _compute_role(self):
        for user in self:
            user.president = user.role_ids.filtered(
                lambda r: r.current and r.name == "president"
            )
            user.secretary = user.role_ids.filtered(
                lambda r: r.current and r.name == "secretary"
            )
            user.treasurer = user.role_ids.filtered(
                lambda r: r.current and r.name == "treasurer"
            )
