...
class Period(models.Model):
    ...
    category_ids = fields.Many2many(
        "category", string="Categories", compute="_compute_category_ids"
    )
    member_ids = fields.Many2many(
        "res.partner", string="Members", compute="_compute_membership_ids"
    )
    ...
    def _update_periods(self):
        ...
        current_period_id = Period.search(
            [("start_date", "<=", today), ("end_date", ">=", today)],
            order="start_date asc",
            limit=1,
        ).id
        upcoming_period_ids = Period.search(
            [("start_date", ">", today)], order="start_date asc"
        ).ids
        upcoming_period_id = upcoming_period_ids[0] if upcoming_period_ids else []
        _logger.info(
            "Update periods: current is #%d; upcoming are #%s",
            current_period_id,
            ",".join(str(p_id) for p_id in upcoming_period_ids),
        )

        for period in Period.search([]):
            period.write(
                {
                    "active": period.id == current_period_id
                    or period.id in upcoming_period_ids,
    ...
    def _compute_year(self):
        raise NotImplementedError(
            _("Field 'Year' cannot be computed. Its only purpose is to be searched.")
        )
    ...
    def _search_year(self, operator, operand):
        if operator in ("like", "ilike"):
            ...
            raise NotImplementedError(
                _("The operator '%s' is not supported when searching field 'Year'.")
                % operator
            )
    ...
    def prepare_duplication_wizard(self, default=None):
        ...
        most_recent_period = self.env["period"].search(
            [
                "|",
                ("active", "=", False),
                ("active", "=", True),
        ...
        return {
            ...
            "view_id": self.env.ref("practice.period_wizard_form_view").id,
            "context": ctx,
            "target": "new",
        }
    ...
    def _compute_membership_ids(self):
        for record in self:
            record.membership_ids = self.env["membership"].search(
                [
                    ("period_category_id.period_id.id", "=", record.id or False),
                ]
            )
            record.count_members = len(
                record.membership_ids.filtered(lambda m: m.state == "member")
            )
    ...
    def _get_alias_name(self, name=""):
        if not name:
            name = self.name
        local_part = (
            self.env["ir.config_parameter"].sudo().get_param("mail_local_part") or ""
        )
    ...
    def _check_name_unique(self):
        ...
        for period in self:
            if self.search_count([("name", "=", period.name)]) > 1:
                raise ValidationError(
                    _(
                        "The name of the period must be unique! Please change it accordingly."
                    )
                )
