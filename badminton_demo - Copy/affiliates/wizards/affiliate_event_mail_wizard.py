...
class AffiliateEventMailWizard(models.TransientModel):
    _name = "affiliate.event.mail.wizard"
    _description = "Affiliate Event Mail Wizard"

    affiliate_event_id = fields.Many2one(
        "affiliate.event", string="Affiliate Event", required=True, ondelete="cascade"
    )

    # === fields related to 1st email, for players ===
    send_to_players = fields.Boolean("Send to Players", default=True)
    default_attendee_ids = fields.One2many(
        "calendar.attendee",
        related="affiliate_event_id.attendee_ids",
        string="Default Players",
    )
    attendee_ids = fields.Many2many(
        "calendar.attendee",
        "affiliate_event_mail_wizard_attendee_rel",
        column1="wizard_id",
        column2="attendee_id",
        string="Players",
        domain="[('id', 'in', default_attendee_ids)]",
    )
    composer_id = fields.Many2one(
        "mail.compose.message", string="Composer (Players)", ondelete="cascade"
    )
    # composer related fields
    template_id = fields.Many2one(
        "mail.template",
        string="Mail Template",
        related="composer_id.template_id",
        readonly=False,
        domain=[("model", "=", "calendar.attendee"), ("kind", "=", "affiliate")],
    )
    subject = fields.Char(
        related="composer_id.subject", string="Subject (Players)", readonly=False
    )
    body = fields.Html(
        related="composer_id.body", string="Body (Players)", readonly=False
    )
    ...
    others_composer_id = fields.Many2one(
        "mail.compose.message", string="Composer (Others)", ondelete="cascade"
    )
    ...
    # composer related fields
    others_partner_ids = fields.Many2many(
        "res.partner", related="others_composer_id.partner_ids", readonly=False
    )
    others_subject = fields.Char(
        related="others_composer_id.subject", string="Subject (Others)", readonly=False
    )
    others_body = fields.Html(
        related="others_composer_id.body", string="Body (Others)", readonly=False
    )
    others_template_id = fields.Many2one(
        "mail.template",
        string="Mail Template (Others)",
        related="others_composer_id.template_id",
        readonly=False,
        domain=[("model", "=", "affiliate.event"), ("kind", "=", "affiliate")],
    )

    @api.model
    def default_get(self, fields):
        res = super(AffiliateEventMailWizard, self).default_get(fields)
        if self.env.context.get(
            "active_model"
        ) == "affiliate.event" and self.env.context.get("active_id"):
            ic_event = self.env["affiliate.event"].browse(self.env.context["active_id"])
        else:
            raise NotImplementedError(
                _(
                    'Only the model "affiliate.event" is supported for this mail wizard. '
                    'Current model: "{}".'
                ).format(self.env.context.get("active_model", ""))
            )
        ...
        others_composer = self.env["mail.compose.message"].create(
            {
                ...
                "subtype_id": self.env.ref(
                    "affiliates.mt_affiliate_event_communication"
                ).id,
            }
        )

        # by default, others_composer.model == 'affiliate.event'
        res.update(
            {
                "affiliate_event_id": ic_event.id,
                "composer_id": composer.id,
                "others_composer_id": others_composer.id,
                "send_to_players": self.env.context.get("send_mail_to_players", False),
                "others_send_to_partners": self.env.context.get(
                    "send_mail_to_others", False
                ),
                "others_partner_ids": ic_event.referee_id
                | ic_event.affiliate_id.responsible_id,
                "others_include_players": not self.env.context.get(
    ...
    def _onchange_others_template_id(self):
        ...
        if self.others_composer_id:
            ...
            self.others_subject = (
                self.others_composer_id.subject if self.others_template_id else ""
            )
    ...
    def _onchange_others_include_followers(self):
        self.ensure_one()
        followers = self.affiliate_event_id.message_follower_ids.partner_id
    ...
    @api.onchange("affiliate_event_id")
    def _onchange_affiliate_event_id(self):
        self.attendee_ids = self.default_attendee_ids.filtered(
            lambda a: a.state in ("needsAction", "tentative")
        )
    ...
    def change_event_state(self):
        ...
        if role == "to_open":
            self.affiliate_event_id.action_open()
        elif role == "to_confirm":
            self.affiliate_event_id.action_confirm()
        elif role == "to_cancel":
            self.affiliate_event_id.action_cancel()
        ...
        def get_subtype_msg(composer):
            ...
            if not sub_type or composer.composition_mode == "mass_mail":
                return ""
            msg = _(
                "and to subscribers of <a href=# data-oe-model={r_model} data-oe-id={r_id}>{r_name}</a>"
            )
            return msg.format(
                r_model=sub_type._name, r_id=sub_type.id, r_name=sub_type.name
            )
        ...
        if self.others_send_to_partners:
            ...
            msg = self.others_template_id._get_sent_mail_message(
                self.others_partner_ids
            )
