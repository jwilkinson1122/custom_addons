...
class ResPartner(models.Model):
        ...
        if model == "calendar.event":
            ...
            if model not in _models_fields:
                raise NotImplementedError(
                    'Model "{}" not supported in method get_attendee_detail_multi_model.'.format(
                        model
                    )
                )
            calendar_event_field = _models_fields[model]
            real_meeting_id = (
                self.env[model].browse(meeting_id)[calendar_event_field].id
            )
    ...
    def get_attendee_detail(self, meeting_ids):
        ...
        """
        if (
            "meeting_ids_model" in self._context
            and "calendar_event_field" in self._context
        ):
            model, field = (
                self._context["meeting_ids_model"],
                self._context["calendar_event_field"],
            )
            meetings = self.env[model].browse(meeting_ids)[field]
            assert meetings._name == "calendar.event", _(
                "Model associated to 'many2manyattendee' widget should be 'calendar.event'"
            )
