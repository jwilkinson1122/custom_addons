...
class Followers(models.Model):
    ...
    mirror_follower_id = fields.Many2one(
        "mail.followers",
        string="Mirror Follower",
        help='As followers are replicated between "Event" and "Affiliate Event" models, '
    ...
    def create(self, values):
        ...
        if (
            values.get("res_model") in ("calendar.event", "affilpractice.event")
            and values.get("res_id")
            and not self.env.context.get("stop_propagation")
        ):
            # 'calendar.event' and 'affilpractice.event' models are "mirrored" (through delegation inheritance),
            ...
            if values["res_model"] == "calendar.event":
                mirrored_record = self.env["affilpractice.event"].search(
                    [("event_id", "=", values["res_id"])], limit=1
                )
            elif values["res_model"] == "affilpractice.event":
                mirrored_record = (
                    self.env["affilpractice.event"].browse(values["res_id"]).event_id
                )
            ...
            if mirrored_record:
                ...
                if not mirrored_follower:
                    ...
                    mirrored_follower = res.with_context(stop_propagation=True).copy(
                        default=values
                    )
                else:
    ...
    def write(self, values):
        ...
        if not self.env.context.get("stop_propagation") and values:
            self.mapped("mirror_follower_id").with_context(stop_propagation=True).write(
                values
            )
        return res
    ...
    def unlink(self):
        if not self.env.context.get("stop_propagation"):
            (self.mapped("mirror_follower_id") - self).with_context(
