...
class patient_evaluation(models.Model):
    ...
    _description = "pod patient evaluation"
    ...
    glycemia = fields.Float(
        string="Glycemia", help="Last blood glucose level. Can be approximative."
    )
    evaluation_summary = fields.Text(string="Evaluation Summary")
    temperature = fields.Float(
        string="Temperature (celsius)", help="Temperature in celcius"
    )
    osat = fields.Integer(
        string="Oxygen Saturation", help="Oxygen Saturation(arterial)."
    )
    bpm = fields.Integer(
        string="Heart Rate", help="Heart rate expressed in beats per minute"
    )
    ...
    diagnosis_id = fields.Many2one("pathology", "Presumptive Diagnosis")
    ldl = fields.Integer(
        string="Last LDL", help="Last LDL Cholesterol reading. Can be approximative"
    )
    visit_type = fields.Selection(
        [
            ("new", "New Health Condition"),
            ("follow", "FollowUp"),
            ("chronic", "Chronic Condition ChechUp"),
            ("child", "Well Child Visit"),
            ("women", "Well Woman Visit"),
            ("man", "Well Man Visit"),
        ],
        string="Visit",
    )
    urgency = fields.Selection(
        [("a", "Normal"), ("b", "Urgent"), ("c", "Podiatry Emergency")],
        string="Urgency",
    )
    ...
    signs_and_symptoms_ids = fields.One2many(
        "signs.and.sympotoms", "patient_evaluation_id", "Signs and Symptoms"
    )
    ...
    appointment_date_id = fields.Many2one("appointment", "Appointment Date")
    next_appointment_id = fields.Many2one("appointment", "Next Appointment")
    derived_from_physician_id = fields.Many2one("physician", "Derived from Doctor")
    derived_to_physician_id = fields.Many2one("physician", "Derived to Doctor")
    secondary_conditions_ids = fields.One2many(
        "secondary_condition", "patient_evaluation_id", "Secondary Conditions"
    )
    diagnostic_hypothesis_ids = fields.One2many(
        "diagnostic_hypotesis", "patient_evaluation_id", "Procedures"
    )
    procedure_ids = fields.One2many(
        "directions", "patient_evaluation_id", " Procedures "
    )
