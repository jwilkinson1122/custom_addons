...
class patient_psc(models.Model):
    _name = "patient.psc"
    _description = "pod patient psc"
    _rec_name = "patient_id"

    @api.model
    def default_get(self, fields):
        result = super(patient_psc, self).default_get(fields)
    ...
    psc_gets_hurt_2 = fields.Selection(
    ...
    @api.onchange(
        ...
        "psc_act_as_younger",
        "psc_acts_as_driven_by_motor",
        "psc_afraid_of_new_situations",
        "psc_blames_others",
        "psc_teases_others",
        "psc_daydreams_too_much",
        "psc_tires_easily",
        "psc_distracted_easily",
        "psc_trouble_concentrating",
        "psc_trouble_sleeping",
        "psc_trouble_with_teacher",
        "psc_visit_doctor_finds_ok",
        ...
        "psc_down_on_self",
        "psc_feels_hopeless",
        "psc_feels_is_bad_child",
        "psc_fidgety",
        "psc_fights_with_others",
        "psc_gets_hurt_often",
        "psc_having_less_fun",
        "psc_irritable_angry",
        "psc_less_interested_in_friends",
        "psc_less_interest_in_school",
        "psc_refuses_to_share",
        "psc_sad_unhappy",
        "psc_school_grades_dropping",
        "psc_spend_time_alone",
        "psc_takes_things_from_others",
        "psc_takes_unnecesary_risks",
    )
    def onchange_selections(self):
        self.psc_total = (
            int(self.psc_aches_pains)
            + int(self.psc_absent_from_school)
            + int(self.psc_act_as_younger)
            + int(self.psc_acts_as_driven_by_motor)
            + int(self.psc_afraid_of_new_situations)
            + int(self.psc_blames_others)
            + int(self.psc_teases_others)
            + int(self.psc_daydreams_too_much)
            + int(self.psc_tires_easily)
            + int(self.psc_distracted_easily)
            + int(self.psc_trouble_concentrating)
            + int(self.psc_trouble_sleeping)
            + int(self.psc_trouble_with_teacher)
            + int(self.psc_visit_doctor_finds_ok)
            + +int(self.psc_wants_to_be_with_parents)
            + int(self.psc_worries_a_lot)
            + int(self.psc_does_not_get_people_feelings)
            + int(self.psc_down_on_self)
            + int(self.psc_feels_hopeless)
            + int(self.psc_feels_is_bad_child)
            + int(self.psc_fidgety)
            + int(self.psc_fights_with_others)
            + int(self.psc_gets_hurt_often)
            + int(self.psc_having_less_fun)
            + int(self.psc_irritable_angry)
            + int(self.psc_less_interested_in_friends)
            + int(self.psc_less_interest_in_school)
            + int(self.psc_refuses_to_share)
            + int(self.psc_sad_unhappy)
            + int(self.psc_school_grades_dropping)
            + int(self.psc_spend_time_alone)
            + int(self.psc_takes_things_from_others)
