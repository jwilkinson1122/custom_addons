...
class patient(models.Model):
    ...
    def print_report(self):
        return self.env.ref("pod_mgmt_system.report_print_patient_card").report_action(
            self
        )
    ...
    current_insurance_id = fields.Many2one("insurance", string="Insurance")
    ...
    primary_care_physician_id = fields.Many2one(
        "physician", string="Primary Care Doctor"
    )
    patient_status = fields.Char(string="Hospitalization Status", readonly=True)
    patient_condition_ids = fields.One2many("patient.condition", "patient_id")
    patient_psc_ids = fields.One2many("patient.psc", "patient_id")
    ...
    cage_ids = fields.One2many("patient.cage", "patient_id")
    sex_oral = fields.Selection(
        [("0", "None"), ("1", "Active"), ("2", "Passive"), ("3", "Both")],
        string="Oral Sex",
    )
    ...
    motorcycle_rider = fields.Boolean(
        "Motorcycle Rider", help="The patient rides motorcycles"
    )
    helmet = fields.Boolean(
        "Uses helmet", help="The patient uses the proper motorcycle helmet"
    )
    traffic_laws = fields.Boolean(
        "Obeys Traffic Laws", help="Check if the patient is a safe driver"
    )
    ...
    car_seat_belt = fields.Boolean(
        "Seat belt", help="Safety measures when driving : safety belt"
    )
    ...
    menstrual_history_ids = fields.One2many("patient.menstrual.history", "patient_id")
    ...
    mammography_history_ids = fields.One2many(
        "patient.mammography.history", "patient_id"
    )
    pap_history_ids = fields.One2many("patient.pap.history", "patient_id")
    colposcopy_history_ids = fields.One2many("patient.colposcopy.history", "patient_id")
    ...
    pregnancy_history_ids = fields.One2many("patient.pregnency", "patient_id")
    family_history_ids = fields.Many2many(
        "family.condition", string="Family Condition Lines"
    )
    perinatal_ids = fields.Many2many("preinatal")
    ...
    housing = fields.Selection(
        ...
        "Housing conditions",
        help="Housing and sanitary living conditions",
        sort=False,
    )
    ...
    fam_apgar_timesharing = fields.Selection(
        ...
        "Time sharing",
        help="Is the patient satisfied with the level of time that they spend together ?",
        sort=False,
    )
    fam_apgar_affection = fields.Selection(
        ...
        "Family affection",
        help="Is the patient satisfied with the level of affection coming from the family ?",
        sort=False,
    )
    ...
    lab_test_ids = fields.One2many("patient.lab.test", "patient_id")
    fertile = fields.Boolean("Fertile")
    ...
    vaccination_ids = fields.One2many("vaccination", "patient_vaccines_id")
    appointments_ids = fields.One2many(
        "appointment", "patient_id", string="Appointments"
    )
    lastname = fields.Char("Last Name")
    report_date = fields.Date("Date", default=datetime.today().date())
    orthotic_ids = fields.One2many("patient.orthotic1", "patient_orthotic_id")
    ...
    def create(self, vals_list):
        ...
        for val in vals_list:
            ...
            if appointment:
                # Create a new partner record for the patient
                partner_vals = {
                    "name": self.env["res.partner"].browse(val["patient_id"]).name
                }
                new_partner = res_partner_obj.create(partner_vals)
                val.update({"patient_id": new_partner.id})
            ...
            if val.get("date_of_birth"):
                ...
                d2 = datetime.today().date()
                rd = relativedelta(d2, d1)
                age = f"{rd.years}y {rd.months}m {rd.days}d"
                val.update({"age": age})

            # Generate a sequence number for the patient
            patient_sequence = self.env["ir.sequence"].next_by_code("patient")
            if patient_sequence:
                val.update({"name": patient_sequence})

        # Use `super()` correctly without conflicting with the class name
            ...
            if rec.date_of_birth:
                if rec.deceased == True:
                    if rec.date_of_death <= rec.date_of_birth:
                        raise UserError(
                            _("Date Of Death Can Not Less Than Date Of Birth.")
                        )
