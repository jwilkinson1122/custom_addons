...
class insurance(models.Model):
    _name = "insurance"
    _description = "pod insurance"
    _rec_name = "insurance_compnay_id"

    number = fields.Char("Number")
    insurance_partner_id = fields.Many2one("res.partner", "Owner", required=True)
    ...
    insurance_compnay_id = fields.Many2one(
        ...
        string="Insurance Compnay",
    )
    category = fields.Char("Category")
    notes = fields.Text("Extra Info")
    member_exp = fields.Date("Expiration Date")
    insurance_plan_id = fields.Many2one("insurance.plan", "Plan")
