...
class appointment(models.Model):
    ...
    inpatient_registration_id = fields.Many2one(
        "inpatient.registration", string="Inpatient Registration"
    )
    ...
    doctor_id = fields.Many2one("physician", "Physician", required=True)
    ...
    patient_psc_ids = fields.Many2many(
        "patient.psc", string="Pediatrics Symptoms Checklist"
    )
    prescription_order_ids = fields.One2many(
        "prescription.order", "appointment_id", string="Prescription"
    )
    ...
    def create(self, vals_list):
        for vals in vals_list:
            vals["name"] = self.env["ir.sequence"].next_by_code("appointment") or "APT"
    ...
    def onchange_patient(self):
        if not self.inpatient_registration_id:
            self.patient_id = ""
        inpatient_obj = self.env["inpatient.registration"].browse(
    ...
    def create_invoice(self):
        lab_req_obj = self.env["appointment"]
