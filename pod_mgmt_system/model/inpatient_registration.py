...
class inpatient_registration(models.Model):
    _name = "inpatient.registration"
    _description = "Podiatry Inpatient Registration"

    name = fields.Char(
        string="Registration Code", copy=False, readonly=True, index=True
    )
    patient_id = fields.Many2one("patient", string="Patient", required=True)
    practiceization_date = fields.Datetime(string="Hospitalization date", required=True)
    discharge_date = fields.Datetime(string="Expected Discharge date", required=True)
    attending_physician_id = fields.Many2one("physician", string="Attending Physician")
    operating_physician_id = fields.Many2one("physician", string="Operating Physician")
    ...
    pathology_id = fields.Many2one("pathology", string="Reason for Admission")
    info = fields.Text(string="Extra Info")
    bed_transfers_ids = fields.One2many(
        "bed.transfer", "inpatient_id", string="Transfer Bed"
    )
    diet_belief_id = fields.Many2one("diet.belief", string="Belief")
    ...
    orthotic_ids = fields.One2many(
        "inpatient.orthotic", "inpatient_registration_id", string="Orthotic"
    )

    @api.model
    def default_get(self, fields):
        result = super(inpatient_registration, self).default_get(fields)
        patient_id = self.env["ir.sequence"].next_by_code("inpatient.registration")
        if patient_id:
            if "name" in fields:
                result.update(
                    {
