...
class patient_rounding(models.Model):
    ...
    _rec_name = "inpatient_registration"
    ...
    inpatient_registration = fields.Many2one(
        "inpatient.registration", string="Registration Code", required=True
    )
    ...
    evolution = fields.Selection(
        [("n", "Status Quo"), ("i", "Improving"), ("w", "Worsening")],
        string="Evolution",
    )
    round_summary = fields.Text(string="Round Summary")
    gcs = fields.Many2one("icu.glasgow", string="GCS")
    right_pupil = fields.Integer(string="R")
    pupillary_reactivity = fields.Selection(
        [("brisk", "Brisk"), ("sluggish", "Sluggish"), ("nonreactive", "Nonreactive")],
        string="Pupillary_Reactivity",
    )
    pupil_dilation = fields.Selection(
        [("normal", "Normanl"), ("miosis", "Miosis"), ("mydriasis", "Mydriasis")],
    ...
    chest_expansion = fields.Selection(
        [("symmentric", "Symmentrical"), ("asymmentric", "Asynmmentrical")],
        string="Expansion",
    )
    ...
    chest_drainages = fields.One2many(
        "icu.chest_drainage",
        "patient_rounding_chest_drainage_id",
        string="Chest Drainages",
    )
    ecg = fields.Many2one("icu.ecg", string="ECG")
    ...
    edema = fields.Selection(
        [("none", "None"), ("peripheral", "Peripheral"), ("anasarca", "Anasarca")],
        string="Edema",
    )
    ...
    vomiting = fields.Selection(
        [("none", "None"), ("vomiting", "Vomiting"), ("hematemesis", "Hematemesis ")],
        string="Vomiting",
    )
    ...
    procedures_ids = fields.One2many(
        "rounding_procedure",
        "patient_rounding_procedure_id",
        string="Procedures",
    )
    practice_location_id = fields.Many2one(
        "stock.location", string="Hospitalization Location"
    )
    medicaments_ids = fields.One2many(
        "patient.rounding.medicament",
        "patient_rounding_medicament_id",
        string="Medicaments",
    )
    supplies_ids = fields.One2many(
        "patient.rounding.supply",
        "patient_rounding_supply_id",
        string="Podiatry Supplier",
    )
    vaccines_ids = fields.One2many(
        "patient.rounding.vaccine",
        "patient_rounding_vaccine_id",
        string="Vaccines",
    )
