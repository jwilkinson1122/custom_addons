...
class health_services_invoice(models.TransientModel):
    _name = "health.service.invoice"
    _description = "pod health service invoice"

    def create_service_invoice(self):
        active_id = self._context.get("active_id")
        lab_req_obj = self.env["health_service"]
        ...
        invoice_vals = {
            "name": self.env["ir.sequence"].next_by_code("health_service_inv_seq"),
        ...
        for p_line in lab_req.service_line_ids:
            if p_line.to_invoice == True:
                ...
                taxes = p_line.product_id.taxes_id.filtered(
                    lambda r: not p_line.product_id.company_id
                    or r.company_id == p_line.product_id.company_id
                )
                tax_ids = taxes.ids
