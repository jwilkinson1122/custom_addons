...
class create_prescription_invoice(models.TransientModel):
    ...
    def create_prescription_invoice(self):
        ...
        lab_req_obj = self.env["prescription.order"]
            ...
            for p_line in lab_req.prescription_line_ids:
                ...
                if p_line.medicament_id.product_id.id:
                    invoice_line_account_id = (
                        p_line.medicament_id.product_id.property_account_income_id.id
                        or p_line.medicament_id.product_id.categ_id.property_account_income_categ_id.id
                        or False
                    )
                ...
                tax_ids = []
                taxes = p_line.medicament_id.product_id.taxes_id.filtered(
                    lambda r: not p_line.medicament_id.product_id.company_id
                    or r.company_id == p_line.medicament_id.product_id.company_id
                )
                tax_ids = taxes.ids
