...
class CompletePairOrder(models.TransientModel):
    ...
    frame = fields.Many2one(
        "product.product", string="Frames", domain="[('categ_id', '=', 'Frames')]"
    )
    lens = fields.Many2one(
        "product.product", string="Lens", domain="[('categ_id', '=', 'Lens')]"
    )
    contact_lens = fields.Many2one(
        "product.product",
        string="Contact Lens",
        domain="[('categ_id', '=', 'Contact Lens')]",
    )
    lens_treatment = fields.Many2one(
        "product.product",
        string="Lens Treatment",
        domain="[('categ_id', '=', 'Lens Treatment')]",
    )
    service = fields.Many2one(
        "product.product", string="Services", domain="[('categ_id', '=', 'Service')]"
    )
    ...
    def show_btn(self):
        ...
        section = (
            ...
            if self.env.context.get("is_complete_pair", False)
                ...
                "Contact Lens"
                if self.env.context.get("is_contact_lens", False)
                else (
                    "Frame Only"
                    if self.env.context.get("is_frame", False)
                    else (
                        "Lens Only"
                        if self.env.context.get("is_lens", False)
                        else (
                            "Service"
                            if self.env.context.get("is_services", False)
                            else (
                                "Miscellaneous"
