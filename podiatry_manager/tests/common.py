...
class TestPointOfSaleCommon(ValuationReconciliationTestCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.company_data['company'].write({
            'podiatry_manager_update_stock_quantities': 'real',
        })
        ...
        cls.pos_config.write({'payment_method_ids': [(4, cls.credit_payment_method.id), (
            4, cls.bank_payment_method.id), (4, cls.cash_payment_method.id)]})
...
class TestPoSCommon(ValuationReconciliationTestCommon):
    ...
    def create_ui_order_data(self, pos_order_lines_ui_args, customer=False, is_invoiced=False, payments=None, uid=None):
        ...
        def create_order_line(product, quantity, discount=0.0):
            ...
            tax_values = (
                tax_ids.compute_all(price_unit_after_discount,
                                    self.currency, quantity)
                if tax_ids
                else {
        ...
        total_amount_base = sum(line[2]['price_subtotal']
                                for line in order_lines)
        return {
            'data': {
    ...
    def _run_test(self, args):
        ...
        _logger.info(
            'DONE: Checks for journal entries before closing the session.')
        ...
        _logger.info(
            'DONE: Checks for journal entries after closing the session.')
        ...
        for uid in orders_map:
            ...
            if expected_values[uid].get('invoice'):
                self._assert_account_move(
                    invoice, expected_values[uid]['invoice'])
    ...
    def _assert_account_move(self, account_move, expected_account_move_vals):
        if expected_account_move_vals:
            ...
            self.assertRecordValues(account_move.line_ids.filtered(
                line_ids_predicate), expected_account_move_vals.pop('line_ids'))
