...
from odoo.addons.podiatry_manager.tests.common import TestPointOfSaleCommon
...
class TestPointOfSaleFlow(TestPointOfSaleCommon):
    ...
    def test_order_refund(self):
        ...
        self.assertEqual(refund.state, 'paid',
                         "The refund is not marked as paid")
    ...
    def test_order_refund_lots(self):
        ...
        refund_lot_id = [
            lot_id.lot_name for lot_id in refund.lines.pack_lot_ids]
        self.assertEqual(
            order_lot_id,
            refund_lot_id,
    ...
    def test_order_to_invoice(self):
        ...
        context_make_payment = {"active_ids": [
            self.pos_order_pos1.id], "active_id": self.pos_order_pos1.id}
        ...
        self.assertEqual(self.pos_order_pos1.state, 'paid',
                         "Order should be in paid state.")
        self.assertFalse(self.pos_order_pos1.account_move,
        ...
        """In order to test the reports on Bank Statement defined in podiatry_manager module, I create a bank statement line, confirm it and print the reports"""
    ...
    def test_create_from_ui(self):
        ...
        self.assertEqual(num_starting_orders + 1,
                         len(current_session.order_ids), "Submitted order not encoded")
        ...
        with mute_logger('odoo.addons.podiatry_manager.models.pos_order'):
        ...
        self.assertEqual(len(rescue_session), 1,
                         "One (and only one) rescue session should be created for orphan orders")
        self.assertIn("(RESCUE FOR %s)" % current_session.name, rescue_session.name,
        ...
        total_cash_payment = sum(rescue_session.mapped('order_ids.payment_ids').filtered(
            lambda payment: payment.payment_method_id.type == 'cash').mapped('amount'))
    ...
    def test_order_to_payment_currency(self):
        ...
        context_make_payment = {"active_ids": [
            self.pos_order_pos0.id], "active_id": self.pos_order_pos0.id}
        ...
        self.assertEqual(self.pos_order_pos0.state, 'draft',
                         'Order should be in draft state.')
