...
class TestPointOfSaleHttpCommon(AccountTestInvoicingHttpCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        env = cls.env
        cls.env.user.groups_id += env.ref('podiatry_manager.group_pos_manager')
        ...
        env['ir.property'].sudo().search(
            [('name', '=', 'property_product_pricelist')]).unlink()
        ...
        discount = env.ref('podiatry_manager.product_product_consumable')
        cls.tip = env.ref('podiatry_manager.product_product_tip')
        (all_pos_product - discount - cls.tip)._write({'active': False})

        # In DESKS categ: Desk Pad
        pos_categ_desks = env.ref('podiatry_manager.pos_category_desks')

        # In DESKS categ: Whiteboard Pen
        pos_categ_misc = env.ref('podiatry_manager.pos_category_miscellaneous')

        # In CHAIR categ: Letter Tray
        pos_categ_chairs = env.ref('podiatry_manager.pos_category_chairs')
        ...
        env['product.pricelist'].create({
            ...
            'item_ids': [(0, 0, {
                ...
                'applied_on': '0_product_variant',
                'min_quantity': 2,
                'product_id': env.ref('podiatry_manager.product_product_consumable').id,
            })],
        })
...
class TestUi(TestPointOfSaleHttpCommon):
    def test_01_pos_basic_order(self):
        ...
        self.env['ir.module.module'].search(
            [('name', '=', 'podiatry_manager')], limit=1).state = 'installed'

        self.start_tour("/pos/ui?config_id=%d" %
                        self.main_pos_config.id, 'pos_pricelist', login="accountman")
        self.start_tour("/pos/ui?config_id=%d" %
                        self.main_pos_config.id, 'pos_basic_order', login="accountman")
        self.start_tour("/pos/ui?config_id=%d" % self.main_pos_config.id,
                        'ProductScreenTour', login="accountman")
        self.start_tour("/pos/ui?config_id=%d" % self.main_pos_config.id,
                        'PaymentScreenTour', login="accountman")
        self.start_tour("/pos/ui?config_id=%d" % self.main_pos_config.id,
                        'ReceiptScreenTour', login="accountman")

        for order in self.env['pos.order'].search([]):
            self.assertEqual(order.state, 'paid', "Validated order has payment of " + str(
                order.amount_paid) + " and total of " + str(order.amount_total))
    ...
    def test_02_pos_with_invoiced(self):
        self.main_pos_config.open_session_cb(check_coa=False)
        self.start_tour("/pos/ui?config_id=%d" %
                        self.main_pos_config.id, 'ChromeTour', login="accountman")
        n_invoiced = self.env['pos.order'].search_count(
            [('state', '=', 'invoiced')])
    ...
    def test_04_product_configurator(self):
        ...
        self.main_pos_config.open_session_cb(check_coa=False)
        self.start_tour("/pos/ui?config_id=%d" % self.main_pos_config,
                        'ProductConfiguratorTour', login="accountman")

    def test_05_ticket_screen(self):
        self.main_pos_config.open_session_cb(check_coa=False)
        self.start_tour("/pos/ui?config_id=%d" % self.main_pos_config.id,
                        'TicketScreenTour', login="accountman")
    ...
    def test_fixed_tax_negative_qty(self):
        ...
        self.main_pos_config.open_session_cb(check_coa=False)
        self.start_tour("/pos/ui?config_id=%d" % self.main_pos_config.id,
                        'FixedTaxNegativeQty', login="accountman")
        ...
        self.assertEqual(
            lines[0].account_id, bank_pm.receivable_account_id or self.env.company.account_default_pos_receivable_account_id)
