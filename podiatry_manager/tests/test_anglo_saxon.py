...
class TestAngloSaxonCommon(common.TransactionCase):
    ...
    def setUp(self):
        ...
        self.pos_config = self.env.ref('podiatry_manager.pos_config_main')
        ...
        self.company.podiatry_manager_update_stock_quantities = 'real'
...
class TestAngloSaxonFlow(TestAngloSaxonCommon):
    ...
    def test_create_account_move_line(self):
        ...
        self.assertEqual(self.pos_order_pos0.amount_paid, 450,
        ...
        self.assertEqual(current_session_id.state, 'closed',
        ...
        self.assertFalse(self.pos_order_pos0.account_move,
                         'There should be no invoice in the order.')
        ...
        aml_output = aml.filtered(
            lambda l: l.account_id.id == account_output.id)
        aml_expense = aml.filtered(
            lambda l: l.account_id.id == expense_account.id)
    ...
    def _prepare_pos_order(self):
        ...
        self.assertEqual(self.product.value_svl, 30,
                         "Value should be (5*5 + 5*1) = 30")
    ...
    def test_fifo_valuation_no_invoice(self):
        ...
        context_make_payment = {"active_ids": [
            pos_order_pos0.id], "active_id": pos_order_pos0.id}
        self.pos_make_payment_0 = self.PosMakePayment.with_context(context_make_payment).create({
            'amount': 7 * 450.0,
            'payment_method_id': self.cash_payment_method.id,
        })
    ...
    def test_fifo_valuation_with_invoice(self):
        ...
        line = pos_order_pos0.account_move.line_ids.filtered(
            lambda l: l.debit and l.account_id == self.category.property_account_expense_categ_id)
        self.assertEqual(pos_order_pos0.account_move.journal_id,
                         self.pos_config.invoice_journal_id)
        self.assertEqual(
            line.debit, 27, 'As it is a fifo product, the move\'s value should be 5*5 + 2*1')
