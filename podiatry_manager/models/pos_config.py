...
class PosConfig(models.Model):
    ...
    def _get_group_pos_manager(self):
        return self.env.ref('podiatry_manager.group_pos_manager')
    ...
    iface_scan_via_proxy = fields.Boolean(
        string='Scan via Proxy', help="Enable barcode scanning with a remotely connected barcode scanner and card swiping with a Vantiv card reader.")
    ...
    iface_orderline_customer_notes = fields.Boolean(
        string='Customer Notes', help='Allow to write notes for customer on Orderlines. This will be shown in the receipt.')
    ...
    set_maximum_difference = fields.Boolean(
        'Set Maximum Difference', help="Set a maximum difference allowed between the expected and counted money during the closing of the session.")
    ...
    session_ids = fields.One2many(
        'pos.session', 'config_id', string='Sessions')
    ...
    payment_method_ids = fields.Many2many(
        'pos.payment.method', string='Payment Methods', default=lambda self: self._default_payment_methods())
    ...
    rounding_method = fields.Many2one(
        'account.cash.rounding', string="Cash rounding")
    cash_rounding = fields.Boolean(string="Cash Rounding")
    only_round_cash_method = fields.Boolean(
    ...
    def _compute_allowed_pricelist_ids(self):
        for config in self:
            if config.use_pricelist:
                ...
                config.allowed_pricelist_ids = self.env['product.pricelist'].search([
                ]).ids
    ...
    def _compute_last_session(self):
        ...
        for pos_config in self:
            ...
            if session:
                ...
                pos_config.last_session_closing_date = session[0]['stop_at'].astimezone(
                    timezone).date()
    ...
    def _compute_current_session_user(self):
        for pos_config in self:
            session = pos_config.session_ids.filtered(lambda s: s.state in [
                                                      'opening_control', 'opened', 'closing_control'] and not s.rescue)
            if session:
                pos_config.pos_session_username = session[0].user_id.sudo(
                ).name
        ...
        for config in self:
            if config.journal_id and config.journal_id.company_id.id != config.company_id.id:
                raise ValidationError(
                    _("The sales journal of the point of sale %s must belong to its company.", config.name))
    ...
    def _check_company_invoice_journal(self):
        for config in self:
            if config.invoice_journal_id and config.invoice_journal_id.company_id.id != config.company_id.id:
                raise ValidationError(
                    _("The invoice journal of the point of sale %s must belong to the same company.", config.name))
    ...
    def _onchange_tipproduct(self):
        if self.iface_tipproduct:
            self.tip_product_id = self.env.ref(
                'podiatry_manager.product_product_tip', False)
        else:
    ...
    def create(self, values):
        ...
        val.update(name=_('POS order line %s',
                   values['name']), code='pos.order.line')
    ...
    def write(self, vals):
        ...
        if opened_session:
            ...
            for key in self._get_forbidden_change_fields():
                if key in vals.keys():
                    field_name = self._fields[key].get_description(self.env)[
                        "string"]
    ...
    def _check_modules_to_install(self):
        ...
        if expected:
            ...
            modules = modules.filtered(
                lambda module: module.state not in STATES)
            if modules:
    ...
    def _force_http(self):
        enforce_https = self.env['ir.config_parameter'].sudo(
        ).get_param('podiatry_manager.enforce_https')
    ...
    def _open_session(self, session_id):
        # The pricelist company might have changed after the first opening of the session
        self._check_pricelists()
        return {
            'name': _('Session'),
            'view_mode': 'form,tree',
