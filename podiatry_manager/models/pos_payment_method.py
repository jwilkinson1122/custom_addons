...
class PosPaymentMethod(models.Model):
    ...
    _description = "Sale Payment Methods"
    _order = "id asc"
    ...
    name = fields.Char(string="Method", required=True, translate=True,
                       help='Defines the name of the payment method that will be displayed in the Sale when the payments are selected.')
    ...
    receivable_account_id = fields.Many2one('account.account',
                                            ...
                                            help="Leave empty to use the default account from the company setting.\n"
                                            "Overrides the company's receivable account (for Sale) used in the journal entries.")
    ...
    open_session_ids = fields.Many2many('pos.session', string='Pos Sessions',
                                        compute='_compute_open_session_ids', help='Open PoS sessions that are using this payment method.')
    config_ids = fields.Many2many('pos.config', string='Sale Configurations')
    ...
    type = fields.Selection(selection=[('cash', 'Cash'), ('bank', 'Bank'), (
        'pay_later', 'Customer Account')], compute="_compute_type")
