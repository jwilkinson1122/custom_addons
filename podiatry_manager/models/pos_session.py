...
class PosSession(models.Model):
    ...
    cash_real_transaction = fields.Monetary(
        string='Transaction', readonly=True)
    cash_real_expected = fields.Monetary(string="Expected", readonly=True)

    order_ids = fields.One2many('pos.order', 'session_id',  string='Orders')
    order_count = fields.Integer(compute='_compute_order_count')
    statement_ids = fields.One2many(
        'account.bank.statement', 'pos_session_id', string='Cash Statements', readonly=True)
    ...
    def _compute_cash_balance(self):
        for session in self:
            ...
            if cash_payment_method:
                ...
                result = self.env['pos.payment'].read_group([('session_id', '=', session.id), (
    ...
    def _compute_total_payments_amount(self):
        ...
        for session in self:
            session.total_payments_amount = session_amount_map.get(
                session.id) or 0
    ...
    def _compute_order_count(self):
        ...
        sessions_data = {
            order_data['session_id'][0]: order_data['session_id_count'] for order_data in orders_data}
        for session in self:
    ...
    def _compute_picking_count(self):
        for session in self:
            ...
            session.failed_pickings = bool(self.env['stock.picking'].search(
    ...
    def _compute_cash_all(self):
        # Only one cash register is supported by podiatry_manager.
        for session in self:
            session.cash_journal_id = session.cash_register_id = session.cash_control = False
            cash_payment_methods = session.payment_method_ids.filtered(
                'is_cash_count')
    ...
    def create(self, values):
        ...
        if self.user_has_groups('podiatry_manager.group_pos_user'):
        ...
        update_stock_at_closing = pos_config.company_id.podiatry_manager_update_stock_quantities == "closing"
        ...
        if self.user_has_groups('podiatry_manager.group_pos_user'):
            res = super(PosSession, self.with_context(
                ctx).sudo()).create(values)
        else:
    ...
    def _validate_session(self, balancing_account=False, amount_to_balance=0, bank_payment_method_diffs=None):
        bank_payment_method_diffs = bank_payment_method_diffs or {}
        self.ensure_one()
        sudo = self.user_has_groups('podiatry_manager.group_pos_user')
        if self.order_ids or self.statement_ids.line_ids:
            ...
            try:
                ...
                if sudo:
                    data = self.sudo().with_company(self.company_id)._create_account_move(
                        balancing_account, amount_to_balance, bank_payment_method_diffs)
                else:
                    raise e
            ...
            if self.move_id.line_ids:
                ...
                self.env['pos.order'].search(
                    [('session_id', '=', self.id), ('state', '=', 'paid')]).write({'state': 'done'})
            else:
    ...
    def _create_diff_account_move_for_split_payment_method(self, payment_method, diff_amount):
        ...
        get_diff_vals_result = self._get_diff_vals(
            payment_method.id, diff_amount)
    ...
    def _get_diff_vals(self, payment_method_id, diff_amount):
        ...
        amounts = self._update_amounts({'amount': 0, 'amount_converted': 0}, {
                                       'amount': diff_amount}, self.stop_at)
    ...
    def get_closing_control_data(self):
        ...
        other_payment_method_ids = self.payment_method_ids - \
            default_cash_payment_method_id if default_cash_payment_method_id else self.payment_method_ids
        cash_in_count = 0
        cash_out_count = 0
        cash_in_out_list = []
        ...
        return {
            ...
            'is_manager': self.user_has_groups("podiatry_manager.group_pos_manager"),
        ...
        for order in self.order_ids:
            ...
            if not order_is_invoiced:
                ...
                for order_line in order.lines:
                    ...
                    # Combine tax lines
                    for tax in line['taxes']:
                        tax_key = (tax['account_id'] or line['income_account_id'],
                                   tax['tax_repartition_line_id'], tax['id'], tuple(tax['tag_ids']))
    ...
    def _create_non_reconciliable_move_lines(self, data):
        ...
        tax_vals = [self._get_tax_vals(key, amounts['amount'], amounts['amount_converted'],
                                       amounts['base_amount_converted']) for key, amounts in taxes.items()]
        # Check if all taxes lines have account_id assigned. If not, there are repartition lines of the tax that have no account_id.
        tax_names_no_account = [line['name']
                                for line in tax_vals if line['account_id'] == False]
        ...
        if not float_is_zero(rounding_difference['amount'], precision_rounding=self.currency_id.rounding) or not float_is_zero(rounding_difference['amount_converted'], precision_rounding=self.currency_id.rounding):
            rounding_vals = [self._get_rounding_difference_vals(
    ...
    def _create_bank_payment_moves(self, data):
        ...
        for payment_method, amounts in combine_receivables_bank.items():
            combine_receivable_line = MoveLine.create(self._get_combine_receivable_vals(
        ...
        for payment, amounts in split_receivables_bank.items():
            split_receivable_line = MoveLine.create(self._get_split_receivable_vals(
    ...
    def _create_pay_later_receivable_lines(self, data):
        MoveLine = data.get('MoveLine')
        combine_receivables_pay_later = data.get(
        ...
        for payment_method, amounts in combine_receivables_pay_later.items():
            vals.append(self._get_combine_receivable_vals(
                payment_method, amounts['amount'], amounts['amount_converted']))
        for payment, amounts in split_receivables_pay_later.items():
            vals.append(self._get_split_receivable_vals(
                payment, amounts['amount'], amounts['amount_converted']))
    ...
    def _apply_diff_on_account_payment_move(self, account_payment, payment_method, diff_amount):
        source_vals, dest_vals = self._get_diff_vals(
            payment_method.id, diff_amount)
        ...
        new_balance_compare_to_zero = self.currency_id.compare_amounts(
            new_balance, 0)
    ...
    def _create_cash_statement_lines_and_cash_move_lines(self, data):
        ...
        for payment, amounts in split_receivables_cash.items():
            ...
            split_cash_receivable_vals[statement].append(self._get_split_receivable_vals(
                payment, amounts['amount'], amounts['amount_converted']))
        ...
        for payment_method, amounts in combine_receivables_cash.items():
            if not float_is_zero(amounts['amount'], precision_rounding=self.currency_id.rounding):
                ...
                combine_cash_receivable_vals[statement].append(self._get_combine_receivable_vals(
                    payment_method, amounts['amount'], amounts['amount_converted']))
        ...
        for statement in self.statement_ids:
            split_cash_statement_lines[statement] = BankStatementLine.create(split_cash_statement_line_vals[statement]).mapped(
                'move_id.line_ids').filtered(lambda line: line.account_id.internal_type == 'receivable')
            combine_cash_statement_lines[statement] = BankStatementLine.create(combine_cash_statement_line_vals[statement]).mapped(
                'move_id.line_ids').filtered(lambda line: line.account_id.internal_type == 'receivable')
    ...
    def _create_invoice_receivable_lines(self, data):
        ...
        data.update(
            {'split_invoice_receivable_lines': split_invoice_receivable_lines})
        return data
    ...
    def _create_stock_output_lines(self, data):
        ...
        for stock_moves in [stock_output, stock_return]:
            for account, amounts in stock_moves.items():
                stock_output_vals[account].append(self._get_stock_output_vals(
    ...
    def _reconcile_account_move_lines(self, data):
        ...
        for statement in self.statement_ids:
            ...
            lines_by_account = [all_lines.filtered(
                lambda l: l.account_id == account and not l.reconciled) for account in accounts if account.reconcile]
        ...
        if self.company_id.account_default_pos_receivable_account_id.reconcile:
            ...
            for payment in split_inv_payment_receivable_lines:
                lines = split_inv_payment_receivable_lines[payment] | split_invoice_receivable_lines.get(
    ...
    def _prepare_line(self, order_line):
        ...
        def get_income_account(order_line):
            ...
            income_account = product.with_company(
                order_line.company_id)._get_product_accounts()['income']
        ...
        def check_refund(x): return x.qty * x.price_unit < 0
        is_refund = check_refund(order_line)
        tax_data = tax_ids.with_context(force_sign=sign).compute_all(price_unit=price, quantity=abs(
            order_line.qty), currency=self.currency_id, is_refund=is_refund)
    ...
    def _get_sale_vals(self, key, amount, amount_converted):
        ...
        if applied_taxes:
            name = '%s with %s' % (title, ', '.join(
                [tax.name for tax in applied_taxes]))
        partial_vals = {
            'name': name,
    ...
    def _get_stock_output_vals(self, out_account, amount, amount_converted):
        partial_args = {'account_id': out_account.id,
                        'move_id': self.move_id.id}
    ...
    def _update_amounts(self, old_amounts, amounts_to_add, date, round=True, force_company_currency=False):
        ...
        if not amounts_to_add.get('base_amount') == None:
            ...
            if self.is_in_company_currency or force_company_currency:
                ...
                base_amount_converted = self._amount_converter(
                    base_amount, date, round)
    ...
    def _get_other_related_moves(self):
        ...
        diff_lines_ref = [self._get_diff_account_move_ref(
            pm) for pm in self.payment_method_ids if pm.type == 'bank' and pm.split_transactions]
    ...
    def action_view_order(self):
        return {
            ...
            'view_mode': 'tree,form',
            'views': [
                (self.env.ref('podiatry_manager.view_pos_order_tree_no_session_id').id, 'tree'),
                (self.env.ref('podiatry_manager.view_pos_pos_form').id, 'form'),
            ],
            'type': 'ir.actions.act_window',
    ...
    def _alert_old_session(self):
        ...
        sessions = self.sudo().search([('start_at', '<=', (fields.datetime.now(
        ) - timedelta(days=7))), ('state', '!=', 'closed')])
        for session in sessions:
            if self.env['mail.activity'].search_count([('res_id', '=', session.id), ('res_model', '=', 'pos.session')]) == 0:
                session.activity_schedule(
                    'podiatry_manager.mail_activity_old_session',
    ...
    def _check_if_no_draft_orders(self):
        draft_orders = self.order_ids.filtered(
    ...
    def try_cash_in_out(self, _type, amount, reason, extras):
        ...
        self.env['cash.box.out']\
            ...
            .run()
        message_content = [
            f"Cash {extras['translatedType']}", f'- Amount: {extras["formattedAmount"]}']
        if reason:
