...
class PosController(http.Controller):
    ...
    def pos_web(self, config_id=False, **k):
        ...
        if not pos_session:
            return request.redirect('/web#action=podiatry_manager.action_client_pos_menu')
        ...
        response = request.render('podiatry_manager.index', context)
    ...
    def test_suite(self, mod=None, **kwargs):
        ...
        return request.render('podiatry_manager.qunit_suite', qcontext=context)

    @http.route('/pos/sale_details_report', type='http', auth='user')
    def print_sale_details(self, date_start=False, date_stop=False, **kw):
        r = request.env['report.podiatry_manager.report_saledetails']
        pdf, _ = request.env.ref('podiatry_manager.sale_details_report').with_context(
            date_start=date_start, date_stop=date_stop)._render_qweb_pdf(r)
    ...
    def load_onboarding_data(self):
        convert.convert_file(request.env.cr, 'podiatry_manager',
                             'data/podiatry_manager_onboarding.xml', None, mode='init', kind='data')
