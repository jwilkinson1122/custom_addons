odoo.define('podiatry_manager.Chrome', function(require) {
    'use strict';
    ...
    const PopupControllerMixin = require('podiatry_manager.PopupControllerMixin');
    const Registries = require('podiatry_manager.Registries');
    const IndependentToOrderScreen = require('podiatry_manager.IndependentToOrderScreen');
    const contexts = require('podiatry_manager.PosContext');
    const { identifyError, posbus } = require('podiatry_manager.utils');
    ...
    const models = require('podiatry_manager.models');
    ...
    class Chrome extends PopupControllerMixin(PosComponent) {
        ...
        async _closePos() {
            ...
            if (!this.env.pos || this.env.pos.db.get_orders().length === 0) {
                window.location = '/web#action=podiatry_manager.action_client_pos_menu';
            }
        ...
        _onPlaySound({ detail: name }) {
            let src;
            if (name === 'error') {
                src = "/podiatry_manager/static/src/sounds/error.wav";
            } else if (name === 'bell') {
                src = "/podiatry_manager/static/src/sounds/bell.wav";
            }
        ...
        _preloadImages() {
            ...
            for (let imageName of staticImages) {
                const image = new Image();
                image.src = `/podiatry_manager/static/src/img/${imageName}`;
            }
        }
        ...
        _buildChrome() {
            if ($.browser.chrome) {
                ...
                if (parseInt(chrome_version, 10) >= 50) {
                    loadCSS('/podiatry_manager/static/src/css/chrome50.css');
                }
            }
