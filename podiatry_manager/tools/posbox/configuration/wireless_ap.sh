...
if [ -z "${WIRED_IP}" ] ; then
	...
	if [ -f "${WIFI_NETWORK_FILE}" ] && [ -z "${FORCE_HOST_AP}" ] ; then
		...
		/home/pi/odoo/addons/podiatry_manager/tools/posbox/configuration/connect_to_wifi.sh &
	else
		...
		nginx -c /home/pi/odoo/addons/podiatry_manager/tools/posbox/configuration/nginx_ap.conf
