...
class ClinicRegister(models.Model):
    ...
    patient_id = fields.Many2one('res.partner', change_default=True, string='Patient', domain=[
                                 ('patient', '!=', '/')], store=True)
    poly_id = fields.Many2one('clinic.poly', string='Poly Destination')
    date = fields.Datetime(
        string='Date', default=lambda self: fields.datetime.now())  # using lambda
    ...
    def create(self, vals):
        vals['name'] = self.env['ir.sequence'].next_by_code(
            'clinic.register.sequence')  # search for DFT and %(y)s , 5
        return super(ClinicRegister, self).create(vals)

    def name_get(self):
        result = []
        for me_id in self:
            result.append((me_id.id, "%s - %s" %
                          (me_id.name, me_id.patient_id.name)))
        return result
    ...
    def create_patient(self):
        view_id = self.env.ref('odoo_clinic.clinic_patient_form_view')
        return {
            'name': 'Create Patient',
    ...
    def action_cancel(self):
        for me_id in self:
            ...
            if checkup_ids:
                checkup_names = [unicodedata.normalize('NFKD', checkup.name).encode(
                    'ascii', 'ignore') for checkup in checkup_ids]
                raise Warning("Please cancel %s checkup first!" %
                              checkup_names)
