...
class res_partner(models.Model):
    ...
    patient_history = fields.One2many(
        'clinic.checkup', 'patient_id', 'Patient History')

    _sql_constraints = [
        ('unique_patient', 'unique(patient,name)',
         'Combination of code and name has been recorded, please check again!'),
    ]  # check again
...
class res_partner_doctor(models.Model):
    ...
    doctor_history = fields.One2many(
        'clinic.checkup', 'doctor_id', 'Doctor History')

    _sql_constraints = [
        ('unique_doctor', 'unique(doctor,name)',
         'Combination of code and name has been recorded, please check again!'),
    ]  # check again

    @api.model
    def create(self, vals):
        vals['doctor'] = self.env['ir.sequence'].next_by_code(
            'clinic.doctor.sequence')
