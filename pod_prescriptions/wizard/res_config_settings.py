...
class ResConfigSettings(models.TransientModel):
    ...
    group_auto_done_setting = fields.Boolean(
        string="Lock Confirmed Prescriptions", implied_group='pod_prescriptions.group_auto_done_setting')
    # group_proforma_prescriptions = fields.Boolean(
    #     string="Pro-Forma Invoice", implied_group='pod_prescriptions.group_proforma_prescriptions',
    #     help="Allows you to send pro-forma invoice.")
    group_warning_prescription = fields.Boolean(
        string="Prescription Order Warnings", implied_group='pod_prescriptions.group_warning_prescription')
    ...
    automatic_invoice = fields.Boolean(
        ...
        config_parameter='pod_prescriptions.automatic_invoice',
    )
    deposit_default_product_id = fields.Many2one(
        related='company_id.prescriptions_down_payment_product_id',
        readonly=False,
        # previously config_parameter='pod_prescriptions.default_deposit_product_id',
    )
    ...
    invoice_mail_template_id = fields.Many2one(
        ...
        config_parameter='pod_prescriptions.default_invoice_email_template',
    ...
    # Modules
    module_prescriptions_product_matrix = fields.Boolean("Prescriptions Grid Entry")
    ...
    module_prescriptions_amazon = fields.Boolean("Amazon Sync")
    module_prescriptions_loyalty = fields.Boolean("Coupons & Loyalty")
    module_prescriptions_margin = fields.Boolean("Margins")
    module_prescriptions_pdf_quote_builder = fields.Boolean("PDF Quote builder")
    ...
    def _onchange_group_product_variant(self):
        ...
        if self.module_prescriptions_product_matrix and not self.group_product_variant:
            self.module_prescriptions_product_matrix = False
    ...
    def set_values(self):
        ...
        if self.default_invoice_policy != 'order':
            self.env['ir.config_parameter'].set_param('pod_prescriptions.automatic_invoice', False)
