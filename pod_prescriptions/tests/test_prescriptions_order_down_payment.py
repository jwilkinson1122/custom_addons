...
from .common import TestPrescriptionCommon


@tagged('post_install', '-at_install')
class TestPrescriptionOrderDownPayment(TestPrescriptionCommon):

    @classmethod
    def setUpClass(cls, chart_template_ref=None):
        super().setUpClass(chart_template_ref=chart_template_ref)

        PrescriptionOrder = cls.env['prescriptions.order'].with_context(tracking_disable=True)
        ...
        # create a generic Prescription Order with all classical products and empty pricelist
        cls.prescriptions_order = PrescriptionOrder.create({
            'partner_id': cls.partner_a.id,
            'partner_invoice_id': cls.partner_a.id,
            'partner_shipping_id': cls.partner_a.id,
            'pricelist_id': cls.company_data['default_pricelist'].id,
        })
        cls.rxl_product_order = cls.env['prescriptions.order.line'].create({
            'name': cls.company_data['product_order_no'].name,
            'product_id': cls.company_data['product_order_no'].id,
            'product_uom_qty': 2,
            'product_uom': cls.company_data['product_order_no'].uom_id.id,
            'price_unit': 100,
            'order_id': cls.prescriptions_order.id,
            'tax_id': False,
        })
        cls.rxl_serv_deliver = cls.env['prescriptions.order.line'].create({
        ...
        cls.rxl_serv_order = cls.env['prescriptions.order.line'].create({
        ...
        cls.rxl_product_deliver = cls.env['prescriptions.order.line'].create({
    ...
    def make_downpayment(cls, **kwargs):
        ...
        downpayment = cls.env['prescriptions.advance.payment.inv'].with_context(rx_context).create(payment_params)
    ...
    def test_tax_breakdown(self):
        self.prescriptions_order.order_line[0].tax_id = self.tax_15 + self.tax_10
        self.prescriptions_order.order_line[1].tax_id = self.tax_10
        self.prescriptions_order.order_line[2].tax_id = self.tax_10
        self.make_downpayment()
        invoice = self.prescriptions_order.invoice_ids
        down_pay_amt = self.prescriptions_order.amount_total / 2
    ...
    def test_tax_breakdown_other_currency(self):
        self.prescriptions_order.currency_id = self.currency_data['currency']  # rate = 2.0
        self.prescriptions_order.order_line[0].tax_id = self.tax_15 + self.tax_10
        self.prescriptions_order.order_line[1].tax_id = self.tax_10
        self.prescriptions_order.order_line[2].tax_id = self.tax_10
        self.make_downpayment()
    ...
    def test_tax_breakdown_fixed_payment_method_with_taxes_on_all_lines(self):
        self.prescriptions_order.order_line[0].tax_id = self.tax_15
        self.prescriptions_order.order_line[1].tax_id = self.tax_10
        self.prescriptions_order.order_line[2].tax_id = self.tax_10
        self.prescriptions_order.order_line[3].tax_id = self.tax_10
        self.make_downpayment(advance_payment_method='fixed', fixed_amount=222.5, amount=0)
        invoice = self.prescriptions_order.invoice_ids
        down_pay_amt = 222.5
        # pylint: disable=C0326
    ...
    def test_tax_price_include_breakdown(self):
        ...
        self.prescriptions_order.order_line[0].tax_id = tax_10_incl + self.tax_10
        self.prescriptions_order.order_line[1].tax_id = self.tax_10
        self.prescriptions_order.order_line[2].tax_id = self.tax_10
        self.make_downpayment()
        invoice = self.prescriptions_order.invoice_ids
    ...
    def test_tax_price_include_include_base_amount_breakdown(self):
        ...
        self.prescriptions_order.order_line[0].tax_id = tax_10_pi_ba + self.tax_10
        self.prescriptions_order.order_line[1].tax_id = self.tax_10
        self.prescriptions_order.order_line[2].tax_id = self.tax_10
        self.make_downpayment()
        invoice = self.prescriptions_order.invoice_ids
    ...
    def test_tax_breakdown_with_discount(self):
        self.prescriptions_order.order_line[0].tax_id = self.tax_10
        self.prescriptions_order.order_line[1].tax_id = self.tax_10
        self.prescriptions_order.order_line[1].discount = 25.0
        self.prescriptions_order.order_line[2].tax_id = self.tax_15
        self.make_downpayment()
        invoice = self.prescriptions_order.invoice_ids
        down_pay_amt = self.prescriptions_order.amount_total / 2
        # pylint: disable=C0326
    ...
    def test_tax_price_include_include_base_amount_breakdown_with_discount(self):
        ...
        self.tax_10.sequence = 2
        self.prescriptions_order.order_line[0].tax_id = tax_10_pi_ba + self.tax_10
        self.prescriptions_order.order_line[0].discount = 25.0
        self.prescriptions_order.order_line[1].tax_id = self.tax_10
        self.prescriptions_order.order_line[2].tax_id = self.tax_10
        self.make_downpayment()
        invoice = self.prescriptions_order.invoice_ids
    ...
    def test_tax_fixed_amount_breakdown(self):
        ...
        self.prescriptions_order.order_line[0].tax_id = tax_group_1
        self.prescriptions_order.order_line[1].tax_id = tax_group_2
        self.prescriptions_order.order_line[2].tax_id = tax_10_a
        self.make_downpayment()
        invoice = self.prescriptions_order.invoice_ids
        down_pay_amt = self.prescriptions_order.amount_total / 2
        # pylint: disable=C0326
    ...
    def test_analytic_distribution(self):
        ...
        self.prescriptions_order.order_line[0].tax_id = self.tax_15 + self.tax_10
        self.prescriptions_order.order_line[0].analytic_distribution = {an_acc_01: 100}
        self.prescriptions_order.order_line[1].tax_id = self.tax_10
        self.prescriptions_order.order_line[1].analytic_distribution = {an_acc_01: 50, an_acc_02: 50}
        self.prescriptions_order.order_line[2].tax_id = self.tax_10
        self.prescriptions_order.order_line[2].analytic_distribution = {an_acc_01: 100}
        self.make_downpayment()
        invoice = self.prescriptions_order.invoice_ids
        down_pay_amt = self.prescriptions_order.amount_total / 2
        # pylint: disable=C0326
    ...
    def test_tax_fixed_amount_analytic_distribution(self):
        ...
        self.prescriptions_order.order_line[0].tax_id = tax_group_1
        self.prescriptions_order.order_line[0].analytic_distribution = {an_acc_01: 50, an_acc_02: 50}
        self.prescriptions_order.order_line[1].tax_id = tax_group_2
        self.prescriptions_order.order_line[2].tax_id = tax_10_a
        self.make_downpayment()
        invoice = self.prescriptions_order.invoice_ids
    ...
    def test_tax_price_include_amount_rounding(self):
        ...
        self.prescriptions_order.order_line[0].price_unit = 900
        self.prescriptions_order.order_line[0].product_uom_qty = 1
        self.prescriptions_order.order_line[0].tax_id = tax_21

        self.prescriptions_order.order_line[1].price_unit = 90
        self.prescriptions_order.order_line[1].product_uom_qty = 2
        self.prescriptions_order.order_line[1].tax_id = tax_21

        self.prescriptions_order.order_line[2].price_unit = 49
        self.prescriptions_order.order_line[2].product_uom_qty = 4
        self.prescriptions_order.order_line[2].tax_id = tax_21

        self.prescriptions_order.order_line[3].unlink()
        self.prescriptions_order.action_confirm()

        rx_context = {
            'active_model': 'prescriptions.order',
            'active_ids': [self.prescriptions_order.id],
            'active_id': self.prescriptions_order.id,
        ...
        downpayment = self.env['prescriptions.advance.payment.inv'].with_context(rx_context).create(payment_params)
        downpayment.create_invoices()
        invoice = self.prescriptions_order.invoice_ids
        # pylint: disable=C0326
        expected = [
    ...
    def test_tax_price_include_negative_amount_rounding_final_invoice(self):
        ...
        self.prescriptions_order.order_line[0].product_id = self.company_data['product_delivery_no'].id,
        self.prescriptions_order.order_line[0].product_uom_qty = 1
        self.prescriptions_order.order_line[0].qty_delivered = 0
        self.prescriptions_order.order_line[0].tax_id = tax_21_a
        self.prescriptions_order.order_line[0].price_unit = 1000

        self.prescriptions_order.order_line[1].product_id = self.company_data['product_delivery_no'].id,
        self.prescriptions_order.order_line[1].product_uom_qty = 1
        self.prescriptions_order.order_line[1].qty_delivered = 0
        self.prescriptions_order.order_line[1].tax_id = tax_21_b
        self.prescriptions_order.order_line[1].price_unit = 1000

        self.prescriptions_order.order_line[2:].unlink()
        ...
        downpayment = self.env['prescriptions.advance.payment.inv'].with_context({**rx_context, 'raise_if_nothing_to_invoice': False}).create(payment_params)
        action = downpayment.create_invoices()
        invoice = self.env['account.move'].browse(action['res_id'])
        # pylint: disable=C0326
        expected = [
        ...
        invoice.unlink()
        self.prescriptions_order.order_line[0].qty_delivered = 1
        self.prescriptions_order.order_line[1].qty_delivered = 1
        downpayment = self.env['prescriptions.advance.payment.inv'].with_context(rx_context).create(payment_params)
        action = downpayment.create_invoices()
        invoice = self.env['account.move'].browse(action['res_id'])
        # pylint: disable=C0326
    ...
    def test_tax_price_include_positive_amount_rounding_final_invoice(self):
        ...
        self.prescriptions_order.order_line[0].product_uom_qty = 1
        self.prescriptions_order.order_line[0].qty_delivered = 0
        self.prescriptions_order.order_line[0].tax_id = tax_24_a
        self.prescriptions_order.order_line[0].price_unit = 1000

        self.prescriptions_order.order_line[1].product_id = self.company_data['product_delivery_no'].id,
        self.prescriptions_order.order_line[1].product_uom_qty = 1
        self.prescriptions_order.order_line[1].qty_delivered = 0
        self.prescriptions_order.order_line[1].tax_id = tax_24_b
        self.prescriptions_order.order_line[1].price_unit = 1000
    ...
    def test_tax_price_include_small_amount_rounding_final_invoice(self):
        ...
        self.prescriptions_order.order_line[2].product_id = self.company_data['product_delivery_no'].id,
        self.prescriptions_order.order_line[2].product_uom_qty = 1
        self.prescriptions_order.order_line[2].qty_delivered = 1
        self.prescriptions_order.order_line[2].tax_id = tax_25_a
        self.prescriptions_order.order_line[2].price_unit = 968

        self.prescriptions_order.order_line[3].product_id = self.company_data['product_delivery_no'].id,
        self.prescriptions_order.order_line[3].product_uom_qty = 1
        self.prescriptions_order.order_line[3].qty_delivered = 1
        self.prescriptions_order.order_line[3].tax_id = tax_25_b
        self.prescriptions_order.order_line[3].price_unit = 968

        self.prescriptions_order.order_line[3].copy({
            'order_id':self.prescriptions_order.id,
            'tax_id': tax_25_c,
            'qty_delivered': 1,
        })

        self.prescriptions_order.order_line.qty_delivered_method = 'manual'
