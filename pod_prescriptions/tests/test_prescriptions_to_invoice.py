...
class TestPrescriptionToInvoice(TestPrescriptionCommon):
    ...
    def test_downpayment(self):
        ...
        self.assertEqual(len(self.prescriptions_order.invoice_ids), 2, 'Invoice should be created for the RX')
        ...
        self.assertEqual(len(self.prescriptions_order.invoice_ids), 3, 'Invoice should be created for the RX')
    ...
    def test_downpayment_percentage_tax_icl(self):
        ...
        self.assertEqual(len(self.prescriptions_order.invoice_ids), 1, 'Invoice should be created for the RX')
        downpayment_line = self.prescriptions_order.order_line.filtered(lambda l: l.is_downpayment and not l.display_type)
    ...
    def test_multi_company_invoice(self):
        """Checks that the company of the invoices generated in a multi company environment using the
           'prescriptions.advance.payment.inv' wizard fit with the company of the RX and not with the current company.
        """
        rx_company_id = self.prescriptions_order.company_id.id
        yet_another_company_id = self.company_data_2['company'].id
        rx_for_downpayment = self.prescriptions_order.copy()
        ...
        payment.create_invoices()
        self.assertEqual(self.prescriptions_order.invoice_ids[0].company_id.id, rx_company_id, "The company of the invoice should be the same as the one from the RX")
    ...
    def test_invoice_analytic_distribution_model(self):
        ...
        down_payment = self.env['prescriptions.advance.payment.inv'].with_context(rx_context).create({})
    ...
    def test_invoice_analytic_rule_with_account_prefix(self):
        ...
        rx = self.env['prescriptions.order'].create({'partner_id': self.partner_a.id})
    ...
    def test_invoice_after_product_return_price_not_default(self):
        ...
        invoicing_wizard = self.env['prescriptions.advance.payment.inv'].with_context(rx_context).create({})
        invoicing_wizard.create_invoices()
    ...
    def test_group_invoice(self):
        ...
        rx1 = self.prescriptions_order.with_context(mail_notrack=True).copy()
    ...
    def test_prescriptions_order_standard_flow_with_invoicing(self):
        ...
        self.assertTrue(self.prescriptions_order.invoice_status == 'no', 'Prescription: RX status after invoicing should be "nothing to invoice"')
        ...
        self.assertTrue(self.prescriptions_order.invoice_status == 'to invoice', 'Prescription: RX status after delivery should be "to invoice"')
        ...
        self.assertTrue(self.prescriptions_order.invoice_status == 'invoiced', 'Prescription: RX status after invoicing everything should be "invoiced"')
        self.assertTrue(len(self.prescriptions_order.invoice_ids) == 2, 'Prescription: invoice is missing')

        # go over the rxld quantity
        self.rxl_serv_order.write({'qty_delivered': 10})
        self.assertTrue(self.prescriptions_order.invoice_status == 'upselling', 'Prescription: RX status after increasing delivered qty higher than ordered qty should be "upselling"')
        ...
        self.assertTrue(self.prescriptions_order.invoice_status == 'invoiced', 'Prescription: RX status after invoicing everything (including the upsel) should be "invoiced"')
    ...
    def test_rx_create_multicompany(self):
        ...
        rx_1 = self.env['prescriptions.order'].with_user(self.company_data['default_user_prescriptionsman']).create({
    ...
    def test_prescriptionsperson_in_invoice_followers(self):
        """
        Test if the prescriptionsperson is in the followers list of invoice created from RX
        """
        ...
        # check if the prescriptionsperson is in the followers list of invoice created from RX
