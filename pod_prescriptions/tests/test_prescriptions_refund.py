...
class TestPrescriptionRefund(TestPrescriptionCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        payment = cls.env['prescriptions.advance.payment.inv'].with_context({
        ...
        cls.invoice = cls.prescriptions_order.invoice_ids[0]
    ...
    def test_refund_create(self):
        ...
        for line in self.prescriptions_order.order_line:
            if line.product_id.invoice_policy == 'delivery':
                ...
                if line == self.sol_prod_order:
                    self.assertEqual(line.qty_to_invoice, 0.0, "The ordered prescriptions line are totally invoiced (qty to invoice is zero)")
                    self.assertEqual(line.qty_invoiced, 5.0, "The ordered (prod) prescriptions line are totally invoiced (qty invoiced come from the invoice lines)")
                else:
                    self.assertEqual(line.qty_to_invoice, 0.0, "The ordered prescriptions line are totally invoiced (qty to invoice is zero)")
                    self.assertEqual(line.qty_invoiced, 3.0, "The ordered (serv) prescriptions line are totally invoiced (qty invoiced = the invoice lines)")
        ...
        invoice_refund = self.prescriptions_order.invoice_ids.sorted(key=lambda inv: inv.id, reverse=False)[-1]  # the first invoice, its refund, and the new invoice
        ...
        self.assertEqual(self.prescriptions_order.invoice_count, 2, "The SO should have 2 related invoices: the original, the new refund")
        self.assertEqual(len(self.prescriptions_order.invoice_ids.filtered(lambda inv: inv.move_type == 'out_refund')), 1, "The SO should be linked to only one refund")
        self.assertEqual(len(self.prescriptions_order.invoice_ids.filtered(lambda inv: inv.move_type == 'out_invoice')), 1, "The SO should be linked to only one customer invoices")

        # At this time, the invoice 1 is opend (validated) and its refund is in draft, so the amounts invoiced are not zero for
        # invoiced prescriptions line. The amounts only take validated invoice/refund into account.
        for line in self.prescriptions_order.order_line:
            if line.product_id.invoice_policy == 'delivery':
                self.assertEqual(line.qty_to_invoice, 0.0, "Quantity to invoice should be same as ordered quantity")
    ...
    def test_refund_modify(self):
        ...
        self.assertEqual(invoice_refund.state, 'draft', 'Last Customer invoices should be in draft')
        self.assertEqual(self.prescriptions_order.invoice_count, 3, "The SO should have 3 related invoices: the original, the refund, and the new one")
        self.assertEqual(len(self.prescriptions_order.invoice_ids.filtered(lambda inv: inv.move_type == 'out_refund')), 1, "The SO should be linked to only one refund")
        self.assertEqual(len(self.prescriptions_order.invoice_ids.filtered(lambda inv: inv.move_type == 'out_invoice')), 2, "The SO should be linked to two customer invoices")
        ...
        for line in self.prescriptions_order.order_line:
            if line.product_id.invoice_policy == 'delivery':
                ...
                if line == self.sol_prod_order:
                    ...
                    self.assertEqual(line.qty_invoiced, 3.0, "The ordered (prod) prescriptions line does not change on invoice 2 confirmation")
                    ...
                    self.assertEqual(line.qty_invoiced, 2.0, "The ordered (serv) prescriptions line does not change on invoice 2 confirmation")
        ...
        for line in self.prescriptions_order.order_line:
            if line.product_id.invoice_policy == 'delivery':
                ...
                if line == self.sol_prod_order:
                    ...
                    self.assertEqual(line.qty_invoiced, 3.0, "The ordered prescriptions line are totally invoiced (qty invoiced = ordered qty)")
                    ...
                    self.assertEqual(line.qty_invoiced, 2.0, "The ordered prescriptions line are totally invoiced (qty invoiced = ordered qty)")
    ...
    def test_refund_invoice_with_downpayment(self):
        prescriptions_order_refund = self.env['prescriptions.order'].create({
            'partner_id': self.partner_a.id,
            'partner_invoice_id': self.partner_a.id,
            'partner_shipping_id': self.partner_a.id,
            'pricelist_id': self.company_data['default_pricelist'].id,
        })
        sol_product = self.env['prescriptions.order.line'].create({
            'product_id': self.company_data['product_order_no'].id,
            'product_uom_qty': 5,
            'order_id': prescriptions_order_refund.id,
            'tax_id': False,
        })
        ...
        prescriptions_order_refund.action_confirm()
        ...
        so_context = {
            'active_model': 'prescriptions.order',
            'active_ids': [prescriptions_order_refund.id],
            'active_id': prescriptions_order_refund.id,
            'default_journal_id': self.company_data['default_journal_prescription'].id,
        }

        downpayment = self.env['prescriptions.advance.payment.inv'].with_context(so_context).create({
        ...
        sol_downpayment = prescriptions_order_refund.order_line[2]
        dp_invoice = prescriptions_order_refund.invoice_ids[0]
        ...
        payment = self.env['prescriptions.advance.payment.inv'].with_context(so_context).create({
            'deposit_account_id': self.company_data['default_account_revenue'].id
        })
        payment.create_invoices()

        so_invoice = max(prescriptions_order_refund.invoice_ids)
        self.assertEqual(len(so_invoice.invoice_line_ids.filtered(lambda l: not (l.display_type == 'line_section' and l.name == "Down Payments"))),
                         len(prescriptions_order_refund.order_line.filtered(lambda l: not (l.display_type == 'line_section' and l.name == "Down Payments"))), 'All lines should be invoiced')
        ...
        self.assertEqual(so_invoice.amount_total, prescriptions_order_refund.amount_total - sol_downpayment.price_unit, 'Downpayment should be applied')
        ...
        credit_note_wizard.refund_moves()
        invoice_refund = prescriptions_order_refund.invoice_ids.sorted(key=lambda inv: inv.id, reverse=False)[-1]
        invoice_refund.action_post()
