...
class TestPrescriptionRefund(TestPrescriptionCommon):
    ...
    def test_refund_create(self):
        ...
        self.assertEqual(self.prescriptions_order.invoice_count, 2, "The RX should have 2 related invoices: the original, the new refund")
        self.assertEqual(len(self.prescriptions_order.invoice_ids.filtered(lambda inv: inv.move_type == 'out_refund')), 1, "The RX should be linked to only one refund")
        self.assertEqual(len(self.prescriptions_order.invoice_ids.filtered(lambda inv: inv.move_type == 'out_invoice')), 1, "The RX should be linked to only one customer invoices")
    ...
    def test_refund_modify(self):
        ...
        self.assertEqual(invoice_refund.state, 'draft', 'Last Customer invoices should be in draft')
        self.assertEqual(self.prescriptions_order.invoice_count, 3, "The RX should have 3 related invoices: the original, the refund, and the new one")
        self.assertEqual(len(self.prescriptions_order.invoice_ids.filtered(lambda inv: inv.move_type == 'out_refund')), 1, "The RX should be linked to only one refund")
        self.assertEqual(len(self.prescriptions_order.invoice_ids.filtered(lambda inv: inv.move_type == 'out_invoice')), 2, "The RX should be linked to two customer invoices")
    ...
    def test_refund_invoice_with_downpayment(self):
        ...
        rxl_product = self.env['prescriptions.order.line'].create({
        ...
        downpayment = self.env['prescriptions.advance.payment.inv'].with_context(rx_context).create({
        ...
        rxl_downpayment = prescriptions_order_refund.order_line[2]
        ...
        payment = self.env['prescriptions.advance.payment.inv'].with_context(rx_context).create({
            'deposit_account_id': self.company_data['default_account_revenue'].id
        })
        payment.create_invoices()

        rx_invoice = max(prescriptions_order_refund.invoice_ids)
        ...
        self.assertEqual(rx_invoice.amount_total, prescriptions_order_refund.amount_total - rxl_downpayment.price_unit, 'Downpayment should be applied')
