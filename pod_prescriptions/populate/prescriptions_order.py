...
class PrescriptionOrderLine(models.Model):
    ...
    def _populate(self, size):
        ...
        self.confirm_prescriptions_order(0.60, rx_line)
        return rx_line

    def confirm_prescriptions_order(self, sample_ratio, rx_line):
        # Confirm sample_ratio * 100 % of so
        random = populate.Random('confirm_prescriptions_order')
        order_ids = self.filter_confirmable_prescriptions_orders(rx_line.order_id).ids
