...
class PrescriptionOrder(models.Model):
    ...
    _sql_constraints = [
        ('date_order_conditional_required',
         "CHECK((state = 'sales' AND date_order IS NOT NULL) OR state != 'sales')",
    ...
    journal_id = fields.Many2one(
        ...
        domain=[('type', '=', 'sales')], check_company=True,
        ...
        if operator == 'in' and value:
            self.env.cr.execute("""
                SELECT array_agg(rx.id)
                    ...
                    JOIN prescriptions_order_line_invoice_rel rxli_rel ON rxli_rel.order_line_id = rxl.id
    ...
    def _compute_invoice_status(self):
        confirmed_orders = self.filtered(lambda rx: rx.state == 'sales')
    ...
    def _compute_amount_paid(self):
        ...
        for order in self:
            order.amount_paid = sum(
                tx.amount for tx in order.transaction_ids if tx.state in ('authorized', 'sales')
            )
    ...
    def _compute_expected_date(self):
        """ For service and consumable, we only take the min dates. This method is extended in pod_prescriptions_stock to
            take the picking_policy of RX into account.
        """
    ...
    def _find_mail_template(self):
        ...
        If the RX is confirmed, we return the mail template for the prescriptions confirmation.
        ...
        """
        self.ensure_one()
        if self.env.context.get('proforma') or self.state != 'sales':
    ...
    def _track_subtype(self, init_values):
        self.ensure_one()
        if 'state' in init_values and self.state == 'sales':
    ...
    def _filter_product_documents(self, documents):
        return documents.filtered(
            lambda document:
                ...
                or (self.state == 'sales' and document.attached_on == 'prescriptions_order')
        )
