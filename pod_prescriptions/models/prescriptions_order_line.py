...
class PrescriptionOrderLine(models.Model):
    ...
    qty_delivered_method = fields.Selection(
        ...
        help="According to product configuration, the delivered quantity can be automatically computed by mechanism:\n"
             ...
             "  - Timesheet: the quantity is the sum of hours recorded on tasks linked to this prescriptions line\n"
    ...
    invoice_lines = fields.Many2many(
        ...
        relation='prescriptions_order_line_invoice_rel', column1='order_line_id', column2='invoice_line_id',
    ...
    def _get_prescriptions_order_line_multiline_description_prescription(self):
        """ Compute a default multiline description for this prescriptions order line.

        In most cases the product description is enough but sometimes we need to append information that only
        exists on the prescriptions order line itself.
        e.g:
        - custom attributes and attributes that don't create variants, both introduced by the "product configurator"
        - in event_prescription we need to know specifically the prescriptions order line as well as the product to generate the name:
          the product is not sufficient because we also need to know the event_id and the event_ticket_id (both which belong to the prescriptions order line).
        """
        self.ensure_one()
    ...
    def _get_pricelist_price(self):
        ...
        :return: the product prescriptions price in the order currency (without taxes)
        :rtype: float
        """
        self.ensure_one()
    ...
    def _compute_discount(self):
        for line in self:
            ...
            if not line.pricelist_item_id:
                ...
                # to the existing prescriptions price.
                continue
    ...
    # (cf pod_prescriptions_stock) instead of simply removing the default and specifying
    # the compute attribute & method in pod_prescriptions_stock.
    ...
    def _compute_qty_delivered_method(self):
        """ Prescription module compute delivered qty for product [('type', 'in', ['consu']), ('service_type', '=', 'manual')]
            ...
            This is true when only prescriptions is installed: pod_prescriptions_stock redifine the behavior for 'consu' type,
            and prescriptions_timesheet implements the behavior of 'service' + service_type=timesheet.
        """
        for line in self:
            if line.is_expense:
    ...
    def _compute_qty_delivered(self):
        """ This method compute the delivered quantity of the RX lines: it covers the case provide by prescriptions module, aka
    ...
    def _compute_qty_to_invoice(self):
        ...
        """
        for line in self:
            if line.state == 'sales' and not line.display_type:
    ...
    def _compute_invoice_status(self):
        ...
        - no: if the RX is not in status 'sales', we consider that there is nothing to
        ...
        - upselling: this is possible only for a product invoiced on ordered quantities for which
          ...
          occurs only in state 'sales', the upselling opportunity is removed from the list.
        ...
        for line in self:
            ...
            elif line.state == 'sales' and line.product_id.invoice_policy == 'order' and\
    ...
    def _compute_untaxed_amount_to_invoice(self):
        """ Total of remaining amount to invoice on the prescriptions order line (taxes excl.) as
    ...
    def _compute_product_uom_readonly(self):
        for line in self:
            ...
            line.product_uom_readonly = line.ids and line.state in ['sales', 'cancel']
    ...
    def create(self, vals_list):
        ...
        for line in lines:
            if line.product_id and line.state == 'sales':
    ...
    def write(self, values):
        ...
        if 'product_uom_qty' in values:
            precision = self.env['decimal.precision'].precision_get('Product Unit of Measure')
            self.filtered(
                lambda r: r.state == 'sales' and float_compare(r.product_uom_qty, values['product_uom_qty'], precision_digits=precision) != 0)._update_line_quantity(values)
    ...
    def _check_line_unlink(self):
        ...
        * Sections and Notes can always be deleted.
        ...
        :returns: Prescriptions Order Lines that cannot be deleted
        :rtype: `prescriptions.order.line` recordset
        """
    ...
    def _expected_date(self):
        self.ensure_one()
        if self.state == 'sales' and self.order_id.date_order:
    ...
    def _prepare_invoice_line(self, **optional_values):
        """Prepare the values to create the new invoice line for a prescriptions order line.
    ...
    def _get_product_catalog_lines_data(self, **kwargs):
        ...
        ``PrescriptionOrder._is_readonly`` for more details) or if `self` contains multiple records.
