...
class TeamStaffPortal(CustomerPortal):
    ...
    @http.route(
        route=["/my/teams", "/my/teams/page/<int:page>"],
        type="http",
        auth="user",
        website=True,
    )
    def view_teams(self, page=0, **kw):
        ...
        pgr = pager(url="/my/teams", total=teams_count, page=page, step=10, scope=5)
        teams = http.request.env["sports.team"].search(
            self._prepare_teams_domain(), offset=pgr["offset"], limit=teams_count
        )
        return http.request.render(
            template="pod_practice_management.portal_my_teams",
            qcontext={
    ...
    @http.route(
        route=["/my/team", "/my/team/page/<int:page>"],
        type="http",
        auth="user",
        website=True,
    )
    def view_team(self, team_id, page=0, **kw):
        ...
        return http.request.render(
            template="pod_practice_management.portal_my_team_players",
            qcontext={
                "team": team,
    ...
    @http.route(
        route=["/my/players", "/my/players/page/<int:page>"],
        type="http",
        auth="user",
        website=True,
    )
    def view_players(self, page=0, **kw):
        ...
        players = http.request.env["sports.patient"].search(
            players_domain, offset=pgr["offset"], limit=players_count
        )
        return http.request.render(
            template="pod_practice_management.portal_my_players",
            qcontext={
                "players_count": players_count,
                "players": players,
                "pager": pgr,
    ...
    @http.route(route=["/my/player"], type="http", auth="user", website=True)
    def view_player(self, player_id, team_id=None, **kw):
        ...
        return http.request.render(
            template="pod_practice_management.portal_my_player_injuries",
            qcontext={
                "player": player,
                "injuries": injuries,
                "team": team,
