...
class TestSale(common.PodiatrySavePointCase):
    ...
    def test_invoice_multiple_coverage_template(self):
        method = self.browse_ref("pod_careplan_sale.by_customer")
        ...
        invoices = sale_orders.with_context( active_model=sale_orders._name
        )._create_invoices()
    ...
    def test_discount(self):
        method = self.browse_ref("pod_careplan_sale.no_invoice")
        self.plan_definition2.third_party_bill = False
        self.plan_definition.is_breakdown = True
        self.plan_definition.is_billable = True
        self.agreement.invoice_group_method_id = method
        ...
        self.assertFalse(group.pod_sale_discount_id)
        discount = self.env["pod.request.group.discount"].new(
            {"request_group_id": group.id}
        )
        discount.pod_sale_discount_id = self.discount
        ...
        for line in sale_order.mapped("order_line"):
            line.qty_delivered = line.product_uom_qty sale_order.with_context(active_model=sale_order._name)._create_invoices()
    ...
    def test_careplan_add_function_01(self):
        encounter_action = self.env["pod.encounter"].create_encounter( patient=self.patient_01.id, center=self.center.id, careplan_data=[
                {
        ...
        encounter = self.env["pod.encounter"].browse(encounter_action["res_id"])
    ...
    def test_careplan_raises_01(self):
        ...
        with self.assertRaises(ValidationError):
            self.env["pod.encounter"].create_encounter( patient=self.patient_01.id, center=self.center.id, careplan_data=[
                    {
    ...
    def test_careplan_raises_02(self):
        with self.assertRaises(ValidationError):
            self.env["pod.encounter"].create_encounter( patient=self.patient_01.id, center=self.center.id, careplan_data=[{"service": self.agreement_line3.product_id.id}],
            )
    ...
    def test_careplan_add_wizard(self):
        ...
        careplan_wizard = (
            self.env["pod.encounter.add.careplan"] .with_context(default_encounter_id=encounter.id)
    ...
    def test_document(self, mock):
        ...
        action = group.with_context( model_name="pod.document.reference"
        ).action_view_request()
        self.assertEqual(
