...
class TestPodiatryCoverageAgreement(common.AgrementSavepointCase):
    ...
    def test_join_agreements_constrain_01(self):
        ...
        agr.invoice_group_method_id = self.env.ref(
            "pod_careplan_sale.by_customer"
        )
        with self.assertRaises(ValidationError):
            self.env["pod.coverage.agreement.join"].with_context( active_model=(agr | agr2)._name, active_ids=(agr | agr2).ids,
            ).create({}).run()
