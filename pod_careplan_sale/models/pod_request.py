...
class PodiatryRequest(models.AbstractModel):
    ...
    sale_order_line_ids = fields.One2many( string="Sale order lines", comodel_name="sale.order.line", inverse_name="pod_res_id", domain=lambda self: self._get_sale_order_domain(),
    )
    ...
    sub_payor_id = fields.Many2one(
        "res.partner", domain="[('payor_id', '=', payor_id), ('is_sub_payor', '=', True)]", readonly=True, states={"draft": [("readonly", False)]},
    )
    payor_id = fields.Many2one(
        "res.partner", related="coverage_id.coverage_template_id.payor_id", readonly=True, states={"draft": [("readonly", False)]},
    )
    authorization_method_id = fields.Many2one(
        "pod.authorization.method", tracking=True, readonly=True
    )
    invoice_group_method_id = fields.Many2one(
        "invoice.group.method", string="Invoice Group Method", tracking=True, readonly=True,
    )
    qty = fields.Integer( default=1, required=True, readonly=True, states={"draft": [("readonly", False)]},
    )
    ...
    def _get_sale_order_query_vals(self, is_insurance=False):
        ...
        return (
            self.with_context( lang=partner.lang or self.env.user.lang
