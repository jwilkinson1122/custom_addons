...
class SaleOrderLine(models.Model):
    ...
    authorization_method_id = fields.Many2one(
        "pod.authorization.method", readonly=True, index=True,
    )
    ...
    coverage_template_id = fields.Many2one(
        "pod.coverage.template", related="order_id.coverage_id.coverage_template_id",
    )
        ...
        if not self.env.context.get("not_sale_share_values", False):
            ...
            if shared_vals:
                ...
                self.mapped("order_id").mapped("order_line").filtered(
                    lambda r: r not in self ).with_context(not_sale_share_values=True).write(shared_vals)
        return res
