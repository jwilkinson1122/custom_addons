...
class SaleOrder(models.AbstractModel):
    ...
    coverage_template_id = fields.Many2one(
        "pod.coverage.template", readonly=True, related="coverage_id.coverage_template_id",
    )
    coverage_agreement_id = fields.Many2one(
        "pod.coverage.agreement", index=True,
    )
    patient_id = fields.Many2one("pod.patient", readonly=True, index=True)
    patient_name = fields.Char( states={"draft": [("readonly", False)], "sent": [("readonly", False)]}
    )
    subscriber_id = fields.Char( states={"draft": [("readonly", False)], "sent": [("readonly", False)]}
    )
    ...
    def write(self, vals):
        ...
        if not self.env.context.get("not_sale_share_values", False):
            ...
            if shared_vals:
                self.mapped("order_line").with_context( not_sale_share_values=True
                ).write(shared_vals)
        return res
