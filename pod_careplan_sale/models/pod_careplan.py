...
class PodiatryCareplan(models.Model):
    ...
    def _add_request_group(
        self, service=False, qty=1, order_by=False, authorization_number=False, authorization_number_extra_1=False, performer=False, agreement_line=False,
        **kwargs
    ):
        self.ensure_one()
        if not service:
        ...
        call = (
            self.env["pod.careplan.add.plan.definition"] .with_context(default_careplan_id=self.id)
            .create(
                {
