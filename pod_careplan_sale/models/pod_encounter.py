...
class PodiatryEncounter(models.Model):
    ...
    invoice_line_ids = fields.One2many( comodel_name="account.move.line", inverse_name="encounter_id", string="Invoice Lines",
    )
    ...
    def _get_sale_order_vals(
        self, partner=False, coverage=False, agreement=False, third_party_partner=False, invoice_group_method=False,
        *kwargs
    ):
        vals = {
    ...
    def _generate_sale_order(
        self,
        order_lines, agreement=False, partner=False, coverage=False, third_party_partner=False, invoice_group_method=False,
        **kwargs
    ):
        ...
        if not order:
            vals = self._get_sale_order_vals( partner=partner, coverage=coverage, agreement=agreement, third_party_partner=third_party_partner, invoice_group_method=invoice_group_method,
                **kwargs,
            )
            order = (
    ...
    # flake8: noqa: C901
    def _add_careplan(
        self, payor=False, sub_payor=False, coverage_template=False, subscriber_id=False, coverage=False, service=False, order_by=False, performer=False, authorization_number=False, qty=1, subscriber_magnetic_str=False,
        **kwargs
    ):
        if coverage:
        ...
        self.ensure_one()
        careplan = (
            self.env["pod.encounter.add.careplan"] .with_context(default_encounter_id=self.id)
            .create(
                {
            ...
            .with_context( default_order_by_id=order_by.id or False, default_service_id=service.id or False,
            )
            .run()
        )
        careplan._add_request_group( service=service, qty=qty, order_by=order_by, authorization_number=authorization_number, performer=performer,
            **kwargs,
        )
        return careplan
