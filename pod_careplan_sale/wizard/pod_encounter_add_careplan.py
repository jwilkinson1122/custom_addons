...
class PodiatryEncounterAddCareplan(models.TransientModel):
    ...
    encounter_id = fields.Many2one(
        "pod.encounter", required=True, readonly=True, domain=[("state", "in", get_encounter_states)],
    )
    patient_id = fields.Many2one(
        "pod.patient", related="encounter_id.patient_id", readonly=True
    )
    center_id = fields.Many2one(
        "res.partner", default=default_center, required=True, domain=[("is_center", "=", True)],
    )
    payor_id = fields.Many2one(
        "res.partner", required=True, domain="[('is_payor', '=', True)]"
    )
    sub_payor_id = fields.Many2one(
        "res.partner", domain="[('payor_id', '=', payor_id), ('is_sub_payor', '=', True)]",
    )
    coverage_id = fields.Many2one(
        "pod.coverage", domain="[('patient_id','=', patient_id)]"
    )
    coverage_template_id = fields.Many2one(
        "pod.coverage.template", required=True, domain="[('payor_id', '=', payor_id)]",
    )
    ...
    def get_careplan_values(self):
        return {
            ...
            "coverage_id": self.patient_id.get_coverage( template=self.coverage_template_id, coverage=self.coverage_id, subscriber_id=self.subscriber_id, magnetic_str=self.subscriber_magnetic_str,
            ).id,
