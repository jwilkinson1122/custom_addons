...
class TestCondition(TransactionCase):
    def setUp(self):
        ...
        self.finding_no_warning = self.env["pod.clinical.finding"].create(
            {"name": "Finding no warning"}
        )
        self.finding_warning = self.env["pod.clinical.finding"].create(
    ...
    def test_create_condition_without_warning(self):
        ...
        self.assertEqual(self.patient.pod_warning_count, 0)

    def test_create_condition_with_warning(self):
        condition_warning = self.env["pod.condition"].create(
            {
                "patient_id": self.patient.id,
        ...
        self.assertEqual(self.patient.pod_condition_count, 1)
        self.assertEqual(self.patient.pod_warning_count, 1)
    ...
    def test_create_allergy_without_warning(self):
        allergy_no_warning = self.env["pod.condition"].create(
            {
                "patient_id": self.patient.id,
                "is_allergy": True,
    ...
    def test_create_allergy_with_warning(self):
        allergy_warning = self.env["pod.condition"].create(
            {
                "patient_id": self.patient.id,
                "is_allergy": True,
    ...
    def test_create_again_an_archived_condition(self):
        ...
        self.assertEqual(self.patient.pod_condition_count, 0)
        condition_2 = self.env["pod.condition"].create(
            {
                "patient_id": self.patient.id,
                "clinical_finding_id": self.finding_warning.id,
            }
