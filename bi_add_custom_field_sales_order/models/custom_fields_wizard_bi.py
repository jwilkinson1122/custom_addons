...
class ir_model_fields(models.Model):
	...
	where_to_add = fields.Selection([('after', 'After'), ('before', 'Before')], string="Where to Add", required=True)
	...
	after_which_field_sale = fields.Many2one('ir.model.fields',string="Field", ondelete="cascade", required=True)
	...
	def custom_field_add(self):
		...
		search_inherited_view_id = ir_ui_view_obj.search([('name', '=', 'bi_inherit_sale_order_view')])
		if not search_inherited_view_id:
			...
			'''
			arch = "<data><field name=" + '"' + self.after_which_field_sale.name + '"' + " " + "position=" + '"' + self.where_to_add + '"' + ">" + "<field name=" + '"' + self.name + '"' + "/>" + "</field>" + "</data>"
			registry = self.env
			type = 'form'
			view_id = registry['ir.model.data']._xmlid_to_res_id("%s.%s" % (MODULE, name))
			if view_id:
			...
			search_id = self.search([('state', '=', 'manual'), ('model_id', '=', 'sale.order')])
			middel_arch_data = ''
			for c_field_id in search_id:
				if c_field_id.after_which_field_sale.name == "date_order":
					if c_field_id.where_to_add == "after":
						middel_arch_data += '''<xpath expr="//field[@name='date_order'][2]" position="after">''' + "<field name=" + '"' + c_field_id.name + '"' + "/>" + "</xpath>"
					else:
						middel_arch_data += '''<xpath expr="//group[@name='order_details']//div[hasclass('o_td_label')][1]" position="before">''' + "<field name=" + '"' + c_field_id.name + '"' + "/>" + "</xpath>"
				
				elif c_field_id.after_which_field_sale.name == "pricelist_id":
					if c_field_id.where_to_add == "after":
						middel_arch_data += '''<xpath expr="//group[@name='order_details']//div[hasclass('o_row')]" position="after">''' + "<field name=" + '"' + c_field_id.name + '"' + "/>" + "</xpath>"
					else:
						middel_arch_data += '''<xpath expr="//label[@for='pricelist_id']" position="before">''' + "<field name=" + '"' + c_field_id.name + '"' + "/>" + "</xpath>"
				
				elif c_field_id.after_which_field_sale.name == "company_id":
					middel_arch_data += '''<xpath expr="//page[@name='other_information']//field[@name='company_id']" ''' +  ''' position="''' + c_field_id.where_to_add +'">' + "<field name=" + '"' + c_field_id.name + '"' + "/>" + "</xpath>"

				else:
					middel_arch_data += "<field name=" + '"' + c_field_id.after_which_field_sale.name + '"' + " " + "position=" + '"' + c_field_id.where_to_add + '"' + ">" + "<field name=" + '"' + c_field_id.name + '"' + "/>" + "</field>"
