...
registry.category("web_tour.tours").add('prescriptions_account_tour', {
    url: "/web",
    rainbowManMessage: () => markup(_t("Wow... 6 prescriptions processed in a few seconds, You're good.<br/>The tour is complete. Try uploading your own prescriptions now.")),
    sequence: 170,
    steps: () => [{
    trigger: '.o_app[data-menu-xmlid="prescriptions.menu_root"]',
    content: markup(_t("Want to become a <b>paperless company</b>? Let's discover NWPL Prescriptions.")),
    position: 'bottom',
}, {
    trigger: 'body:not(:has(.o_FileViewer)) img[src="https://img.youtube.com/vi/Ayab6wZ_U1A/0.jpg"]',
    content: markup(_t("Click on a thumbnail to <b>preview the prescription</b>.")),
    position: 'bottom',
    ...
    content: markup(_t("Let's process prescriptions in your Inbox.<br/><i>Tip: Use Tags to filter prescriptions and structure your process.</i>")),
    position: 'bottom',
    run: function (actions) {
        $('.o_search_panel_filter_value:eq(0) .o_search_panel_label_title').click();
    },
}, {
    trigger: 'body:not(:has(.o_FileViewer)) .o_prescriptions_kanban',
    extra_trigger: '.o_prescriptions_kanban',
    content: markup(_t("Click on a card to <b>select the prescription</b>.")),
    position: 'bottom',
    ...
    extra_trigger: '.o_prescriptions_kanban',
    content: _t("Let's process this prescription, coming from our scanner."),
    position: 'bottom',
}, {
    trigger: '.o_inspector_split',
    extra_trigger: '[title="Mails_inbox.pdf"]',
    content: _t("As this PDF contains multiple prescriptions, let's split and process in bulk."),
    position: 'bottom',
}, {
    trigger: '.o_page_splitter_wrapper:nth(3)',
    extra_trigger: '.o_prescriptions_pdf_canvas:nth(5)', // Makes sure that all the canvas are loaded.
    content: markup(_t("Click on the <b>page separator</b>: we don't want to split these two pages as they belong to the same prescription.")),
    position: 'right',
}, {
    trigger: '.o_prescriptions_pdf_page_selector:nth(5)',
    extra_trigger: '.o_prescriptions_pdf_manager',
