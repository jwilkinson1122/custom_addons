...
class TestInvoices(AccountTestInvoicingCommon):
    ...
    def test_suspense_statement_line_id(self):
        reconcile_activity_type = self.env['mail.activity.type'].create({
            ...
            "folder_id": self.env.ref("prescriptions.prescriptions_finance_folder").id,
            "res_model": "account.move",
            "tag_ids": [(6, 0, [self.env.ref('prescriptions.prescriptions_finance_status_tc').id])],
        })
        ...
        prescriptions = self.env['prescriptions.prescription'].search([('request_activity_id', '=', activity.id)])
        self.assertTrue(prescriptions.exists())
        ...
        workflow_rule_vendor_bill = self.env['prescriptions.workflow.rule'].create({
            'domain_folder_id': prescriptions.folder_id.id,
            'name': "Create a new Vendor Bill from prescription",
            'create_model': 'account.move.in_invoice',
        })
        vendor_bill_action = workflow_rule_vendor_bill.apply_actions(prescriptions.ids)
