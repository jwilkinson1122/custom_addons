...
class TestCasePrescriptionsBridgeAccount(TransactionCase):

    def setUp(self):
        super(TestCasePrescriptionsBridgeAccount, self).setUp()
        ...
        self.workflow_rule_vendor_bill = self.env['prescriptions.workflow.rule'].create({
    ...
    def test_bridge_folder_workflow(self):
        ...
        """
        self.assertEqual(self.prescription_txt.res_model, 'prescriptions.prescription', "failed at default res model")
        multi_return = self.workflow_rule_vendor_bill.apply_actions([self.prescription_txt.id, self.prescription_gif.id])
        ...
        self.assertEqual(self.prescription_txt.res_model, 'account.move', "failed at workflow_bridge_dms_account"
                                                                           " new res_model")
        vendor_bill_txt = self.env['account.move'].search([('id', '=', self.prescription_txt.res_id)])
        self.assertTrue(vendor_bill_txt.exists(), 'failed at workflow_bridge_dms_account vendor_bill')
        self.assertEqual(self.prescription_txt.res_id, vendor_bill_txt.id, "failed at workflow_bridge_dms_account res_id")
        self.assertEqual(vendor_bill_txt.move_type, 'in_invoice', "failed at workflow_bridge_dms_account vendor_bill type")
        vendor_bill_gif = self.env['account.move'].search([('id', '=', self.prescription_gif.res_id)])
        self.assertEqual(self.prescription_gif.res_id, vendor_bill_gif.id, "failed at workflow_bridge_dms_account res_id")

        single_return = self.workflow_rule_vendor_bill.apply_actions([self.prescription_txt.id])
    ...
    def test_bridge_account_account_settings_on_write(self):
        ...
        on invoices.
        """
        folder_test = self.env['prescriptions.folder'].create({'name': 'folder_test'})
        self.env.user.company_id.prescriptions_account_settings = True
        ...
        for invoice_type in ['in_invoice', 'out_invoice', 'in_refund', 'out_refund']:
            ...
            setting = self.env['prescriptions.account.folder.setting'].create({
            ...
            self.assertEqual(txt_doc.folder_id, folder_test, 'the text test prescription have a folder')
            ...
            self.assertEqual(txt_doc.attachment_id.id, attachment_txt_alternative_test.id,
                             "the attachment of the prescription should have swapped")
    ...
    def test_journal_entry(self):
        ...
        on invoices.
        """
        folder_test = self.env['prescriptions.folder'].create({'name': 'Bills'})
        self.env.user.company_id.prescriptions_account_settings = True
        ...
        prescriptions = self.env['prescriptions.prescription'].search([('attachment_id', 'in', attachments.ids)])
    ...
    def test_bridge_account_workflow_settings_on_write(self):
        ...
        """
        self.env.user.company_id.prescriptions_account_settings = True
        tag_category_a = self.env['prescriptions.facet'].create({
            'folder_id': self.folder_a.id,
            'name': "categ_a",
        })
        tag_a = self.env['prescriptions.tag'].create({
            'facet_id': tag_category_a.id,
            'name': "tag_a",
        })
        tag_b = self.env['prescriptions.tag'].create({
            'facet_id': tag_category_a.id,
            'name': "tag_b",
        })
        tag_action_a = self.env['prescriptions.workflow.action'].create({
            'action': 'add',
            'facet_id': tag_category_a.id,
            'tag_id': tag_a.id,
        })
        ...
        self.env['prescriptions.account.folder.setting'].create({
            'folder_id': self.folder_a.id,
            'journal_id': invoice_test.journal_id.id,
            'tag_ids': tag_b,
        })
        prescription_test = self.env['prescriptions.prescription'].create({
            'name': 'test reconciliation workflow',
            'folder_id': self.folder_a.id,
            'datas': TEXT,
        })
        self.workflow_rule_vendor_bill.apply_actions([prescription_test.id])
        self.assertEqual(prescription_test.tag_ids, tag_a | tag_b,
            "The prescription should have the workflow action's tag(s)")

    def test_bridge_account_sync_partner(self):
        """
        Tests that the partner is always synced on the prescription, regardless of settings
        """
        partner_1, partner_2 = self.env['res.partner'].create([{'name': 'partner_1'}, {'name': 'partner_2'}])
        self.prescription_txt.partner_id = partner_1
        self.workflow_rule_vendor_bill.apply_actions([self.prescription_txt.id, self.prescription_gif.id])
        move = self.env['account.move'].browse(self.prescription_txt.res_id)
        self.assertEqual(move.partner_id, partner_1)
        move.partner_id = partner_2
        self.assertEqual(self.prescription_txt.partner_id, partner_2)

    def test_workflow_create_misc_entry(self):
        misc_entry_rule = self.env.ref('prescriptions_account.misc_entry_rule')
        misc_entry_action = misc_entry_rule.apply_actions([self.prescription_txt.id, self.prescription_gif.id])
        move = self.env['account.move'].browse(self.prescription_txt.res_id)
    ...
    def test_workflow_create_bank_statement_raise(self):
        with self.assertRaises(UserError): # Could not make sense of the given file.
            self.env.ref('prescriptions_account.bank_statement_rule').apply_actions([self.prescription_txt.id, self.prescription_gif.id])

    def test_workflow_create_vendor_bill(self):
        vendor_bill_entry_rule = self.env.ref('prescriptions_account.vendor_bill_rule_financial')
        vendor_bill_entry_action = vendor_bill_entry_rule.apply_actions([self.prescription_txt.id])
        move = self.env['account.move'].browse(self.prescription_txt.res_id)
    ...
    def test_workflow_create_vendor_receipt(self):
        ...
        vendor_receipt_rule = self.env.ref('prescriptions_account.prescriptions_vendor_receipt_rule')
        vendor_receipt_action = vendor_receipt_rule.apply_actions([self.prescription_txt.id])
        move = self.env['account.move'].browse(self.prescription_txt.res_id)
    ...
    def test_workflow_rule_form_journal(self):
        with Form(self.env.ref('prescriptions_account.vendor_bill_rule_financial')) as rule:
