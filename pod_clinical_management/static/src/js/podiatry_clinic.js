...
publicWidget.registry.PodiatryClinic = publicWidget.Widget.extend({
  ...
  _onSpecialisedChange: async function () {
    ...
    await jsonrpc("/specialised_doctors", {
      ...
      self.$el
        .find("#doctor_id")
        .prepend('<option value="">Select Doctor</option>');
      records.forEach(function (record) {
        self
          .$("#doctor_id")
          .append(`<option value="${record.id}">${record.name}</option>`);
      });
    });
  },
  _onDoctorChange: async function () {
    ...
    await jsonrpc("/doctors_shifts", {
      ...
      self.$el
        .find("#time_shift")
        .prepend('<option value="">Select Appointment Time</option>');
      records.forEach(function (record) {
        self
          .$("#time_shift")
          .append(`<option value="${record.id}">${record.name}</option>`);
