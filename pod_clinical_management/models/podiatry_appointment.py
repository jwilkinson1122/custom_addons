...
class PodiatryAppointment(models.Model):
    """Patient podiatry appointment details"""

    _name = "podiatry.appointment"
    _description = "Podiatry Appointment for patients"
    ...
    patient_phone = fields.Char(
        related="patient_id.phone", string="Phone", help="Phone number of the patient"
    )
    patient_age = fields.Integer(
        related="patient_id.patient_age", string="Age", help="Age of the patient"
    )
    ...
    reason = fields.Text(
        string="Please describe the reason",
    ...
    def create(self, vals):
        ...
        res = super(PodiatryAppointment, self).create(vals)
        res.state = "new"
        return res
    ...
    def _compute_time_shifts(self):
        ...
        for record in self:
            record.time_shift_ids = (
                self.env["podiatry.time.shift"]
                .search([("id", "in", record.doctor_id.time_shift_ids.ids)])
                .ids
            )
        ...
        for record in self:
            if record.specialist_id:
                record.doctor_ids = (
                    ...
                    .search([("specialised_in_id", "=", record.specialist_id.id)])
                    .ids
                )
            else:
    ...
    def action_prescription(self):
        ...
        return {
            ...
            "res_id": self.env["podiatry.prescription"]
            .search([("appointment_id", "=", self.id)], limit=1)
            .id,
        }
