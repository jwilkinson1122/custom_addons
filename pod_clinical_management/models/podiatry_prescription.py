...
class PodiatryPrescription(models.Model):
    """Prescription of patient from the podiatry clinic"""
    ...
    sequence_no = fields.Char(
        ...
        help="Sequence number of the podiatry prescription",
    )
    ...
    cost = fields.Float(
        related="treatment_id.cost", string="Treatment Cost", help="Cost of treatment"
    )
    ...
    state = fields.Selection(
        [("new", "New"), ("done", "Prescribed"), ("invoiced", "Invoiced")],
        default="new",
        string="state",
        help="state of the appointment",
    )
    medicine_ids = fields.One2many(
        "podiatry.prescription_lines",
        "prescription_id",
        string="Medicine",
        help="medicines",
    )
    invoice_data_id = fields.Many2one(
        comodel_name="account.move", string="Invoice Data", help="Invoice Data"
    )
    ...
    def create(self, vals):
        ...
        res = super(PodiatryPrescription, self).create(vals)
        return res

    @api.depends("appointment_id")
    def _compute_appointment_ids(self):
        """Computes and assigns the `appointment_ids` field for each record.
        This method searches for all `podiatry.appointment` records that have
        a state of `new` and a date equal to today's date. It then updates
        the `appointment_ids` field of each `PodiatryPrescription` record
        with the IDs of these found appointments."""
        for rec in self:
            rec.appointment_ids = (
                self.env["podiatry.appointment"]
                .search([("state", "=", "new"), ("date", "=", fields.Date.today())])
                .ids
            )

    def action_prescribed(self):
        """Marks the prescription and its associated appointment as `done`.
        This method updates the state of both the PodiatryPrescription instance
        and its linked podiatry.appointment instance to `done`, indicating that
    ...
    def _compute_grand_total(self):
        """Computes the grand total cost of the podiatry prescription.
        ...
        field of the `PodiatryPrescription` model."""
        self.grand_total = self.cost
        for rec in self.medicine_ids:
            self.grand_total += rec.total


class PodiatryPrescriptionLines(models.Model):
    """Prescription lines of the podiatry clinic prescription"""

    _name = "podiatry.prescription_lines"
    _description = "Podiatry Prescriptions Lines"
    ...
    medicament_form = fields.Selection(
        [
            ("tablet", "Tablets"),
            ("capsule", "Capsules"),
            ("liquid", "Liquid"),
    ...
    quantity = fields.Integer(
        string="Quantity", required=True, help="Quantity of medicine"
    )
    ...
    price = fields.Float(
        related="medicament_id.list_price", string="Price", help="Cost of medicine"
    )
    total = fields.Float(string="Total Price", help="Total price of medicine")
    prescription_id = fields.Many2one(
        "podiatry.prescription", help="Relate the model with podiatry_prescription"
    )
