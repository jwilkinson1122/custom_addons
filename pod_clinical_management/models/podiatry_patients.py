...
class PodiatryPatients(models.Model):
    """To create Patients in the clinic, use res.partner model and customize it"""

    _inherit = "res.partner"

    company_type = fields.Selection(
        selection_add=[("person", "Patient"), ("company", "Medicine Distibutor")],
        help="Patient type",
    )
    dob = fields.Date(string="Date of Birth", help="DOB of the patient")
    ...
    start_date = fields.Date(string="Member Since", help="Patient insurance start date")
    expiration_date = fields.Date(
        string="Expiration Date", help="Patient insurance expiration date"
    )
    insureds_name = fields.Char(string="Insured's Name", help="Name of the insured's")
    identification_number = fields.Char(
        string="Identification Number", help="Identification Number of insured's"
    )
    is_patient = fields.Boolean(string="Is Patient", help="To set it's a patient")
    medical_questionnaire_ids = fields.One2many(
    ...
    def create(self, vals):
        """Overrides the create method to handle additional logic for PodiatryPatients.
        When a new PodiatryPatient is created, It then proceeds to create a portal
        ...
        res = super(PodiatryPatients, self).create(vals)
        if "company_type" in vals and vals["company_type"] == "person":
            ...
            try:
                    ...
                    ._create_user_from_template(
                        {
                            ...
                            "groups_id": [
                                ...
                                self.env.ref(
                                    "pod_clinical_managementgroup_podiatry_doctor"
                                ).id,
                ...
                raise UserError(_("Email already used for another podiatrist"))
        return res
