...
class PodiatryTimeShift(models.Model):
    """Doctors time shift, different time slots"""

    _name = "podiatry.time.shift"
    _description = "Podiatry Time Shift"
    _rec_name = "name"

    name = fields.Char(string="Name", readonly=True, help="name of the time shifts")
    shift_type = fields.Selection(
        ...
        string="Shift Type",
        help="Selection field for the shift type",
    )
    ...
    def create(self, vals_list):
        ...
        newly created `podiatry.time.shift` record(s) to a string that represents
        the shift time range."""
        res = super(PodiatryTimeShift, self).create(vals_list)
