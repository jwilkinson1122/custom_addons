...
class PodiatryClinic(Controller):
    """Controller for a podiatry clinic website that allows users to view clinic
    details and schedule appointments online."""

    @route("/podiatry_doctor", auth="public", website=True)
    def podiatry_clinic(self):
        """Renders the podiatry clinic page with patient, specialist, and doctor information.
        This method retrieves the current user's partner ID as the patient ID,
        fetches all records from the `podiatry.specialist` model, and all records
        from the `hr.employee` model to display on the podiatry clinic webpage."""
        patient_id = request.env.user.partner_id
        specialised_id = request.env["podiatry.specialist"].sudo().search([])
        doctor_id = request.env["hr.employee"].sudo().search([])
        return request.render(
            "pod_clinical_managementwebsite_podiatry_template",
            {
    ...
    def create_appointment(self, **kw):
        ...
        if len(kw.get("time_shift")) == 0:
            ...
            patient_appointment = (
                request.env["podiatry.appointment"]
                .sudo()
                .create(
                    {
    ...
    def success_appointment(self, **kwargs):
        ...
        return request.render(
            "pod_clinical_managementwebsite_rental_success_template", {"token": kwargs}
        )

    @http.route(
        "/podiatry_clinic/appointment_card/<int:token>",
        type="http",
        auth="public",
        website=True,
    )
    def appointment_card(self, token):
        ...
        appointment = request.env["podiatry.appointment"].sudo().browse(token)
        ...
        pdf_content, _ = report_service._render_qweb_pdf(
            "pod_clinical_managementaction_appointment_card", data=data
        )
        pdf_http_headers = [
    ...
    def get_doctors_shifts(self, doctor_id):
        ...
        doctors_shift = (
            request.env["hr.employee"].sudo().browse(int(doctor_id)).time_shift_ids
        )
    ...
    def get_all_doctors(self):
        ...
        doctor_id = request.env["hr.employee"].sudo().search([])
        return request.render(
            "pod_clinical_managementwebsite_all_doctors", {"doctor_ids": doctor_id}
        )
