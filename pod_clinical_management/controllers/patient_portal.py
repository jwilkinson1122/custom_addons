...
class PatientPortal(portal.CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        """Extends the base method to include the count of podiatry prescriptions
        ...
        if "prescriptions_count" in counters:
            prescriptions_count = (
                request.env["podiatry.prescription"].sudo().search_count([])
            )
    ...
    def portal_my_prescriptions(self, **kwargs):
        ...
        if (
            request.env.ref("pod_clinical_managementgroup_podiatry_manager")
            in request.env.user.groups_id
        ):
            domain = []
        elif (
            request.env.ref("pod_clinical_managementgroup_podiatry_doctor")
            in request.env.user.groups_id
        ):
            domain = [
                (
        ...
        prescriptions = request.env["podiatry.prescription"].sudo().search(domain)
        return request.render(
            "pod_clinical_managementportal_my_prescriptions",
    ...
    def view_prescriptions(self, id):
        ...
        prescription = request.env["podiatry.prescription"].browse(id)
        return request.render(
            "pod_clinical_managementprescription_portal_template",
