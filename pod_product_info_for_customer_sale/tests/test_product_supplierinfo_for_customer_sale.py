...
class TestProductSupplierinfoForCustomerSale(TransactionCase):
    ...
    def test_pod_product_info_for_customer_sale(self):
    ...
    def test_pod_product_info_for_customer_sale_variant(self):
        so = self.env["sale.order"].create(
            {"partner_id": self.customer.id, "pricelist_id": self.pricelist.id}
        )
        line = self.env["sale.order.line"].create(
    ...
    def test_pod_product_info_for_customer_sale_template(self):
    ...
    def test_pod_product_info_for_customer_sale_variant_wo_template(self):
        customerinfo = self._create_partnerinfo(
            "customer", self.customer, self.product_variant_2, empty_variant=True
        )
        so = self.env["sale.order"].create(
