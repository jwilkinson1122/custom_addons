...
class AccountMove(models.Model):
    ...
    def _get_invoiced_lot_values(self):
        ...
        all_invoices_amls = current_invoice_amls.prescription_line_ids.invoice_lines.filtered(lambda aml: aml.move_id.state == 'posted').sorted(lambda aml: (aml.date, aml.move_name, aml.id))
        ...
        stock_move_lines = current_invoice_amls.prescription_line_ids.move_ids.move_line_ids.filtered(lambda sml: sml.state == 'done' and sml.lot_id).sorted(lambda sml: (sml.date, sml.id))
    ...
    @api.depends('line_ids.prescription_line_ids.order_id')
    def _compute_delivery_date(self):
        # EXTENDS 'account'
        super()._compute_delivery_date()
        for move in self:
            prescription_order_effective_date = list(filter(None, move.line_ids.prescription_line_ids.order_id.mapped('effective_date')))
            effective_date_res = max(prescription_order_effective_date) if prescription_order_effective_date else False
            # if multiple prescription order we take the bigger effective_date
    ...
    def _compute_incoterm_location(self):
        ...
        for move in self:
            prescription_locations = move.line_ids.prescription_line_ids.order_id.mapped('incoterm_location')
            incoterm_res = next((incoterm for incoterm in prescription_locations if incoterm), False)
...
class AccountMoveLine(models.Model):
    ...
    def _prescription_can_be_reinvoice(self):
        self.ensure_one()
        return self.move_type != 'entry' and self.display_type != 'cogs' and super(AccountMoveLine, self)._prescription_can_be_reinvoice()
