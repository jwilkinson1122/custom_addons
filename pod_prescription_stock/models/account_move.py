...
class AccountMove(models.Model):
    ...
    def _stock_account_get_last_step_stock_moves(self):
        ...
        for invoice in self.filtered(lambda x: x.move_type == 'out_invoice'):
            rslt += invoice.mapped('invoice_line_ids.prescriptions_line_ids.move_ids').filtered(lambda x: x.state == 'done' and x.location_dest_id.usage == 'customer')
        for invoice in self.filtered(lambda x: x.move_type == 'out_refund'):
            rslt += invoice.mapped('reversed_entry_id.invoice_line_ids.prescriptions_line_ids.move_ids').filtered(lambda x: x.state == 'done' and x.location_id.usage == 'customer')
            # Add refunds generated from the SO
            rslt += invoice.mapped('invoice_line_ids.prescriptions_line_ids.move_ids').filtered(lambda x: x.state == 'done' and x.location_id.usage == 'customer')
        return rslt
    ...
    def _get_invoiced_lot_values(self):
        ...
        stock_move_lines = current_invoice_amls.prescriptions_line_ids.move_ids.move_line_ids.filtered(lambda sml: sml.state == 'done' and sml.lot_id).sorted(lambda sml: (sml.date, sml.id))
...
class AccountMoveLine(models.Model):
    ...
    def _stock_account_get_anglo_saxon_price_unit(self):
        ...
        so_line = self.prescriptions_line_ids and self.prescriptions_line_ids[-1] or False
        if so_line:
