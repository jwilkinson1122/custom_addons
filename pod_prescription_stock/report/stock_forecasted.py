...
class StockForecasted(models.AbstractModel):
    ...
    def _get_report_header(self, product_template_ids, product_ids, wh_location_ids):
        ...
        so_lines = self.env['prescriptions.order.line'].search(domain)
        out_sum = 0
        if so_lines:
        ...
        res['draft_prescriptions_orders'] = so_lines.mapped("order_id").sorted(key=lambda so: so.name).read(fields=['id', 'name'])
        res['draft_prescriptions_orders_matched'] = self.env.context.get('prescriptions_line_to_match_id') in so_lines.ids
        res['qty']['out'] += out_sum
        return res
