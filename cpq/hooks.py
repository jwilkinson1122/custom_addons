...
def _link_attribute_values_to_options(env):
    ...
    for val in values_to_update:
        ...
        if match:
            ...
            _logger.info("Linked CPQ value '%s' to option '%s'", val.name, match.name)
    ...
    #     match = ProductOption.search([('code', '=', val.code)], limit=1)
    #     if match:
    #         _logger.info("Linking CPQ value '%s' to option '%s'", val.name, match.name)
    #         val.linked_option_id = match.id
...
def _ensure_cpq_attributes_link_product_attributes(env):
    ...
    for cpq_attr in CPQAttribute.search([]):
        ...
        if not match:
            ...
            _logger.info("[OK] Created new product.attribute for CPQ attribute: %s", name)
        else:
            _logger.info("Matched CPQ attribute '%s' to existing product.attribute ID %s", name, match.id)
...
def sync_cpq_values_to_product_attribute_values(env):
    ...
    for cpq_val in CPQVal.search([]):
        ...
        if not product_attr:
            _logger.warning("! No linked product.attribute for CPQ attribute: %s", cpq_attr.name)
            continue
...
def check_cpq_value_links(env):
    ...
    if not issues:
        return "<p style='color: green; font-weight: bold;'>All CPQ attribute values are properly linked.</p>"
    ...
    <h3 style="color: #c00;">! Found %s CPQ value(s) with linking issues</h3>
...
def fix_cpq_links(env):
    ...
    # Step 1: Ensure all CPQ Attributes are linked to a product.attribute
    for cpq_attr in CPQAttr.search([("linked_product_attribute_id", "=", False)]):
        ...
        if not match:
            ...
            _logger.info("[*] Created product.attribute for CPQ attribute: %s", cpq_attr.name)
        cpq_attr.linked_product_attribute_id = match.id

    # Step 2: Ensure all CPQ Values are linked to product.attribute.value
    for val in CPQVal.search([]):
        ...
        if not val.linked_option_id and val.code:
            ...
            if opt_match:
                ...
                _logger.info("[*] Linked CPQ value '%s' to option '%s'", val.name, opt_match.name)
        ...
        if prod_val:
            ...
            _logger.info("[*] Created product.attribute.value for CPQ value: %s", val.name)
...
#         val.product_attribute_value_id = prod_val.id
...
def repair_cpq_attribute_links(env):
    ...
    for val in CPQAttrVal.search([]):
        ...
        if not cpq_attr or not cpq_attr.exists():
            _logger.warning("Skipping CPQ value ID %s ('%s') — missing attribute_id", val.id, val.name)
            continue
...
def cleanup_broken_cpq_values(env, auto_delete=False):
    ...
    if auto_delete:
        broken.unlink()
        _logger.info("🧹 Deleted %s broken CPQ value records.", len(broken))
