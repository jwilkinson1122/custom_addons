...
class ProductConfiguratorController(Controller):
    ...
    def cpq_configure(self, product_tmpl_id, configuration):
        ...
        # Prepare config for logging and order line
        try:
        ...
        values = {
            ...
            "price_unit": (
                configuration.get("total_price", 0) / configuration.get("quantity_to_make", 1)
                if configuration.get("quantity_to_make", 1)
                else 0
            ),
        }
    ...
    # @route('/cpq/<int:product_tmpl_id>/configure', type='json', auth='user')
    # def cpq_configure(self, product_tmpl_id, configuration):
    #     _logger.info("🛰️ [CPQ] Received configuration payload: %s", json.dumps(configuration, indent=2))
    #     cpq_logger.info("🧩 Product Template ID: %s", product_tmpl_id)
    #     cpq_logger.info("💡 Current User: %s", request.env.user.name)
    ...
    #     cpq_logger.info("✅ Sending CPQ configuration response: %s", json.dumps(response, indent=2))
