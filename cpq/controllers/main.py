...
class ProductConfiguratorController(Controller):
    ...
    def _log_context(self, ctx, label="Context"):
        try:
            formatted = json.dumps(ctx, indent=2, default=str)
    ...
    @route("/cpq/<int:product_tmpl_id>/data", type="json", auth="user")
    def cpq_tmpl_data(self, product_tmpl_id, **kwargs):
        env = request.env
    ...
    #     self._log_context(ctx, label="Merged Context")
    ...
    #     sale_order = None
    #     sale_order_line = env["sale.order.line"].sudo().browse(active_id) if active_model == "sale.order.line" and active_id else None

    #     if sale_order_line and sale_order_line.exists():
    #         sale_order = sale_order_line.order_id if sale_order_line.order_id else env["sale.order"].sudo().browse(active_sale_order_id)
    ...
    #         raise UserError(_("No active Sale Order found. Please launch from an order line or create a quotation."))
    ...
    # def _prepare_cpq_line_vals(self, env, sale_order, base_product, product_tmpl, config, summary_html, config_json):
        ...
        return {
            ...
            "price_unit": round(config.get("total_price", 0) / config.get("quantity_to_make", 1), 2) if config.get("quantity_to_make", 1) else 0,
